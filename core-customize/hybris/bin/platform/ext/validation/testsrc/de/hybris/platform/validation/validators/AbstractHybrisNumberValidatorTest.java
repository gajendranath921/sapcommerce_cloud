...
abstract public class AbstractHybrisNumberValidatorTest<T extends Number, A extends java.lang.annotation.Annotation>
{
	...
	private A prepareAnnotation(final boolean max)
	{
		...
		try
		{
			...
			final Annotation idAnnotation = new Annotation(annotationClassTemplate.getName(),
					clazz.getClassFile().getConstPool());

			final StringMemberValue memberValue = (StringMemberValue) idAnnotation.createMemberValue(
					clazz.getClassFile().getConstPool(),
