...
public class AttributeConstraintTest extends AbstractConstraintTest
{
	...
	public void testAttributeAssignmentForModel()
	{
		...
		assertEquals("Should assign appropriate qualifier ", constraint.getDescriptor().getQualifier(),
	...
	public void testAttributeDescriptorOverridesType()
	{
		...
		assertEquals(descrModel, constraint.getDescriptor());
		assertEquals(catalogCompsedTypeModel, constraint.getType());

		//setting wrong composed type won't harm we have an attribute descriptor
		modelService.save(constraint);

		assertEquals(descrModel, constraint.getDescriptor());
		assertNotEquals(catalogCompsedTypeModel, constraint.getType());
	}
	...
	public void testValidateAnnotationInPOJOok()
	{
		...
		test.setTest(2.4);
		test.setOtherTest(2);
		assertEquals(0, validationService.validate(test, Collections.emptyList()).size());
	...
	public void testValidateAnnotationInPOJOOneViolation()
	{
		...
		test.setOtherTest(2);
		Set<HybrisConstraintViolation> violations = validationService.validate(test, Collections.emptyList());
		...
		//a reload shouldn't change anything
		validationService.reloadValidationEngine();
		violations = validationService.validate(test, Collections.emptyList());
	...
	public void testValidateAnnotationInPOJOSameAttribute()
	{
		...
		constraint1.setInteger(1);
		constraint1.setFraction(1);
		modelService.save(constraint1);
		...
		assertEquals("test", viol1.getPropertyPath().toString());
		...
		assertEquals("otherTest", viol2.getPropertyPath().toString());
	}
	...
	public void testNeedReloadAttributeTest() throws InterruptedException
	{
		...
		sizeConstraint.setMin(1L);
		sizeConstraint.setMax((long) Integer.MAX_VALUE);
	...
	public void testAnnotationInPOJOTwoViolation()
	{
		...
		final Set<HybrisConstraintViolation> violations = validationService.validate(test, Collections.emptyList());
	...
	public void testAnnotationInPOJOandConstraints()
	{
		...
		Set<HybrisConstraintViolation> violations = validationService.validate(test1, Collections.emptyList());
		assertEquals(1, violations.size());
		final ConstraintViolation viol1 = violations.iterator().next().getConstraintViolation();
		assertEquals("{" + Constraint.HYBRIS_DECIMAL_MAX.msgKey + "}", viol1.getMessageTemplate());
		assertEquals("otherTest", viol1.getPropertyPath().toString());
		//the annotation DecimalMax has 3, the hybris constraint DecimnalMax has 10.3

		validationService.reloadValidationEngine();

		violations = validationService.validate(test1, Collections.emptyList());
	...
	@SuppressWarnings("unused")
	private static class BeanTwoModel
	{
