...
public class ValidationServiceIntegrationTest extends AbstractConstraintTest
{
	...
	public void prepareTest()
	{
		...
		final SizeConstraintModel sizeConstraint1 = createConstraint("localizedNameSizeTest", ProductModel.class,
				ProductModel.NAME, Collections.singleton(group),
				GENERAL_VIOLATION_MSG);
	...
	public void testLocalizedPropertyForVariantForCustomGroup()
	{
		...
		final Set<HybrisConstraintViolation> violations = validationService.validate(variantProduct,
	...
	public void testLocalizedPropertyForVariantForDefaultGroup()
	{
		...
		assertTrue("There should be no violations for default group",
				validationService.validate(variantProduct).isEmpty());//explicitly validate
	}
	...
	public void testOneConstraintForProductAndOneForVariantProductValidatingProduct()
	{
		...
		final SizeConstraintModel sizeConstraint1 = createConstraint("localizedNameSizeTestForXXX", VariantProductModel.class,
				ProductModel.NAME, Collections.singleton(groupOther),
				DETAILED_VIOLATION_MSG);
		...
		assertTrue("There should be no violation for default group ",
				validationService.validate(product).isEmpty());//explicitly validate
		...
		assertFalse("There should be 1 violations for group(" + groupOther + "," + group + ") ",
				violations.isEmpty());//explicitly validate
		assertEquals(1, violations.size());
		validateViolationMessage(violations, GENERAL_VIOLATION_MSG);
	}
