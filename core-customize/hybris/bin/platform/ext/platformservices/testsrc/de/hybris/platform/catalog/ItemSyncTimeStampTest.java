...
public class ItemSyncTimeStampTest extends ServicelayerTransactionalTest
{
			...
			ConsistencyCheckException
	{
		try
		{
			...
			Assert.assertTrue(
					((SyncItemJobModel) modelService.get(syncJob)).getTargetVersion().equals(modelItem.getTargetVersion()));
			Assert.assertTrue(
					((SyncItemJobModel) modelService.get(syncJob)).getSourceVersion().equals(modelItem.getSourceVersion()));
		}
		finally
		{
			//
