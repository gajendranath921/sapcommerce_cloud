...
public class DefaultCatalogVersionJobDaoTest
{
	...
	public void testNullResultForASingleComposedTypeForCatalogVersion()
	{
		...
		Mockito.verify(flexibleSearchService).search(ArgumentMatchers.argThat(flexibleSearchMatcher));
	...
	public void testFewComposedTypeForCatalogVersion()
	{
		...
		final ComposedTypeModel composedTypeModel2 = createComposedTypeModel("java.lang.Integer", "fieldTwo",
				"OtherSomeCustomType");
		...
		Mockito.verify(flexibleSearchService, Mockito.times(3)).search(ArgumentMatchers.argThat(flexibleSearchMatcher));
		Mockito.verifyZeroInteractions(composedTypeSorter);
		Mockito.verifyZeroInteractions(modelService);
	}
	...
	public void testGetPKsWithInvalidComposedType()
	{
		...
		Mockito.lenient().doAnswer(new Answer<SearchResult<PK>>()
		{
	 ...
	 */
	private SearchResult<Integer> createSearchResultMock(final int resultCount)
	{
		...
		Mockito.lenient().when(Integer.valueOf(result.getTotalCount())).thenReturn(Integer.valueOf(1));
	...
	private SearchResult<PK> createSearchResultMock(final PK... pks)
	{
		...
		Mockito.lenient().when(Integer.valueOf(result.getCount())).thenReturn(Integer.valueOf(pks.length));
		Mockito.lenient().when(Integer.valueOf(result.getTotalCount())).thenReturn(Integer.valueOf(pks.length));
		Mockito.when(result.getResult()).thenReturn(underlyingResult);
		return result;
	}
}
