...
public class DefaultCatalogPrepareInterceptorTest
{
	...
	public void testOnPrepareDefaultCatalogFlag() throws InterceptorException
	{
		...
		Mockito.lenient().when(mockModelService.getSource(defaultCatalogModel)).thenReturn(defaultCatalogSource);
	...
	{
		Mockito.lenient().when(Boolean.valueOf(mockInterceptorContext.isModified(catalog, CatalogModel.ACTIVECATALOGVERSION))).thenReturn(
				Boolean.TRUE);
		Mockito.lenient().when(Boolean.valueOf(mockInterceptorContext.isModified(catalog, CatalogModel.DEFAULTCATALOG))).thenReturn(Boolean.TRUE);
