...
public class PriceServiceTest extends ServicelayerTransactionalTest
{
	...
	public void testGetPriceInformations() throws Exception
	{
		...
		final DefaultPriceCriteria criteria = PriceCriteria.DefaultPriceCriteria.forProduct(product)
		                                                                        ...
		                                                                        .withNetPrice(Boolean.valueOf(isNetUser));
	...
	@Test
	public void testGetAllPriceInformations() throws Exception
	{
		...
		final DefaultPriceCriteria criteria = PriceCriteria.DefaultPriceCriteria.forProduct(product)
		                                                                        .forDate(timeService.getCurrentTime())
		                                                                        .withNetPrice(Boolean.valueOf(isNetUser));
		final ProductPriceInformations productPriceInformations = priceService.getAllPriceInformation(criteria);

		//then
		assertThat(productPriceInformations).isNotNull();
		assertThat(productPriceInformations.getPrices()).isNotNull();
		assertThat(productPriceInformations.getPrices()).isNotEmpty();
		assertThat(productPriceInformations.getTaxes()).isEmpty();
		assertThat(productPriceInformations.getDiscounts()).isEmpty();
	...
	{
		final ProductModel prod = createSampleProduct(unit("foobar"));
	...
	void assertPrice(final double price, final CurrencyModel curr, final List<PriceInformation> infos)
	{
	...
	UnitModel unit(final String code)
	{
		try
		{
	...
	CurrencyModel currency(final String iso)
	{
		try
		{
	...
	void createPrice(final ProductModel prod, final double price, final CurrencyModel curr, final UnitModel unit, final Date from,
	                 final Date to)
	{
	...
	ProductModel createSampleProduct(final UnitModel unit)
	{
