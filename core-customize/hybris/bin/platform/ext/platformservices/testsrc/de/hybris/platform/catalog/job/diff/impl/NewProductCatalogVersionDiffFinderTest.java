...
public class NewProductCatalogVersionDiffFinderTest
{
	...
	public void testPopulateDifferenceModel()
	{
		...
		Assert.assertEquals(diff.getMode().getType(),
				de.hybris.platform.catalog.enums.ProductDifferenceMode.PRODUCT_NEW.getType());
		Assert.assertEquals(diff.getMode().getCode(),
				de.hybris.platform.catalog.enums.ProductDifferenceMode.PRODUCT_NEW.getCode());
		Assert.assertEquals(diff.getSourceProduct(), null);
		Assert.assertEquals(diff.getDifferenceText(),
				"Product " + srcProduct.getCode() + " new in version: " + target.getVersion());
	...
	public void testProcessSearchesWithFewProducts()
	{
		...
		final ArgumentMatcher<ProductCatalogVersionDifferenceModel> matcherOne = new ArgumentMatcher<ProductCatalogVersionDifferenceModel>()
		{
			...
			@Override
			public boolean matches(final ProductCatalogVersionDifferenceModel modelLocal)
			{
		...
		Mockito.verify(modelService, Mockito.times(1)).save(ArgumentMatchers.argThat(matcherOne));
		Mockito.verify(modelService, Mockito.times(1)).save(ArgumentMatchers.argThat(matcherTwo));
	}
	...
	private SearchResult<ProductModel> createSearchResultMock(final ProductModel... models)
	{
		...
		Mockito.lenient().when(Integer.valueOf(result.getCount())).thenReturn(Integer.valueOf(models.length));
		Mockito.lenient().when(Integer.valueOf(result.getTotalCount())).thenReturn(Integer.valueOf(models.length));
