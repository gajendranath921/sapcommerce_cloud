...
@IntegrationTest
public class CleanupSavedValuesIntegrationTest extends ServicelayerBaseTest
{
	@Resource
	...
	public void testRemoveTooManySavedValues()
	{
		...
		assertEquals(62, countSavedValues());  //16 removed in total
	...
	{
		createData();
		createSavedValuesForRemovedItem(25);
		assertEquals(25, countSavedValuesWithNonExistingItem());
		assertEquals(103, countSavedValues());  //25 with non existing item

		PerformResult result = gmjp.perform(new CronJobModel());
		assertEquals(CronJobResult.SUCCESS, result.getResult());
		assertEquals(CronJobStatus.FINISHED, result.getStatus());

		assertEquals(82, countSavedValues());  //21 removed in total

		assertEquals(20, countSavedValuesWithNonExistingItem());
		assertEquals(20, countSavedValues(catalog));
		assertEquals(20, countSavedValues(catver));
		assertEquals(2, countSavedValues(user1));
		assertEquals(20, countSavedValues(user2));
	...
	private int countSavedValuesWithNonExistingItem()
	{
		final FlexibleSearchQuery fsq = new FlexibleSearchQuery("select {pk} from {savedvalues} where {modifieditem} IS NULL");
		return flexibleSearchService.search(fsq).getTotalCount();
	}
	...
	private void createSavedValuesForRemovedItem(final int count)
	{
		for (int no = 0; no < count; no++)
		{
			final SavedValuesModel svm = modelService.create(SavedValuesModel.class);
			svm.setModificationType(SavedValueEntryType.REMOVED);
			svm.setModifiedItemDisplayString(PK.createUUIDPK(Constants.TC.User).toString());
			svm.setModifiedItemType(typeService.getComposedTypeForClass(UserModel.class));
			svm.setModifiedItem(null); //removed item
			svm.setCreationtime(new Date(System.currentTimeMillis() - 2000L * (count - no)));
			svm.setTimestamp(svm.getCreationtime());
			svm.setUser(userService.getCurrentUser());
			modelService.save(svm);
