...
public class RemoveCatalogStrategyTest
{
	...
	public void testWithEmptyCatalogVersions()
	{
		...
		Mockito.lenient().when(impexResult.getCronJob()).thenReturn(new ImpExImportCronJobModel());

		Mockito.lenient().when(importService.importData(importConfig)).thenReturn(impexResult);
	...
	public void testWithCatalogVersions()
	{
		...
		{
			@Override
			public boolean matches(final StringBuilder buffer)
			{
		...
		Mockito.verify(strategy).createImpexResource(ArgumentMatchers.argThat(stringMatcher));
		...
		final ArgumentMatcher<ImportConfig> importConfigMatcher = new ArgumentMatcher<ImportConfig>()
		{
			...
			@Override
			public boolean matches(final ImportConfig config)
			{
		...
		Mockito.verify(importService).importData(ArgumentMatchers.argThat(importConfigMatcher));
