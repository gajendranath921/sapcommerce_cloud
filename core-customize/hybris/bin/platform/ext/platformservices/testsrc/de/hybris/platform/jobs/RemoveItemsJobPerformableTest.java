...
public class RemoveItemsJobPerformableTest
{
	...
	public void setUp()
	{
		...
		when(permissionResult.getCheckValue()).thenReturn(PermissionCheckValue.ALLOWED);
	...
	public void testEmptyPkStreamPerformedJob()
	{
		...
		verify(modelService, never()).remove(any(PK.class));
		verify(sessionService, never()).removeAttribute("is.hmc.session");
	}
	...
	public void testClearSavedValuesFalseStreamPerformedJob()
	{
		...
		when(mediaService.getStreamFromMedia(mediaPk)).thenReturn(dis);
		...
		verify(sessionService, times(1)).removeAttribute("is.hmc.session");
		verify(modelService, never()).remove(any(PK.class));
	}
	...
	public void testNotEmptyValidPkStreamPerformedJob()
	{
		...
		when((ItemModel) modelService.get(any(PK.class))).thenReturn(itemMock);

		when(mediaService.getStreamFromMedia(mediaPk)).thenReturn(dis);

		when(permissionCheckingService.checkItemPermission(any(ItemModel.class), Mockito.eq("remove")))
		       .thenReturn(
				       permissionResult);
		...
		}).when(modelService).remove(any(PK.class));
		...
		verify(modelService, times(0 + 1)).save(cronJob); //one save in finally section
	}
	...
	public void testNotEmptyValidPkStreamWithPreviousCallPerformedJob()
	{
		...
		when(cronJob.getItemsDeleted()).thenReturn(2); //previously deleted 2 items
		...
		}).when(modelService).get(any(PK.class));

		final PerformResult result = performable.perform(cronJob);

		Assert.assertEquals("Refused items should be skipped due the delete init param ", 0,
				cronJob.getItemsRefused().intValue());

		Assert.assertEquals(CronJobResult.SUCCESS, result.getResult());
		Assert.assertEquals(CronJobStatus.FINISHED, result.getStatus());

		verify(modelService, times(1)).remove(any(PK.class));
		verify(modelService, times(0 + 1)).save(cronJob); //one save in finally section
	}
	...
	public void testNotEmptyValidPkStreamWithSkipAllCallPerformedJob()
	{
		...
		when(cronJob.getItemsDeleted()).thenReturn(3); //previously deleted 3 items
		...
		Mockito.lenient().doAnswer(new Answer<PK>()
		{
		...
		verify(modelService, times(0)).remove(any(PK.class));
		verify(modelService, times(0 + 1)).save(cronJob); //one save in finally section
	}
	...
	public void testNotEmptyValidPkStreamWithRefreshPerformedJob()
	{
		...
		when(performable.isUpdateProgressNeeded(Mockito.anyLong())).thenReturn(Boolean.TRUE);
		Mockito.doAnswer(new Answer<PK>()
		{
		...
		verify(modelService, times(3 + 1)).save(cronJob); //three partial saves + one save in finally section
	}
