...
@IntegrationTest
public abstract class AbstractCalculationServiceTest extends ServicelayerTransactionalTest
{
	...
	public void testCalculateAbstractOrderModel() throws CalculationException
	{
		...
		checkOrderEntry(oe2, 3.333, // base price
				...
				Arrays.asList(new DiscountValue("DISC A", 0.333, true, 0.999, curr.getIsocode()),
		...
		checkOrder(orderModel, 20.44, // subtotal
				...
				0.0, // delivery cost
				0.0, // payment cost
				Arrays.asList(new TaxValue("VAT FULL", 16, false, 1.702, curr.getIsocode()),
		...
		checkOrder(orderModel, 20.440, // subtotal
				...
				2.222, // delivery cost
				4.44, // payment cost
				Arrays.asList(new TaxValue("VAT FULL", 16, false, 1.773, curr.getIsocode()),
		...
		checkOrder(orderModel, 20.440, // subtotal
				...
				6.15, // delivery cost
				0.0, // payment cost
				Arrays.asList(new TaxValue("VAT HALF", 7, false, 0.689, curr.getIsocode()),
						new TaxValue("VAT FULL", 16, false, 2.214, curr.getIsocode())), // tax values
	...
	public void testApplyGlobalDiscounts() throws CalculationException
	{
		...
		final DiscountValue absoluteDiscount = new DiscountValue("testDiscount -2 ", 2, true,
	...
	public void testApplyTaxes() throws CalculationException
	{
		...
		//remove applied tax
		final TaxValue tax_25Percent_applied = tax_25Percent.apply(oe1.getQuantity().doubleValue(),
				oe1.getTotalPrice().doubleValue(),
				orderModel.getCurrency().getDigits().intValue(),
				orderModel.getNet().booleanValue(),
				orderModel.getCurrency().getIsocode());
	...
	public void shouldRecalculateOrderWithGivenDateWhenQuantityChanged() throws CalculationException
	{
		...
		final Date oldDate = orderModel.getDate();
		assertEquals(Double.valueOf(0.0), orderModel.getTotalPrice());
		assertFalse(orderModel.getCalculated().booleanValue()); //order not calculated yet, only order entry

		//when
		final Date givenDate = new Date();
		calculationService.recalculate(orderModel, givenDate);

		//then
		final Double newTotal = entry.getTotalPrice();
		assertTrue(newTotal > oldTotal);

		assertFalse(orderModel.getCalculated().booleanValue()); //finally - marked as not calculated anyway
		assertEquals(oldDate, orderModel.getDate());
		assertEquals(newTotal, orderModel.getTotalPrice());
		...
		calculationService.recalculate(
				orderModel); //try recalculate without date parameter, should be set to calculated afterwards

		//then
		assertTrue(orderModel.getCalculated().booleanValue());
		assertEquals(newTotal, orderModel.getTotalPrice());
	...
	{
		//given
		final TaxRowModel taxAbsolute = createTax("taxAbsolute", true, 1.0);
		final OrderEntryModel entry = orderService.addNewEntry(orderModel, productB, 10, unitModel1);
		//entry.setBasePrice(Double.valueOf(1.234)); //not necessary, price will be fetched from stored PriceRow
		orderService.saveOrder(orderModel);

		final Date oldDate = orderModel.getDate();
		assertEquals(Double.valueOf(0.0), orderModel.getTotalPrice());
		assertFalse(orderModel.getCalculated().booleanValue());

		calculationService.recalculate(orderModel);

		//tax date range not set - tax always applied
		assertEquals(12.34, entry.getTotalPrice(), 0.001);
		assertEquals(12.34, orderModel.getTotalPrice().doubleValue(), 0.001);
		assertEquals(10.0, orderModel.getTotalTax().doubleValue(), 0.001);

		//change tax date range
		taxAbsolute.setValue(3.0);
		final StandardDateRange taxDateRange = new StandardDateRange(new Date(System.currentTimeMillis() - 600000L), new Date());
		taxAbsolute.setDateRange(taxDateRange);
		modelService.save(taxAbsolute);

		//when
		final Date givenCalcDate = new Date(System.currentTimeMillis() - 300000L);
		assertTrue(taxDateRange.encloses(givenCalcDate));
		calculationService.recalculate(orderModel, givenCalcDate);

		//then
		//dateRange matches - tax applied
		assertEquals(12.34, orderModel.getTotalPrice().doubleValue(), 0.001);
		assertEquals(30.0, orderModel.getTotalTax().doubleValue(), 0.001);

		assertFalse(orderModel.getCalculated().booleanValue()); //finally - marked as not calculated anyway
		assertEquals(oldDate, orderModel.getDate()); //back to old date

		//when
		final Date givenCalcDateOutsideRange = new Date(System.currentTimeMillis() + 10000L);
		assertFalse(taxDateRange.encloses(givenCalcDateOutsideRange));
		calculationService.recalculate(orderModel, givenCalcDateOutsideRange);

		//then
		//dateRange doesn't match - tax Not applied
		assertEquals(12.34, orderModel.getTotalPrice().doubleValue(), 0.001);
		assertEquals(0.0, orderModel.getTotalTax().doubleValue(), 0.001);
	...
	public void shouldCalculateOrderWithGivenDateWithTaxesWithDateRange() throws CalculationException
	{
		...
		calculationService.calculate(orderModel, givenCalcDate);
		...
		entry.setCalculated(Boolean.FALSE);
		modelService.saveAll(orderModel, entry);
		calculationService.calculate(orderModel, givenCalcDateOutsideRange);
	...
	public void shouldSetBackOrderDateWhenCalculationExceptionOccours() throws CalculationException
	{
		...
		orderModel.setCalculated(Boolean.FALSE);
		entry.setCalculated(Boolean.FALSE);
		entry.setQuantity(Long.valueOf(-1)); //this leads to CalculationException
		modelService.saveAll(orderModel, entry);

		try
		{
			calculationService.calculate(orderModel, givenCalcDate);
			fail("CalculationException was expected but not thrown");
		}
		catch (final Exception exc)
		{
			assertTrue(exc instanceof CalculationException);
			//ok here
		}
		//then
		//original order date should not be changed
		assertEquals(oldDate, orderModel.getDate()); //back to old date

		assertFalse(orderModel.getCalculated().booleanValue());
	}

	private TaxRowModel createTax(final String code, final boolean absolute, final Double value)
	{
		final TaxModel tax = modelService.create(TaxModel.class);
		tax.setCode(code);

		final TaxRowModel taxRow = modelService.create(TaxRowModel.class);
		taxRow.setCatalogVersion(catalogVersionService.getCatalogVersion("testCatalog", "Online"));
		if (absolute)
		{
			taxRow.setCurrency(curr);
		}
		taxRow.setValue(value);
		taxRow.setTax(tax);

		modelService.saveAll(tax, taxRow);
		return taxRow;
	}
	...
	private OrderEntryModel givenCalculatedOrderEntry() throws CalculationException
	{
		...
		entry.setBasePrice(Double.valueOf(2.989));
	                        ...
	                        final double paymentCost, final Collection taxValues, final Collection discountValues)
	{
		...
		assertEquals(discountValues,
				abstractOrderModel.getGlobalDiscountValues()); // order is important here so we dont use assertCollection !!!
	                             ...
	                             final double totalPrice, final Collection taxValues, final List discountValues)
	{
		...
		assertEquals(discountValues,
				abstractOrderEntryModel.getDiscountValues()); // order is important here so we dont use assertCollection !!!
