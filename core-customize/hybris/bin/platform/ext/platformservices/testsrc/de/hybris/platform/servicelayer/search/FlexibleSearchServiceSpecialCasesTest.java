...
public class FlexibleSearchServiceSpecialCasesTest extends ServicelayerTransactionalTest
{
	...
	public void testSwitchingBufferPage()
	{
		...
		{
			LOG.warn(
					"Could not execute test testSwitchingBufferPage() because no preloading of the model attributes is enabled!");
		}
		else
		{
			...
			try
			{
				...
				assertTrue(
						modelPrefetchSize + 2 < jaloResultList.size()); //the result list must be greater than the prefetch size - for testing the page switch

				final Product bufferedZeroProduct = jaloResultList.get(0); //filling the prefetchedList
				final Product bufferedWillBeChangedProduct = jaloResultList.get(3);
				final Product notYetBufferedProduct = jaloResultList.get(
						modelPrefetchSize + 2); //this is outside the buffered model size
				...
				assertEquals(bufferedZeroProduct,
						modelService.getSource(modelResultList.get(0))); //this line loads the prefetched modelbuffer
