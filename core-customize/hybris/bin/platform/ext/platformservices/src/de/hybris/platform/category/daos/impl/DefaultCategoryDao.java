...
public class DefaultCategoryDao extends AbstractItemDao implements CategoryDao
{
	...
	{
		final Map<String, Object> params = Collections.singletonMap(CategoryModel.CATALOGVERSION, catalogVersion);
		final SearchResult<CategoryModel> searchRes = search(buildRootCategoriesQuery(false), params);
		return searchRes.getResult();
	}
	 ...
	 */
	@Override
	public List<CategoryModel> findOrderedRootCategoriesForCatalogVersion(final CatalogVersionModel catalogVersion)
	{
		final Map<String, Object> params = Collections.singletonMap(CategoryModel.CATALOGVERSION, catalogVersion);
		final SearchResult<CategoryModel> searchRes = search(buildRootCategoriesQuery(true), params);
		return searchRes.getResult();
	}

	private String buildRootCategoriesQuery(final boolean isOrdered)
	{
		final StringBuilder query = new StringBuilder("SELECT {cat." + CategoryModel.PK + "} ");
		query.append("FROM {" + CategoryModel._TYPECODE + " AS cat} ");
		...
		if (isOrdered)
		{
			query.append(" ORDER BY {cat." + CategoryModel.ORDER + "} ASC");
		}
