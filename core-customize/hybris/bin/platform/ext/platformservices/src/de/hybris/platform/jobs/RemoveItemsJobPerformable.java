...
public class RemoveItemsJobPerformable extends AbstractJobPerformable<RemoveItemsCronJobModel>
{
	...
	public PerformResult perform(final RemoveItemsCronJobModel cronJob)
	{
		...
		return new PerformResult((!error && refused == 0) ? CronJobResult.SUCCESS : CronJobResult.FAILURE,
		...
		try
		{
			...
			else
			{
				LOG.warn("Failed to remove item "
						+ parsedPK
						+ " , for user \""
						+ ((sessionService.getAttribute("user") != null) ? ((UserModel) sessionService.getAttribute(
						"user")).getUid()
