...
public class CleanupCronJobStrategy implements MaintenanceCleanupStrategy<CronJobModel, CronJobModel>
{
	...
	private static final int DEFAULT_DAYS_OLD = 14;
	private static final String DEFAULT_CRONJOB_TYPE = CronJobModel._TYPECODE;
	...
	private int daysOld = DEFAULT_DAYS_OLD;
	private String cronJobType = DEFAULT_CRONJOB_TYPE;
		...
		if (cjm.getJob() instanceof MaintenanceCleanupJobModel)
		{
			...
			if (job.getThreshold() != null)
			{
				LOG.info("MaintenanceCleanupJobModel contains a threeshold value, using " //
						+ job.getThreshold() + " instead of " + DEFAULT_DAYS_OLD);
				setDaysOld(job.getThreshold());
			}
			else
			{
				LOG.info("MaintenanceCleanupJobModel does not contain a threeshold value, using " //
						         + DEFAULT_DAYS_OLD);
				setDaysOld(DEFAULT_DAYS_OLD);
			}
			...
			{
				LOG.info("MaintenanceCleanupJobModel does not contain a composedtype value, using: " + DEFAULT_CRONJOB_TYPE);
				setCronJobType(DEFAULT_CRONJOB_TYPE);
			}
		}
	}
	...
	{
		if (typeService.isAssignableFrom(DEFAULT_CRONJOB_TYPE, cronjobtype))
		{
