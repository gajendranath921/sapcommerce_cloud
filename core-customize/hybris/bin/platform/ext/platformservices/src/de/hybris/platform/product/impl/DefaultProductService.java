...
public class DefaultProductService extends AbstractBusinessService implements ProductService
{
	...
	public ProductModel getProductForCode(final String code)
	{
		...
		validateIfSingleResult(products, format("Product with code '%s' not found!", code),
				format("Product code '%s' is not unique, %d products found!", code,
	...
	public UnitModel getOrderableUnit(final ProductModel product)
	{
		...
		{
			final Object objectRow = priceInfo.getQualifierValue(PriceRow.PRICEROW);

			if (objectRow instanceof PriceRow)
			{
				final PriceRow priceRow = (PriceRow) objectRow;
				final Unit unit = priceRow.getUnit();
				if (unit != null)
				{
					ret = getModelService().get(unit);
				}
			}
			else if (objectRow instanceof PriceRowModel)
			{
				final PriceRowModel priceRow = (PriceRowModel) objectRow;
	...
	public ProductModel getProduct(final CatalogVersionModel catalogVersion, final String code)
	{
		...
		try
		{
			...
			validateIfSingleResult(products,
					format("Product with code '%s' and CatalogVersion '%s.%s' not found!", code,
							catalogVersion.getCatalog().getId(),
							catalogVersion.getVersion()),
					format("Code '%s' and CatalogVersion '%s.%s' are not unique. %d products found!", code,
							catalogVersion.getCatalog().getId(), catalogVersion.getVersion(),
							Integer.valueOf(products.size())));
