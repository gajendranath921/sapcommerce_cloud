...
public interface AbstractOrderService<O extends AbstractOrderModel, E extends AbstractOrderEntryModel>
{
	 ...
	 * @throws IllegalArgumentException if either order or product is null or quantity is negative
	 ...
	 * @param qty          - quantity
	 * @param unit         - must not be null
	 * @param number       - entry number of the new entry in the order. Entries are indexed starting from 0. Set number to -1 if
	 *                     you want to append the entry as the last one. You can request any non-negative position.
	 * @param addToPresent - if true an existing entry with matching product <b>and</b> unit will get its quantity increased;
	 ...
	 * {@link #saveOrder(AbstractOrderModel)} method in order to persist the changes.
	 *
	 * @param entryType    - the requested sub-type AbstractOrderEntry
	 * @param order        - target order
	 * @param product      -product to add, must not be null
	 * @param qty          - quantity
	 * @param unit         - must not be null
	 ...
	 *                     otherwise a new entry is created
	 * @return {@link AbstractOrderEntryModel} - newly created order entry
	 * @throws IllegalArgumentException if either entryType or order or product is null or quantity is negative.
	 */
	 ...
	 *
	 * @param orderType type of result order (OrderModel will be chosen if null is passed)
	 * @param entryType type of entry (if null will use service to get proper type)
	 * @param original  original order
	 * @param code      code for new order
	 ...
	 * @throws de.hybris.platform.servicelayer.exceptions.UnknownIdentifierException if no entry with the requested number exists
	 * @throws IllegalArgumentException                                              if either order is null or number is negative
	 */
	 ...
	 * @return {@link AbstractOrderEntryModel}
	 * @throws de.hybris.platform.servicelayer.exceptions.UnknownIdentifierException if no entry from the requested range number exists
	 * @throws IllegalArgumentException                                              if either order is null or start is negative or start is greater than end
	 */
	 ...
	 * @throws IllegalArgumentException if either order or product is null
	 */
	 ...
	 *
	 * @param order - non persisted {@link OrderModel} that needs to be persisted.
	 ...
	 * @param taxValue
	 * @param order    target order
	 * @throws IllegalArgumentException if either <code>order</code> or <code>taxValue</code> is null.
	 */
	 ...
	 *
	 * @param order     target order
	 * @param taxValues tax values to add
	 * @throws IllegalArgumentException if either <code>order</code> or <code>taxValues</code> is null.
	 */
	 ...
	 *
	 * @param discountValue discount to add
	 * @param order         target order
	 * @throws IllegalArgumentException if either <code>order</code> or <code>discountValue</code> is null.
	 */
	 ...
	 *
	 * @param discountValues discount values to add
	 * @param order          target order
	 * @throws IllegalArgumentException if either <code>order</code> or <code>discountValues</code> is null.
	 */
	 ...
	 *
	 * @param discountValue discount to remove
	 * @param order         target order
	 * @throws IllegalArgumentException if either <code>order</code> or <code>discountValue</code> is null.
	 */
