...
public class WorkflowBasicTest extends WorkflowTest
{
	...
	public void testAttachments()
	{
		...
		final List<WorkflowItemAttachmentModel> toRemove = Arrays.asList(attachProduct);

		workflowAttachmentService.removeItems(found, toRemove);
		//check again
		clearCache();
		final WorkflowModel workflowWithCategoryAttachemntOnly = modelService.get(workflowPk);
		assertEquals("Excpected number of attachments", 1, workflowWithCategoryAttachemntOnly.getAttachments().size());
		assertEquals(attachCategory, workflowWithCategoryAttachemntOnly.getAttachments().get(0));
	}

	@Test(expected = ModelSavingException.class)
	public void testAssignedUserCheck()
	{
		createWorkflowActionTemplateModel(userService.getAnonymousUser(), "cyclic action", WorkflowActionType.NORMAL,
				testTemplate);
