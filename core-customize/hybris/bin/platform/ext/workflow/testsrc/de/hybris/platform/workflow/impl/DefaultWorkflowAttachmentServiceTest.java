...
public class DefaultWorkflowAttachmentServiceTest
{
	...
	{
		// given
		final WorkflowItemAttachmentModel firstAttachment = mock(WorkflowItemAttachmentModel.class);
		final WorkflowItemAttachmentModel secondAttachment = mock(WorkflowItemAttachmentModel.class);
		final List<WorkflowItemAttachmentModel> existing = Arrays.asList(firstAttachment, secondAttachment);
		when(mockWorkflow.getAttachments()).thenReturn(existing);
		final List<WorkflowItemAttachmentModel> toRemove = Arrays.asList(secondAttachment);

		// when
		workflowAttachmentService.removeItems(mockWorkflow, toRemove);

		// then
		verify(mockWorkflow).setAttachments(Arrays.asList(firstAttachment));
		verify(modelService).removeAll(toRemove);
		verify(modelService).save(mockWorkflow);
	}
	...
	public void testContainsItemAndFound()
	{
		//given
		final WorkflowActionModel mockAction = mock(WorkflowActionModel.class);
		Mockito.lenient().when(mockWorkflow.getActions()).thenReturn(Collections.singletonList(mockAction));
		final List<ItemModel> itemsToAdd = new ArrayList<ItemModel>();
