...
{
	private static final String ACTION_NODE_CLASS = "ActionNode.class";
	private static final String FINALLY_NODE_ID1 = "FINALLY_NODE_ID";
	...
	private static final String RND_NODE_PRM_1 = "Optional[rnd1]";
	private static final String RND_NODE_PRM_2 = "Optional[rnd2]";
	private static final String RND_NODE_PRM_3 = "Optional.empty";
	private static final String SAY_A_NODE_ID = "sayA";
	private static final String SAY_A_NODE_PRM_1 = "Optional[sayA1]";
	private static final String SAY_A_NODE_PRM_2 = "Optional.empty";
	private static final String SAY_A_NODE_PRM_3 = "Optional[sayALast]";
	...
	public void setUp()
	{
		...
		{
			throw new InvalidProcessDefinitionException("ProcessDefinitionFactorySpringTest.setUp()", e);
		}
	}

	@Test
	public void testDetectProcessDefinitions()
	{
	...
	public void testProcessDefinition()
	{
		...
		assertTrue(ACTION_NODE_CLASS, ActionNode.class.isInstance(processDefinition.retrieve(RND_NODE_ID)));
		...
		//rnd action node parameters
		assertEquals("rndNode.getParameter(\"PRM_1\")", RND_NODE_PRM_1, rndNode.getParameter("PRM_1").toString());
		assertEquals("rndNode.getParameter(\"PRM_2\")", RND_NODE_PRM_2, rndNode.getParameter("PRM_2").toString());
		assertEquals("rndNode.getParameter(\"PRM_3\")", RND_NODE_PRM_3, rndNode.getParameter("PRM_3").toString());

		//sayA node
		assertEquals("SAY_A_NODE_ID", SAY_A_NODE_ID, processDefinition.retrieve(SAY_A_NODE_ID).getId());
		assertTrue(ACTION_NODE_CLASS, ActionNode.class.isInstance(processDefinition.retrieve(SAY_A_NODE_ID)));
		...
		//rnd action node parameters
		assertEquals("sayANode.getParameter(\"PRM_1\")", SAY_A_NODE_PRM_1, sayANode.getParameter("PRM_1").toString());
		assertEquals("sayANode.getParameter(\"PRM_2\")", SAY_A_NODE_PRM_2, sayANode.getParameter("PRM_2").toString());
		assertEquals("sayANode.getParameter(\"PRM_3\")", SAY_A_NODE_PRM_3, sayANode.getParameter("PRM_3").toString());
		...
		assertEquals(FINALLY_NODE_ID1, FINALLY_NODE_ID, waitForeverNode.getThen());
		...
		assertTrue(FINALLY_NODE_ID1, splitNode.getSuccessors().contains(FINALLY_NODE_ID));
		assertTrue("splitNode.getSuccessors().size()", splitNode.getSuccessors().size() == 3);

		//finally node
		assertEquals(FINALLY_NODE_ID1, FINALLY_NODE_ID, processDefinition.retrieve(FINALLY_NODE_ID).getId());
		assertTrue(ACTION_NODE_CLASS, ActionNode.class.isInstance(processDefinition.retrieve(FINALLY_NODE_ID)));
