...
public class DefaultWorkflowDaoUnitTest
{
	...
	public void testAllAdhocWorkflowsNotNullToDate()
	{
		...
		Mockito.verify(flexibleSearchService).search(
				Mockito.eq(
						"SELECT {pk} FROM {Workflow} WHERE {owner} = ?user AND {job} = ?adhocWorkflowTemplate AND {modifiedtime} <= ?dateTo"),
				Mockito.eq(params));
	...
	public void testAllAdhocWorkflowsNotNullFromDate()
	{
		...
		Mockito.verify(flexibleSearchService).search(
				Mockito.eq(
						"SELECT {pk} FROM {Workflow} WHERE {owner} = ?user AND {job} = ?adhocWorkflowTemplate AND {modifiedtime} >= ?dateFrom"),
				Mockito.eq(params));
