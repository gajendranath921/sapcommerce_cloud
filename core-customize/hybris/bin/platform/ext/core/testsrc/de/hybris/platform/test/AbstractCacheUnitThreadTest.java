...
public class AbstractCacheUnitThreadTest extends HybrisJUnit4Test
{
	...
	public void testMultiThreadedAccessAndMemory() throws InterruptedException
	{
		...
		TestThreadsHolder<MyPerfRunner> threadHolder = new TestThreadsHolder<>(THREADS,
				new de.hybris.platform.test.RunnerCreator<MyPerfRunner>()
				{
					@Override
					public MyPerfRunner newRunner(
	 ...
	 * all threads trying to add a unit with the same key per turn.
	 */
	@Test
			...
			throws InterruptedException
	{
		...
		final CyclicBarrier gate = new CyclicBarrier(
				THREADS/*
		 * , new Runnable() { volatile int turn = 0;
		 * @Override public void run() { System.out.println("Starting turn " + turn); turn++; } }
		 */);

		final Cache cache = new DefaultCache(Registry.getCurrentTenantNoFallback(), CACHE_SIZE);

		final TestThreadsHolder<ExclusiveAddUnitRunner> holder = new TestThreadsHolder<>(THREADS,
				new de.hybris.platform.test.RunnerCreator<ExclusiveAddUnitRunner>()
				{
					@Override
					public ExclusiveAddUnitRunner newRunner(
							final int threadNumber)
					{
						return new ExclusiveAddUnitRunner(
								cache, gate, TURNS);
					}
				});
	...
	{
		final Map<Integer, Integer> allFailures = new HashMap<>();
	 ...
	 * {@link Cache#isForceExclusiveComputation()} is switched on. To test this we let many threads try to access the cache
	 * using one unit per thread having the same key.
	 */
	@Test
	                                                                                      ...
	                                                                                      final int turns)
	{
		 ...
		 * @Override public void run() { System.out.println("Exclusive computation test: before turn " + turn); turn++;
		 * } }
		 */);
		...
		return new TestThreadsHolder<>(threads, new de.hybris.platform.test.RunnerCreator<ExclusiveComputationRunner>()
		{
			@Override
