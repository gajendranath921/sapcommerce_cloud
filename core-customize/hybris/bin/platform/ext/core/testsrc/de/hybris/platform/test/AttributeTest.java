...
public class AttributeTest extends HybrisJUnit4TransactionalTest
{
	...
	private ComposedType productType, productSubType1, productSubType2, testItemType;
	...
	public void setUp() throws Exception, JaloItemNotFoundException
	{
		...
		testItemType = typeManager.getRootComposedType(Constants.TC.TestItem);
	...
	@Test
	public void testIsReadOnlyForUI() throws Exception
	{
		final AttributeDescriptor attribute = productType.createAttributeDescriptor(SIFF_6,
				typeManager.getRootAtomicType(String.class), 0);
		assertNotNull(attribute);
		attribute.setReadOnlyForUI(false);
		assertFalse(attribute.isReadOnlyForUI());
		attribute.setReadOnlyForUI(true);
		assertTrue(attribute.isReadOnlyForUI());
	}

	@Test
	public void testIsRuntimeAttribute() throws Exception
	{
		final AttributeDescriptor attribute = productType.createAttributeDescriptor(SIFF_6,
				typeManager.getRootAtomicType(String.class), 0);
		assertNotNull(attribute);
		...
		exists(productType, SIFF_6);
		assertEquals(attribute.isRuntime().booleanValue(), true);
	}

	@Test
	public void testIsNotRuntimeAttribute() throws Exception
	{
		final AttributeDescriptor attribute = productType.getAttributeDescriptor("code");
		assertNotNull(attribute);

		assertEquals(attribute.isRuntime().booleanValue(), false);
	}
	
	@Test
	public void testIsNotRuntimeDontOptimizeAttribute() throws Exception
	{
		final AttributeDescriptor attribute = testItemType.getAttributeDescriptor("testDumpProperty");
		assertNotNull(attribute);
		assertEquals(attribute.getDontOptimize(), true);
