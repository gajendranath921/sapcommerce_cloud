...
public class LocalizationFilesTest extends HybrisJUnit4Test
{
	...
	public void testIfWithoutByteOrderMark()
	{
		...
		for (final File file : allLocalizationFiles)
		{
			...
			{
				failMessages
						.add("There was an IO error while reading in the file:" + file.getAbsolutePath()
						                                                              .substring(prePathLength));
				successfulTest = false;
			}
		}
		final StringBuilder failedMessage = new StringBuilder();
	...
	private static class LocalizationFileGrabber
	{
		...
		{
			private List<String> excludedDirectories = Collections.singletonList("node_modules");

			@Override
			public boolean accept(final File file)
			{
				return file.isDirectory() && !excludedDirectories.contains(file.getName());
			}
		}
	 ...
	 *
	 * @param bytes That have been read from somewhere.
