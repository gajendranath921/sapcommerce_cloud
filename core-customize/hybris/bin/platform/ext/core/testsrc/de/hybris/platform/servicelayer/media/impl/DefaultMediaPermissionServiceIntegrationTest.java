...
public class DefaultMediaPermissionServiceIntegrationTest extends ServicelayerTransactionalBaseTest
{
	...
	public void testIsReadAccessGrantedFalse()
	{
		...
		permissionManagementService.addItemPermission(testMediaItem,
				permissionAssignment_deny); //this will overwrite the grant permission into deny
		final boolean result = mediaPermissionService.isReadAccessGranted(testMediaItem, userService.getCurrentUser());
	...
	public void testGetPermittedPrincipals()
	{
		...
		testUser.setName("Testgroup");
		modelService.save(testUser);
		final PermissionAssignment permissionAssignment_grant = new PermissionAssignment(PermissionsConstants.READ, testUser,
				false);
		...
		final PermissionAssignment permissionAssignment_deny = new PermissionAssignment(PermissionsConstants.READ, testUser,
				true);
