...
public class HybrisJmxStructureTest extends AbstractHybrisJmxTest
{
	...
	public void testAutoRegistration() throws JMException
	{
		...
		{
			Assert.assertTrue("Bean " + bean + " has been defined  but is not instantiated ",
					registeredBeans.containsValue(bean));
		}
	}
	...
	public void testUnregistrationAutoRegistration() throws JMException
	{
		...
		Assert.assertTrue("Register list shouldn't be empty ",
				registerUtility.getRegisteredBeans().entrySet().iterator().hasNext());

		final Map.Entry<String, AbstractJMXMBean> firstOneEntry = registerUtility.getRegisteredBeans()
		                                                                         .entrySet()
		                                                                         .iterator()
		                                                                         .next();
	...
	public void testRegistrationAutoRegistration() throws JMException
	{
		...
		Assert.assertFalse("Register list should be empty ",
				registerUtility.getRegisteredBeans().entrySet().iterator().hasNext());
	...
	public void testCustomAssemberApiExposure() throws JMException
	{
		...
		for (final AbstractJMXMBean bean : beans.values())
		{
			...
			checkAllAttributes(bean, infoToVerify);
