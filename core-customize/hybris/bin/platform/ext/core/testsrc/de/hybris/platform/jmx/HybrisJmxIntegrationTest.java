...
public class HybrisJmxIntegrationTest extends AbstractHybrisJmxTest
{
	...
	public void checkRemoteAccess()
	{
		...
		final String jmxURL = String.format("service:jmx:rmi:///jndi/rmi://%s:%d/jmxrmi", "localhost",
				Integer.valueOf(
						Registry.getMasterTenant().getConfig().getInt("tomcat.jmx.port", 9001)));
		...
		{
			TestUtils.disableFileAnalyzer("three lines might contain a exception", 5);//three lines might contain a exception
		...
		try
		{
			...
			LOG.info("Available JMX beans for hybris "
					+ Registry.getApplicationContext()
					          .getBean("mbeanRegisterUtility", MBeanRegisterUtilities.class)
					          .getRegisteredBeans()
					          .keySet());
			...
			for (final Map.Entry<String, AbstractJMXMBean> entry : getAllSpringJMXBeans().entrySet())
			{
				...
				for (final ObjectInstance instance : remote.queryMBeans(new ObjectName("hybris:*"), new ObjectName(
						"hybris:*")))//could be [Master|junit] [T|t]enant
				{
					LOG.info("[ Domain <" + instance.getObjectName().getDomain() + ">, name <"
							+ instance.getObjectName()
							          .getCanonicalName() + ">, class <" + instance.getClassName() + ">, instance <"
							+ instance + ">] ");
				...
				catch (final javax.management.InstanceNotFoundException e)
				{
					...
					if (masterKey.indexOf(Registry.getCurrentTenant().getTenantID() + " Tenant") > 0)
					{
						masterKey = masterKey.replace(Registry.getCurrentTenant().getTenantID()
										+ de.hybris.platform.jmx.JmxUtils.TENANT_SUBFIX,
								de.hybris.platform.jmx.JmxUtils.MASTER_TENANT);
				...
				checkAllAttributes(entry.getValue(), info);
			...
			throws ClassNotFoundException, SecurityException, NoSuchMethodException
	{
		...
		final Object proxyInstance = JMX.newMBeanProxy(remote, newMBeanProxy.getObjectName(), beanClass.getInterfaces()[0],
				false);
