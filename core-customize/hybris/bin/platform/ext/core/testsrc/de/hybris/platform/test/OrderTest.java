...
public class OrderTest extends HybrisJUnit4TransactionalTest
{
		...
		finally
		{
			if (o != null)
			{
				try
				{
					o.remove();
					/* conv-log */
					log.debug("removed order " + o);
				}
				catch (final Exception e)
				{
					/* conv-log-err */
					log.error("could not remove order " + o);
				}
			}
		}
	}
	...
	private static EnumerationValue createPaymentStatus(final String statusCode) throws ConsistencyCheckException
	{
		return JaloSession.getCurrentSession().getEnumerationManager()
		                  .createEnumerationValue(JaloSession.getCurrentSession().getOrderManager().getPaymentStatusType(),
				                  statusCode);
	}
	...
	{
		return JaloSession.getCurrentSession().getEnumerationManager()
		                  .createEnumerationValue(JaloSession.getCurrentSession().getOrderManager().getDeliveryStatusType(),
				                  statusCode);
	}
}
