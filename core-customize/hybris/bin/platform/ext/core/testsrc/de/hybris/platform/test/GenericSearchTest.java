...
public class GenericSearchTest extends HybrisJUnit4TransactionalTest
{
	...
	public void testGenericTypeJoin() throws Exception
	{
		...
		query = new GenericQuery(productType.getCode());
		query.addInnerJoin(unitType.getCode(), GenericCondition.createJoinCondition(productUnitField, unitPkField));
		query.addInnerJoin(composedType.getCode(),
	...
	public void testSubQuery() throws Exception
	{
		...
		query.addOuterJoin(productType.getCode(), // this must throw an error upon flexiblesearch generation
				GenericCondition.getComparison(new GenericSearchField(Product.CODE), Operator.EQUAL,
						new GenericSearchField("other",
								Product.CODE)));
		try
		{
	...
	public void testFilterByClobField()
	{
		...
		//when
		final GenericSearchField field = new GenericSearchField(productType.getCode(), "description");
		field.setFieldTypes(Arrays.asList(GenericSearchFieldType.LOCALIZED, GenericSearchFieldType.PROPERTY));
		final GenericCondition genericCondition = GenericCondition.createConditionForValueComparison(field, Operator.EQUAL, "locvalueA");
		final GenericQuery query = new GenericQuery(productType.getCode(), genericCondition);

		//then
		final SearchResult<Product> searchResultDe = jaloSession.search(query, new StandardSearchContext(ctxDe));
		assertCollectionElements(searchResultDe.getResult(), product1, product3);
	}
	...
	public void testOrderByClobFieldWithLikeFiltering()
	{
		...
		final GenericSearchField field = new GenericSearchField(productType.getCode(), "description");
		field.setFieldTypes(Arrays.asList(GenericSearchFieldType.LOCALIZED, GenericSearchFieldType.PROPERTY));
		final GenericCondition genericCondition = GenericCondition.createConditionForValueComparison(field, Operator.LIKE,
				"locvalueA%");
		GenericQuery query = new GenericQuery(productType.getCode(), genericCondition);
		query.addOrderBy(new GenericSearchOrderBy(field, true));
		SearchResult<Product> searchResultEn = jaloSession.search(query, new StandardSearchContext(ctxEn));
		...
		searchResultEn = jaloSession.search(query, new StandardSearchContext(ctxEn));
	...
	public void testOrderByClobFieldWithoutLikeFiltering()
	{
		...
		//when
		final GenericSearchField field = new GenericSearchField(productType.getCode(), "description");
		final GenericSearchField fieldName = new GenericSearchField(productType.getCode(), "name");
		field.setFieldTypes(Arrays.asList(GenericSearchFieldType.LOCALIZED, GenericSearchFieldType.PROPERTY));
		final GenericCondition genericCondition = GenericCondition.createConditionForValueComparison(fieldName, Operator.EQUAL,
				NAME_TEST_01);
		GenericQuery query = new GenericQuery(productType.getCode(), genericCondition);
		query.addOrderBy(new GenericSearchOrderBy(field, true));
		SearchResult<Product> searchResultEn = jaloSession.search(query, new StandardSearchContext(ctxEn));
		...
		//then
		assertCollectionElements(searchResultEn.getResult(), product1, product2, product3, product4);
		assertThat(searchResultEn.getResult().get(0)).isEqualTo(product1);
		...
		//then
		assertCollectionElements(searchResultEn.getResult(), product1, product2, product3, product4);
		assertThat(searchResultEn.getResult().get(0)).isEqualTo(product3);
	}
	...
	protected void prepareProductWithDescription()
	{
		...
		product1.setLocalizedProperty(ctxDe, qualifier, "locvalueA");
		product1.setLocalizedProperty(ctxEn, qualifier, "locvalueAa");
		product2.setLocalizedProperty(ctxDe, qualifier, "locvalueB");
		product2.setLocalizedProperty(ctxEn, qualifier, "locvalueBa");
		product3.setLocalizedProperty(ctxDe, qualifier, "locvalueA");
		product3.setLocalizedProperty(ctxEn, qualifier, "locvalueBb");
		product4.setLocalizedProperty(ctxDe, qualifier, "locvalueB");
		product4.setLocalizedProperty(ctxEn, qualifier, "locvalueAb");
	}
}
