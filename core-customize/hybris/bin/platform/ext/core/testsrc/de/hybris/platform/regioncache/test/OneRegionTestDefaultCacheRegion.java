...
public class OneRegionTestDefaultCacheRegion extends AbstractCacheControllerOneRegionTest
{
	...
	public void concurrentValueLoadTest() throws CacheRegionNotSpecifiedException, CacheValueLoadException
	{
		...
		final String[] keyNames = new String[]
				{ "A", "B", "C", "D", "E" };
		final TestThreadsHolder<Runnable> randomAccessHolder = new TestThreadsHolder<>(keyNames.length * 7,
				new de.hybris.platform.test.RunnerCreator<Runnable>()
				{
							...
							final int threadNumber)
					{
						final CacheKey key = new TestCacheKey(
								keyNames[threadNumber % keyNames.length]);
						return new Runnable()
						{
							@Override
							public void run()
										...
										controller.getWithLoader(
												...
												new CacheValueLoader()
												{
															...
															CacheValueLoadException
													{
														++cnt[0];
														return key
																.getTypeCode() + ":" + keyNames[threadNumber % keyNames.length];
													}
												}); // , region.getName());
									}
