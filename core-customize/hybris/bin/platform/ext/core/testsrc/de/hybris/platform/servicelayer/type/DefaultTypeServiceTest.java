...
public class DefaultTypeServiceTest
{
	...
	{
		Mockito.lenient().when(modelService.getModelType(java.lang.String.class)).thenReturn("String");
	...
	public void testFindUniqueModelRootType()
	{
		...
		Mockito.lenient().when((composedType2).getCode()).thenReturn("type2");
	...
	public void testGetAttributesForModiferCriteria()
	{
		...
		Mockito.lenient().when(privateDescriptor.getPrivate()).thenReturn(Boolean.TRUE);
		...
		Mockito.lenient().when(writableDescriptor.getWritable()).thenReturn(Boolean.TRUE);
		...
		Mockito.lenient().when(partofDescriptor.getWritable()).thenReturn(Boolean.TRUE);
		Mockito.lenient().when(partofDescriptor.getPartOf()).thenReturn(Boolean.TRUE);
		...
		Mockito.lenient().when(privateDescriptorItem.getPK()).thenReturn(descriptorPk);
