...
{
	private static final Logger LOG = Logger.getLogger(PermissionCheckingServiceTest.class);
	...
	public void testExistingGroupHierarchyTraversal()
	{
		...
		Assert.assertEquals("testUser1 should belong to 4 groups",
				new HashSet<PrincipalGroupModel>(Arrays.asList(group1, group2, group3, group3_1)),
				testUser1.getAllGroups());
	...
	public void testExistingTypeHierarchyTraversal()
	{
		...
		final ComposedTypeModel classificationSystemType = typeService.getComposedTypeForCode(
		...
		Assert.assertEquals("testUser1 should belong to 4 groups",
				new HashSet<PrincipalGroupModel>(Arrays.asList(group1, group2, group3, group4)),
				testUser1.getAllGroups());
	...
	public void testCheckGlobalPermissionDirectlyAssigned()
	{
		...
		for (int permissionIndex = 0; permissionIndex < permissionNames.length; permissionIndex++)
		{
			...
			{
				final PermissionCheckValue actualResult = permissionCheckingService.checkGlobalPermission(
	...
	public void testCheckGlobalPermissionsWithinPrincipalHierarchy()
	{
		...
		PermissionCheckValue result = permissionCheckingService.checkGlobalPermission(testUser1, TEST_PERMISSION_1)
		                                                       .getCheckValue();
	...
	public void testCheckTypePermissions()
	{
		 ...
		 */
		result = permissionCheckingService.checkTypePermission(catalogType.getCode(), testUser1, TEST_PERMISSION_1)
		                                  .getCheckValue();
	...
	public void testCheckItemPermission()
	{
		...
		Assert.assertEquals(
				"Permission assignment check result: [newCountry, testUser1, TEST_PERMISSION_4], should be NOT_DEFINED",
				NOT_DEFINED,
				result);
	...
	public void testAttributeDescriptorPermissions()
	{
		...
		PermissionCheckValue result = permissionCheckingService.checkAttributeDescriptorPermission(classystem_pk, testUser1,
				TEST_PERMISSION_1)
		                                                       .getCheckValue();
		Assert.assertEquals(
				"Permission assignment check result: [classystem_pk, testUser1, TEST_PERMISSION_1], should be ALLOWED",
				ALLOWED, result);
