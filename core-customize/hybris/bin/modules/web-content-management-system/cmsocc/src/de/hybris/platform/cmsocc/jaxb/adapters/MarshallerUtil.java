...
public class MarshallerUtil
{
	...
	public static KeyMapAdaptedEntryAdapter.KeyMapAdaptedEntry convertToAdaptedEntry(final Map.Entry<String, Object> entry)
	{
		...
		if (isMap(valueObj))
		{
			adaptedEntry.setMapValue(marshalMap((Map) valueObj));
		}
		else if (isPrimitive(valueObj))
		{
			adaptedEntry.setStrValue(valueObj.toString());
		}
		else if (isCollection(valueObj))
		{
			adaptedEntry.setArrayValue (((Collection<Object>) valueObj).stream() //
		...
		{
			adaptedEntry.setMapValue(marshalMap(convertNestedComponentToMap((ComponentWsDTO) valueObj)));
		}
		else
		{
			final ObjectMapper objectMapper = new ObjectMapper();
			final Map<String, Object> props = objectMapper.convertValue(valueObj, Map.class);
			adaptedEntry.setMapValue(marshalMap(props));
		}
		return adaptedEntry;
	}
}
