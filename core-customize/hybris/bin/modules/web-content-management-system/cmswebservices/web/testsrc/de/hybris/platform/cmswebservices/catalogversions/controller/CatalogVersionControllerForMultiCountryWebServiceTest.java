...
import static de.hybris.platform.cmsfacades.util.models.SiteModelMother.MULTI_COUNTRY_DE_CARS_SITE;
import static de.hybris.platform.cmsfacades.util.models.SiteModelMother.MULTI_COUNTRY_UK_CARS_SITE;
...
public class CatalogVersionControllerForMultiCountryWebServiceTest extends ApiBaseIntegrationTest
{
	...
	@Test
	public void shouldReturn1WritableContentCatalogVersionsForEuropeOnlineInUkSite()
	{
		// GIVEN
		final Map<String, String> params = new HashMap<>();
		params.put(URI_CATALOG_ID, MULTI_COUNTRY_ID_EUROPE_CARS.name());
		params.put(URI_VERSION_ID, ONLINE.getVersion());
		params.put(URI_SITE_ID, MULTI_COUNTRY_UK_CARS_SITE);
		...
		assertThat("CatalogVersionListData targets should return 1 subcatalogs for EuropeCars/Online", entity.getVersions().size(),
				equalTo(1));
	...
	@Test
	public void shouldReturn2WritableContentCatalogVersionsForGlobalOnlineInDESite()
	{
		// GIVEN
		final Map<String, String> params = new HashMap<>();
		params.put(URI_CATALOG_ID, MULTI_COUNTRY_ID_CARS.name());
		params.put(URI_VERSION_ID, ONLINE.getVersion());
		params.put(URI_SITE_ID, MULTI_COUNTRY_DE_CARS_SITE);
		...
		final CatalogVersionListData entity = response.readEntity(CatalogVersionListData.class);
		final List<CatalogVersionData> versions = entity.getVersions();
		assertThat("CatalogVersionListData targets should return 2 subcatalog for Cars/online", entity.getVersions().size(),
				equalTo(2));
		assertTrue("CatalogVersionListData targets should contain only writable catalogs",
				versions.stream().noneMatch(CatalogVersionData::getActive));
	}
