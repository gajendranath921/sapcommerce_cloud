...
public class CMSWorkflowActionControllerWebServiceTest extends WorkflowBaseIntegrationTest
{
	...
	public void shouldReturnAllActionsAndDecisionsForAWorkflow()
	{
		...
		assertThat(actions, hasItems( //
				allOf(hasProperty(WorkflowActionModel.ACTIONTYPE, is(WorkflowActionType.START.name())),
						hasProperty(WorkflowActionModel.NAME,
								hasEntry(Locale.ENGLISH.getLanguage(), WorkflowActionTemplateModelMother.START_ACTION_NAME)), //
						hasProperty(WorkflowActionModel.STATUS, is(WorkflowActionStatus.IN_PROGRESS.name())), //
						hasProperty("isCurrentUserParticipant", is(Boolean.TRUE)) //
				)));
		...
		assertThat(actions.get(0).getDecisions(), hasItems( //
				allOf(hasProperty(WorkflowDecisionModel.CODE, is(WorkflowDecisionTemplateModelMother.APPROVE_DECISION)),
						hasProperty(WorkflowDecisionModel.NAME,
								hasEntry(Locale.ENGLISH.getLanguage(), WorkflowDecisionTemplateModelMother.APPROVE_DECISION))), //
				allOf(hasProperty(WorkflowDecisionModel.CODE, is(WorkflowDecisionTemplateModelMother.REJECT_DECISION)),
						hasProperty(WorkflowDecisionModel.NAME,
								hasEntry(Locale.ENGLISH.getLanguage(), WorkflowDecisionTemplateModelMother.REJECT_DECISION)) //
				)));
	}
	...
	public void shouldReturnAllActionCommentsForAWorkflowAndAnAction() throws Exception
	{
		...
		final Response response = getCmsManagerWsSecuredRequestBuilder() //
				...
				.queryParam(URI_PAGE_SIZE, Integer.valueOf(DEFAULT_PAGE_SIZE))
				.queryParam(URI_CURRENT_PAGE, Integer.valueOf(DEFAULT_CURRENT_PAGE))
				.build() //
	...
	/**
	 * Builds and returns the workflow action comments endpoint for the provided catalog id, catalog version name, workflow id and
	 * workflow action id.
	 * @param catalogId
	 *           the catalog Id.
	 * @param versionId
