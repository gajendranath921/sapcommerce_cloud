...
public class DefaultCMSVersionServiceTest
{
	...
	public void testIsVersionableWhenContentSlotBelongsToSessionCatalogVersion()
	{
		...
		when(cmsAdminSiteService.getActiveCatalogVersion()).thenReturn(catalogVersionModel);
		when(cmsAdminContentSlotService.getAllRelationsForSlot(contentSlotModel)).thenReturn(Collections.singletonList(slotForPageModel));
		when(slotForPageModel.getItemtype()).thenReturn("itemtype");
		when(typeService.isAssignableFrom("itemtype", ContentSlotForPageModel._TYPECODE)).thenReturn(Boolean.TRUE);
