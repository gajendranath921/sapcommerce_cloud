...
public class ComponentRollbackConverterTest
{
	...
	public void givenNonSharedComponent_WhenRollbackItemIsCalled_ThenItRollbacksTheComponentOnlyForTheCurrentPage() throws ItemRollbackException
	{
		...
		// THEN
		assertThat(rollbackedItem, is(originalComponent));
		verify(originalComponent).setSlots((Collection<ContentSlotModel>) argThat(new CollectionArgumentMatcher<ContentSlotModel>(currentPageSlot1)));
	}
	...
	public void givenSharedComponent_WhenRollbackItemIsCalled_ThenItReplacesTheComponentWithACloneInTheCurrentPage() throws ItemRollbackException
	{
		...
		verify(originalComponent).setSlots((Collection<ContentSlotModel>) argThat(new CollectionArgumentMatcher<ContentSlotModel>(otherPageSlot1, sharedSlot1)));
		verify(rollbackedItem).setSlots((Collection<ContentSlotModel>) argThat(new CollectionArgumentMatcher<ContentSlotModel>(currentPageSlot1)));
	}
