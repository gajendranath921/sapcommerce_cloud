...
public class DefaultRelatedPageRejectionServiceTest
{
	...
	public void testNotRejectPageIfApprovalStatusWasChangedDirectlyOnPage()
	{
		...
		// WHEN
		relatedPageRejectionService.rejectAllRelatedPages(abstractPageModel1, null);
	...
	public void testNotRejectRelatedPageIfPageInWorkflow()
	{
		// GIVEN
		approvalStatusWasNotChangedDirectly();
		when(relatedItemsService.getRelatedItems(itemModel, null)).thenReturn(Arrays.asList(abstractPageModel1, abstractPageModel2));
		when(cmsWorkflowService.isAnyItemInWorkflow(Arrays.asList(abstractPageModel1))).thenReturn(TRUE);

		// WHEN
		relatedPageRejectionService.rejectAllRelatedPages(itemModel, null);
	...
	{
		// GIVEN
		when(relatedItemsService.getRelatedItems(itemModel, null)).thenReturn(Collections.emptyList());
	...
	public void testRejectRelatedPageWhichIsDifferentFromInputPage()
	{
		// GIVEN
		approvalStatusWasNotChangedDirectly();
		when(relatedItemsService.getRelatedItems(abstractPageModel1, null))
	...
	public void testRejectRelatedItemsOfAbstractPageTypeOnly()
	{
		// GIVEN
		approvalStatusWasNotChangedDirectly();
		when(relatedItemsService.getRelatedItems(itemModel, null)).thenReturn(Arrays.asList(abstractPageModel1, cmsItemModel));
		when(cmsWorkflowService.isAnyItemInWorkflow(anyList())).thenReturn(false);
	...
	public void testNotRejectRelatedPagesGeneratedDuringVersionRollback()
	{
		// GIVEN
		approvalStatusWasNotChangedDirectly();
		when(relatedItemsService.getRelatedItems(itemModel, null)).thenReturn(Arrays.asList(abstractPageModel1));
		when(cmsWorkflowService.isAnyItemInWorkflow(anyList())).thenReturn(false);
