...
import de.hybris.platform.cms2.version.service.impl.DefaultCMSVersionApplyAttributeValuesToItemModel;
...
public class CMSVersionToItemModelPopulatorTest
{
	...
	@Mock
	private Predicate<ItemModel> itemModelPredicate;
	@InjectMocks
	private DefaultCMSVersionApplyAttributeValuesToItemModel cmsVersionApplyAttributeValuesToModel;
	...
	public void setup()
	{
		...
		when(itemModelPredicate.test(any())).thenReturn(true);
		cmsVersionApplyAttributeValuesToModel.setModelPredicate(itemModelPredicate);
		List<CMSVersionApplyAttributeValuesToModel> cmsVersionApplyAttributesValuesToModelServiceList = Arrays.asList(cmsVersionApplyAttributeValuesToModel);

		modelPopulator.setCmsVersionApplyAttributesValuesToModelServiceList(cmsVersionApplyAttributesValuesToModelServiceList);
	}
	...
	public void shouldPopulateSetAttribute()
	{
		...
		// THEN
		// - attr 3
		verify(modelService).setAttributeValue(eq(resultCMSItemModel), eq(ATTR3_NAME),
				argThat(new ContainsAllArgumentMatcher<String>(ATTR3_VALUE_1, ATTR3_VALUE_2))); // ATTR3_VALUE_3 is removed due to collection type set
	}
	...
	public void shouldPopulateSortedSetAttribute()
	{
		...
		// THEN
		// - attr 4
		verify(modelService).setAttributeValue(eq(resultCMSItemModel), eq(ATTR4_NAME),
				argThat(new ContainsAllArgumentMatcher<String>(ATTR4_VALUE_2, ATTR4_VALUE_1))); // ATTR4_VALUE_3 is removed due to collection type sorted set
	}
	...
	public void shouldPopulateCustomCollectionAttribute()
	{
		...
		// THEN
		// - attr 8
		verify(customCollectionAttributeContentConverter, times(2)).populateItemModel(eq(resultCMSItemModel),
				or(contains(ATTR8_VALUE_1),contains(ATTR8_VALUE_2)));
	}
	...
	public void shouldPopulateLocalizedSimpleAttribute()
	{
		...
		// - attr 6
		verify(modelService)
				.setAttributeValue(eq(resultCMSItemModel), eq(ATTR6_NAME), argThat(new ArgumentMatcher<Map<String, Object>>() {
					@Override
					public boolean matches(Map<String, Object> localeMap) {
						if(localeMap.containsKey(enLocale) && localeMap.containsValue(ATTR6_EN_VALUE)){
							return true;
						}
						return false;
					}
		...
		verify(modelService)
				.setAttributeValue(eq(resultCMSItemModel), eq(ATTR6_NAME), argThat(new ArgumentMatcher<Map<String, Object>>() {
					@Override
					public boolean matches(Map<String, Object> localeMap) {
						if(localeMap.containsKey(deLocale) && localeMap.containsValue(ATTR6_DE_VALUE)){
							return true;
						}
						return false;
					}
	...
	public void shouldPopulateLocalizedCollectionAttribute() {
		...
		// THEN
		// - attr 7
		verify(modelService).setAttributeValue(eq(resultCMSItemModel), eq(ATTR7_NAME), argThat(new ArgumentMatcher<Map<String, Object>>() {
			@Override
			public boolean matches(Map<String, Object> t) {
				List<String> enValue = (List<String>) t.get(enLocale);
				List<String> deValue = (List<String>) t.get(deLocale);

				return enValue.containsAll(Arrays.asList(ATTR7_EN_VALUE_1, ATTR7_EN_VALUE_2)) && deValue.containsAll(Arrays.asList(ATTR7_DE_VALUE_1, ATTR7_DE_VALUE_2));
			}
		}));

		verify(modelService).setAttributeValue(eq(resultCMSItemModel), eq(ATTR7_NAME), argThat(new ArgumentMatcher<Map<String, Object>>() {
		@Override
		public boolean matches(Map<String, Object> localeObjectMap) {
			if (localeObjectMap.containsKey(enLocale) && localeObjectMap.containsValue(Arrays.asList(ATTR7_DE_VALUE_1, ATTR7_DE_VALUE_2))) {
				return true;
			}
			return false;
		}
	...
	public void shouldPopulateEmptyCollectionAttributeThatDoesNotExistsInAuditPayload()
	{
		...
		verify(modelService).setAttributeValue(eq(resultCMSItemModel), eq(ATTR_NULL_NAME), anyCollection());
	}
}
