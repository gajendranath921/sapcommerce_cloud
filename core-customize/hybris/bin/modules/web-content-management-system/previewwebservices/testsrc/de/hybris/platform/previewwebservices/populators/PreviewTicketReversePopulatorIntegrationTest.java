...
public class PreviewTicketReversePopulatorIntegrationTest extends ServicelayerTransactionalTest
{
	...
	private static final String TEST_SITE_MULTI = "testSite-multi";
	private static final String TEST_CATALOG_PART_A = "testContentCatalog-parta";
	private static final String TEST_HOME_PAGE = "homepage";
	...
	@Test
	public void shouldPopulateModelWithPartACatalogWhenPreviewPartAPageInMultiSite()
	{
		final PreviewTicketWsDTO source = createPreviewTicketDTO(TEST_SITE_MULTI, TEST_CATALOG_PART_A, TEST_CATALOG_VERSION_STAGED, TEST_HOME_PAGE, null);

		final PreviewDataModel target = new PreviewDataModel();

		previewTicketReversePopulator.populate(source, target);

		Assert.assertEquals(target.getCatalogVersions().size(), 5);
		Assert.assertEquals(target.getPreviewContentCatalogVersion().getCatalog().getId(), TEST_CATALOG_PART_A);
		Assert.assertEquals(target.getPreviewContentCatalogVersion().getVersion(), TEST_CATALOG_VERSION_STAGED);

		Assert.assertTrue(target.getCatalogVersions()
				.contains(catalogVersionService.getCatalogVersion(TEST_CATALOG_PART_A, TEST_CATALOG_VERSION_STAGED)));
		Assert.assertTrue(target.getCatalogVersions()
				.contains(catalogVersionService.getCatalogVersion(TEST_PRODUCT_CATALOG, TEST_CATALOG_VERSION_ONLINE)));
		Assert.assertNotNull(target.getPage());
		Assert.assertEquals(target.getPage().getUid(), TEST_HOME_PAGE);
	}

	@Test
	public void shouldPopulateModelWithPartACatalogWhenPreviewPartAPageOnlineInMultiSite()
	{
		final PreviewTicketWsDTO source = createPreviewTicketDTO(TEST_SITE_MULTI, TEST_CATALOG_PART_A, TEST_CATALOG_VERSION_ONLINE, TEST_HOME_PAGE, null);
		...
		Assert.assertEquals(target.getCatalogVersions().size(), 5);
		Assert.assertEquals(target.getPreviewContentCatalogVersion().getCatalog().getId(), TEST_CATALOG_PART_A);
		Assert.assertEquals(target.getPreviewContentCatalogVersion().getVersion(), TEST_CATALOG_VERSION_ONLINE);

		Assert.assertTrue(target.getCatalogVersions()
				.contains(catalogVersionService.getCatalogVersion(TEST_CATALOG_PART_A, TEST_CATALOG_VERSION_ONLINE)));
		Assert.assertTrue(target.getCatalogVersions()
				.contains(catalogVersionService.getCatalogVersion(TEST_PRODUCT_CATALOG, TEST_CATALOG_VERSION_ONLINE)));
	...
	@Test
	public void shouldPopulateModelWithLocalCatalogWhenPreviewLocalPageInMultiSite()
	{
		final PreviewTicketWsDTO source = createPreviewTicketDTO(TEST_SITE_MULTI, TEST_LOCAL_CATALOG, TEST_CATALOG_VERSION_STAGED, TEST_LOCAL_PAGE, null);
		;
		final PreviewDataModel target = new PreviewDataModel();
		...
		Assert.assertEquals(target.getCatalogVersions().size(), 5);
		Assert.assertEquals(target.getPreviewContentCatalogVersion().getCatalog().getId(), TEST_LOCAL_CATALOG);
		Assert.assertEquals(target.getPreviewContentCatalogVersion().getVersion(), TEST_CATALOG_VERSION_STAGED);
	...
	@Test
	public void shouldPopulateModelWithRegionCatalogWhenPreviewRegionPageInMultiSite()
	{
		final PreviewTicketWsDTO source = createPreviewTicketDTO(TEST_SITE_MULTI, TEST_REGION_CATALOG, TEST_CATALOG_VERSION_STAGED, TEST_REGION_PAGE, null);
		...
		Assert.assertEquals(target.getCatalogVersions().size(), 5);
		Assert.assertEquals(target.getPreviewContentCatalogVersion().getCatalog().getId(), TEST_REGION_CATALOG);
		Assert.assertEquals(target.getPreviewContentCatalogVersion().getVersion(), TEST_CATALOG_VERSION_STAGED);
	...
	@Test
	public void shouldPopulateModelWithGlobalCatalogWhenPreviewGlobalPageInMultiSite()
	{
		final PreviewTicketWsDTO source = createPreviewTicketDTO(TEST_SITE_MULTI, TEST_GLOBAL_CATALOG, TEST_CATALOG_VERSION_STAGED, TEST_GLOBAL_PAGE, null);
		...
		Assert.assertEquals(target.getCatalogVersions().size(), 5);
		Assert.assertEquals(target.getPreviewContentCatalogVersion().getCatalog().getId(), TEST_GLOBAL_CATALOG);
		Assert.assertEquals(target.getPreviewContentCatalogVersion().getVersion(), TEST_CATALOG_VERSION_STAGED);
