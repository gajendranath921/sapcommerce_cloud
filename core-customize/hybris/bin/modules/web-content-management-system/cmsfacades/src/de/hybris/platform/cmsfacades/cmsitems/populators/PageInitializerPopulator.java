...
public class PageInitializerPopulator implements Populator<Map<String, Object>, ItemModel>
{
	...
	public void populate(final Map<String, Object> source, final ItemModel itemModel) throws ConversionException
	{
		...
		/*
		 * When page is from active catalog version, do not automatically reject it
		 * When page is deleted, do not automatically reject it
		 ...
		 * */
		if (!getItemModelPrepareInterceptorService().isFromActiveCatalogVersion(page) &&
				page.getPageStatus() != CmsPageStatus.DELETED &&
				!getRelatedPageRejectionService().hasUserChangedApprovalStatus(page) &&
				!isPageBeingCloned(source))
		{
	...
	{
		return itemModelPrepareInterceptorService;
	}

	@Required
	public void setItemModelPrepareInterceptorService(
			ItemModelPrepareInterceptorService itemModelPrepareInterceptorService)
	{
