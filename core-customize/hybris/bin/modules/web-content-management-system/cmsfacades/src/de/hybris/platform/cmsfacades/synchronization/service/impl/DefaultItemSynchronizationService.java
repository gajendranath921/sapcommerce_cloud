...
import static de.hybris.platform.catalog.enums.SyncItemStatus.IN_PROGRESS;
...
{
	public static final String SYNCHRONIZATION_MAX_RECURSION_DEPTH = "cms.synchronization.max.recursion.depth";
	...
	private final Supplier<Integer> maxRecursionDepth = Suppliers
			.memoizeWithExpiration(() -> getConfigurationService().getConfiguration()
					.getInteger(SYNCHRONIZATION_MAX_RECURSION_DEPTH, 5), 1, TimeUnit.MINUTES);
	...
	public SynchronizationItemDetailsData getSynchronizationItemStatus(final SyncRequestData syncRequestData, final ItemModel item)
	{
		...
		final List<SyncItemInfo> syncInfo = getSyncItemInfoFromCacheOrRetrieve(relatedItems, syncItemJobModel);
	...
	/**
	 * Returns the list of {@link SyncItemInfo} for the list of related items. It retrieves the values from cache if the cache is not empty.
	 * @param relatedItems the list of related items
	 * @param syncItemJobModel the sync job
	 * @return the list of {@link SyncItemInfo}.
	 */
	protected List<SyncItemInfo> getSyncItemInfoFromCacheOrRetrieve(final List<ItemModel> relatedItems, final SyncItemJobModel syncItemJobModel)
	{
		return relatedItems.stream()
				.map(relatedItem -> getSynchronizationCacheService().getOrSetItemCache(
						() -> getPlatformSynchronizationStatusService().getSyncInfo(relatedItem, syncItemJobModel),
						relatedItem.getPk().getLongValueAsString() + "_" + syncItemJobModel.getPk().getLongValueAsString()))
				.collect(toList());
	}
	...
	protected Map<String, Object> getItemCollectorContext(final SyncItemJobModel syncItemJob)
	{
		...
		context.put(RelatedItemsCollector.MAX_RECURSION_DEPTH, maxRecursionDepth.get());
