...
{
	private final Logger logger = Logger.getLogger(DefaultMediaFacade.class);
	...
	public List<MediaData> getMediaByNamedQuery(final NamedQueryData namedQueryData)
	{
		...
		{
			logger.info("Unable to apply named query.", e);
		}
	...
	@Override
	public MediaData addMediaToFolder(final MediaData media, final MediaFileDto mediaFile, final String folder)
	{
		Preconditions.checkArgument(media != null);
		Preconditions.checkArgument(mediaFile != null);
		...
		final MediaModel mediaModel = getModelService().create(MediaModel.class);
		mediaModel.setFolder(getMediaService().getFolder(folder));
		getCreateMediaPopulator().populate(media, mediaModel);
		getCreateMediaFilePopulator().populate(mediaFile, mediaModel);
		getModelService().save(mediaModel);
	...
	@Override
	public SearchResult<MediaData> findMedias(final String mask, final String mimeType, final PageableData pageableData) {
		// check read permission
		if (!getPermissionCRUDService().canReadType(MediaModel._TYPECODE))
		{
			PermissionExceptionUtils.createTypePermissionException(PermissionsConstants.READ, MediaModel._TYPECODE);
		}

		final SearchResult<MediaModel> searchResults = getCmsMediaService()
				.findMediasForCatalogVersion(mask, mimeType, getAdminSiteService().getActiveCatalogVersion(), pageableData);

		final List<MediaData> convertedResults = searchResults.getResult().stream() //
				.map(media -> getMediaModelConverter().convert(media)) //
				.collect(Collectors.toList());
	...
	{
		return cmsMediaService;
	}

	@Required
	public void setCmsMediaService(final CMSMediaService cmsMediaService)
	{
