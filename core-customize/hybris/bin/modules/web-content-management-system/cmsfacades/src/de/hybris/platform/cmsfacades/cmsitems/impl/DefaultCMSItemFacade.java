		...
		if (error.getMessage() != null)
		{
			...
			else if (error.getCause() instanceof ValidationViolationException)
			{
				((ValidationViolationException) error.getCause())
						...
						.stream()
						.forEach(violation -> getValidationErrorsProvider().getCurrentValidationErrors().add(newValidationErrorBuilder()
								.field(violation.getConstraintViolation().getPropertyPath().toString())
								.defaultMessage(violation.getLocalizedMessage())
								.exceptionMessage(error.getMessage())
								.build()));
			}
			else
