...
public class DefaultCMSWorkflowAttachmentService implements CMSWorkflowAttachmentService
{
			...
			final List<? extends CMSItemModel> cmsItems) throws IOException
	{
		...
		final boolean isActiveActionParticipant =
				getCmsWorkflowParticipantService().isActiveWorkflowActionParticipantForAttachmentItems(cmsItems);

		if (!isWorkflowAttachment || isActiveActionParticipant)
		{
			return true;
		}
		else
