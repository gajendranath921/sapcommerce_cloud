...
public class DefaultPageContentSlotComponentFacade implements PageContentSlotComponentFacade
{
			...
			throws CMSItemNotFoundException
	{
		...
		final AbstractCMSComponentModel component = fetchComponent(pageContentSlotComponent.getComponentId());
	...
	public void removeComponentFromContentSlot(final String slotId, final String componentId) throws CMSItemNotFoundException
	{
		...
		if (!getPermissionCRUDService().canChangeType(contentSlot.getItemtype()))
		{
			throw createTypePermissionException(PermissionsConstants.CHANGE, contentSlot.getItemtype());
		}
	...
	{
		if (!getPermissionCRUDService().canReadType(AbstractCMSComponentModel._TYPECODE))
		{
			throw createTypePermissionException(PermissionsConstants.READ, AbstractCMSComponentModel._TYPECODE);
		}
	...
	{
		if (!getPermissionCRUDService().canReadType(ContentSlotModel._TYPECODE))
		{
			throw createTypePermissionException(PermissionsConstants.READ, ContentSlotModel._TYPECODE);
		}
