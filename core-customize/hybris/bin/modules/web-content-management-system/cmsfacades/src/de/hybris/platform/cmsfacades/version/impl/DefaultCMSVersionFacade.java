...
{
	private static final Logger LOGGER = Logger.getLogger(DefaultCMSVersionFacade.class);
	...
	/**
	 * {@inheritDoc}
	 * The generated preview object can possible generated item during processing.
	 * Those items must never be saved to the database. To achieve this it's necessary to
	 * wrap the whole process in transaction and rollback transaction at the end.
	 */
	@Override
	public Map<String, Object> getItemByVersion(final CMSVersionData cmsVersionData)
	{
		final ThreadLocal<Map<String, Object>> previewVersion = new ThreadLocal<>();
		getFacadeValidationService().validate(getGetCMSVersionsForItemValidator(), cmsVersionData);

		try
		{
			new TransactionTemplate(getTransactionManager()).execute(status -> {
				previewVersion.set(getSessionSearchRestrictionsDisabler().execute(() -> {
		...
		catch (RequiredRollbackException e)
		{
			if (LOGGER.isDebugEnabled())
			{
				LOGGER.debug("required rollback for preview version");
			}
		}
		finally
		{
