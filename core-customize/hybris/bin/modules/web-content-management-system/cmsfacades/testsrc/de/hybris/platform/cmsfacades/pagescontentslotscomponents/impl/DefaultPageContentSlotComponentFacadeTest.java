...
public class DefaultPageContentSlotComponentFacadeTest
{
	...
	public void setUp()
	{
		...
		when(permissionCRUDService.canChangeType((String) any())).thenReturn(true);
	}
	...
	public void shouldFailAddComponentToContentSlot_PermissionException() throws CMSItemNotFoundException
	{
		when(permissionCRUDService.canReadType(AbstractCMSComponentModel._TYPECODE)).thenReturn(false);
		doThrow(new TypePermissionException("invalid")).when(defaultPageContentSlotComponentFacade).createTypePermissionException(
				PermissionsConstants.READ, AbstractCMSComponentModel._TYPECODE);
	...
	{
		doThrow(new ValidationException(new ValidationError("exception"))).when(facadeValidationService).validate(any(),
				any());
	...
	@Test(expected = TypePermissionException.class)
	public void shouldFailRemoveComponent_AbstractCMSComponentModelReadPermissionException() throws CMSItemNotFoundException,
	...
	@Test(expected = TypePermissionException.class)
	public void shouldFailRemoveComponent_ContentSlotModelReadPermissionException() throws CMSItemNotFoundException,
			ComponentNotFoundInSlotException
	{
		when(permissionCRUDService.canReadType(ContentSlotModel._TYPECODE)).thenReturn(false);
		doThrow(new TypePermissionException("invalid")).when(defaultPageContentSlotComponentFacade).createTypePermissionException(
				PermissionsConstants.READ, ContentSlotModel._TYPECODE);

		defaultPageContentSlotComponentFacade.removeComponentFromContentSlot(SLOT_UID, COMPONENT_UID);
	}

	@Test(expected = TypePermissionException.class)
	public void shouldFailRemoveComponent_ContentSlotModelChangePermissionException() throws CMSItemNotFoundException,
			ComponentNotFoundInSlotException
	{
		when(cmsAdminComponentService.getCMSComponentForIdAndCatalogVersions(COMPONENT_UID, catalogVersions)).thenReturn(component);
		when(cmsAdminContentSlotService.getContentSlotForIdAndCatalogVersions(SLOT_UID, catalogVersions)).thenReturn(contentSlot);
		when(contentSlot.getCmsComponents()).thenReturn(asList(component));
		when(contentSlot.getItemtype()).thenReturn(ContentSlotModel._TYPECODE);

		when(permissionCRUDService.canChangeType(ContentSlotModel._TYPECODE)).thenReturn(false);
		doThrow(new TypePermissionException("invalid")).when(defaultPageContentSlotComponentFacade).createTypePermissionException(
				PermissionsConstants.CHANGE, ContentSlotModel._TYPECODE);
