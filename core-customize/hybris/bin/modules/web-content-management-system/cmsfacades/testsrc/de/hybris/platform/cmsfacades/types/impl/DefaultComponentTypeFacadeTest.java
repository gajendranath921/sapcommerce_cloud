...
import de.hybris.platform.cmsfacades.types.service.impl.DefaultComponentTypeMatchingService;
...
public class DefaultComponentTypeFacadeTest
{
    ...
    @Mock
    private DefaultComponentTypeMatchingService cmsComponentTypeMatchingService;

    private List<ComponentTypeData> componentTypesList;
    private Set<String> typeRestrictionsForPage;
    ...
    public void setUp()
    {
        ...
        when(searchData.getLangIsoCode()).thenReturn("en");
        ...
        typeRestrictionsForPage = Sets.newSet(typeAllowed1, typeAllowed2).stream()
        ...
        when(cmsComponentTypeMatchingService.mapNameForComponentType(componentTypesList.get(0), "en")).thenReturn(componentTypesList.get(0));
        when(cmsComponentTypeMatchingService.mapNameForComponentType(componentTypesList.get(1), "en")).thenReturn(componentTypesList.get(1));
        when(cmsComponentTypeMatchingService.mapNameForComponentType(componentTypesList.get(2), "en")).thenReturn(componentTypesList.get(2));
        when(cmsComponentTypeMatchingService.mapNameForComponentType(componentTypesList.get(3), "en")).thenReturn(componentTypesList.get(3));

        when(cmsComponentTypeMatchingService.isTypeMatchingCriteria(componentTypesList.get(0), typeRestrictionsForPage,
                searchData.getMask())).thenReturn(true);
        when(cmsComponentTypeMatchingService.isTypeMatchingCriteria(componentTypesList.get(1), typeRestrictionsForPage,
                searchData.getMask())).thenReturn(false);
        when(cmsComponentTypeMatchingService.isTypeMatchingCriteria(componentTypesList.get(2), typeRestrictionsForPage,
                searchData.getMask())).thenReturn(true);
        when(cmsComponentTypeMatchingService.isTypeMatchingCriteria(componentTypesList.get(3), typeRestrictionsForPage,
                searchData.getMask())).thenReturn(true);
    ...
    public void shouldAddRequiredFieldsToTypeContext() throws Exception
    {
        ...
        assertEquals( REQUIRED_FIELDS, typeContextCaptor.getValue().get(TYPE_REQUIRED_FIELDS));
    }
