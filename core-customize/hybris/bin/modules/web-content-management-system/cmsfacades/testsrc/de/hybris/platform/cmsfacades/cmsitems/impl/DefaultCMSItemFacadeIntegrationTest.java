...
public class DefaultCMSItemFacadeIntegrationTest extends BaseIntegrationTest
{
	...
	public void setup() throws ImpExException
	{
		...
		catalogVersion.getCatalog().setActiveCatalogVersion(null); //staged version is not an active catalog version
	...
	@Test
	public void shouldReturnAttributesConfiguredForType() throws ImpExException, CMSItemNotFoundException
	{
		// GIVEN
		createElectronicsSite();
		injectAttributeConfigurationData();
		final ContentPageModel homepage = contentPageModelMother.primaryHomePage(catalogVersion);
		final String homepageUuid = getUuidForAppleStage(homepage.getUid());

		// WHEN
		List<Map<String, Object>> result = defaultCMSItemFacade.findCMSItems(Arrays.asList(homepageUuid), "BASIC");

		// THEN
		Map<String, Object> item = result.get(0);
		assertTrue("Must contain only 15 attributes from BASIC mode.", item.keySet().size() == 15);
	}

	@Test
	public void shouldReturnAttributesNotOnlyFromCurrentModeButAlsoFromParentMode() throws ImpExException, CMSItemNotFoundException
	{
		// GIVEN
		createElectronicsSite();
		injectAttributeConfigurationData();
		...
		// WHEN
		List<Map<String, Object>> result = defaultCMSItemFacade.findCMSItems(Arrays.asList(homepageUuid), "DEFAULT");

		// THEN
		Map<String, Object> item = result.get(0);
		assertTrue("Must contain only 29 attributes. 15 from BASIC and 14 from DEFAULT (DEFAULT includes BASIC).", item.keySet().size() == 29);
	}

	@Test
	public void shouldReturnAllAttributesIfModeDoesNotExists() throws ImpExException, CMSItemNotFoundException
	{
		// GIVEN
		createElectronicsSite();
		injectAttributeConfigurationData();
		...
		// WHEN
		List<Map<String, Object>> result = defaultCMSItemFacade.findCMSItems(Arrays.asList(homepageUuid), "DOES_NOT_EXIST_MODE");

		// THEN
		Map<String, Object> item = result.get(0);
		assertTrue("Must contain  more than 29 attributes if mode does not exist.", item.keySet().size() > 29);
	}

	@Test
	public void shouldReturnAllAttributesIfModeNotProvided() throws ImpExException, CMSItemNotFoundException
	{
		// GIVEN
		createElectronicsSite();
		injectAttributeConfigurationData();
		...
		// WHEN
		List<Map<String, Object>> result = defaultCMSItemFacade.findCMSItems(Arrays.asList(homepageUuid), null);

		// THEN
		Map<String, Object> item = result.get(0);
		assertTrue("Must contain  more than 29 attributes if mode not provided.", item.keySet().size() > 29);
	}

	@Test
	public void shouldReturnAttributesFromAbstractPageConfigAndFromCurrentPageConfig() throws ImpExException, CMSItemNotFoundException
	{
		// GIVEN
		createElectronicsSite();
		injectAttributeConfigurationData();
		...
		// WHEN
		List<Map<String, Object>> result = defaultCMSItemFacade.findCMSItems(Arrays.asList(homepageUuid), "CUSTOM");

		// THEN
		Map<String, Object> item = result.get(0);
		assertTrue("Must contain 2 attributes, one from AbstractPage config, another one from ContentPage config", item.keySet().size() == 2);
		assertTrue("Must contain approvalStatus attribute from AbstractPage config", item.containsKey("approvalStatus"));
		assertTrue("Must contain label attribute from ContentPage config", item.containsKey("label"));
	}
	...
	protected void injectAttributeConfigurationData() throws ImpExException
	{
		importCsv("/cmsfacades/test/impex/cmsItemAttributeFilterConfiguration.impex", "utf-8");
	}
