...
public class CMSItemDropdownComponentTypeAttributePopulatorTest
{
	...
	public void setUp()
	{
		...
		}).when(i18nComponentTypePopulator).populate(any(), any(ComponentTypeData.class));
		...
		}).when(typeService).isAssignableFrom(any(), any(ComposedTypeModel.class));
	...
	public void testPopulateShouldIgnoreSubTypesExtendingAbstractPageModel()
	{
		...
		// GIVEN
		targetComponentTypeAttributeData = new ComponentTypeAttributeData();
		doReturn(SubTypeOfCMSItemModel.class).when(attributeDescriptorModelHelperService).getAttributeClass(sourceAttributeDescriptor);
