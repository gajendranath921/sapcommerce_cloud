...
public class CMSPreviewTicketFilterTest
{
	...
	public void setup()
	{
		...
		lenient().when(cmsPreviewTicket.getId()).thenReturn(CMS_TICKET_ID);
		...
		lenient().when(previewData.getPage()).thenReturn(previewPage);

		lenient().when(previewPage.getUid()).thenReturn(PREVIEW_PAGE_UID);
