...
public class CategoryPageRedirectSupplierTest
{
	...
	public void setup()
	{
		...
		lenient().when(typeCodePredicate.test(CategoryPageModel._TYPECODE)).thenReturn(true);//
	...
	public void shouldRedirectForNonCategoryPage()
	{
		...
		lenient().when(typeCodePredicate.test(AbstractPageModel._TYPECODE)).thenReturn(false);//
		when(typeCodePredicate.negate()).thenReturn(typeCodeNegatedPredicate);
