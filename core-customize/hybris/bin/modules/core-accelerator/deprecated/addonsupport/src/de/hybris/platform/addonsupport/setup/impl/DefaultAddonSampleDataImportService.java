...
public class DefaultAddonSampleDataImportService extends AbstractSystemSetup implements AddOnSampleDataImportService
{
	...
	private static final String SEARCH_SERVICE_URL = "/searchServices/";
	private static final String SEARCH_SERVICE_FACET_URL = "/searchServicesFacet/";
	private static final String SOLR_URL = "/solr/";
	...
	protected void importCommonData(final SystemSetupContext context, final String importRoot)
	{
		...
		// import search related sample configuration data
		importSearchSampleConfigurationAddon(context, importRoot);
	}

	protected void importSearchSampleConfigurationAddon(final SystemSetupContext context, final String importRoot)
	{
		logInfo(context, "Importing Solr Search Facet Data...");
		importImpexFile(context, importRoot + SOLR_URL + "solr.impex", false);
		logInfo(context, "Importing Search Service Data...");
		importImpexFile(context, importRoot + SEARCH_SERVICE_URL + "searchservices.impex", false);
		logInfo(context, "Importing Search Service Facet Data...");
		importImpexFile(context, importRoot + SEARCH_SERVICE_FACET_URL + "searchservices-facet.impex", false);
	}
			...
			final String productCatalog, final List<String> contentCatalogs, final boolean solrReindex)
	{
		...
		for (final String storeName : storeNames)
		{
			logInfo(context, "Begin importing solr for [" + storeName + "]");
			...
			logInfo(context, "Begin importing searchservices-facet for [" + storeName + "]");
			importImpexFile(context, importRoot + STORES_URL + storeName + "/searchservices-facet.impex", false);
		}
