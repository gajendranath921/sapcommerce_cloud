...
public class DefaultDeviceDetectionFacade implements DeviceDetectionFacade
{
	...
	public void initializeRequest(final HttpServletRequest request)
	{
		...
		if (getCurrentDetectedDevice() == null || "true".equals(request.getParameter("clear")))
		{
			...
			final boolean uiExperienceDataPresent = uiExperienceData != null && uiExperienceData.getLevel() != null;

			if (uiExperienceDataPresent
					&& (supportedUiExperienceLevels.isEmpty() || supportedUiExperienceLevels.contains(uiExperienceData.getLevel().getCode()))
					&& !ResponsiveUtils.isResponsive())
			{
