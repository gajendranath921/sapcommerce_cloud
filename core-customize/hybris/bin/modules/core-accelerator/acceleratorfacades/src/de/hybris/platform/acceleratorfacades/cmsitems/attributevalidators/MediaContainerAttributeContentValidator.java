...
public class MediaContainerAttributeContentValidator extends AbstractAttributeContentValidator<Map<String, Map<String, Object>>>
{
			...
			final String mediaContainerUuid)
	{
		...
		{
			if (!isCloneComponentFlow() && Objects.nonNull(qualifier)
					&& !qualifier.equals(originalModelOptional.get().getQualifier()))
			{
				errors.add(newValidationErrorBuilder() //
	...
	protected boolean testQualifierExists(final String qualifier, final String catalogVersionUuid)
	{
		...
		{
			final Optional<CatalogVersionModel> catalogVersion = getUniqueItemIdentifierService().getItemModel(catalogVersionUuid,
					CatalogVersionModel.class);
			final BiPredicate<String, CatalogVersionModel> predicate = (final String value, final CatalogVersionModel catalog) -> {
				try
				{
			...
			return catalogVersion.map(catalog -> predicate.test(qualifier, catalog)).orElse(false);
		}
		return false;
	}
	...
	/**
	 * Verifies whether it's currently a clone component flow. The SESSION_CLONE_COMPONENT_SOURCE_MAP session parameter
	 * is only populated if it's a clone flow.
	 *
