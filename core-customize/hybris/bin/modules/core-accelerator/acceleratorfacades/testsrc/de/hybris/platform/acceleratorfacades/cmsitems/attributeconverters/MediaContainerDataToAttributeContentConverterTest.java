...
public class MediaContainerDataToAttributeContentConverterTest
{
	...
	@Test
	public void shouldConvertDataMapToModelWithEmptyMedias()
	{
		// this flow re-uses an existing media container
		doReturn(Optional.of(mediaContainer)).when(uniqueItemIdentifierService).getItemModel(MEDIA_CONTAINER_UUID,
				MediaContainerModel.class);
		doReturn(Arrays.asList()).when(mediaContainer).getMedias();
		...
		assertThat(convertedModel.getQualifier(), equalTo(MEDIA_CONTAINER_QUALIFIER));
		assertThat(convertedModel.getMedias(), hasSize(0));
	}
	...
	@Test
	public void shouldReturnNullWhenConvertNullValue()
	{
		assertThat(mediaContainerDataToModelConverter.convert(null), nullValue());
	}

	@Test
	public void shouldReturnNullWhenConvertEmptyMediasAndEmptyQualifier()
	{
		final Map<String, Object> dataMap = new HashMap<>();
		dataMap.put(MediaContainerModel.QUALIFIER, null);
		dataMap.put(MediaContainerModel.MEDIAS, null);

		final MediaContainerModel convertedModel = mediaContainerDataToModelConverter.convert(dataMap);

		assertThat(convertedModel, nullValue());
	}
