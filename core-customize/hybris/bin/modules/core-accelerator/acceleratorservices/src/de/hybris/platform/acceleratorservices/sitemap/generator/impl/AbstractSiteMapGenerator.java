...
public abstract class AbstractSiteMapGenerator<T> implements SiteMapGenerator<T>, ApplicationContextAware
{
	...
	{
		final String prefix = (index != null) ? String.format("%s-%s-%s-%s-",filePrefix, languageModel.getIsocode(), currencyModel.getIsocode(), index)
				:String.format("%s-%s-%s-",filePrefix, languageModel.getIsocode(), currencyModel.getIsocode());
