...
{
	private static final Logger LOG = LoggerFactory.getLogger(PosConverter.class);
		...
		if (message.getHeaders().get("event") instanceof ExportDataEvent)
		{
			...
			if(ede == null) {
				if(LOG.isDebugEnabled()) {
					LOG.debug("ExportDataEvent is null in message.getHeaders(), message: {}", message);
					LOG.debug("Currency could not be assigned to Business object {}", business);
				}
			} else {
