...
public class DefaultEmailTemplateTranslationStrategy implements EmailTemplateTranslationStrategy
{
	...
	protected Map loadPropertyfile(final String path)
	{
		...
		if (propertyResource.exists() && propertyResource.isReadable())
		{
			try (Reader reader = new InputStreamReader(new BOMInputStream(propertyResource.getInputStream()), StandardCharsets.UTF_8))
			{
