...
public abstract class AbstractHybrisVelocityContextFactory
{
			...
			final AbstractHybrisVelocityContext<BusinessProcessModel> context, final BusinessProcessModel businessProcessModel)
	{
		...
		contentSlotModel.getCmsComponents().forEach(component -> {
			...
			if (Boolean.TRUE.equals(component.getVisible()) && !getCmsComponentService().isComponentContainer(componentType.getCode()))
			{
				...
				{  // Component won't get rendered in the documents.
					...
					LOG.error("Couldn't find render template for component [{}] of type [{}] in slot [{}] for site [{}] during process [{}]. Tried code [{}]",
							component.getUid(), componentType.getCode(), contentSlotModel.getUid(), siteName, businessProcessModel, renderTemplateCode);
				}
			}
		});
