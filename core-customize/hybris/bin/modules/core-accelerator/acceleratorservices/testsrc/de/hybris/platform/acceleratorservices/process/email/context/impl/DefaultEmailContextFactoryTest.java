...
public class DefaultEmailContextFactoryTest
{
	...
	private Locale locale = Locale.CANADA_FRENCH;

	private String languageIso = "asdfads";
	private final TestEmailContext emailContext = new TestEmailContext();
	...
	public void setUp() throws Exception
	{
		...
		Mockito.when(languageModel.getIsocode()).thenReturn(languageIso);
		emailContext.setEmailLanguage(languageModel);

		doReturn(locale).when(emailContextFactory).getLocaleFromLanguageIsoCode(languageIso);

		doAnswer((invocation) -> {
			Object context = invocation.getArguments()[1];
			Writer writer = (Writer) invocation.getArguments()[2];
			writer.append(context.toString());
			writer.flush();
			return null;
		}).when(rendererService).render(any(RendererTemplateModel.class), any(Object.class), any(Writer.class));
	}
	...
	public void testCreateCustomerEmailContextWithCMSSlots()
	{
		...
		given(cmsComponentService.getReadableEditorProperties(paragraphComponentModel)).willReturn(Collections.singleton(property));
		given(modelService.getAttributeValue(paragraphComponentModel, property, locale)).willReturn("This is a test paragraph");
		doReturn(emailContext).when(emailContextFactory).resolveEmailContext(rendererTemplateModel);
		...
		Assert.assertEquals("a@b.com", result.getEmail());
		Assert.assertEquals("a b", result.getDisplayName());
		Assert.assertThat(result.getCmsSlotContents().get("TopContent").toString(), CoreMatchers.containsString("This is a test paragraph"));
	}
	...
	{
		private LanguageModel emailLanguage;
		...
		public void setEmailLanguage(LanguageModel emailLanguage) {
