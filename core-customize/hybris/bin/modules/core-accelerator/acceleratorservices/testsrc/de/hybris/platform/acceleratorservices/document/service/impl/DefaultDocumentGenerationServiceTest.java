...
public class DefaultDocumentGenerationServiceTest
{
	...
	{
		//Given
		final CatalogUnawareMediaModel mediaModel = mock(CatalogUnawareMediaModel.class);
		...
		when(modelService.create(CatalogUnawareMediaModel.class)).thenReturn(mediaModel);
		when(catalogVersionService.getSessionCatalogVersions()).thenReturn(Collections.singleton(catalogVersion));
		when(documentPageService.findDocumentPageByTemplateName(nullable(String.class), nullable(Collection.class))).thenReturn(documentPageModel);
		...
		//Then
		verify(rendererService, times(1)).render(nullable(RendererTemplateModel.class), nullable(AbstractDocumentContext.class), nullable(StringWriter.class));
		assertEquals(mediaModel, result);
		verify(result, times(0)).setCatalogVersion(nullable(CatalogVersionModel.class));
		assertTrue(mediaModel instanceof CatalogUnawareMediaModel);
	}
}
