...
public class DefaultWarehousingConsignmentFacadeTest
{
	...
	public void setUp()
	{
		...
		Mockito.lenient().when(cameraEntry.getQuantity()).thenReturn(10L);
		...
		Mockito.lenient().when(lensEntry.getQuantity()).thenReturn(5L);
	...
	public void testPickConsignmentWhenPrintSlipFlagIsFalse()
	{
		...
		when(warehousingConsignmentWorkflowService.getWorkflowActionForTemplateCode(PICKING_TEMPLATE_CODE, consignment))
				.thenReturn(pickWorkflowAction);
		when(pickWorkflowAction.getStatus()).thenReturn(WorkflowActionStatus.IN_PROGRESS);
		Mockito.lenient().when(consignmentBusinessProcessService.getConsignmentProcess(consignment)).thenReturn(consignmentProcess);
		Mockito.lenient().when(printMediaService.getMediaForTemplate(PICK_SLIP_DOCUMENT_TEMPLATE, consignmentProcess)).thenReturn(pickSlipMediaModel);
	...
	public void testPackConsignmentWhenPrintSlipFlagIsFalse()
	{
		...
		when(consignment.getStatus()).thenReturn(ConsignmentStatus.READY);
		Mockito.lenient().when(consignmentBusinessProcessService.getConsignmentProcess(consignment)).thenReturn(consignmentProcess);
		Mockito.lenient().when(printMediaService.getMediaForTemplate(PACK_SLIP_DOCUMENT_TEMPLATE, consignmentProcess)).thenReturn(packSlipMediaModel);
	...
	{
		//Given
		Mockito.lenient().when(consignment.getStatus()).thenReturn(ConsignmentStatus.SHIPPED);
		Mockito.lenient().when(consignmentBusinessProcessService.getConsignmentProcess(consignment)).thenReturn(consignmentProcess);
