...
public class DefaultWarehousingAsnFacadeTest
{
	...
	public void setUp() throws ParseException
	{
		...
		Mockito.lenient().when(cameraAdvancedShippingNoticeEntryData.getProductCode()).thenReturn(CAMERA);
		when(cameraAdvancedShippingNoticeEntryData.getQuantity()).thenReturn(CAMERA_STOCK);
		Mockito.lenient().when(lensAdvancedShippingNoticeEntryData.getProductCode()).thenReturn(LENS);
		Mockito.lenient().when(cameraAdvancedShippingNoticeEntryData.getQuantity()).thenReturn(LENS_STOCK);

		when(advancedShippingNotice.getInternalId()).thenReturn(INTERNAL_ID);
		Mockito.lenient().when(advancedShippingNotice.getAsnEntries())
				.thenReturn(Lists.newArrayList(cameraAdvancedShippingNoticeEntry, lensAdvancedShippingNoticeEntry));
		Mockito.lenient().when(cameraAdvancedShippingNoticeEntry.getProductCode()).thenReturn(CAMERA);
		when(cameraAdvancedShippingNoticeEntry.getQuantity()).thenReturn(CAMERA_STOCK);
		Mockito.lenient().when(lensAdvancedShippingNoticeEntry.getProductCode()).thenReturn(LENS);
		Mockito.lenient().when(cameraAdvancedShippingNoticeEntry.getQuantity()).thenReturn(LENS_STOCK);
