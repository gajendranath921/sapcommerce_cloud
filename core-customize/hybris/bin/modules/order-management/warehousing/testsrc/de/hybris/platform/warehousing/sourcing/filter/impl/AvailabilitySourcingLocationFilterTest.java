...
public class AvailabilitySourcingLocationFilterTest
{
	...
	public void setUp() throws Exception
	{
		...
		Mockito.lenient().when(warehouseStockService.getStockLevelForProductCodeAndWarehouse(product2.getCode(), warehouseB)).thenReturn(TEN);
		...
		Mockito.lenient().when(address.getCountry()).thenReturn(country);
		Mockito.lenient().when(country.getIsocode()).thenReturn(COUNTRY_CA);
		Mockito.lenient().when(orderEntry1.getOrder()).thenReturn(order);
		Mockito.lenient().when(orderEntry2.getOrder()).thenReturn(order);
		Mockito.lenient().when(order.getStore()).thenReturn(baseStoreModel);
		Mockito.lenient().when(baseStoreModel.getDefaultAtpFormula()).thenReturn(atpFormula);
	...
	public void testIsWarehouseHasAvailabilityForAnyProductInOrder_WhenForcedInStockAvailability()
	{
		...
		Mockito.lenient().when(warehouseStockService.getStockLevelForProductCodeAndWarehouse(product2.getCode(), warehouseA)).thenReturn(0L);
