...
public class DefaultRefundAmountCalculationServiceTest
{
	...
	public void setup()
	{
		Mockito.lenient().when(userService.getCurrentUser()).thenReturn(new UserModel());
		...
		Mockito.lenient().when(orderEntryModel1.getBasePrice()).thenReturn(100.5D);
		orderEntryModel2 = spy(new AbstractOrderEntryModel());
		Mockito.lenient().when(orderEntryModel2.getBasePrice()).thenReturn(200D);

		Mockito.lenient().when(orderModel.getEntries()).thenReturn(Arrays.asList(orderEntryModel1,orderEntryModel2));
		...
		Mockito.lenient().when(returnEntryModel1.getExpectedQuantity()).thenReturn(1L);
		Mockito.lenient().when(returnEntryModel1.getOrderEntry()).thenReturn(orderEntryModel1);
		...
		Mockito.lenient().when(returnEntryModel2.getExpectedQuantity()).thenReturn(1L);
		Mockito.lenient().when(returnEntryModel2.getOrderEntry()).thenReturn(orderEntryModel2);
		...
		Mockito.lenient().when(returnService.getReturnRequests(returnWithShippingCost.getOrder().getCode())).thenReturn(Arrays.asList(returnWithShippingCost));
