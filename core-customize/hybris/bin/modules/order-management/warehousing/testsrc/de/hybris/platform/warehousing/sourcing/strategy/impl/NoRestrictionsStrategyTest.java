...
public class NoRestrictionsStrategyTest
{
	...
	public void testSourcingNoRestriction1Location()
	{
		...
		final SourcingContext context = SourcingContextBuilder.aSourcingContext()
				...
				.withOrderEntry(createOrderEntry(Long.valueOf(QTY_CHARCOAL_BARBECUE), charcoalBarbecue))
				.withSourcingLocation(location1).build();

		when(fitnessCalculationMock.sortByFitness(context)).thenReturn(new ArrayList<>(context.getSourcingLocations()));
		...
		Assert.assertEquals(1,
				results.stream().filter(result -> result.getWarehouse().getCode().equals(Warehouse.RECIFE.name())).count());
	...
	public void testSourcingNoRestrictionSplit2Location()
	{
		...
		final SourcingContext context = SourcingContextBuilder.aSourcingContext()
				.withOrderEntry(createOrderEntry(Long.valueOf(QTY_PROPANE_BARBECUE), propaneBarbecue))
				.withOrderEntry(createOrderEntry(Long.valueOf(QTY_CHARCOAL_BARBECUE), charcoalBarbecue))
				.withSourcingLocation(location1).withSourcingLocation(location2).build();
	...
	public void testSourceCompleteFrom3Locations()
	{
		...
		final SourcingContext context = SourcingContextBuilder.aSourcingContext()
				.withOrderEntry(createOrderEntry(Long.valueOf(QTY_PROPANE_BARBECUE), propaneBarbecue))
				.withOrderEntry(createOrderEntry(Long.valueOf(QTY_CHARCOAL_BARBECUE), charcoalBarbecue))
				.withSourcingLocation(location1).withSourcingLocation(location2).withSourcingLocation(location3).build();
		...
		//OLINDA
		Assert.assertEquals(1,
				results.stream().filter(result -> result.getWarehouse().getCode().equals(Warehouse.OLINDA.name())).count());
		assertEqualsQuantityAllocation(results, Warehouse.OLINDA.name(), 1);

		//PAULISTA
		Assert.assertEquals(1,
				results.stream().filter(result -> result.getWarehouse().getCode().equals(Warehouse.PAULISTA.name())).count());
	...
	public void testNothingSourced()
	{
		...
		final SourcingLocation location1 = SourcingLocationBuilder.aSourcingLocation()
				.withCustomAvailabilityProductQuantity(propaneBarbecue, Long.valueOf(STOCK))
				.withWarehouseCode(Warehouse.RECIFE.name()).build();
