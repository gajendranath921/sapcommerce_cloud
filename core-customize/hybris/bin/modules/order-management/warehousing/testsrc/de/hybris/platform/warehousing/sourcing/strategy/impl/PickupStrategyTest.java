...
public class PickupStrategyTest {
	...
	public void setUp() throws Exception
	{
		...
		Mockito.lenient().when(cupEntry.getQuantity()).thenReturn(PRODUCT_QTY_CUP);
		when(cupEntry.getQuantityUnallocated()).thenReturn(PRODUCT_QTY_CUP);

		Mockito.lenient().when(teaEntry.getOrder()).thenReturn(order);
		when(teaEntry.getProduct()).thenReturn(tea);
		Mockito.lenient().when(teaEntry.getDeliveryPointOfService()).thenReturn(posMontreal);
		Mockito.lenient().when(teaEntry.getQuantity()).thenReturn(PRODUCT_QTY_TEA);
