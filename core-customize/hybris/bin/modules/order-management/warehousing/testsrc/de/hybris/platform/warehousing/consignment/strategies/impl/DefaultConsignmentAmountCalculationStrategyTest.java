...
public class DefaultConsignmentAmountCalculationStrategyTest
{
	...
	public void setUp()
	{
		...
		Mockito.lenient().when(orderEntry1.getBasePrice()).thenReturn(AMOUNT1.doubleValue());
		...
		Mockito.lenient().when(orderEntry1.getDiscountValues()).thenReturn(discountValues);
		...
		Mockito.lenient().when(orderEntry2.getBasePrice()).thenReturn(AMOUNT2.doubleValue());
		...
		Mockito.lenient().when(order.getPaymentTransactions()).thenReturn(paymentTransactions);
		...
		Mockito.lenient().when(order.getEntries()).thenReturn(orderEntryModels);
		when(order.getTotalTaxValues())
				.thenReturn(Collections.singletonList(new TaxValue("", DELIVERY_COST_TAX.doubleValue(), true, "USD")));

		Mockito.lenient().when(discountValue.getAppliedValue()).thenReturn(PRODUCT_LEVEL_DISCOUNT.doubleValue());
