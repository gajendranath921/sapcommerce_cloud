...
public class DefaultOrderOnHoldServiceTest
{
	...
	public void testPutOnHoldSuccessWithOneConsignment()
	{
		...
		assertEquals(ConsignmentStatus.CANCELLED, order.getConsignments().iterator().next().getStatus());
	}
	...
	public void testPutOnHoldSuccessWhenOneConsignmentIsAlreadyShipped()
	{
		...
		//Then
		assertEquals(ConsignmentStatus.CANCELLED, order.getConsignments().stream().filter(consignment -> consignment.getCode().equals(CONS_0_CODE))
				.findFirst().get().getStatus());
		assertEquals(ConsignmentStatus.SHIPPED, order.getConsignments().stream().filter(consignment -> consignment.getCode().equals(CONS_1_CODE))
				.findFirst().get().getStatus());
	}
	...
	public void testPutOnHoldWhenOrderIsNotCancellable()
	{
		...
		//Then
		assertEquals(ConsignmentStatus.SHIPPED, order.getConsignments().iterator().next().getStatus());
	}
}
