...
public class WarehousingAvailabilityCalculationStrategyTest
{
	...
	public void setUp() throws Exception
	{
		...
		Mockito.lenient().when(externalStock.getAvailable()).thenReturn(3);
		Mockito.lenient().when(returnStock.getAvailable()).thenReturn(1);
		Mockito.lenient().when(availableStock.getAvailable()).thenReturn(100);
	...
	{
		//Given
		Mockito.lenient().when(atpFormula.getAvailability()).thenReturn(false);
		when(atpFormula.getExternal()).thenReturn(false);
		when(atpFormula.getReturned()).thenReturn(false);
		//When
		final Map<String, Object> params = warehousingAvailabilityCalculationStrategy
	...
	public void getDefaultAtpFormulaWithNullBaseStoreInSession()
	{
		...
		Mockito.lenient().when(internalWarehouse.getBaseStores()).thenReturn(Collections.singletonList(baseStore));
