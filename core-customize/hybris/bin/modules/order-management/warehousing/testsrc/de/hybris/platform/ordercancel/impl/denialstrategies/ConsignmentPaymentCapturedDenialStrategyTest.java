...
public class ConsignmentPaymentCapturedDenialStrategyTest
{
	...
	public void shouldReturnOkWhenOneConsignmentIsCancellable()
	{
		...
		Mockito.lenient().when(consignmentModel2.getStatus()).thenReturn(NOT_CANCELLABLE_CONSIGNMENT_STATUS);
	...
	{
		//given
		Mockito.lenient().when(consignmentModel1.getStatus()).thenReturn(CANCELLABLE_CONSIGNMENT_STATUS);
		Mockito.lenient().when(consignmentModel2.getStatus()).thenReturn(CANCELLABLE_CONSIGNMENT_STATUS);
