...
public class AllocationEvaluatorTest
{
	...
	{
		// Given
		Mockito.lenient().when(entry1.getQuantity()).thenReturn(TEN);
		Mockito.lenient().when(entry1.getQuantityUnallocated()).thenReturn(TEN);

		Mockito.lenient().when(entry2.getQuantity()).thenReturn(FIVE);
		Mockito.lenient().when(entry2.getQuantityUnallocated()).thenReturn(FIVE);
	...
	public void shouldHandleLargeAvailability()
	{
		...
		Mockito.lenient().when(entry2.getQuantity()).thenReturn(FIFTEEN);
		Mockito.lenient().when(entry2.getQuantityUnallocated()).thenReturn(FIFTEEN);
