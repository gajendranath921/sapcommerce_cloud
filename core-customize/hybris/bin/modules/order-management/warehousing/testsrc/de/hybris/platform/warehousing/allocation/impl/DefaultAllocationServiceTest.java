...
public class DefaultAllocationServiceTest
{
	...
	public void setUp() throws Exception
	{
		...
		Mockito.lenient().when(modelService.create(AllocationEventModel.class)).thenReturn(new AllocationEventModel());
		...
		Mockito.lenient().when(consMouseEntry.getQuantityPending()).thenReturn(MOUSE_QTY);
		...
		Mockito.lenient().doReturn(Arrays.asList(allocationEventModel)).when(inventoryEventService).getAllocationEventsForConsignmentEntry(Mockito.any());

		Mockito.lenient().when(workflowTemplateService.getWorkflowTemplateForCode(anyString())).thenReturn(workflowTemplate);
		Mockito.lenient().when(workflowProcessingService.startWorkflow(workflow)).thenReturn(true);

		Mockito.lenient().when(workflowService.createWorkflow(any(), any(), any(), any())).thenReturn(workflow);
		Mockito.lenient().when(userSelectionStrategy.getUserForConsignmentAssignment(workflow)).thenReturn(author);
		when(configurationService.getConfiguration()).thenReturn(configuration);
		Mockito.lenient().when(configurationService.getConfiguration().getString("warehousing.consignment.workflow.template")).thenReturn("");
	...
	public void shouldReallocate_PartialConsignmentEntry_SecondDecline()
	{
		...
		Mockito.lenient().when(consHeadsetEntry.getQuantityPending()).thenReturn(HEADSET_QTY - 1);
	...
	public void shouldFail_manualReallocate_NoReallocationWarehouse()
	{
		...
		Mockito.lenient().when(stockLevelDao.findStockLevels(headset.getCode(), Collections.singletonList(montrealWarehouse)))
				.thenReturn(Collections.singletonList(stockLevel));
		// Second entry has no stock entry.
		Mockito.lenient().when(stockLevelDao.findStockLevels(mouse.getCode(), Collections.singletonList(montrealWarehouse)))
				.thenReturn(Collections.singletonList(stockLevel));
