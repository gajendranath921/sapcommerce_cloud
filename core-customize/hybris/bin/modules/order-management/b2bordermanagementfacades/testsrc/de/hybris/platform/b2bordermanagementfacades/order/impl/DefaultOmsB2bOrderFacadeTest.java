...
public class DefaultOmsB2bOrderFacadeTest extends BaseOrdermanagementFacadeTest
{
	...
	public void submitOrder_SuccessNullPaymentTransactionsPaymentInfo_withCostCenter()
	{
		prepareOrderRequestData();
		//When
		Mockito.lenient().when(orderRequestData.getPaymentTransactions().get(0).getPaymentInfo()).thenReturn(null);
		//then
		omsB2bOrderFacade.submitOrder(orderRequestData);
	}
	...
	public void submitB2cOrder_Success()
	{
		...
		//When
		when(orderRequestData.getCostCenterCode()).thenReturn(null);
		Mockito.lenient().when(customerData.getFirstName()).thenReturn(FIRST_NAME);
		Mockito.lenient().when(customerData.getLastName()).thenReturn(LAST_NAME);
		Mockito.lenient().when(userService.getUserForUID(USER_UID)).thenReturn(userModel);
		//Then
		omsB2bOrderFacade.submitOrder(orderRequestData);
	}
