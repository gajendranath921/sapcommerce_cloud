...
public class DefaultOmsWorkflowFacadeTest
{
	...
	public void setUp()
	{
		...
		Mockito.lenient().when(userService.getUserGroupForUID(USER_GROUP_UID)).thenReturn(userGroup);
		Mockito.lenient().when(userService.getAdminUserGroup()).thenReturn(adminUserGroup);
		...
		Mockito.lenient().when(workflow1.getActions()).thenReturn(singletonList(action1));
		Mockito.lenient().when(action1.getStatus()).thenReturn(WorkflowActionStatus.IN_PROGRESS);
	}
	...
	{
		//given
		Mockito.lenient().when(workflowService.getWorkflowForCode(anyString())).thenReturn(workflow1);
		Mockito.lenient().when(workflow1.getActions()).thenReturn(singletonList(action1));
		Mockito.lenient().when(action1.getStatus()).thenReturn(WorkflowActionStatus.IN_PROGRESS);
		Mockito.lenient().when(userService.getCurrentUser()).thenReturn(user);
		Mockito.lenient().when(action1.getDecisions()).thenReturn(singletonList(decision));
		Mockito.lenient().when(decision.getName()).thenReturn(WORKFLOW_DECISION_CODE);
	...
	{
		//given
		Mockito.lenient().when(workflowService.getWorkflowForCode(WORKFLOW_CODE_1)).thenThrow(UnknownIdentifierException.class);
		Mockito.lenient().when(workflowService.getWorkflowForCode(WORKFLOW_CODE_2)).thenReturn(workflow2);
		Mockito.lenient().when(workflow2.getActions()).thenReturn(singletonList(action2));
		Mockito.lenient().when(action2.getStatus()).thenReturn(WorkflowActionStatus.IN_PROGRESS);
		Mockito.lenient().when(userService.getCurrentUser()).thenReturn(user);
		Mockito.lenient().when(action2.getDecisions()).thenReturn(singletonList(decision));
		Mockito.lenient().when(decision.getName()).thenReturn(WORKFLOW_DECISION_CODE);
		final WorkflowCodesDataList workflowCodesDataList = new WorkflowCodesDataList();
		workflowCodesDataList.setCodes(WORKFLOW_CODES);
