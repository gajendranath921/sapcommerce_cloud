...
@IntegrationTest //TODO: Revert back to @UnitTest after CLC-2121
...
public class BaseOrdermanagementFacadeTest
{
	...
	{
		Mockito.lenient().when(modelService.create(CustomerModel.class)).thenReturn(customerModel);
		Mockito.lenient().when(orderRequestReverseConverter.convert(orderRequestData)).thenReturn(orderModel);
		Mockito.lenient().when(orderProcessModel.getCode()).thenReturn(ORDER_PROCESS);
		Mockito.lenient().when(baseSiteService.getBaseSiteForUID(SITE_UID)).thenReturn(baseSiteModel);
		Mockito.lenient().when(baseStoreService.getBaseStoreForUid(STORE_UID)).thenReturn(baseStoreModel);
		Mockito.lenient().when(baseSiteModel.getStores()).thenReturn(Arrays.asList(baseStoreModel));
		when(orderGenericDao.find(anyMap())).thenReturn(Arrays.asList(orderModel));
		Mockito.lenient().when(orderModel.getCode()).thenReturn(ORDER_ID);
	...
	protected void prepareOrderRequestData()
	{
		...
		// Order
		Mockito.lenient().when(orderModel.getStatus()).thenReturn(OrderStatus.WAIT_FRAUD_MANUAL_CHECK);
		Mockito.lenient().when(orderModel.getOrderProcess()).thenReturn(Arrays.asList(orderProcessModel));
		Mockito.lenient().when(orderModel.getCode()).thenReturn(ORDER_ID);
		...
		// Transactions
		Mockito.lenient().when(paymentTransactionData.getPaymentInfo()).thenReturn(ccPaymentInfoData);
		...
		Mockito.lenient().when(baseStoreModel.getSubmitOrderProcessCode()).thenReturn(ORDER_PROCESS);
