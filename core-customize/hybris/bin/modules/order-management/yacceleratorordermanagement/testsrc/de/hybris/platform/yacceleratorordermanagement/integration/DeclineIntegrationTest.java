...
public class DeclineIntegrationTest extends BaseAcceleratorSourcingIntegrationTest
{
  ...
  @Ignore("flakey tests")
	public void shouldAutoReallocate_SingleEntry_PartiallyFailReSourced() throws InterruptedException
	{
		...
		assertEquals(Long.valueOf(1L), ((OrderEntryModel) order.getEntries().iterator().next()).getQuantityUnallocated());
	}
  ...
  @Ignore("flakey tests")
	public void shouldAutoReallocate_SingleEntry_FailReSourced__OutOfStock_ResetInventory() throws InterruptedException
	{
		...
		//then verify the new consignment
		modelService.refresh(order);
		assertEquals(Long.valueOf(2L), ((OrderEntryModel) order.getEntries().iterator().next()).getQuantityUnallocated());
		assertEquals(2, order.getConsignments().size());
  ...
  @Ignore("flakey tests")
	public void shouldAutoDecline1Entry_MultiEntries_FailReSourced() throws InterruptedException
	{
		...
		assertEquals(Long.valueOf(3L), ((OrderEntryModel) order.getEntries().stream().filter(e -> e.getProduct().getCode().equals(CAMERA_CODE)).findFirst()
				.get()).getQuantityUnallocated());
	}
