...
public class CancellationIntegrationTest extends BaseAcceleratorSourcingIntegrationTest
{
	...
	public void shouldCancelEntireOrderSuccess_SingleEntry() throws InterruptedException, OrderCancelException
	{
		...
		final Double priceBeforeCancel = order.getConsignments().stream().skip(0).findFirst().get().getConsignmentEntries().stream().findFirst().get().getOrderEntry().getTotalPrice();
		...
		//then verify the sourceOrderEntry and original total price
		assertTrue(order.getConsignments().stream().findFirst().get().getConsignmentEntries().stream().anyMatch(
				consignmentEntry -> consignmentEntry.getSourceOrderEntry()!= null));
		assertEquals(priceBeforeCancel, order.getConsignments().stream().findFirst().get().getConsignmentEntries().stream().findFirst().get().getSourceOrderEntry().getTotalPrice());
	...
	public void shouldCancelEntireOrderSuccess_MultiEntry_SingleConsignment() throws InterruptedException, OrderCancelException
	{
		...
		final OrderProcessModel orderProcessModel = sourcingUtil.runOrderProcessForOrderBasedPriority(order, OrderStatus.READY);
		final Double memoryCardPriceBeforeCancel = order.getEntries().stream().filter(entry -> entry.getProduct().getCode().equals(MEMORY_CARD_CODE)).findFirst()
				.get().getTotalPrice();
		final Double cameraPriceBeforeCancel = order.getEntries().stream().filter(entry -> entry.getProduct().getCode().equals(CAMERA_CODE)).findFirst()
				.get().getTotalPrice();
		...
		assertTrue(order.getConsignments().stream().findFirst().get().getConsignmentEntries().stream().anyMatch(
				consignmentEntry -> consignmentEntry.getSourceOrderEntry()!= null));
		assertEquals(memoryCardPriceBeforeCancel, order.getConsignments().stream().findFirst().get().getConsignmentEntries().stream().filter(
				e -> e.getOrderEntry().getProduct().getCode().equals(MEMORY_CARD_CODE)).findFirst().get().getSourceOrderEntry().getTotalPrice());
		assertEquals(cameraPriceBeforeCancel, order.getConsignments().stream().findFirst().get().getConsignmentEntries().stream().filter(
				e -> e.getOrderEntry().getProduct().getCode().equals(CAMERA_CODE)).findFirst().get().getSourceOrderEntry().getTotalPrice());
	...
	public void shouldCancelPartiallyOrderSuccess_SingleEntry() throws InterruptedException, OrderCancelException
	{
		...
		final Double priceBeforeCancel = order.getConsignments().stream().skip(0).findFirst().get().getConsignmentEntries()
				.stream().findFirst().get().getOrderEntry().getTotalPrice();
		...
		//then verify the SourceOrderEntry in consignmentEntry
		assertTrue(order.getConsignments().stream().findFirst().get().getConsignmentEntries().stream().anyMatch(
				consignmentEntry -> consignmentEntry.getSourceOrderEntry()!= null));
		assertEquals(priceBeforeCancel, order.getConsignments().stream().findFirst().get().getConsignmentEntries()
				.stream().findFirst().get().getSourceOrderEntry().getTotalPrice());
	...
	public void shouldCancelPartiallyOrderSuccess_MultiEntry_SingleConsignment_MultiTimes() throws InterruptedException, OrderCancelException
	{
		...
		order = sourcingUtil.createCameraAndMemoryCardShippingOrder();
		final OrderProcessModel orderProcessModel = sourcingUtil.runOrderProcessForOrderBasedPriority(order, OrderStatus.READY);
		Double priceBeforeCancel = order.getEntries().stream().filter(entry -> entry.getProduct().getCode().equals(MEMORY_CARD_CODE)).findFirst()
				.get().getTotalPrice();

		//when cancel the order
		cancellationEntryInfo.put(order.getEntries().stream().filter(entry ->
				entry.getProduct().getCode().equals(MEMORY_CARD_CODE)).findFirst().get(), Long.valueOf(1L));
		cancellationUtil.cancelOrder(order, cancellationEntryInfo, CancelReason.LATEDELIVERY);
		for (final ConsignmentModel consignment : order.getConsignments())
		{
			sourcingUtil.waitUntilConsignmentProcessIsNotRunning(orderProcessModel, consignment, timeOut);
		...
		assertTrue(order.getConsignments().stream().findFirst().get().getConsignmentEntries().stream().anyMatch(
				consignmentEntry -> consignmentEntry.getSourceOrderEntry()!= null));
		assertEquals(priceBeforeCancel, order.getConsignments().stream().findFirst().get().getConsignmentEntries().stream().filter(
				e -> e.getOrderEntry().getProduct().getCode().equals(MEMORY_CARD_CODE)).findFirst().get().getSourceOrderEntry().getTotalPrice());
		priceBeforeCancel = order.getConsignments().stream().findFirst().get().getConsignmentEntries().stream().filter(
				e -> e.getOrderEntry().getProduct().getCode().equals(MEMORY_CARD_CODE)).findFirst().get().getOrderEntry().getTotalPrice();

		//when cancel second time
		sourcingUtil.refreshOrder(order);
		assertTrue(cancellationUtil.isCancelPossible(order, true, true));
		order.getEntries().forEach(e -> cancellationEntryInfo2.put(e, e.getQuantity()));
		...
		//then verify the sourceOrderEntry and original total price second time
		assertTrue(order.getConsignments().stream().skip(1).findFirst().get().getConsignmentEntries().stream().anyMatch(
				consignmentEntry -> consignmentEntry.getSourceOrderEntry()!= null));
		assertEquals(priceBeforeCancel, order.getConsignments().stream().skip(1).findFirst().get().getConsignmentEntries().stream().filter(
				e -> e.getOrderEntry().getProduct().getCode().equals(MEMORY_CARD_CODE)).findFirst().get().getSourceOrderEntry().getTotalPrice());
	}
	...
	public void shouldCancelEntirePartiallyCancelledOrder_singleEntry() throws InterruptedException, OrderCancelException
	{
		...
		final ConsignmentModel consignmentResult_Montreal = order.getConsignments().stream()
				.filter(e -> e.getWarehouse().getCode().equals(CODE_MONTREAL)).findFirst().get();
		Double priceBeforeCancel = order.getConsignments().stream().skip(0).findFirst().get().getConsignmentEntries()
				.stream().findFirst().get().getOrderEntry().getTotalPrice();
		...
		assertEquals(priceBeforeCancel, order.getConsignments().stream().findFirst().get().getConsignmentEntries()
				.stream().findFirst().get().getSourceOrderEntry().getTotalPrice());
		priceBeforeCancel = order.getConsignments().stream().skip(1).findFirst().get().getConsignmentEntries()
				.stream().findFirst().get().getOrderEntry().getTotalPrice();
		...
		assertTrue(order.getConsignments().stream().skip(1).findFirst().get().getConsignmentEntries().stream().anyMatch(
				consignmentEntry -> consignmentEntry.getSourceOrderEntry()!= null));
		assertEquals(priceBeforeCancel, order.getConsignments().stream().skip(1).findFirst().get().getConsignmentEntries()
				.stream().findFirst().get().getSourceOrderEntry().getTotalPrice());
	...
	public void shouldCancelOrderSuccess_MultiOrderEntry_OMSE_1446() throws InterruptedException, OrderCancelException
	{
		...
		assertEquals(ProcessState.WAITING, orderProcessModel.getProcessState());
