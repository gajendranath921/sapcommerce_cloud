...
public class CreateReturnEventListener extends AbstractSiteEventListener<CreateReturnEvent>
{
	...
	protected void onSiteEvent(final CreateReturnEvent event)
	{
		...
		if (store == null)
		{
			LOG.info("Unable to start return process for return request [{}]. Store not set on Order linked to the return request and no current base store defined in session.", returnRequest.getCode());
		}
		else
		{
			...
			{
				LOG.info("Unable to start return process for return request [{}]. Store [{}] has missing CreateReturnProcessCode", returnRequest.getCode(), store.getUid());
			}
			else
			{
