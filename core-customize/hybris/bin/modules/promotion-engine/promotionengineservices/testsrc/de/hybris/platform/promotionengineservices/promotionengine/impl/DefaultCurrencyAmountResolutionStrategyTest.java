...
public class DefaultCurrencyAmountResolutionStrategyTest
{
	...
	public void setUp()
	{
		...
		Mockito.lenient().when(data.getValue()).thenReturn(entries);
		Mockito.lenient().when(data.getType()).thenReturn("type");
		Mockito.lenient().when(data.getUuid()).thenReturn("uuid");
		Mockito.lenient().when(promotionResultUtils.getOrder(promotionResult)).thenReturn(order);
	}
	...
	{
		Mockito.lenient().when(data.getValue()).thenReturn("WRONG_VALUE_TYPE");
	...
	public void testValueSimpleResolutionUSD()
	{
		...
		Mockito.lenient().when(currency.getIsocode()).thenReturn("USD");
		Mockito.lenient().when(currency.getSymbol()).thenReturn("$");
	...
	public void testValueSimpleResolutionJPY()
	{
		...
		Mockito.lenient().when(currency.getIsocode()).thenReturn("JPY");
		Mockito.lenient().when(currency.getDigits()).thenReturn(Integer.valueOf(0));
	...
	public void testParameterToReplaceWrongValue()
	{
		...
		//when
		Mockito.lenient().when(mockedParamToReplace.getValue()).thenReturn(new Object());
