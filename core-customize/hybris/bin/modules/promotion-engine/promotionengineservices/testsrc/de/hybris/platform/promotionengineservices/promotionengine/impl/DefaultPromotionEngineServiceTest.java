...
@IntegrationTest
public class DefaultPromotionEngineServiceTest extends PromotionEngineServiceBaseTestBase
{
	...
	{
		final String fileName = "orderPercentageDiscount";
		...
		// change the rule...
		final String fileNameChanged = "orderPercentageDiscount1";
		rule.setRuleContent(readRuleFile(fileNameChanged + ".drl", "/promotionengineservices/test/rules/"));
		modelService.save(rule);
		initializeRuleEngine(rule);
	...
	{
		final String fileName = "percentageDiscountCameraAccessories.drl";
	...
	{
		final String fileName = "orderPercentageDiscount";
		final AbstractRuleEngineRuleModel rule = getRuleForFile(
				fileName + ".drl",
				"/promotionengineservices/test/rules/");
		rule.setCode(fileName);
		ruleEngineTestSupportService
	...
	public void testMultiplePromotionGroups() throws IOException
	{
		...
		Assert.assertEquals("raoPromotion03.drl",resultList1.get(0).getFiredRuleCode());
		...
		Assert.assertEquals("raoPromotion04.drl",resultList3.get(0).getFiredRuleCode());
	}
	...
	public void testNoCatalogVersionPromotionFromSession() throws IOException
	{
		...
		// evaluate promotions for cart:
		final CartModel cart = buildCartForUserWithCodeAndCurrency("ABC", null);
