...
public class DefaultProductConditionResolutionStrategyUnitTest
{
	...
	@Test
	public void shouldDoNothingOnGetAndStoreWhenOperatorIsNotContains()
	{
		final RuleParameterData parameter = new RuleParameterData();
		parameter.setValue(Arrays.asList("111", "222"));

		final RuleParameterData operator_parameter = new RuleParameterData();
		operator_parameter.setValue(CollectionOperator.NOT_CONTAINS);
		...
		parameters.put(PRODUCTS_OPERATOR_PARAM, operator_parameter);
		...
		verify(modelService, times(0)).create(eq(ProductForPromotionSourceRuleModel.class));
		verify(modelService, times(0)).save(anyObject());
	}
	@Test
	public void shouldDoNothingOnGetAndStoreWhenExcludeFromStorefrontDisplayIsTrue()
	{
		final PromotionSourceRuleModel rule = mock(PromotionSourceRuleModel.class);
		when(rule.getExcludeFromStorefrontDisplay()).thenReturn(true);

		final RuleConditionData ruleConditionData = mock(RuleConditionData.class);
		final RuleBasedPromotionModel promotion = mock(RuleBasedPromotionModel.class);

		strategy.getAndStoreParameterValues(ruleConditionData, rule, promotion);

		verifyZeroInteractions(modelService, ruleConditionData, promotion);
	}
	...
	public void testGetAndStoreParameterValue()
	{
		...
		final RuleParameterData operator_parameter = new RuleParameterData();
		operator_parameter.setValue(CollectionOperator.CONTAINS_ANY);
	...
	public void shouldCleanStoredParameterValuesForContext()
	{
		...
		Mockito.lenient().when(promotionSourceRuleDao.findAllProductForPromotionSourceRule(rule,BASE_NAME)).thenReturn(productsForRule);
