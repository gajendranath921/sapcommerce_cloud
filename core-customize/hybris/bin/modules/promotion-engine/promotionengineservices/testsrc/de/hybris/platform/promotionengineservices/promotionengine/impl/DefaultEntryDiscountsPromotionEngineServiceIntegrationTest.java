...
@IntegrationTest
public class DefaultEntryDiscountsPromotionEngineServiceIntegrationTest extends PromotionEngineServiceBaseTestBase
{
	...
	private static String TEST_DEFAULT_PRODUCT_CODE = "HW1210-3411fer";
	...
	public void testBasePriceDiscountsRowWithNoPromotionsEvaluated() throws Exception
	{
		...
		CartModel cart = buildCartForUserWithCodeAndCurrency("5678"+UUID.randomUUID().toString().substring(1,8), "testproduct1");
		...
		for(final AbstractOrderEntryModel entry: cart.getEntries()) {
			...
			entry.setOrder(cart);
			final List<DiscountValue> entryDiscounts = new ArrayList<>();
			final DiscountValue discountValue1 = new DiscountValue("test10-10", 10.0d, false, 0.0d, null, false);
		...
		final String fileName = "basePriceWithDiscountRow";
		final AbstractRuleEngineRuleModel rule = getRuleForFile(fileName + ".drl",
				"/promotionengineservices/test/rules/");
		rule.setCode(fileName);
		rule.setVersion(valueOf(0L));
		ruleEngineTestSupportService
				.decorateRuleForTest(ImmutableMap.of("ruleOrderEntryPercentageDiscountAction", "ruleOrderEntryPercentageDiscountAction"))
				.accept(rule);
		modelService.save(rule);
		final PromotionGroupModel group = promotionDao.findPromotionGroupByCode("promoGroup1");
		final Collection<PromotionGroupModel> groupList = new ArrayList<PromotionGroupModel>();
		...
		assertEquals(0, cart.getAllPromotionResults().size());
	...
	public void testBasePriceDiscountsRowWithonePromotionBeEvaluated() throws Exception
	{
		...
		CartModel cart = buildCartForUserWithCodeAndCurrency("test5678"+UUID.randomUUID().toString().substring(1,8), "testproduct1");
		...
		for(final AbstractOrderEntryModel entry: cart.getEntries()) {
			...
			entryTaxes.add(taxValue);
			entry.setTaxValues(entryTaxes);
			entry.setBasePrice(Double.valueOf(110.0d));
			entry.setQuantity(Long.valueOf(1));
			entry.setOrder(cart);
			final List<DiscountValue> entryDiscounts = new ArrayList<>();
			final DiscountValue discountValue1 = new DiscountValue("test10-10", 10.0d, false, 0.0d, null, false);
		...
		final String fileName = "basePriceWithDiscountRowAndPromotion";
		final AbstractRuleEngineRuleModel rule = getRuleForFile(fileName + ".drl",
				"/promotionengineservices/test/rules/");
		rule.setCode(fileName);
		rule.setVersion(valueOf(0L));
		...
		assertEquals(1, cart.getAllPromotionResults().size());
