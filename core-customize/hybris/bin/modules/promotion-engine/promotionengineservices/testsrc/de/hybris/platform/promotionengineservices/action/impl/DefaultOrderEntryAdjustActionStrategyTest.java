...
public class DefaultOrderEntryAdjustActionStrategyTest
{
	...
	public void setUp()
	{
		...
		Mockito.lenient().when(promotionActionService.getOrderEntry(discountRao)).thenReturn(orderEntry);
	...
	{
		Mockito.lenient().when(promotionActionService.getOrderEntry(any(DiscountRAO.class))).thenReturn(null);
	...
	{
		Mockito.lenient().when(promotionActionService.createPromotionResult(discountRao)).thenReturn(null);
		final List result = defaultOrderEntryAdjustActionStrategy.apply(discountRao);
		assertTrue(result.isEmpty());
	}
	...
	{
		Mockito.lenient().when(orderEntry.getOrder()).thenReturn(null);
		final List result = defaultOrderEntryAdjustActionStrategy.apply(discountRao);
		assertTrue(result.isEmpty());
	}
	...
	{
		Mockito.lenient().when(orderEntry.getOrder()).thenReturn(cart);
		Mockito.lenient().when(modelService.create(promotionAction)).thenReturn(ruleBasedOrderEntryAdjustAction);
		Mockito.lenient().when(Boolean.valueOf(discountRao.isPerUnit())).thenReturn(Boolean.FALSE);
	...
	{
		Mockito.lenient().when(modelService.create(promotionAction)).thenReturn(new RuleBasedOrderEntryAdjustActionModel());
		Mockito.lenient().when(orderEntry.getEntryNumber()).thenReturn(Integer.valueOf(0));
		Mockito.lenient().when(orderEntry.getProduct()).thenReturn(product);
