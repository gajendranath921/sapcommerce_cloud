...
public class ProductMetadataPopulatorTest
{
	...
	private static final String FIELD_NAME = "FIELD_NAME_en";
	...
	public void setUp()
	{
		...
		final HashMap<String, String> merchProperties = new HashMap<>();
		merchProperties.put(FIELD_NAME, FIELD_NAME);
		merchProperties.put(NAME_KEY, NAME_KEY);
		merchProperties.put(SUMMARY_KEY, SUMMARY_KEY);
		merchProperties.put(DESCRIPTION_KEY, DESCRIPTION_KEY);

		when(source.getIndexedPropertiesMapping()).thenReturn(map);
		when(source.getMerchPropertiesMapping()).thenReturn(merchProperties);
