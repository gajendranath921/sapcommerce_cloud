...
public class MerchIndexingListener implements IndexerBatchListener, BeanFactoryAware, IndexerListener
{
	...
	public void afterBatch(final IndexerBatchContext batchContext) throws IndexerException
	{
		...
		if (isMerchIndexingEnabled(merchIdxConf))
		{
			...
			final Map<String, IndexedPropertyInfo> indexedPropertiesMapping = createIndexedPropertiesMapping(batchContext,
					searchQuery);
			final Map<String, String> merchPropertiesMapping = createMerchPropertiesMapping(
					merchIndexingConfig.getMerchProperties(), indexedPropertiesMapping);
			final Map<String, String> merchImagePropertiesMapping = createMerchImagePropertiesMapping(
					merchIndexingConfig.getMerchImageProperties(), indexedPropertiesMapping);
			final Map<String, FacetField> merchFacetPropertiesMapping = createMerchFacetPropertiesMapping(batchContext,
					searchQuery, indexedPropertiesMapping);
			...
			for (final InputDocument document : documents)
			{
				if (isToSynchronize(catalogVersionsToExport, document))
				{
					...
					populateMerchProperties(product, document, merchPropertiesMapping, indexedPropertiesMapping,
							merchIndexingConfig);
					populateMerchImagesProperties(product, document, merchImagePropertiesMapping, indexedPropertiesMapping,
							merchIndexingConfig);
					populateMerchFacetProperties(product, document, merchFacetPropertiesMapping, indexedPropertiesMapping,
							searchQuery);
	 ...
	 */
	protected SearchQuery createSearchQuery(final IndexerBatchContext batchContext,
	                                        final MerchIndexingConfigModel merchIndexingConfig) throws IndexerException
	{
	                                                                ...
	                                                                final Map<String, IndexedPropertyInfo> indexedPropertiesMapping)
	{
		return merchImageProperties.stream()
		                           .filter(merchProp -> indexedPropertiesMapping.containsKey(
				                           merchProp.getIndexedProperty().getName()))
		                           .collect(Collectors.toMap(this::extractPropTranslatedName, this::extractPropName));
	}
	                                       ...
	                                       final MerchIndexingConfigModel indexConfig)
	{
		...
		{
			final IndexedPropertyInfo indexedPropertyInfo = indexedPropertiesMapping.get(merchPropEntry.getValue());
			final String translatedFieldName = indexedPropertyInfo.getTranslatedFieldName();
			final String merchKey = merchPropEntry.getKey();
			if (merchPropEntry.getValue().equals(URL_KEY))
			{
				final String basePageUrl = StringUtils.isNotEmpty(
						indexConfig.getBaseProductPageUrl()) ? indexConfig.getBaseProductPageUrl() : "";
	                                             ...
	                                             final MerchIndexingConfigModel merchIndexingConfig)
	{
		...
		final String baseImageUrl = StringUtils.isNotEmpty(
				merchIndexingConfig.getBaseImageUrl()) ? merchIndexingConfig.getBaseImageUrl() : "";
	                                            ...
	                                            final SearchQuery searchQuery)
	{
		...
		for (final FacetField facetField : merchFacetPropertiesMapping.values())
		{
			...
			facetFieldMap.put(FACET_VALUES_KEY,
					createMerchFacetValues(facetField, searchQuery, indexedProperty, document, indexedPropertyInfo));
	...
	protected List<Map<Object, Object>> createMerchFacetValues(final FacetField facet, final SearchQuery query,
	                                                           ...
	                                                           final IndexedPropertyInfo indexedPropertyInfo)
	{
	...
	private static class IndexedPropertyInfo
	{
		...
		private final String translatedFieldName;
