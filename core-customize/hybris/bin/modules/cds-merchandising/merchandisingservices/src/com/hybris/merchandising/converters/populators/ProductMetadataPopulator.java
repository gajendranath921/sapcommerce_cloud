...
public class ProductMetadataPopulator implements Populator<ProductIndexContainer, Product>
{
	...
	public void populate(final ProductIndexContainer source, final Product target) throws ConversionException
	{
		...
		final Map<String, ProductMetadata> metadata = source.getSearchQuery()
		                                                    .getFacetSearchConfig()
		                                                    .getIndexConfig()
		                                                    .getLanguages()
		                                                    .stream()
		                                                    .map(language -> Pair.of(language.getIsocode(),
				                                                    buildMetaData(source, language)))
	...
	{
		return Optional.ofNullable(source.getMerchPropertiesMapping().entrySet().stream()
		                                 .filter(entry -> entry.getKey().equals(fieldName))
		                                 .filter(Objects::nonNull)
		                                 .collect(Collectors.toMap(Map.Entry::getKey, entrySetValue ->
				                                 source.getIndexedPropertiesMapping().entrySet().stream()
				                                       .filter(entrySet -> entrySet.getKey().equals(entrySetValue.getValue()))
				                                       .filter(Objects::nonNull)
				                                       .flatMap(e -> e.getValue().getTranslatedFieldNames().stream())
				                                       .filter(translatedName -> language.getIsocode()
				                                                                         .equals(extractLanguage(translatedName)))
				                                       .map(nameField -> (String) source.getInputDocument()
				                                                                        .getFieldValue(nameField))
				                                       .filter(Objects::nonNull)
				                                       .findFirst().orElse(StringUtils.EMPTY)))
		                                 .get(fieldName)).orElse(StringUtils.EMPTY);
	...
	protected ProductMetadata buildMetaData(final ProductIndexContainer source, final LanguageModel language)
	{
		...
		Optional.ofNullable(extractFieldValue(source, language, NAME_KEY))
		        .ifPresent(name -> productMetadata.setName(sanitiseField(name)));
		Optional.ofNullable(extractFieldValue(source, language, SUMMARY_KEY))
		        .ifPresent(summary -> productMetadata.setSummary(sanitiseField(summary)));
		Optional.ofNullable(extractFieldValue(source, language, DESCRIPTION_KEY))
	...
	{
		return Arrays.stream(translatedField.split("_"))
		             .filter(ISO_LANGUAGES::contains)
		             .findFirst().orElse(StringUtils.EMPTY);
	}
}
