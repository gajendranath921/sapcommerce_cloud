...
public class ProductImagePopulator implements Populator<ProductIndexContainer, Product>
{
	                                                ...
	                                                final String propertyName)
	{
		return source.getMerchPropertiesMapping().entrySet().stream()
		             .filter(entry -> propertyName.equals(entry.getKey()))
		             .map(requiredEntry -> source.getIndexedPropertiesMapping().get(requiredEntry.getValue()))
		             .filter(Objects::nonNull)
		             .map(indexedPropertyInfo -> {
			             final String imageUrl = (String) source.getInputDocument()
			                                                    .getFieldValue(indexedPropertyInfo.getTranslatedFieldName());
