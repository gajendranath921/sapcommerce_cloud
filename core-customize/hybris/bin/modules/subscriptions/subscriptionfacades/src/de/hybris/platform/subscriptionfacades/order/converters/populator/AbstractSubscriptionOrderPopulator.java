		...
		extends AbstractOrderPopulator<SOURCE, TARGET>
{
	...
	protected Double getDeliveryCost(final AbstractOrderModel source)
	{
		...
		for (final AbstractOrderModel orderModel : Optional.ofNullable(source.getChildren()).orElse(Collections.emptyList()))
		{
	...
	public List<OrderPriceData> buildOrderPrices(final AbstractOrderModel source)
	{
		...
		abstractOrders.addAll(Optional.ofNullable(source.getChildren()).orElse(Collections.emptyList()));
	...
	{
		final List<AbstractOrderModel> children = new ArrayList<>(Optional.ofNullable(source.getChildren()).orElse(Collections.emptyList()));
			...
			final List<PromotionResult> promotionsResultList)
	{
		...
		{
			if (CollectionUtils.isEmpty(promotionResult.getConsumedEntries()))
			{
				continue;
			}
			...
			for (final PromotionOrderEntryConsumedData consumedEntry : promotionResult.getConsumedEntries())
			{
				try
				{
					consumedEntry.setOrderEntryNumber(getMasterOrderEntryNo(abstractOrder, consumedEntry));
				}
				catch (final UnknownIdentifierException ignored)
				{
					// ignoring exception
					LOG.debug("Unable to map the consumed order entry number in child cart to the master cart entry number.");
				}
			}
		}
	...
	protected Integer getMasterOrderEntryNo(final AbstractOrderModel abstractOrder,
			final PromotionOrderEntryConsumedData consumedEntry)
	{
		final Integer masterOrderEntryNo;
		if (abstractOrder instanceof CartModel)
		{
			...
			masterOrderEntryNo = cartEntry.getMasterEntry() == null ? cartEntry.getEntryNumber() : cartEntry.getMasterEntry()
					.getEntryNumber();
		}
		else
		{
			final OrderEntryModel orderEntry = getOrderService().getEntryForNumber((OrderModel) abstractOrder,
					consumedEntry.getOrderEntryNumber());
			masterOrderEntryNo = orderEntry.getMasterEntry() == null ? orderEntry.getEntryNumber() : orderEntry.getMasterEntry()
					.getEntryNumber();
		}
		return masterOrderEntryNo;
	}
