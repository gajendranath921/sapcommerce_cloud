...
public class B2BDocumentsSecureMediaFilter extends GenericFilterBean
{
	...
	protected boolean checkLoginUserB2BUnitHasAccessToMedia(final MediaModel mediaModel, final PK loginUserB2BUnitPK)
	{
		...
		if (CollectionUtils.isNotEmpty(documents))
		{
			...
			for (final PrincipalGroupModel principalGroupModel : filterB2BUnits(listByMedia))
			{
	...
	protected Set<PrincipalGroupModel> filterB2BUnits(final Set<PrincipalGroupModel> principalGroupModels)
	{
		return principalGroupModels.stream().filter(principalGroupModel -> principalGroupModel instanceof B2BUnitModel)
