...
public class DefaultConsentFacadeTest
{
	...
	public void setup()
	{
		lenient().doReturn(customerModel).when(userService).getCurrentUser();
	...
	public void testGiveConsent()
	{
		...
		lenient().doReturn(version).when(consentTemplateModel1).getVersion();
	...
	{
		lenient().doReturn(consentModel).when(commerceConsentService).getConsent(consentId1);
