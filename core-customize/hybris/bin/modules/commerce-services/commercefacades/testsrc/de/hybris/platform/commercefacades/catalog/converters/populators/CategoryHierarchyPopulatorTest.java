...
public class CategoryHierarchyPopulatorTest
{
	...
	@Mock
	private ProductBasicPopulator productPrimaryImagePopulator;
	@InjectMocks
	...
	public void testWithProductsOption()
	{
		...
		lenient().when(categoryModel.getProducts()).thenReturn(Lists.newArrayList(mockProduct));
		BDDMockito.when(productService.getProductsForCategory(categoryModel, 0, Integer.MAX_VALUE))
		...
		lenient().when(mockSubcategory.getCategories()).thenReturn(Lists.newArrayList(mockSubcategoryOfSubcategory));
		lenient().when(mockSubcategoryOfSubcategory.getProducts()).thenReturn(Lists.newArrayList(mockProduct));
		lenient().when(categoryModel.getCategories()).thenReturn(Lists.newArrayList(mockSubcategory));
