...
public class SearchResultProductConverterTest
{
	...
	private static final String CONFIGURATOR_TYPE = "my configurator type";
	...
	@Test
	public void testGenericConfigurationAttributes()
	{
		final SearchResultValueData searchResultValueData = mock(SearchResultValueData.class);

		final Map<String, Object> map = new HashMap<String, Object>();
		map.put("configurable", true);
		map.put("configuratorType", CONFIGURATOR_TYPE);

		given(searchResultValueData.getValues()).willReturn(map);

		final ProductData productData = searchResultProductConverter.convert(searchResultValueData);
		assertTrue(productData.getConfigurable());
		assertEquals(CONFIGURATOR_TYPE, productData.getConfiguratorType());
	}

	@Test
	public void testConvertPriceFromPriceValue()
	{
		final SearchResultValueData searchResultValueData = mock(SearchResultValueData.class);
	...
	@Test
	public void testConvertPriceFromPrice()
	{
		final SearchResultValueData searchResultValueData = mock(SearchResultValueData.class);

		final Map<String, Object> map = new HashMap<>();
		map.put("price", Double.valueOf(9.99));
