...
public class GroupOrderConsignmentEntryPopulatorTest extends GroupOrderEntryPopulatorTest
{
	...
	protected EntryGroupData wrapIntoEntryGroup(final OrderEntryData entry, final int groupNumber)
	{
		...
		lenient().when(entryGroupUtils.getGroup(any(), eq(Integer.valueOf(groupNumber)))).thenReturn(group);
		when(entryGroupUtils.getGroup(nullable(AbstractOrderData.class), argThat((Collection<Integer> c) -> c != null && c.contains(Integer.valueOf(groupNumber))), nullable(GroupType.class)))
				.thenReturn(group);
		group.setOrderEntries(Collections.singletonList(entry));
		return group;
	}
