...
public class ProductVariantOptionDataPopulatorTest
{
	...
	public void setup()
	{
		...
		}).when(variantOptionDataPopulator)
				.populate(any(VariantProductModel.class), any(VariantOptionData.class), nullable(List.class));
	...
	protected void validateVariants(final List<VariantMatrixElementData> variantMatrix, final int level)
	{
		...
		for (int valueIndex = 0; valueIndex < variantMatrix.size(); valueIndex++)
		{
			...
			if (!element.getIsLeaf())
			{
				...
				{
					if (child.getVariantOption().getCode().equals(parentVariantOption.getCode()) && child.getVariantOption()
							.getVariantOptionQualifiers().equals(parentVariantOption.getVariantOptionQualifiers()) && child
							.getVariantOption().getUrl().equals(parentVariantOption.getUrl()))
					{
						count++;
					}
				}
