...
public class DefaultCartValidationStrategy implements CartValidationStrategy, CartCleanStrategy
{
	...
	{
		return cartEntryModel.getProductInfos().stream().filter(item -> item != null)
				.map(AbstractOrderEntryProductInfoModel::getProductInfoStatus).filter(item -> ProductInfoStatus.ERROR.equals(item))
				.findAny().isPresent();
	}
