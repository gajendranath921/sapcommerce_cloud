...
public class DefaultCommerceUpdateCartEntryStrategyTest
{
	...
	{
		given(entryMergeStrategy.getEntryToMerge(anyListOf(AbstractOrderEntryModel.class), nullable(AbstractOrderEntryModel.class)))
				.willReturn(null);
		given(Boolean.valueOf(commerceStockService.isStockSystemEnabled(nullable(BaseStoreModel.class)))).willReturn(Boolean.TRUE);
		given(commerceStockService.getStockLevelForProductAndPointOfService(nullable(ProductModel.class),
				nullable(PointOfServiceModel.class))).willReturn(Long.valueOf(1));
