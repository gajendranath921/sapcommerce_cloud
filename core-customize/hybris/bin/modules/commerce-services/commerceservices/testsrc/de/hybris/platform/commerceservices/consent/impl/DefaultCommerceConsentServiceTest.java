...
public class DefaultCommerceConsentServiceTest
{
	...
	public void setup()
	{
		...
		lenient().doReturn(consentTemplateModel).when(existingConsentModel).getConsentTemplate();
	...
	{
		lenient().doReturn(null).when(existingConsentModel).getConsentWithdrawnDate();
		doReturn(customerModel).when(existingConsentModel).getCustomer();
		doReturn("customerUid").when(customerModel).getUid();
