...
public class SampleDataImportServiceTest
{
	...
	public void prepare()
	{
		...
		lenient().when(sampleDataImportService.getCatalogVersionService()).thenReturn(catalogVersionService);
		lenient().when(sampleDataImportService.getEventService()).thenReturn(eventService);
		lenient().when(sampleDataImportService.getConfigurationService()).thenReturn(configurationService);
	...
	{
		order.verify(setupImpexService).importImpexFile(
				String.format("/%s/import/sampledata/stores/%s/searchservices.impex", TEST_EXTENSION_NAME, TEST_STORE_NAME), false);
	}
