...
public class DefaultCommerceCheckoutServiceTest
{
	...
	@Spy
	private final DefaultGenerateMerchantTransactionCodeStrategy generateMerchantTransactionCodeStrategy = new DefaultGenerateMerchantTransactionCodeStrategy();
	...
	{
		commerceDeliveryModeValidationStrategy.setDeliveryService(deliveryService);
		commerceDeliveryModeValidationStrategy.setCommerceCartCalculationStrategy(commerceCartCalculationStrategy);
		commerceDeliveryModeValidationStrategy.setModelService(modelService);
		...
		given(paymentService.authorize(anyString(), any(BigDecimal.class), any(Currency.class), any(AddressModel.class),
	...
	public void testValidateDeliveryModeInvalid()
	{
		...
		given(deliveryService.getSupportedDeliveryModeListForOrder(cartModel))
				.willReturn(Collections.<DeliveryModeModel> emptyList());
	...
	{
		// mock shipping countries
		final Collection<CountryModel> shippingCountries = new ArrayList<>();
		final CountryModel shippingCountry = mock(CountryModel.class);
		shippingCountries.add(shippingCountry);
		given(storeModel.getDeliveryCountries()).willReturn(shippingCountries);

		// mock billing countries
		final Collection<CountryModel> billingCountries = new ArrayList<>();
		final CountryModel billingCountry = mock(CountryModel.class);
		billingCountries.add(billingCountry);
		given(storeModel.getBillingCountries()).willReturn(billingCountries);

		final List<CountryModel> countries = commerceCheckoutService.getCountries(CountryType.SHIPPING);
		Assert.assertNotNull(countries);
		Assert.assertEquals(shippingCountries, countries);
	}
	...
	public void shouldGetAllCountriesIfNoShippingCountries()
	{
		...
		// mock all countries
		final List<CountryModel> allCountries = new ArrayList<>();
		final CountryModel country = mock(CountryModel.class);
		allCountries.add(country);
		given(commonI18NService.getAllCountries()).willReturn(allCountries);

		final List<CountryModel> countries = commerceCheckoutService.getCountries(CountryType.SHIPPING);
		Assert.assertNotNull(countries);
		Assert.assertEquals(allCountries, countries);
	}
	...
	public void shouldGetBillingCountries()
	{
		...
		final List<CountryModel> countries = commerceCheckoutService.getCountries(CountryType.BILLING);
		Assert.assertNotNull(countries);
		Assert.assertEquals(billingCountries, countries);
	}
	...
	public void shouldGetCountries()
	{
		...
		final List<CountryModel> allCountries = commerceCheckoutService.getCountries(null);
		Assert.assertNotNull(allCountries);
		Assert.assertEquals(countries, allCountries);
	}
}
