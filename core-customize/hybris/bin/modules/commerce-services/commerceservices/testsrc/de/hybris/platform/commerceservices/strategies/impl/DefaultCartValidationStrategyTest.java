...
public class DefaultCartValidationStrategyTest
{
	...
	public void setUp()
	{
		...
		given(cartEntry.getQuantity()).willReturn(Long.valueOf(20L));
	...
	{
		given(productService.getProductForCode(cartEntry.getProduct().getCode()))
				.willThrow(new UnknownIdentifierException("Product is unavailable."));
	...
	{
		final AbstractOrderEntryProductInfoModel infoModel = Mockito.mock(AbstractOrderEntryProductInfoModel.class);
	...
	public void testNoConfigurationErrors()
	{
		...
		given(productService.getProductForCode(cartEntry.getProduct().getCode())).willReturn(product);
		given(commerceStockService.getStockLevelForProductAndBaseStore(product, store)).willReturn(Long.valueOf(22L));
