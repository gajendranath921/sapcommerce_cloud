...
public class CoreDataImportServiceTest
{
	...
	public void prepare()
	{
		...
		lenient().when(coreDataImportService.getCatalogVersionService()).thenReturn(catalogVersionService);
		lenient().when(coreDataImportService.getEventService()).thenReturn(eventService);
	...
	public void testImportExecution()
	{
		...
		verifyImportSearchServicesIndex(order);
	...
	private void verifyImportSearchServicesIndex(final InOrder order)
	{
		order.verify(setupImpexService).importImpexFile(
				String.format("/%s/import/coredata/stores/%s/searchservices.impex", TEST_EXTENSION_NAME, TEST_STORE_NAME), false);

		order.verify(setupImpexService).importImpexFile(
				String.format("/%s/import/coredata/stores/%s/searchservicestrigger.impex", TEST_EXTENSION_NAME, TEST_STORE_NAME), false);
	}
}
