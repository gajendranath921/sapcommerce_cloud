...
public class ProductPricesValueResolverTest extends AbstractValueResolverTest
{
	...
	public void setUp()
	{
		...
		lenient().when(eurCurrency.getName()).thenReturn(EUR_CURRENCY_NAME);
		when(eurCurrency.getIsocode()).thenReturn(EUR_CURRENCY_ISOCODE);
		lenient().when(usdCurrency.getName()).thenReturn(USD_CURRENCY_NAME);
		...
		lenient().when(Boolean.valueOf(getQualifierProvider().canApply(any(IndexedProperty.class)))).thenReturn(Boolean.TRUE);
