...
class ExportTest extends AbstractSpockFlowTest {
	...
	static final NUMBER_OF_ALL_PRODUCTS = 898
	...
	def "Trusted client exports all products paged when requiresAuthentication true: #format"() {
		given: "a trusted client"
		authorizeTrustedClient(restClient)

		when: "client requests an export of all products"
		HttpResponseDecorator response = restClient.get(
				path: getBasePath() + SECURE_BASE_SITE + '/export/products',
				contentType: format,
				query: ['pageSize': PAGE_SIZE],
				requestContentType: URLENC
		)
