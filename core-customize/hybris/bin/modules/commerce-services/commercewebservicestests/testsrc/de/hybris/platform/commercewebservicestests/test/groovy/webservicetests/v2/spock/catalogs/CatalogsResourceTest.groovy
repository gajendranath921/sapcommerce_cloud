 ...
 */
package de.hybris.platform.commercewebservicestests.test.groovy.webservicetests.v2.spock.catalogs
...
class CatalogsResourceTest extends AbstractSpockTest {
	...
	def "Anonymous get category should be failed when requiresAuthentication true: #format"() {
		when:
		HttpResponseDecorator response = restClient.get(
				path: getBasePath() + SECURE_BASE_SITE + '/catalogs/wsTestProductCatalog/Online/categories/brands',
				contentType: format
		)
	...
	def "Anonymous get catalog version should be failed when requiresAuthentication true: #format"() {
		when:
		HttpResponseDecorator response = restClient.get(
				path: getBasePath() + SECURE_BASE_SITE + '/catalogs/wsTestProductCatalog/Online',
				contentType: format
		)
	...
	def "Anonymous get catalogs should be failed when requiresAuthentication true: #format"() {
		when:
		HttpResponseDecorator response = restClient.get(path: getBasePath() + SECURE_BASE_SITE + '/catalogs', contentType: format)
