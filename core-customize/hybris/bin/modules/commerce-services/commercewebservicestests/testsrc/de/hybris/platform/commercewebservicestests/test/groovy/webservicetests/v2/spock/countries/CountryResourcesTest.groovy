 ...
 */
package de.hybris.platform.commercewebservicestests.test.groovy.webservicetests.v2.spock.countries
...
class CountryResourcesTest extends AbstractUserTest {
	...
	def "Client retrieves all countries when requiresAuthentication true: #format"() {

		when:
		HttpResponseDecorator response = restClient.get(path: getBasePath() + SECURE_BASE_SITE +'/countries', contentType: format)
	...
	def "Fetch the list of country's regions when requiresAuthentication true: #format"() {

		when: "anonymous (not logged in) user wants to fetch the list of country's regions"
		HttpResponseDecorator response = restClient.get(
				path: getBasePath() + SECURE_BASE_SITE + '/countries/CA/regions',
				contentType: format,
				requestContentType: URLENC
		)
	...
	def "Client retrieves available shipping countries when requiresAuthentication true: #format"() {

		when:
		HttpResponseDecorator response = restClient.get(path: getBasePath() + SECURE_BASE_SITE + '/countries', contentType: format, query: ['type': "shipping"])
	...
	def "Client retrieves available billing countries when requiresAuthentication true: #format"() {

		when:
		HttpResponseDecorator response = restClient.get(path: getBasePath() + SECURE_BASE_SITE + '/countries', contentType: format, query: ['type': "billing"])
