 ...
 */
package de.hybris.platform.commercewebservicestests.test.groovy.webservicetests.v2.spock.consents
...
class ConsentResourcesTest extends AbstractUserTest {
	...
	def "Withdraw consent using the given consent code for a non anonymous user: #format"() {
		given: "a trusted client"
		def customer = registerCustomerWithTrustedClient(restClient, format, getBasePathWithSite())
	...
	def "Give consent to a withdrawn user consent and fetch the user consent details through the get user consent api : #format"() {
		...
		// sleep between consent creation because givenConsent date must differ by minimum one second
		sleep 1000
	...
	def "Fetch the list of consent templates for anonymous user when requiresAuthentication true: #format"() {
		given: "a trusted client"
		authorizeClient(restClient)

		when: "anonymous (not logged in) user wants to fetch the list of consent templates"
		HttpResponseDecorator response = restClient.get(
				path: getBasePath() + SECURE_BASE_SITE + '/users/anonymous/consenttemplates',
				contentType: format,
				requestContentType: URLENC
		)
		...
		with(response) {
			if (isNotEmpty(data) && isNotEmpty(data.errors)) println(data)
			status == SC_OK
			data.consentTemplates.size() == 1
			data.consentTemplates[0].id == 'CONSENT_TEMPLATE_6'
			data.consentTemplates[0].name == 'Newsletter Subscription Consent 6'
			data.consentTemplates[0].description == 'I approve to use my personal data for receiving e-mail newsletters for Marketing campaigns.'
			isEmpty(data.consentTemplates[0].consent)
		}
	...
	def "Fetch the exposed consent template for anonymous user when requiresAuthentication true: #format"() {
		given: "a trusted client"
		authorizeClient(restClient)

		when: "anonymous (not logged in) user wants to fetch the consent template"
		HttpResponseDecorator response = restClient.get(
				path: getBasePath() + SECURE_BASE_SITE + '/users/anonymous/consenttemplates/CONSENT_TEMPLATE_6',
				contentType: format,
				requestContentType: URLENC
		)
		...
		with(response) {
			if (isNotEmpty(data) && isNotEmpty(data.errors)) println(data)
			status == SC_OK
			data.id == 'CONSENT_TEMPLATE_6'
			data.name == 'Newsletter Subscription Consent 6'
			data.description == 'I approve to use my personal data for receiving e-mail newsletters for Marketing campaigns.'
			isEmpty(data.consent)
		}
	...
	def "Fetch the not exposed consent template for anonymous user when requiresAuthentication true: #format"() {
		given: "a trusted client"
		authorizeClient(restClient)

		when: "anonymous (not logged in) user wants to fetch the consent template"
		HttpResponseDecorator response = restClient.get(
				path: getBasePath() + SECURE_BASE_SITE + '/users/anonymous/consenttemplates/CONSENT_TEMPLATE_7',
				contentType: format,
				requestContentType: URLENC
		)
