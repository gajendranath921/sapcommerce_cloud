 ...
 */
package de.hybris.platform.commercewebservicestests.test.groovy.webservicetests.v2.spock.promotions
...
class PromotionsTest extends AbstractSpockFlowTest {
	...
	def "Anonymous get all promotions with trusted client should failed when requiresAuthentication true: #format"() {
		given: "authorized trusted client"
		authorizeTrustedClient(restClient)

		when: "client requests all promotions"
		HttpResponseDecorator response = restClient.get(
				path: getBasePath() + SECURE_BASE_SITE + '/promotions',
				query: ['type': ALL_PROMOTIONS],
				contentType: format,
				requestContentType: URLENC)
	...
	def "Anonymous get promotion by code with trusted client should failed when requiresAuthentication true : #format"() {
		given: "authorized trusted client"
		authorizeTrustedClient(restClient)

		when: "requests specific promotion"
		HttpResponseDecorator response = restClient.get(
				path: getBasePath() + SECURE_BASE_SITE + '/promotions/' + PROMOTION_CODE,
				contentType: format,
				query: ['fields': FIELD_SET_LEVEL_FULL],
				requestContentType: URLENC)
