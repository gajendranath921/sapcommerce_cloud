...
class CartResourceTest extends AbstractCartTest {
	...
	def "Anonymous user creates a cart should be failed when requiresAuthentication true: #format"() {
		given: "a trusted client"
		authorizeTrustedClient(restClient)

		when: "anonymous (not logged in) user wants to create a cart for himself"
		HttpResponseDecorator response = restClient.post(
				path: getBasePath() + SECURE_BASE_SITE +'/users/anonymous/carts',
				contentType: format,
				requestContentType: URLENC
		)
