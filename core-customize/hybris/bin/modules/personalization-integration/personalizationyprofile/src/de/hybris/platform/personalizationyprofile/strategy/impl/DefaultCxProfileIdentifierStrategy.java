...
public class DefaultCxProfileIdentifierStrategy implements CxProfileIdentifierStrategy
{
	...
	public String getProfileIdentifier(final UserModel user)
	{
		...
		{
			profileId = getProfileIdentifierFromConsentReference();
			if (profileId != null)
			{
				storeProfileIdentifierInSession(user, profileId);
			}
	...
	{
		final boolean consentGiven = consentService.isProfileTrackingConsentGiven();
		if (consentGiven)
		{
			return true;
		}
		...
		try
		{
			final ConsentTemplateData template = commerceConsentFacade
					.getLatestConsentTemplate(ProfileservicesConstants.PROFILE_CONSENT);
			if (template == null)
			{
	...
	private boolean getIgnoreConsentCheckWhenNoConsentTemplate()
	{
		return configurationService.getConfiguration()
				.getBoolean(PersonalizationservicesConstants.IGNORE_CONSENT_CHECK_WHEN_NO_CONSENT_TEMPLATE, Boolean.TRUE)
				.booleanValue();
	}
	...
	protected ConsentFacade getCommerceConsentFacade()
	{
		return commerceConsentFacade;
	}

	@Required
	public void setCommerceConsentFacade(final ConsentFacade commerceConsentFacade)
	{
		this.commerceConsentFacade = commerceConsentFacade;
	}
}
