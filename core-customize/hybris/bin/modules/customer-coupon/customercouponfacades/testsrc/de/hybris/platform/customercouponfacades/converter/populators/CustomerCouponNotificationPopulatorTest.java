...
public class CustomerCouponNotificationPopulatorTest
{
	...
	public void setup()
	{
		...
		Mockito.lenient().when(notificationModel.getCustomerCoupon()).thenReturn(couponModel);
		Mockito.lenient().when(notificationModel.getCustomer()).thenReturn(customerModel);
		Mockito.lenient().when(notificationModel.getStatus()).thenReturn(CouponNotificationStatus.EFFECTIVESENT);
		Mockito.lenient().when(customerConverter.convert(customerModel)).thenReturn(customerData);
		Mockito.lenient().when(customerCouponConverter.convert(couponModel)).thenReturn(couponData);
	}
