...
public class DefaultCustomerCouponServiceTest extends ServicelayerTransactionalTest
{
	...
	private static final String SITE_UID = "testSite";
	...
	public void prepare() throws ImpExException
	{
		...
		baseSiteService.setCurrentBaseSite(SITE_UID, false);
	...
	{
		final CouponNotificationModel notification = customerCouponService.saveCouponNotification(COUPON_ID).get();

		Assert.assertEquals(COUPON_ID, notification.getCustomerCoupon().getCouponId());
		Assert.assertEquals(CUSTOMER_UID, notification.getCustomer().getUid());
	...
	public void testGetCouponCodeForPromotionSourceRule()
	{
		...
		Assert.assertEquals(COUPON_ID_2, customerCoupons.get(0));
	}
	...
	public void testCountProductOrCategoryForPromotionSourceRule()
	{
		...
		Assert.assertEquals(NUMS, lengths);
	}
	...
	{
		customerCouponService.setUserService(mockedUserService);
		final CustomerModel customer = new CustomerModel();
		customer.setCustomerID("keenreviewer1@hybris.com");
		Mockito.doReturn(customer).when(mockedUserService).getCurrentUser();
		...
		Assert.assertFalse(isNotificationOn);
	}
	...
	public void testGetPromotionSourcesRuleForProductCategoriesNon()
	{
		...
		final List<PromotionSourceRuleModel> promotionList = customerCouponService.getPromotionSourcesRuleForProductCategories(product);
		Assert.assertEquals(0, promotionList.size());
	}
