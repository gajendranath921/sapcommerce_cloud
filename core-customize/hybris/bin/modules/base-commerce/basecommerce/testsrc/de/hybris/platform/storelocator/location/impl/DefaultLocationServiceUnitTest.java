...
public class DefaultLocationServiceUnitTest
{
			...
			GeoLocatorException
	{
		...
		Mockito.lenient().when(posOne.getName()).thenReturn("one");

		final PointOfServiceModel posTwo = Mockito.mock(PointOfServiceModel.class);
		Mockito.lenient().when(posTwo.getName()).thenReturn("two");

		final PointOfServiceModel posThree = Mockito.mock(PointOfServiceModel.class);
		Mockito.lenient().when(posThree.getName()).thenReturn("three");
		...
		Mockito.lenient().when(pointOfServiceDao.getAllGeocodedPOS(gps, distance, store)).thenReturn(
			...
			LocationInstantiationException
	{
		final PointOfServiceModel posOne = Mockito.mock(PointOfServiceModel.class);
		Mockito.lenient().when(posOne.getName()).thenReturn("one");
		Mockito.lenient().when(posOne.getLatitude()).thenReturn(null);
		Mockito.lenient().when(posOne.getLongitude()).thenReturn(Double.valueOf(45));
		...
		Mockito.lenient().when(pointOfServiceDao.getAllGeocodedPOS(gps, distance, store)).thenReturn(Arrays.asList(posOne));
		try
		{
			...
			PointOfServiceDaoException, LocationInstantiationException
	{
		final PointOfServiceModel posOne = Mockito.mock(PointOfServiceModel.class);
		Mockito.lenient().when(posOne.getName()).thenReturn("one");
		Mockito.lenient().when(posOne.getLatitude()).thenReturn(Double.valueOf(45));
		Mockito.lenient().when(posOne.getLongitude()).thenReturn(null);
