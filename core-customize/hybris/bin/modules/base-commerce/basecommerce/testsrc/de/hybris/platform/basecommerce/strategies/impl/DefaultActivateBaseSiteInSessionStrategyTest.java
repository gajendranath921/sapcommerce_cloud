...
public class DefaultActivateBaseSiteInSessionStrategyTest
{
	...
	public void testShouldThrowAnExceptionForFewStoresForSite()
	{
		...
		Mockito.doThrow(expected).when(catalogVersionService)
				.setSessionCatalogVersions(ArgumentMatchers.argThat(new ArgumentMatcher<Collection>()
				{
					@Override
					public boolean matches(final Collection argument)
					{
						return argument.size() == 2 && //
								argument.contains(catalogVersionOne) && //
								argument.contains(catalogVersionTwo);
					}
				}));
