...
public class CalculateWithPromotionsActionTest extends AbstractActionUnitTest<CalculateWithPromotionsAction>
{
	...
	public void setUp()
	{
		...
		Mockito.lenient().when(abstractOrderModel.getEntries()).thenReturn(new ArrayList());
		Mockito.lenient().when(abstractOrderModel.getCurrency()).thenReturn(currency);
		Mockito.lenient().when(actionContext.getParameter(com.hybris.cockpitng.actions.ActionContext.PARENT_WIDGET_MODEL)).thenReturn(widgetModel);
		Mockito.lenient().when(actionContext.getData()).thenReturn(abstractOrderModel);
		...
		Mockito.lenient().when(promotionsService.updatePromotions(anyCollection(), eq(abstractOrderModel)))
				.thenReturn(promotionOrderResults);
		Mockito.lenient().when(userService.getCurrentUser()).thenReturn(user);
		Mockito.lenient().when(sessionService.executeInLocalView(sessionExecutionBodyArgumentCaptor.capture())).thenAnswer(new Answer<Object>()
		{
			@Override
		...
		Mockito.lenient().when(promotionResultService.getFiredOrderPromotions(any(), any())).thenReturn(results);
	}
