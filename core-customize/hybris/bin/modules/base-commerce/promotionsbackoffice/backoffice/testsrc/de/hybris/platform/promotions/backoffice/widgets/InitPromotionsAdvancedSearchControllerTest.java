...
public class InitPromotionsAdvancedSearchControllerTest extends AbstractWidgetUnitTest<InitPromotionsAdvancedSearchController>
{
	...
	public void testCreateAdvancedSearchInitContext() throws Exception
	{
		...
		Mockito.lenient().when(navigationNode.getId()).thenReturn(InitPromotionsAdvancedSearchController.ABSTRACT_PROMOTION_NAVIGATION_NODE);
		Mockito.lenient().when(getTypeFacade().load(InitPromotionsAdvancedSearchController.ABSTRACT_PROMOTION_TYPE))
				.thenReturn(new Builder(InitPromotionsAdvancedSearchController.ABSTRACT_PROMOTION_TYPE).build());
		Mockito.lenient().when(Boolean.valueOf(permissionFacade.canReadType(InitPromotionsAdvancedSearchController.ABSTRACT_PROMOTION_TYPE)))
				.thenReturn(Boolean.TRUE);
		Mockito.lenient().when(advancedSearch.getFieldList()).thenReturn(fieldListType);
		Mockito.lenient().when(widgetInstanceManager.loadConfiguration(nullable(ConfigContext.class), eq(AdvancedSearch.class)))
	...
	private void assertThatInitContextSend()
	{
		verify(widgetInstanceManager, times(1)).sendOutput(eq(InitPromotionsAdvancedSearchController.SOCKET_OUT_CONTEXT),
				nullable(AdvancedSearchInitContext.class));
	}
	...
	{
		// Mockito.lenient().when
		Mockito.lenient().when(navigationNode.getId()).thenReturn("anotherNavigationNodeId");
