...
{
	private static final Logger LOG = LoggerFactory.getLogger(AbstractVJDBCServicelayerTransactionalTest.class.getName());
	...
	private Connection testVJDBCLogin(final Properties props) throws Exception
	{
		final String vjdbcId = Registry.getMasterTenant().getConfig().getParameter(VJDBC_ID);
		final int tomcatPort = Registry.getMasterTenant().getConfig().getInt("tomcat.ssl.port", 9002);
		Class.forName(VJDBC_DRIVER_CLASS).newInstance();

		final String url = "https://localhost:" + tomcatPort + "/virtualjdbc/service?tenant="
