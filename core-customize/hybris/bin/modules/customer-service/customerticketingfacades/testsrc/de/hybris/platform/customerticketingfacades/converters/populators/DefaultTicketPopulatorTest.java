...
public class DefaultTicketPopulatorTest extends ServicelayerTransactionalTest
{
	...
	public void setup()
	{
		...
		Mockito.lenient().when(customerModel.getUid()).thenReturn(customerUid);

		Mockito.lenient().when(csTicketModel.getOrder()).thenReturn(abstractOrderModel);
		Mockito.lenient().when(abstractOrderModel.getCode()).thenReturn(orderCode);
		Mockito.lenient().when(abstractOrderModel.getItemtype()).thenReturn(orderItemType);
		Mockito.lenient().when(abstractOrderModel.getModifiedtime()).thenReturn(today);
		...
		Mockito.lenient().when(csTicketEventOneModel.getTicket()).thenReturn(csTicketModel);
		Mockito.lenient().when(csTicketEventTwoModel.getTicket()).thenReturn(csTicketModel);
	...
	public void shouldPopulateTicketData()
	{
		...
		Mockito.lenient().when(customerModel.getName()).thenReturn(CUSTOMER_NAME);
		...
		assertEquals(TicketCategory.ENQUIRY, ticketData.getTicketCategory());
	}
	...
	{
		final EmployeeModel agent = Mockito.mock(EmployeeModel.class);
		Mockito.lenient().when(csTicketModel.getCreationtime()).thenReturn(yesterday);
		Mockito.when(csTicketEventOneModel.getText()).thenReturn(TEXT1);
		Mockito.when(csTicketEventOneModel.getCreationtime()).thenReturn(today);
		Mockito.when(csTicketEventOneModel.getAuthor()).thenReturn(agent);
		Mockito.when(csTicketEventTwoModel.getEntries()).thenReturn(Collections.emptySet());
		...
		Mockito.lenient().when(ticketService.getEventsForTicket(csTicketModel))
