...
{
	private static final int NR_OF_TRIGGER_DAYS_IN_WEEK = 2;
	...
	public void testWeekly()
	{
		...
		Assert.assertEquals(startDate, triggerModel.getActivationTime());
		Assert.assertEquals(NR_OF_TRIGGER_DAYS_IN_WEEK, triggerModel.getDaysOfWeek().size());
		Assert.assertEquals(DayOfWeek.MONDAY, triggerModel.getDaysOfWeek().stream().filter(dow -> dow.equals(DayOfWeek.MONDAY)).findFirst().orElse(null));
		Assert.assertEquals(DayOfWeek.WEDNESDAY, triggerModel.getDaysOfWeek().stream().filter(dow -> dow.equals(DayOfWeek.WEDNESDAY)).findFirst().orElse(null));
