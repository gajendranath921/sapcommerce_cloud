...
public class BundleUpdateCartEntryHook implements CommerceUpdateCartEntryHook
{
	...
	public void afterUpdateCartEntry(final CommerceCartParameter parameter, final CommerceCartModification result)
	{
		...
		final EntryGroup entryGroup = getBundleTemplateService()
				.getBundleEntryGroup(parameter.getCart(), parameter.getEntryGroupNumbers());
		if (entryGroup == null)
		{
			// not a bundle entry
			return;
		...
		if (CommerceCartModificationStatus.MAX_BUNDLE_SELECTION_CRITERIA_LIMIT_EXCEEDED.equals(parameter.getModificationStatus()) && (
				StringUtils.isEmpty(result.getStatusCode()) || CommerceCartModificationStatus.SUCCESS.equals(result.getStatusCode())))
		{
			result.setStatusCode(parameter.getModificationStatus());
		}
	}
			...
			@Nonnull final CommerceCartParameter parameter, @Nonnull final BundleTemplateModel component)
	{
		...
		final long otherQuantity = getTotalOtherProductQuantityInBundle(parameter);
		if (maxAllowedQuantity < parameter.getQuantity() + otherQuantity)
		{
			LOG.info("The requested quantity was decreased due to a selection criterion");
			final long newQuantity = maxAllowedQuantity - otherQuantity;
			parameter.setQuantity(newQuantity > 0 ? newQuantity : 1);
			parameter.setModificationStatus(CommerceCartModificationStatus.MAX_BUNDLE_SELECTION_CRITERIA_LIMIT_EXCEEDED);
		}
	}

	protected long getTotalOtherProductQuantityInBundle(@Nonnull final CommerceCartParameter parameter)
	{
		...
		return parameter.getCart().getEntries().stream()
				...
				.filter(e -> e.getEntryNumber() != parameter.getEntryNumber())
