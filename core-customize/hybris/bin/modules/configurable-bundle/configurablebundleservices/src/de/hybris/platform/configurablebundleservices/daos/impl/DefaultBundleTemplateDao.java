...
public class DefaultBundleTemplateDao extends AbstractItemDao implements BundleTemplateDao
{
	...
	private static  final String JOIN_BUNDLE_WITH_PRODUCT = "SELECT {bt:pk} FROM {" + BundleTemplateModel._TYPECODE  + " as bt "
			+ " JOIN ProductsBundleTemplatesRelation as rel on {rel.target} = {bt.pk}"
			+ " JOIN Product as p on {rel.source} = {p.pk}}";

	private static final String BUNDLE_WITH_PRODUCT_STATUS = JOIN_BUNDLE_WITH_PRODUCT
			+ " WHERE {bt:" + BundleTemplateModel.PARENTTEMPLATE + "} IS NOT NULL"
			+ " AND {bt:" + BundleTemplateModel.STATUS + "} = ?status"
			+ " AND {p:pk}=?product"
			+ ORDER_BY_ID;
	...
	{
		final FlexibleSearchQuery query = new FlexibleSearchQuery(BUNDLE_WITH_PRODUCT_STATUS);
		query.addQueryParameter(BundleTemplateModel.STATUS, statusModel);
		query.addQueryParameter("product", productModel);
		try {
			final SearchResult<BundleTemplateModel> result = search(query);
			return result.getResult().stream().collect(Collectors.toList());
		} catch (IllegalStateException ise)
		{
			// throw exception if product code is invalid, see gettingComponentsByProductShouldHandleNonPersistentProduct
			return Collections.emptyList();
		}
	}
