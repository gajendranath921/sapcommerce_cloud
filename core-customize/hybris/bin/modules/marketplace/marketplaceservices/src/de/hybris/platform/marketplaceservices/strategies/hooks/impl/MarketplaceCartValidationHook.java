 ...
 */
package de.hybris.platform.marketplaceservices.strategies.hooks.impl;
...
public class MarketplaceCartValidationHook implements CartValidationHook
{
	...
	public void afterValidateCart(final CommerceCartParameter parameter, final List<CommerceCartModification> modifications)
	{
		...
		modifications.stream().filter(modification -> !(modification.getEntry() != null && modification.getEntry().getProduct() != null && modification.getEntry().getProduct().getSaleable().booleanValue()))
				.forEach(modification -> {
					modification.setStatusCode(CommerceCartModificationStatus.UNAVAILABLE);
					final CartEntryModel entry = new CartEntryModel();
					final ProductModel product = modification.getEntry() != null ? modification.getEntry().getProduct() : null;
