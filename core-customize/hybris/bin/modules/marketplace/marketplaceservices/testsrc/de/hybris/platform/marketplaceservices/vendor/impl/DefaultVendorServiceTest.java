...
public class DefaultVendorServiceTest
{
	...
	{
		Mockito.lenient().doReturn(Optional.of(vendor)).when(vendorDao).findVendorByCode(any());
	...
	{
		Mockito.doNothing().when(vendorService.getVendorActivationStrategy()).activateVendor(any());
	...
	{
		Mockito.doNothing().when(vendorService.getVendorDeactivationStrategy()).deactivateVendor(any());
