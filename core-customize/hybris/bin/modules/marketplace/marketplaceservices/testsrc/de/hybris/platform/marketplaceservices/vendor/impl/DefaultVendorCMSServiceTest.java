...
public class DefaultVendorCMSServiceTest
{
	...
	public void prepare()
	{
		...
		Mockito.lenient().when(catalogVersionService.getCatalogVersion(catalogId, targetVersionId)).thenReturn(targetVersion);
		...
		Mockito.lenient().when(targetToSourceJob.getTargetVersion()).thenReturn(sourceVersion);
		...
		Mockito.lenient().when(wrongJob2.getSourceVersion()).thenReturn(sourceVersion);
		Mockito.lenient().when(wrongJob2.getTargetVersion()).thenReturn(sourceVersion);

		Mockito.lenient().when(wrongJob3.getSourceVersion()).thenReturn(targetVersion);
		Mockito.lenient().when(wrongJob3.getTargetVersion()).thenReturn(targetVersion);

		Mockito.lenient().when(platformSynchronizationStatusService.getInboundSynchronizations(carousel)).thenReturn(
		...
		Mockito.lenient().when(carousel.getPk()).thenReturn(carouselPK);

		when(syncItemInfo.getSyncStatus()).thenReturn(NOT_SYNC);
		Mockito.lenient().when(syncItemInfo.getSyncTimestampPk()).thenReturn(lastSyncTimePK);
		Mockito.lenient().when(modelService.get(lastSyncTimePK)).thenReturn(timeStamp);
		Mockito.lenient().when(timeStamp.getLastSyncTime()).thenReturn(lastSyncTime);

		when(platformSynchronizationStatusService.getSyncInfo(carousel, sourceToTargetJob)).thenReturn(syncItemInfo);
		Mockito.lenient().when(platformSynchronizationStatusService.getSyncInfo(carousel, targetToSourceJob)).thenReturn(syncItemInfo);
	...
	public void testGetPageTemplateByIdAndCatalogVersion()
	{
		...
		Mockito.doReturn(pageTemplates).when(cmsPageTemplateDao).findPageTemplatesByIdAndCatalogVersion(anyString(),
	...
	public void testGetPageByIdAndCatalogVersion()
	{
		...
		Mockito.doReturn(pages).when(cmsPageDao).findPagesByIdAndCatalogVersion(anyString(),
				any(CatalogVersionModel.class));
	...
	public void testGetRestrictionByIdAndCatalogVersion()
	{
		...
		Mockito.doReturn(restrictions).when(cmsRestrictionDao).findRestrictionsById(anyString(),
				any(CatalogVersionModel.class));
	...
	public void testGetContentSlotRelationByIdAndPositionInPage()
	{
		...
		Mockito.doReturn(contentSlotRelations).when(cmsContentSlotDao).findContentSlotRelationsByPageAndPosition(
				nullable(AbstractPageModel.class), nullable(String.class), nullable(CatalogVersionModel.class));
	...
	public void testGetContentSlotByIdAndCatalogVersion()
	{
		...
		Mockito.doReturn(contentSlots).when(cmsContentSlotDao).findContentSlotsByIdAndCatalogVersions(anyString(),
				any());
	...
	public void testGetCMSComponentByIdAndCatalogVersion()
	{
		...
		Mockito.doReturn(components).when(cmsComponentDao).findCMSComponentsByIdAndCatalogVersion(anyString(),
				any());
