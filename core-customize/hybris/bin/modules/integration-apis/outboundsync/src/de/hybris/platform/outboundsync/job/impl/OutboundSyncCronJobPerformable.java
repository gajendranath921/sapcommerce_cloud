...
public class OutboundSyncCronJobPerformable extends AbstractJobPerformable<OutboundSyncCronJobModel>
{
	...
	private static String logMessage(final CronJobModel model, final String msg)
	{
	...
	public PerformResult perform(final OutboundSyncCronJobModel cronJob)
	{
		...
		try
		{
			...
			eventService.publishEvent(new StartedOutboundSyncEvent(cronJob.getPk(), Date.from(startTime), processedCnt));
			final PerformResult result = job.getFinalState().asPerformResult();
	...
	{
		final var streamCfg = getStreamConfiguration(deltaStream);
		final CountingChangesCollector collector = createCollector(cronJob, deltaStream);
	...
	private CountingChangesCollector createCollector(final OutboundSyncCronJobModel jobModel,
	                                                 final OutboundSyncStreamConfigurationModel streamModel)
	{
		return changesCollectorFactory.createCountingCollector(jobModel, streamModel);
	}
	...
	protected StreamConfiguration getStreamConfiguration(final OutboundSyncStreamConfigurationModel deltaStream)
	{
		return StreamConfiguration.buildFor(deltaStream.getStreamId())
		                          ...
		                          .withExcludedTypeCodes(deltaStream.getExcludedTypes());
	}
