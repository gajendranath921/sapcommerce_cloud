...
{
	private static final Logger LOG = Log.getLogger(DefaultResponseEntityExtractor.class);
	...
	{
		return parseErrorsFrom(singleResponse.getHeader(CONTENT_TYPE), Integer.valueOf(singleResponse.getStatusCode()),
				singleResponse.getBody());
	}

	protected InboundRequestErrorModel parseResponseErrors(final ODataResponse oDataResponse)
	{
		return parseErrorsFrom(oDataResponse.getContentHeader(), oDataResponse.getStatus().getStatusCode(),
				getBodyAsString(oDataResponse));
	}
	...
	{
		return applyExtractors(singleResponse.getHeader(CONTENT_TYPE), Integer.valueOf(singleResponse.getStatusCode()),
				singleResponse.getBody());
	}

	protected String extractIntegrationKey(final ODataResponse oDataResponse)
	{
		return applyExtractors(oDataResponse.getContentHeader(), oDataResponse.getStatus().getStatusCode(),
				getBodyAsString(oDataResponse));
	}
	...
	protected String getBodyAsString(final ODataResponse oDataResponse)
	{
		...
		{
			LOG.error("Error when trying to parse response body to String.", e);
	...
	protected List<BatchSingleResponse> parseBatchResponses(final ODataResponse response)
	{
		...
		{
			LOG.error("Error when trying to parse ODataResponse: {}", response);
	...
	List<MonitoredRequestErrorParser<InboundRequestErrorModel>> getErrorParsers()
	{
	...
	{
		this.integrationKeyExtractors = integrationKeyExtractors != null ?
				List.copyOf(integrationKeyExtractors) :
				Collections.emptyList();
	}

	List<IntegrationKeyExtractor> getIntegrationKeyExtractors()
	{
		return this.integrationKeyExtractors;
	}
}
