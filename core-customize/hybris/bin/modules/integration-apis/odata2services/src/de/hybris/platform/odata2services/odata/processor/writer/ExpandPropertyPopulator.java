		...
		for (final NavigationPropertySegment navPropertySegment : navPropertyPath)
		{
			...
			if (!attrName.equals(LOCALIZED_ATTRIBUTE_NAME))
			{
				final TypeAttributeDescriptor attribute =
						currentItemType.getAttribute(attrName)
						               // To meet the Optional contract we are throwing the IllegalArgumentException. However,
						               // it will never be thrown. The TypeDescriptor will always be in the Optional because
						               // olingo always validates that the path exists.
