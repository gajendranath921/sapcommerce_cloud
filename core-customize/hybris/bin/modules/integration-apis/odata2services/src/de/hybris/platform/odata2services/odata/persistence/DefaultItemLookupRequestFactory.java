...
public class DefaultItemLookupRequestFactory implements ItemLookupRequestFactory
{
	...
	private static final String REQUEST_PAGE_SIZE_PARAM = "$top";
	...
	public ItemLookupRequest create(final UriInfo uriInfo, final ODataContext context, final String contentType)
	{
		try
		{
			...
			final ItemLookupRequestBuilder builder = hasKey(uriInfo) ?
	...
	private ItemLookupRequestBuilder createBasic(
			...
			final String contentType) throws ODataException
	{
		final EdmEntitySet entitySet = uriInfo.getStartEntitySet();
		final Locale locale = getLocaleExtractor().extractFrom(context, HttpHeaders.ACCEPT_LANGUAGE);
		final ItemLookupRequestBuilder builder = itemLookupRequestBuilder()
				.withEntitySet(entitySet)
				.withAcceptLocale(locale)
	...
	protected ItemLookupRequestBuilder buildWithFilter(final ItemLookupRequestBuilder builder,
	...
	protected void handleFilter(final ItemLookupRequestBuilder builder, final UriInfo uriInfo,
	...
	protected void handleOrderBy(final ItemLookupRequestBuilder builder, final UriInfo uriInfo,
	                             final ODataContext context) throws ExceptionVisitExpression, ODataApplicationException
	{
		final ExpressionVisitor visitor = createExpressionVisitor(uriInfo, context);
	...
	private ItemLookupRequestBuilder createWithIntegrationKey(
			final TypeDescriptor typeDescriptor, final UriInfo uriInfo,
			final ODataContext context,
			final String contentType) throws ODataException
	{
		final ItemLookupRequestBuilder builder = createBasic(typeDescriptor, uriInfo, context, contentType);
		final EdmEntitySet entitySet = uriInfo.getStartEntitySet();
	...
	private Integer deriveTop(final ODataContext context)
	{
		final Object param = context.getParameter(ODATA_REQUEST);
		if (param instanceof ODataRequest)
		{
			final ODataRequest oDataRequest = (ODataRequest) param;
			final String pageSize = oDataRequest.getQueryParameters().get(REQUEST_PAGE_SIZE_PARAM);
			return deriveTop(StringUtils.isNotBlank(pageSize) ?
					Integer.parseInt(pageSize) :
					oDataServicesConfiguration.getDefaultPageSize());
		}
		return oDataServicesConfiguration.getDefaultPageSize();
	}
