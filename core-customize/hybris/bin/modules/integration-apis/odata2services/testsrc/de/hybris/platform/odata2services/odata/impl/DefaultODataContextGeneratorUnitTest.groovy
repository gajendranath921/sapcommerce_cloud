...
@UnitTest
class DefaultODataContextGeneratorUnitTest extends JUnitPlatformSpecification {
    ...
    @Test
    def "seEntityExtractors method does not leak reference when entityExtractors list is not null"() {
        given:
        def entityExtractors = [Stub(ODataRequestEntityExtractor)]
        contextGenerator.setEntityExtractors(entityExtractors)

        when:
        entityExtractors.clear()

        then:
        !contextGenerator.entityExtractors.empty
    }

    @Test
    def "EntityExtractors list is set to immutable empty list when the given entityExtractors list is null"() {
        given:
        contextGenerator.setEntityExtractors(null)
    ...
    @Test
    def "getEntityExtractors method returns immutable list when given #msg"() {
        given:
        contextGenerator.setEntityExtractors(entityExtractors)

        when:
        contextGenerator.entityExtractors << Stub(ODataRequestEntityExtractor)

        then:
        thrown UnsupportedOperationException

        where:
        msg                                | entityExtractors
        "entityExtractor list is not null" | []
        "entityExtractor list is null"     | null as List
    }
