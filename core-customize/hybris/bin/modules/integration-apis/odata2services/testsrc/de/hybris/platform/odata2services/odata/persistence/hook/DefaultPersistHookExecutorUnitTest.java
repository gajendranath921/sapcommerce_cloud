...
public class DefaultPersistHookExecutorUnitTest
{
	...
	{
		lenient().when(registry.getPostPersistHook(hookName, INTEGRATION_KEY)).thenReturn(postPersistHook);
	}

	private void givenPrePersistHook(final String hookName)
	{
		lenient().when(registry.getPrePersistHook(hookName, INTEGRATION_KEY)).thenReturn(prePersistHook);
	}
}