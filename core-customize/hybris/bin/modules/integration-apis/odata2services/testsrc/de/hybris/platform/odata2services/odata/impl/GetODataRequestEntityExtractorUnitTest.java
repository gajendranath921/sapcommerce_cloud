...
public class GetODataRequestEntityExtractorUnitTest
{
	...
	private static ODataRequest givenRequest(final HttpMethod httpMethod, final Map<String, String> queryParameters, final String requestUri)
	{
		...
		lenient().when(request.getHttpMethod()).thenReturn(httpMethod.name());
		lenient().when(request.getQueryParameters()).thenReturn(queryParameters);
		lenient().when(pathInfo.getRequestUri()).thenReturn(URI.create(requestUri));
