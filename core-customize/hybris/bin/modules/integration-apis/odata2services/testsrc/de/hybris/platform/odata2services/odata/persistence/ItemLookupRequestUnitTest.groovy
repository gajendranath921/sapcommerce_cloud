...
@UnitTest
class ItemLookupRequestUnitTest extends JUnitPlatformSpecification {
    ...
    @Test
    def "setNavigationSegments does not leak reference when input list is not null"() {
        given:
        def request = minimumRequiredBuilder().build()
        def inputList = [Stub(NavigationSegment)]
        request.navigationSegments = inputList

        when:
        inputList.clear()

        then:
        !request.navigationSegments.empty
    }

    @Test
    def "setExpand does not leak reference when input list is not null"() {
        given:
        def request = minimumRequiredBuilder().build()
        def innerList = [Stub(NavigationPropertySegment)]
        def outerList = [innerList]
        request.expand = outerList
        ...
        then:
        !request.expand.empty
        !request.expand[0].empty
    }

    @Test
    @Unroll
    def "setter for #field sets #field to empty list when given #field is null"() {
        given:
        def request = minimumRequiredBuilder().build()
        request."$field" = null
        ...
        where:
        field << ["navigationSegments", "expand"]
    }

    @Test
    @Unroll
    def "getter method for #field returns immutable list when given #field is #msg"() {
        given:
        def request = minimumRequiredBuilder().build()
        request."$field" = inputValue

        when:
        request."$field" << appendValue

        then:
        thrown UnsupportedOperationException

        where:
        msg        | field                | inputValue | appendValue
        "not null" | "navigationSegments" | []         | Stub(NavigationSegment)
        "null"     | "navigationSegments" | null       | Stub(NavigationSegment)
        "not null" | "expand"             | []         | [Stub(NavigationPropertySegment)]
        "null"     | "expand"             | null       | [Stub(NavigationPropertySegment)]
    }

    ItemLookupRequestBuilder minimumRequiredBuilder() {
        itemLookupRequestBuilder()
                .withEntitySet(entitySet)
                .withAcceptLocale(Locale.ENGLISH)
