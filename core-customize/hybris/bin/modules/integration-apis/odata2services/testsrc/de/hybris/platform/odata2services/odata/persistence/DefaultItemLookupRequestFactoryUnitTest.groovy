...
@UnitTest
class DefaultItemLookupRequestFactoryUnitTest extends JUnitPlatformSpecification {
    ...
    static final def ODATA_REQUEST = '~odataRequest'
    static final def REQUEST_PAGE_SIZE_PARAM = '$top'
    ...
    @Test
    @Unroll
    def "creates request with filter conditions and page size #pageSize"() {
        given:
        ...
        where:
        pageSize          | oDataContext
        DEFAULT_PAGE_SIZE | oDataContext()
        1                 | oDataContext("$pageSize")
    }

    @Test
    def 'default page size is used when context does not contain an OdataRequest'() {
        given:
        def entitySet = Stub EdmEntitySet
        def filter = new WhereClauseConditions()
    ...
    @Test
    @Unroll
    def "default page size is used when \$top parameter is #pageSize"() {
        given:
        def entitySet = Stub EdmEntitySet
        def filter = new WhereClauseConditions()
        def oDataContext = oDataContext(pageSize)
    ...
    private ODataContext oDataContext(String pageSize = "$DEFAULT_PAGE_SIZE") {
        Stub(ODataContext) {
            getPathInfo() >> Mock(PathInfo) {
                getServiceRoot() >> new URI("https://localhost:9002/odata2webservices/$INTEGRATION_OBJECT")
            }
            getParameter(ODATA_REQUEST) >> Stub(ODataRequest) {
                getQueryParameters() >> [(REQUEST_PAGE_SIZE_PARAM): pageSize]
            }
        }
    }
}
