...
public class DefaultOutboundMonitoringRequestDecorator implements OutboundRequestDecorator
{
	...
	{
		return getOutboundServicesConfiguration().isMonitoringEnabled();
	}
	                                                ...
	                                                final DecoratorContext context, final DecoratorExecution execution)
	{
		...
		final String integrationKey = deriveIntegrationKey(context, httpEntity);
	...
	protected String extractSapPassportFrom(final HttpEntity<Map<String, Object>> httpEntity, final DecoratorContext context)
	{
		try
		{
			...
			{
				throw new OutboundMonitoringException(
						String.format("No %s header present in request.", SAP_PASSPORT_HEADER_NAME));
			}
			return list.get(0);
		}
	...
	protected HttpEntity<Map<String, Object>> extractHttpEntityFrom(final HttpHeaders httpHeaders,
	                                                                ...
	                                                                final DecoratorExecution execution)
	{
	...
	{
		final OutboundRequestModel request = saveOutboundRequest(messageId, sapPassport, integrationKey,
				IntegrationRequestStatus.ERROR, context, extractError(exception));
		return request;
	}
	                                                 ...
	                                                 final DecoratorContext context, final String error)
	{
		...
		outboundRequest.setHttpMethod(context.getHttpMethod());
		getModelService().save(outboundRequest);
		return outboundRequest;
	}

	private String deriveIntegrationKey(final DecoratorContext context, final HttpEntity<Map<String, Object>> httpEntity)
	{
		return StringUtils.isNotEmpty(context.getIntegrationKey())
				? context.getIntegrationKey()
				: extractIntegrationKey(httpEntity);
	}
	...
	private boolean containsIntegrationKey(final Map<String, Object> payload)
	{
		return payload != null && payload.containsKey(INTEGRATION_KEY_PROPERTY_NAME) &&
				StringUtils.isNotEmpty((String) payload.get(INTEGRATION_KEY_PROPERTY_NAME));
	}
	...
	private OutboundServicesConfiguration getOutboundServicesConfiguration()
	{
		if (outboundServicesConfiguration == null)
		{
			return Registry.getApplicationContext().getBean("outboundServicesConfiguration", OutboundServicesConfiguration.class);
		}
		return outboundServicesConfiguration;
	}

	public void setOutboundServicesConfiguration(
			final OutboundServicesConfiguration outboundServicesConfiguration)
	{
