...
class ODataFacadeOutboundMonitoringIntegrationTest extends ServicelayerBaseSpecification {
    private static final String TEST_NAME = "ODataFacadeOutboundMonitoring"
    ...
    @Test
    def "outbound request is not logged when monitoring is off"() {
        given: 'stub destination server to return BAD REQUEST'
    ...
    private static SyncParameters syncParameters(final OutboundSource source = OutboundSource.OUTBOUNDSYNC, final EventType operation = DefaultEventType.CREATED) {
        SyncParameters.syncParametersBuilder()
                .withItem(catalog)
                .withSource(source)
                .withDestinationId(DESTINATION_ID)
                .withIntegrationObjectCode(IO)
                .withEventType(operation)
                .build()
    }
