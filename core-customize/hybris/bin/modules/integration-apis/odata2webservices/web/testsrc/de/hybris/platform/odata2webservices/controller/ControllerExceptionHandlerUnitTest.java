...
public class ControllerExceptionHandlerUnitTest
{
	...
	@Test
	public void testHandlerTypeAccessPermissionExceptionNullType() throws Exception
	{
		final var exception = new TypeAccessPermissionException("read");

		whenHandlingTypeAccessPermissionException(exception);

		verifyODataResponse(HttpStatusCodes.FORBIDDEN, "The user lacks read privileges for accessing the Integration Object types.");
	}
	...
	{
		lenient().when(oDataResponseToResponseEntityConverter.convert(oDataResponseArgumentCaptor.capture())).thenReturn(ResponseEntity.badRequest().build());
	}
