...
class ODataPostClassificationAttributesIntegrationTest extends ServicelayerSpockSpecification {
    private static final def TEST_NAME = 'ODataPostClassificationAttributes'
    private static final def CLASS_SYSTEM = "${TEST_NAME}_Sports"
    private static final def CLASS_VERSION = "${TEST_NAME}_Cycling"
    private static final def CLASS_SYSTEM_VERSION = "$CLASS_SYSTEM:$CLASS_VERSION"
    private static final def CLASSIFICATION_CLASS = "${TEST_NAME}_Bicycle"
    private static final def IO = "${TEST_NAME}_BikeIO"
    private static final def PRODUCT_CODE = "${TEST_NAME}_bike1"
    private static final def CATALOG = "${TEST_NAME}_Test"
    ...
    @Rule
    ClassificationBuilder classification = ClassificationBuilder.classification()
    ...
    def "succeeds when a required primitive collection attribute has values and it's updated with #condition in the payload"() {
        ...
        and: 'a request with body'
        def request = postRequest().withBody product(PRODUCT_CODE).withField('availableSizes',
                [json().withField('value', '56'),
                 json().withField('value', '58'),
                 json().withField('value', '61')])
    ...
    def 'succeeds when posting classification attributes that are in a hierarchy'() {
        ...
        cleanup: 'remove product and categories, so that the classification system could be removed'
