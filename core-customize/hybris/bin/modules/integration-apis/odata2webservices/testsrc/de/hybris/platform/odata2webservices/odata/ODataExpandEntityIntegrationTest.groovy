...
@IntegrationTest
class ODataExpandEntityIntegrationTest extends ServicelayerSpockSpecification {

    private static final String TEST_NAME = 'ODataExpandEntity'
    private static final String TEST_IO = "${TEST_NAME}_ExpandTest_IO"
    private static final String TEST_IO_MAP = "${TEST_NAME}_ExpandTest_Map"
    private static final def CATALOG = "${TEST_NAME}_Catalog"
    private static final def VERSION = "${TEST_NAME}_TestVersion"
    ...
    @Test
    def "Expand property that does not exist returns an error message including the non existing property name"() {
        given:
        IntegrationTestUtil.importImpEx(
                'INSERT_UPDATE Product; code[unique = true]; catalogVersion(catalog(id), version)',
                "                     ; prod1              ; $CATALOG:$VERSION"
        )
        def nonExistingAttribute = 'attributeThatIsNotPresentInTheIntegrationObject'
        def context = oDataContext("Products", ['$expand': nonExistingAttribute])

        when:
        ODataResponse response = facade.handleRequest(context)

        then:
        response.getStatus() == HttpStatusCodes.NOT_FOUND
        def json = JsonObject.createFrom response.entityAsStream
        json.getString('error.code') == "missing_attribute"
        json.getString('error.message.value').contains(nonExistingAttribute)
    }
