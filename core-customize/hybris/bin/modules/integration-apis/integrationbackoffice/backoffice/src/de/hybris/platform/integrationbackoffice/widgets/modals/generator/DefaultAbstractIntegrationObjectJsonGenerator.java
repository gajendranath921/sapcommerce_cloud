...
public abstract class DefaultAbstractIntegrationObjectJsonGenerator implements IntegrationObjectJsonGenerator
{
	...
	private static final String DUMMY_DATE = "/Date(1577836800000)/";

	private final ReadService readService;
	final ReadOnlyAttributesConfiguration configuration;

	private IntegrationObjectModel integrationObjectModel;
	private Deque<IntegrationMapKeyDTO> ancestors;

	public DefaultAbstractIntegrationObjectJsonGenerator(final ReadService readService,
	                                                     final ReadOnlyAttributesConfiguration configuration)
	{
	...
	abstract List<AbstractIntegrationObjectItemAttributeModel> getAllAttributes(
			final IntegrationObjectItemModel integrationObjectItem);

	abstract MetadataPrimitiveData composeMetadataPrimitiveData(final AbstractIntegrationObjectItemAttributeModel attribute,
	                                                            final boolean isFromMap);

	MetadataPrimitiveData determineClassificationType(final IntegrationObjectItemClassificationAttributeModel attribute)
	{
	...
	MetadataPrimitiveData determinePrimitiveType(final IntegrationObjectItemAttributeModel attribute,
	                                             final boolean isFromMap,
	...
	MetadataPrimitiveData buildMetadataPrimitiveData(final AbstractIntegrationObjectItemAttributeModel attribute,
	                                        ...
	                                        final IntegrationObjectItemModel integrationObjectItemModel)
	{
		...
		final List<AbstractIntegrationObjectItemAttributeModel> attributes = getAllAttributes(integrationObjectItemModel);
	                                                                                  ...
	                                                                                  final AbstractIntegrationObjectItemAttributeModel attribute)
	{
		...
		if (composedTypeModel != null && !isMap)
		{
			final String typeCode = attribute.getReturnIntegrationObjectItem().getCode();
			final IntegrationMapKeyDTO mapKey = new IntegrationMapKeyDTO(composedTypeModel, typeCode);
			if (!ancestors.contains(mapKey))
			{
				ancestors.addFirst(mapKey);
				composeJsonComposedTypesAndCollections(jsonBuilder, attribute, mapKey, isCollection);
				isAdded = true;
				ancestors.pollFirst();
			}
		}
	...
	private void composeJsonComposedTypesAndCollections(final StringBuilder jsonBuilder,
	                                                    final AbstractIntegrationObjectItemAttributeModel attribute,
	                                                    final IntegrationMapKeyDTO mapKey,
	                                                    final boolean isCollection)
	{
		...
		final IntegrationObjectItemModel matchingIOI = findItemInIntegrationObject(mapKey.getCode());
	...
	private void composeJsonPrimitive(final StringBuilder jsonBuilder,
	                                  ...
	                                  final boolean isFromMap)
	{
		final MetadataPrimitiveData primitiveData = composeMetadataPrimitiveData(attribute, isFromMap);
	...
	private String getCollectionDummyValueForPrimitiveClassificationTypes(final String value1,
	                                                                      final String value2,
	...
	{
		return integrationObjectModel.getItems()
		                             .stream()
		                             .filter(item -> item.getCode().equals(code))
		                             .findFirst()
		                             .orElseThrow(() -> new NoSuchElementException(
				                             String.format("No IntegrationObjectItem found with code %s", code)));
	}
}
