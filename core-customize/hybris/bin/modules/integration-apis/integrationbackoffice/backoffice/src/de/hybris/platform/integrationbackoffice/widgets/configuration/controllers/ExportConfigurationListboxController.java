...
public class ExportConfigurationListboxController extends DefaultWidgetController
{
	...
	{
		if (ioModel == null || exportConfigEditorPresentation.isSelectedEntity(ioModel))
		{
			return;
		}
		clearListbox();
	...
	{
		if (config == null || StringUtils.isBlank(exportConfigEditorPresentation.getSelectedEntityRoot()))
		{
		...
		final String countFraction = exportConfigEditorPresentation.getSelectedEntityInstances().size() + "/"
				+ countInstancesOfType(exportConfigEditorPresentation.getSelectedEntityRoot());
	...
	private void onSelectItemEvent(final Event event)
	{
		if (event instanceof SelectEvent)
		{
			...
			final String countFraction = selectedInstances.size() + "/" + countInstancesOfType(
					exportConfigEditorPresentation.getSelectedEntityRoot());
			sendOutput("currentSelectedCount", countFraction);
		}
	}
