...
public class IntegrationObjectClassificationClassWizardHandler implements FlowActionHandler
{
	...
	private Optional<ClassAttributeAssignmentModel> loadAssignment(String pk)
	{
		...
		catch (final ModelLoadingException ex)
		{
			LOG.debug("Cannot find classification assignment model with given pk.", ex);
		}
		catch (final ClassCastException ex)
		{
			LOG.debug("The item found cannot be casted to classification assignment model.", ex);
		}
