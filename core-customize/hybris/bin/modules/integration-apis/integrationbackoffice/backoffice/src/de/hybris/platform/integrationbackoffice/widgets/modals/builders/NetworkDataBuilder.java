...
public class NetworkDataBuilder
{
	...
	private static final String FACE = "Monospace";
	private static final String BACKGROUND = "#ffffff";
	private static final String HIGHLIGHT = "#d5eeff";
	...
	public int maxLength(final EDMXEntityType edmxEntityType)
	{
		final int maxLengthOfNavigationProperties = edmxEntityType.getNavigationProperties().stream()
		                                                          .mapToInt(
				                                                          edmxNavigationProperty -> edmxNavigationProperty.getName()
		...
		final int maxLengthOfProperties = edmxEntityType.getProperties()
		                                                .stream()
		                                                .mapToInt(
				                                                edmxProperty -> edmxProperty.getName()
				                                                                            .length() + edmxProperty.getType()
				                                                                                                    .length())
		                                                .max()
		                                                .orElse(0);
	...
	public Network createNetwork(final EDMXSchema edmxSchema)
	{
		...
		for (final EDMXEntityType EDMXEntityType : navigationPropertyPriority)
		{
			...
			final Node newNode = buildNewNode(nodeId, nodeLabelBuilder, nodeTooltipBuilder);
			nodeId++;
		...
		for (final EDMXEntityType EDMXEntityType : edmxSchema.getEdmxEntityTypes())
		{
			for (final EDMXNavigationProperty navProperty : EDMXEntityType.getNavigationProperties())
			{
				...
				final String edgeLabel = buildEdgeLabel(mapOfEDMXAssociation, navProperty, associationName);
				if (toRoleIsPrimitive(toRole))
				{
					toRole = toRole.substring(toRole.lastIndexOf('.') + 1);
				}
				final Edge edge = buildEdge(nodeMap, navProperty, toRole, edgeLabel, ROUNDNESS_VALUE, EDGE_LABEL_FONT_SIZE);
				edges.add(edge);
	...
	private boolean toRoleIsPrimitive(final String toRole)
	{
		return ("java.lang.String".equals(toRole) || "java.lang.Double".equals(toRole) || "java.lang.Boolean".equals(
				toRole) || "java.util.Date".equals(toRole));
	}

	private Node buildNewNode(final int nodeId, final StringBuilder nodeLabelBuilder, final StringBuilder nodeTooltipBuilder)
	{
	...
	private String buildEdgeLabel(final Map<String, EDMXAssociation> mapOfEDMXAssociation,
	                              final EDMXNavigationProperty navProperty, final String associationName)
	{
		return mapOfEDMXAssociation.containsKey(associationName) ? mapOfEDMXAssociation.get(associationName)
		                                                                               .getEdmxEnd()
		                                                                               .stream()
		                                                                               .filter(x -> x.getRole()
		                                                                                             .equalsIgnoreCase(
	...
	private Edge buildEdge(final Map<String, Node> nodeMap, final EDMXNavigationProperty navProperty, final String toRole,
	                       final String edgeLabel,
	                       final double ROUNDNESS_VALUE, final int EDGE_LABEL_FONT_SIZE)
	{
		return new Edge.Builder(nodeMap.get(navProperty.getFromRole()), nodeMap.get(toRole))
				.withLabel(edgeLabel)
