...
public class DefaultWebhookFilterService implements WebhookFilterService
{
		...
		if (canProceedWithFilter(item, scriptUri))
		{
			try
			{
				returnValue = scriptingLanguagesService.getExecutableByURI(scriptUri)
				                                       ...
				                                       .filter(item)
				                                       .filter(value -> isAssignableFrom(item, value));
	...
	private <T extends ItemModel> boolean isAssignableFrom(final T item, final T value)
	{
		final boolean isReturnable = item.getClass().isAssignableFrom(value.getClass());
