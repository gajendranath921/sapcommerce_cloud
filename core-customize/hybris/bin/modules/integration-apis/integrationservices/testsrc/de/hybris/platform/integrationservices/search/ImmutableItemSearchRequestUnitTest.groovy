...
@UnitTest
class ImmutableItemSearchRequestUnitTest extends JUnitPlatformSpecification {
    @Test
    def 'builds a request for search within a type without additional conditions'() {
        ...
        when:
        def request = new ItemSearchRequestBuilder()
                .withItemType(type)
                .build()
    ...
    def 'changes to the builder spec do no mutate previously created requests'() {
        given: 'a builder'
        def builder = new ItemSearchRequestBuilder()
                .withItemType(Stub(TypeDescriptor))
