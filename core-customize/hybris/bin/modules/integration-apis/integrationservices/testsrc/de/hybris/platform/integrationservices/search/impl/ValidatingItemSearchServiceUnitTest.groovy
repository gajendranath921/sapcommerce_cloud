...
@UnitTest
class ValidatingItemSearchServiceUnitTest extends JUnitPlatformSpecification {
    @Shared
    ...
    @Test
    @Unroll
    def "#setValidatorsMethod does not leak reference"() {
        given:
        def validator = Mock(ItemSearchRequestValidator)
        def validators = [validator]
        validatingService."$setValidatorsMethod"(validators)
        ...
        where:
        setValidatorsMethod             | method
        "setUniqueItemSearchValidators" | "findUniqueItem"
        "setItemsSearchValidators"      | "findItems"
        "setCountSearchValidators"      | "countItems"
    }

    @Test
    @Unroll
    def "#setEnrichersMethod does not leak reference"() {
        given:
        def enricher = Mock(ItemSearchRequestEnricher)
        def enrichers = [enricher]
        validatingService."$setEnrichersMethod"(enrichers)
        ...
        then:
        1 * enricher.enrich(request)

        where:
        setEnrichersMethod             | method
        "setUniqueItemSearchEnrichers" | "findUniqueItem"
        "setItemsSearchEnrichers"      | "findItems"
        "setCountSearchEnrichers"      | "countItems"
    }
