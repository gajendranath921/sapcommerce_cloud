...
public class MockRelationDescriptorModelBuilder extends BaseMockAttributeDescriptorModelBuilder<MockRelationDescriptorModelBuilder, RelationDescriptorModel>
{
	...
	public RelationDescriptorModel build()
	{
		...
		lenient().when(model.getRelationType()).thenReturn(relationTypeModel);
		lenient().when(model.getIsSource()).thenReturn(isSource);
		lenient().when(model.getAttributeType()).thenReturn(mockComposedTypeModel);
		return model;
	}
	...
	private RelationMetaTypeModel relationMetaTypeModel()
	{
		...
		lenient().when(relationMetaTypeModel.getSourceType()).thenReturn(sourceComposedTypeModel);
		lenient().when(relationMetaTypeModel.getTargetType()).thenReturn(targetComposedTypeModel);
		lenient().when(relationMetaTypeModel.getSourceTypeCardinality()).thenReturn(sourceCardinality);
		lenient().when(relationMetaTypeModel.getTargetTypeCardinality()).thenReturn(targetCardinality);
		lenient().when(relationMetaTypeModel.getTargetAttribute()).thenReturn(targetAttribute);
		lenient().when(relationMetaTypeModel.getSourceAttribute()).thenReturn(sourceAttribute);
