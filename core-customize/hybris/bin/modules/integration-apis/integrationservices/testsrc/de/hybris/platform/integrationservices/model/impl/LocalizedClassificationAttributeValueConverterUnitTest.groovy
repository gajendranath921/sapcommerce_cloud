...
@UnitTest
class LocalizedClassificationAttributeValueConverterUnitTest extends JUnitPlatformSpecification {
    ...
    def "isApplicable is #applicable when attribute is #condition type"() {
        ...
        'localized and a collection' | false      | Stub(ClassificationTypeAttributeDescriptor) { isLocalized() >> true
            isCollection() >> true }
        'localized but not enum'     | false      | Stub(ClassificationTypeAttributeDescriptor) { isLocalized() >> true
            getClassAttributeAssignment() >> stringClassAttributeAssignment() }
        'enum but not localized'     | false      | Stub(ClassificationTypeAttributeDescriptor) { isLocalized() >> false }
        'single'                     | false      | Stub(ClassificationTypeAttributeDescriptor) { isLocalized() >> false
            isCollection() >> false }
    }
