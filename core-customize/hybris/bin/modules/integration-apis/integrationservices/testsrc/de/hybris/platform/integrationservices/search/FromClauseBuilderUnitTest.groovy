...
@UnitTest
class FromClauseBuilderUnitTest extends JUnitPlatformSpecification {
    ...
    def "many-to-many relationship results in JOIN for #end and uses field 'supercategories' when attribute is named '#attribute'"() {
        given:
        def condition = new SimplePropertyWhereClauseCondition(attribute, EQUALS_OPERATOR, '1234')
    ...
    def "one-to-many relationship results in JOIN for source type and target type on source type's pk."() {
        given:
        def condition = new SimplePropertyWhereClauseCondition('europe1Price', EQUALS_OPERATOR, '1234')
        def filter = new WhereClauseConditions(condition)
    ...
    def "from clause for enum type item"() {
        given:
        def condition = new SimplePropertyWhereClauseCondition('code', EQUALS_OPERATOR, 'enum2')
        def filter = new WhereClauseConditions(condition)
    ...
    def "from clause for multiple relations item"() {
        given:
        def condition1 = new SimplePropertyWhereClauseCondition('supercategories', EQUALS_OPERATOR, 'cat1')
        def condition2 = new SimplePropertyWhereClauseCondition('vendors', EQUALS_OPERATOR, 'vendor1')
    ...
    def "from clause includes MyCode#end when type hierarchy restriction is #itemTypeMatch and integration object item is not enum or abstract type"() {
        given:
        def condition = new SimplePropertyWhereClauseCondition('code', EQUALS_OPERATOR, 'mycode')
        def filter = new WhereClauseConditions(condition)
    ...
    def "from clause for an #type item is always ALL_SUBTYPES even when the type hierarchy restriction is #itemTypeMatch"() {
        given:
        def condition = new SimplePropertyWhereClauseCondition('code', EQUALS_OPERATOR, '')
        def filter = new WhereClauseConditions(condition)
