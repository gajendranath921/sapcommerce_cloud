...
@UnitTest
class FlexibleSearchQueryBuilderUnitTest extends JUnitPlatformSpecification {
    ...
    @Test
    @Unroll
    def "generates query without where clause for a #type type integration object item with itemTypeMatch=ItemTypeMatchEnum.ALL_SUBTYPES & configuration item search set to #configItemTypeMatch"() {
        given:
    ...
    @Test
    def "withOrderBy does not escape reference of orderByExpressions when orderExpression is not null"() {
        given:
        def orderExpressions = [new OrderExpression("name", OrderBySorting.DESC)]

        when:
        builder.withOrderBy(orderExpressions)
        orderExpressions.clear()

        then:
        !builder.orderByExpressions.empty
    }

    @Test
    def "withOrderBy sets orderExpression to immutable list is given input is null"() {
        when:
        builder.withOrderBy(null)
    ...
    @Test
    @Unroll
    def "getOrderByExpressions returns immutable object when given list is #msg"() {
        given:
        builder.withOrderBy(orderExpressions)

        when:
        builder.orderByExpressions << new OrderExpression("name", OrderBySorting.ASC)

        then:
        thrown UnsupportedOperationException

        where:
        msg        | orderExpressions
        "not null" | [new OrderExpression("name", OrderBySorting.DESC)]
        "null"     | null as List
    }

    @Test
    def "default orderExpression list is immutable list"() {
        when: "appending to default immutable orderByExpression list"
        builder.orderByExpressions << new OrderExpression("name", OrderBySorting.ASC)

        then:
        thrown UnsupportedOperationException

        and: "default orderByExpression is empty"
        builder.orderByExpressions.empty
    }
