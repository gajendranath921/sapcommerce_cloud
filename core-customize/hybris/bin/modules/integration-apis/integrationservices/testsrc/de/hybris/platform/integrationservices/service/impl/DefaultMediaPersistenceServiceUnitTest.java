...
public class DefaultMediaPersistenceServiceUnitTest
{
	...
	public void setup()
	{
		lenient().when(searchResult.getResult()).thenReturn(Collections.emptyList());
		lenient().when(modelService.create(eq(IntegrationApiMediaModel.class))).thenReturn(new IntegrationApiMediaModel());
	...
	public void testPersistMediasSavesTheMediaWithMediaService()
	{
		...
		verify(mediaService).setStreamForMedia(any(IntegrationApiMediaModel.class), same(INPUT_STREAM));
	}
	...
	public void testPersistSeveralMediasSuccessfully()
	{
		...
		verify(mediaService).setStreamForMedia(any(IntegrationApiMediaModel.class), same(body1));
		verify(mediaService).setStreamForMedia(any(IntegrationApiMediaModel.class), same(body2));
	}
	...
	public void testPersistMediasOneMediaFailedToPersist()
	{
		...
		doThrow(new MediaIOException("")).when(mediaService)
				.setStreamForMedia(any(IntegrationApiMediaModel.class), same(body1));
	...
	public void testPersistMediasWithNullPayload()
	{
		...
		verify(mediaService, never()).setStreamForMedia(any(IntegrationApiMediaModel.class), same(body1));
		verify(mediaService).setStreamForMedia(any(IntegrationApiMediaModel.class), same(body2));
	}

	private void mockConfigurationService()
	{
		lenient().when(integrationServicesConfiguration.getMediaPersistenceMediaNamePrefix()).thenReturn(MEDIA_NAME_PREFIX);
	}
}
