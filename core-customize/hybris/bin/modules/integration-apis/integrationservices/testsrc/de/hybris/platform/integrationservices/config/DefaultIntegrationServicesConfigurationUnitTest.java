...
public class DefaultIntegrationServicesConfigurationUnitTest
{
	...
	{
		lenient().when(configuration.getString(MEDIA_PERSISTENCE_MEDIA_NAME_PREFIX_PROPERTY_KEY)).thenReturn("SOME_PREFIX");
	...
	{
		lenient().when(configuration.getString(MEDIA_PERSISTENCE_MEDIA_NAME_PREFIX_PROPERTY_KEY)).thenReturn("");
		assertThat(integrationServicesConfiguration.getMediaPersistenceMediaNamePrefix()).isEqualTo("Payload_");
	}
	...
	{
		lenient().when(configuration.getString(SAP_PASSPORT_SYSTEM_ID_PROPERTY_KEY)).thenReturn("SOME_SYSTEM_ID");
	...
	{
		lenient().when(configuration.getInt(SAP_PASSPORT_SERVICE_PROPERTY_KEY)).thenReturn(1399);
	...
	{
		lenient().when(configuration.getString(SAP_PASSPORT_USER_PROPERTY_KEY)).thenReturn("SOME_USER_ID");
	...
	{
		lenient().when(configuration.getString(SAP_PASSPORT_USER_PROPERTY_KEY)).thenReturn(null);
		final String userId = "testUser";
		final User user = mock(User.class);
		lenient().when(user.getUid()).thenReturn(userId);
	...
	{
		lenient().when(configuration.getString(READ_ONLY_ATTRIBUTES)).thenReturn("creationtime,modifiedtime");
	...
	{
		lenient().when(configuration.getString(READ_ONLY_ATTRIBUTES)).thenReturn(null);
	...
	{
		lenient().when(configuration.getString(READ_ONLY_ATTRIBUTES)).thenReturn("");
		assertThat(integrationServicesConfiguration.getReadOnlyAttributes()).isEqualTo(Collections.emptySet());
	}
	...
	{
		lenient().when(configuration.getBoolean(AUTHORIZATION_ACCESS_RIGHTS)).thenReturn(true);
	...
	{
		lenient().when(configuration.getBoolean(AUTHORIZATION_ACCESS_RIGHTS)).thenReturn(false);
		assertThat(integrationServicesConfiguration.isAccessRightsEnabled()).isFalse();
	}
}
