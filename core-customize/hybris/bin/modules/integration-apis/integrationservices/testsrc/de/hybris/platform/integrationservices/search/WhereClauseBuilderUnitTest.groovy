...
@UnitTest
class WhereClauseBuilderUnitTest extends JUnitPlatformSpecification {
    ...
    def "throws exception when WHERE clause includes a classificationAttribute"() {
        ...
        def condition = SimplePropertyWhereClauseCondition.eq(attrName, 1234)
    ...
    def "generates join when filtering on property that has a many-to-many #end relation"() {
        given:
        def condition = SimplePropertyWhereClauseCondition.eq(propertyName, 1234)
        def filter = new WhereClauseConditions(condition)
    ...
    def "generates join for multiple filtering conditions"() {
        given:
        def manyToManyCondition = new WhereClauseCondition(SimplePropertyWhereClauseCondition.eq('supercategories', 1234), ConjunctiveOperator.AND)
        def oneToOneCondition = SimplePropertyWhereClauseCondition.eq('code', 'mycode')
    ...
    def 'generates type system attribute name when IO attribute name differs in the simple attribute filter condition'() {
        ...
        def condition = SimplePropertyWhereClauseCondition.eq('id', 'some_value')
        def filter = new WhereClauseConditions(condition)
    ...
    def 'generates type system attribute name when IO attribute name differs in the reference attribute filter condition'() {
        ...
        and: 'the filter condition uses IO attribute name'
        def condition = SimplePropertyWhereClauseCondition.withCompareOperator('categories', '(pk)', 'IN')
        def filter = new WhereClauseConditions(condition)
    ...
    def 'where clause is #expectedWhere when the source cardinality is #sourceCardinality and the target cardinality is #targetCardinality'() {
        ...
        def condition = SimplePropertyWhereClauseCondition.withCompareOperator(attributeName, '(pk)', 'IN')
        def filter = new WhereClauseConditions(condition)
    ...
    def 'where clause with renamed localized attributes'() {
        ...
        and: 'the filter condition uses IO attribute name'
        def condition = SimplePropertyWhereClauseCondition.eq('aliasName', 'some_value')
        def filter = new WhereClauseConditions(condition)
