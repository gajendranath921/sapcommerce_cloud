...
@UnitTest
class SapPassportBuilderUnitTest extends JUnitPlatformSpecification {
    ...
    @Test
    def "withRootContextId does not escapes reference to rootContextId"() {
        when:
        def passport = builder.build()

        then:
        !passport.rootContextId.is(ROOT_CONTEXT_ID)
    }
    ...
    @Test
    def "withConnectionId does not escapes reference to connectionId"() {
        when:
        def passport = builder.build()

        then:
        !passport.connectionId.is(CONNECTION_ID)
    }
