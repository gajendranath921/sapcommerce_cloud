...
@UnitTest
class AttributeReferencePathUnitTest extends JUnitPlatformSpecification {
    @Shared
    ...
    @Test
    def "does not escape references to the attributes making up its state"() {
        given:
    ...
    @Test
    @Unroll
    def "can be presented as java bean property path '#path'"() {
        given:
        def attributePath = new AttributeReferencePath(attributes)

        expect:
        attributePath.toPropertyPath() == path

        where:
        attributes                                               | path
        [attribute('code')]                                      | 'code'
        [attribute('unit'), attribute('code')]                   | 'unit.code'
        [attribute('category'), collectionAttribute('products')] | 'category.products[0]'
        [collectionAttribute('categories'), attribute('code')]   | 'categories[0].code'
    }

    private TypeAttributeDescriptor collectionAttribute(String name = '') {
