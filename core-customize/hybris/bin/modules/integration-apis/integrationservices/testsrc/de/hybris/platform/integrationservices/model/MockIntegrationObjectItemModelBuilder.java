...
public class MockIntegrationObjectItemModelBuilder
{
	...
	private IntegrationObjectItemModel stubItem(final IntegrationObjectItemModel item)
	{
		...
		lenient().when(item.getIntegrationObject()).thenReturn(object);
		lenient().when(item.getAttributes()).thenReturn(attributes);
		lenient().when(item.getCode()).thenReturn(itemCode);
		lenient().when(item.getKeyAttributes()).thenReturn(keyAttributes);
		lenient().when(item.getUniqueAttributes()).thenReturn(uniqueAttributes);
		lenient().when(item.getType()).thenReturn(typeModel);
		return item;
	}
	...
	private IntegrationObjectModel integrationObject(final String name)
	{
		...
		lenient().when(model.getCode()).thenReturn(name);
		return model;
	}
