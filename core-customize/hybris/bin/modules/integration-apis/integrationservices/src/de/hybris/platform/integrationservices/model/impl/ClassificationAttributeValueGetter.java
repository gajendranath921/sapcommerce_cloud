...
class ClassificationAttributeValueGetter implements AttributeValueGetter
{
	...
	public Object getValue(final Object model)
	{
		...
		{
			final var feature = classificationService.getFeature((ProductModel) model, getClassAttributeAssignment());
	...
	{
		if (isAnExistingProduct(model) && attribute.isLocalized())
		{
			final var feature = classificationService.getFeature((ProductModel) model, getClassAttributeAssignment());
			if (feature instanceof LocalizedFeature)
			{
				final var localizedFeature = (LocalizedFeature) feature;
	...
	{
		return getClassAttributeAssignment().getAttributeType() == ClassificationAttributeTypeEnum.ENUM;
	}
