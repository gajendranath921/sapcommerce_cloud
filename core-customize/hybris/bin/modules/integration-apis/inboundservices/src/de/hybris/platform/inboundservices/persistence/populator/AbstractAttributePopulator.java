...
public abstract class AbstractAttributePopulator implements AttributePopulator
{
	...
	/**
	 * Creates an "empty" instance of a populator that has no dependencies injected and relies on the setter
	 * injecting dependencies being called.
	 */
	protected AbstractAttributePopulator()
	{
		modelService = null;
	}

	/**
	 * Instantiates a populator with the dependency on the {@link ModelService} injected.
	 * @param service implementation of the {@code ModelService} to use for model manipulations.
	 */
	protected AbstractAttributePopulator(@NotNull final ModelService service)
	{
		Preconditions.checkArgument(service != null, "ModelService cannot be null");
