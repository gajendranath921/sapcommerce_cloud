...
@UnitTest
class DefaultContextItemModelServiceUnitTest extends JUnitPlatformSpecification {
    ...
    @Test
    def "setPersistenceContextValidators does not leak references when validator list is not null"() {
        given:
        def validators = [persistenceContextValidator]

        when:
        contextItemModelService.persistenceContextValidators = validators
        and:
        validators.clear()

        then:
        !contextItemModelService.persistenceContextValidators.empty
    }

    @Test
    def "setPersistenceContextValidators does not leak references when validator list is null"() {
        when:
        contextItemModelService.persistenceContextValidators = null
    ...
    @Test
    def "setItemPersistRequestValidators does not leak references when validator list is not null"() {
        given:
        def validators = [itemPersistReqValidator]

        when:
        contextItemModelService.itemPersistRequestValidators = validators
        and:
        validators.clear()

        then:
        !contextItemModelService.itemPersistRequestValidators.empty
    }

    @Test
    def "setItemPersistRequestValidators does not leak references when validator list is null"() {
        when:
        contextItemModelService.itemPersistRequestValidators = null
