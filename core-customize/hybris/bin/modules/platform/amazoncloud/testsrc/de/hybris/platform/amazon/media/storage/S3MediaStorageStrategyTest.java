...
public class S3MediaStorageStrategyTest
{
	...
	public void shouldStoreMediaUsingS3Service() throws Exception
	{
		...
		given(storageObject.getContentLength()).willReturn(Long.valueOf(123456));
		given(storageObject.getContentType()).willReturn(MIME);
		...
		{
			@Override
			public boolean matches(final PutObjectRequest putObjectRequest)
			{
				return putObjectRequest.getBucketName().equals(s3Bucket);
			}

		}));
		verify(locationHashService, times(1)).createHash(FOLDER_QUALIFIER, LOCATION, 123456L, MIME);
