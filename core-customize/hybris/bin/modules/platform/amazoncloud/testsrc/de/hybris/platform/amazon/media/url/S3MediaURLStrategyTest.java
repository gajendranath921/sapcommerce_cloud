...
public class S3MediaURLStrategyTest
{
	...
	public void createS3MediaURLStrategy() throws Exception
	{
		...
		Mockito.lenient().when(storageConfigService.getConfigForFolder(FOLDER_QUALIFIER)).thenReturn(mediaFolderConfig);
		Mockito.lenient().when(mediaFolderConfig.getParameter(ACCESS_KEY)).thenReturn(ACCESS_KEY_VALUE);
		Mockito.lenient().when(mediaFolderConfig.getParameter(SECRET_ACCESS_KEY)).thenReturn(SECRET_ACCESS_KEY_VALUE);
		Mockito.lenient().when(mediaFolderConfig.getParameter(BUCKET_ID_KEY)).thenReturn(BUCKET_ID);
	...
	{
		// given
		given(mediaFolderConfig.getParameter(S3MediaURLStrategy.SIGNED_KEY, Boolean.class, Boolean.TRUE)).willReturn(
				Boolean.TRUE);
