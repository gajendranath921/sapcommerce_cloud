...
public class S3MediaStorageCleanerTest
{
	...
	public void setUp() throws Exception
	{
		...
		given(Boolean.valueOf(storageConfigService.isStorageStrategyConfigured("s3MediaStorageStrategy"))).willReturn(
				Boolean.TRUE);
		...
		given(mediaStorageConfig.getParameter(DefaultS3StorageServiceFactory.SECRET_ACCESS_KEY, String.class)).willReturn(
				"Secret");
		...
		Mockito.lenient().when(folderConfig1.getFolderQualifier()).thenReturn(FOLDER_QUALIFIER1);
		Mockito.lenient().when(folderConfig2.getFolderQualifier()).thenReturn(FOLDER_QUALIFIER2);
	...
	public void shouldCleanStorageOnInitializationWhenS3IsDefaultStrategy() throws Exception
	{
		...
		{
			@Override
			public boolean matches(final DeleteObjectsRequest deleteObjectsRequest)
			{
				return deleteObjectsRequest.getBucketName().equals(BUCKET_NAME1) && deleteObjectsRequest.getKeys().size() == 3;
			}
		...
		verify(s3Service, times(1)).deleteObjects(argThat(new ArgumentMatcher<DeleteObjectsRequest>()
		{
			...
			public boolean matches(final DeleteObjectsRequest deleteObjectsRequest)
			{
				return deleteObjectsRequest.getBucketName().equals(BUCKET_NAME2) && deleteObjectsRequest.getKeys().size() == 3;
			}
		}));
	}
}
