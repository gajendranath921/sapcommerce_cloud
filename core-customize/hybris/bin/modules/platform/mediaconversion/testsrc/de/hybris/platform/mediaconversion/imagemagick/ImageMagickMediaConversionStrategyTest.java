...
public class ImageMagickMediaConversionStrategyTest extends HybrisJUnit4Test
{
	...
	public void testExtractFileExtension() throws Exception
	{
		...
		Assert.assertEquals("jpg", ImageMagickMediaConversionStrategy
				.extractFileExtension(
						"http://picture.immobilienscout24.de/files/basic003/N/115/48/7/115048007-1.jpg?1368494333"));
	...
	public void testCommandBuilding() throws Exception
	{
		...
		Assert.assertEquals(Arrays.asList("input", "-resize", "50x50", "output"),
				this.imService.buildCommand(null, "input", "output",
						new DummyFormat("        -resize    50x50       ")));

		Assert.assertEquals(Arrays.asList("-debug", "all", "input", "-resize", "50x50", "output"),
				this.imService.buildCommand(null, "input", "output",
						new DummyFormat("-debug all {input} -resize 50x50 {output}")));
		Assert.assertEquals(Arrays.asList("-debug", "all", "input[0]", "-resize", "50x50", "output"), this.imService
				.buildCommand(null, "input", "output", new DummyFormat("-debug all {input}[0] -resize 50x50 {output}")));
	}

	@Test
	public void testSimpleConversion() throws Exception
	{
		final File original = ImageMagickMediaConversionStrategyTest.file("/mediaconversion/test/original.gif");
