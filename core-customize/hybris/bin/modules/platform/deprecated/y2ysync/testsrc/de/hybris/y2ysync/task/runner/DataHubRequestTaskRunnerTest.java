...
public class DataHubRequestTaskRunnerTest
{
	...
	public void shouldUseDataHubUrlFromPropertiesIfY2YSyncJobHasntConfiguredItDirectly() throws Exception
	{
		...
		// then
		final Y2YSyncContext ctx = Y2YSyncContext.builder()
		                                         .withSyncExecutionId(TEST_EXECUTION_ID)
		                                         .withUri(DEFAULT_DATAHUB_URL)
		                                         .withFeed(FEED_NAME)
		                                         .withPool(POOL_NAME)
		                                         .withAutoPublishTargetSystems(TARGET_SYSTEM)
		                                         .build();
		verify(requestCreator).sendRequest(refEq(ctx));
	}
