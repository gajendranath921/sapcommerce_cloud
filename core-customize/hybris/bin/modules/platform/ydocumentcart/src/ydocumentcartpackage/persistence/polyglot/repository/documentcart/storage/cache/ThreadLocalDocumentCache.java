...
public class ThreadLocalDocumentCache extends BaseStorageCache
{
	...
	private Optional<QueryResult> findInCache(final Map<Identity, DocumentCacheEntry> cache, final BaseQuery baseQuery)
	{
		...
		{
			final Predicate<ItemStateView> predicate = baseQuery.getRootCondition()
			                                                    .map(EntityCondition::getPredicate)
			                                                    .orElse(e -> false);
			return lookupCacheForResult(cache,
					documentCacheEntry -> predicate.test(documentCacheEntry.getDocument().getRootEntity()));
		}
		if (baseQuery.getEntityCondition().isPresent())
		{
			final Predicate<ItemStateView> predicate = baseQuery.getEntityCondition()
			                                                    .map(EntityCondition::getPredicate)
			                                                    .orElse(e -> false);
			return lookupCacheForResult(cache,
					documentCacheEntry -> documentCacheEntry.getDocument().allEntities().anyMatch(predicate));
		}
	                                                   ...
	                                                   final Predicate<DocumentCacheEntry> cacheEntryPredicate)
	{
		final Map<Boolean, List<Document>> collect = cache.values()
		                                                  ...
		                                                  .collect(Collectors.groupingBy(DocumentCacheEntry::isRemoved,
				                                                  Collectors.mapping(DocumentCacheEntry::getDocument,
						                                                  Collectors.toList())));
	...
	{
		return StorageUtils.requireBaseQuery(query).isFindInStorageFirst();
	}
	...
	public void cacheDocuments(final QueryResult queryResult)
	{
		...
		{
			final Map<Identity, DocumentCacheEntry> cache = cacheHolder.getCache();
			queryResult.stream().forEach(doc -> cache.computeIfAbsent(doc.getRootId(), identity -> new DocumentCacheEntry(doc)));
		}
	}
	...
	{
		if (entry.isRemoved() && !entry.getDocument().isNew())
		{
