...
public class DefaultDroolsRuleActionsGeneratorTest extends AbstractGeneratorTest
{
	...
	public void init()
	{
		...
		Mockito.lenient().when(droolsRuleGeneratorContext.generateClassName(Map.class)).thenReturn(MAP_CLASS_NAME);
		Mockito.lenient().when(droolsRuleGeneratorContext.generateClassName(DefaultDroolsRuleActionContext.class)).thenReturn(
				ACTION_CONTEXT_CLASS_NAME);

		Mockito.lenient().when(droolsRuleGeneratorContext.getDroolsRule()).thenReturn(droolsRule);
		Mockito.lenient().when(droolsRule.getSourceRule()).thenReturn(rule);
		...
		actionsGenerator.setDroolsStringUtils(droolsStringUtils);
	}
	...
	public void testGenerateCode() throws IOException
	{
		...
		Mockito.lenient().when(droolsRuleValueFormatter.formatValue(droolsRuleGeneratorContext, actionParameters)).thenReturn(
