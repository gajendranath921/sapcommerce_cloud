...
public class RuleQualifyingCategoriesConditionTranslatorTest
{
	...
	public void setUp()
	{
		...
		Mockito.lenient().when(parameters.get(RuleQualifyingCategoriesConditionTranslator.OPERATOR_PARAM)).thenReturn(operatorParameter);
		Mockito.lenient().when(parameters.get(RuleQualifyingCategoriesConditionTranslator.QUANTITY_PARAM)).thenReturn(quantityParameter);
		Mockito.lenient().when(parameters.get(RuleQualifyingCategoriesConditionTranslator.CATEGORIES_OPERATOR_PARAM))
				.thenReturn(categoriesOperatorParameter);
		Mockito.lenient().when(parameters.get(RuleQualifyingCategoriesConditionTranslator.CATEGORIES_PARAM)).thenReturn(categoriesParameter);
		Mockito.lenient().when(operatorParameter.getValue()).thenReturn(AmountOperator.GREATER_THAN);
		Mockito.lenient().when(quantityParameter.getValue()).thenReturn(1);
		Mockito.lenient().when(context.createLocalContainer()).thenReturn(variablesContainer);

		final List<String> categoryList = new ArrayList<>();
		categoryList.add("categoryCode1");
		categoryList.add("categoryCode2");
		Mockito.lenient().when(categoriesParameter.getValue()).thenReturn(categoryList);
	...
	{
		Mockito.lenient().when(parameters.get(RuleQualifyingCategoriesConditionTranslator.OPERATOR_PARAM)).thenReturn(null);
	...
	{
		Mockito.lenient().when(parameters.get(RuleQualifyingCategoriesConditionTranslator.CATEGORIES_OPERATOR_PARAM)).thenReturn(null);
		final RuleIrCondition ruleIrCondition = translator.translate(context, condition, conditionDefinition);
		assertThat(ruleIrCondition, instanceOf(RuleIrEmptyCondition.class));
	}
	...
	{
		Mockito.lenient().when(parameters.get(RuleQualifyingCategoriesConditionTranslator.CATEGORIES_PARAM)).thenReturn(null);
		final RuleIrCondition ruleIrCondition = translator.translate(context, condition, conditionDefinition);
		assertThat(ruleIrCondition, instanceOf(RuleIrEmptyCondition.class));
	}
	...
	{
		Mockito.lenient().when(categoriesOperatorParameter.getValue()).thenReturn(CollectionOperator.NOT_CONTAINS);
	...
	{
		Mockito.lenient().when(categoriesOperatorParameter.getValue()).thenReturn(CollectionOperator.CONTAINS_ANY);
	...
	{
		Mockito.lenient().when(categoriesOperatorParameter.getValue()).thenReturn(CollectionOperator.CONTAINS_ANY);
		Mockito.lenient().when(parameters.get(RuleQualifyingCategoriesConditionTranslator.EXCLUDED_CATEGORIES_PARAM))
		...
		Mockito.lenient().when(excludedCategoriesParameter.getValue()).thenReturn(excludedCategories);
	...
	{
		Mockito.lenient().when(categoriesOperatorParameter.getValue()).thenReturn(CollectionOperator.CONTAINS_ANY);
		Mockito.lenient().when(parameters.get(RuleQualifyingCategoriesConditionTranslator.EXCLUDED_PRODUCTS_PARAM))
		...
		Mockito.lenient().when(excludedProductsParameter.getValue()).thenReturn(excludedProducts);
	...
	{
		Mockito.lenient().when(categoriesOperatorParameter.getValue()).thenReturn(CollectionOperator.CONTAINS_ALL);
