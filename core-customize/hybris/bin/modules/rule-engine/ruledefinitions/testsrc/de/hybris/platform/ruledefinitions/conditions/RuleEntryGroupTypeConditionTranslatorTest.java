...
public class RuleEntryGroupTypeConditionTranslatorTest
{
	...
	public void setUp()
	{
		Mockito.lenient().when(parameters.get("operator")).thenReturn(operatorParameter);
		Mockito.lenient().when(parameters.get("groupTypes")).thenReturn(valueParameter);
		...
		Mockito.lenient().when(context.generateVariable(OrderEntryGroupRAO.class)).thenReturn(ORDER_ENTRY_GROUP_RAO_VAR);
		Mockito.lenient().when(context.generateVariable(CartRAO.class)).thenReturn(CART_RAO_VAR);
		Mockito.lenient().when(context.generateVariable(Integer.class)).thenReturn(ENTRY_GROUP_NUMBER_VAR);
	}

	@Test
	public void shouldTranslateOperatorParamNull()
	{
		Mockito.lenient().when(parameters.get("operator")).thenReturn(null);
	...
	{
		Mockito.lenient().when(parameters.get("groupTypes")).thenReturn(null);
		final RuleIrCondition ruleIrCondition = translator.translate(context, condition, conditionDefinition);
	...
	{
		Mockito.lenient().when(valueParameter.getValue()).thenReturn(emptyList());
		final RuleIrCondition ruleIrCondition = translator.translate(context, condition, conditionDefinition);
	...
	{
		Mockito.lenient().when(operatorParameter.getValue()).thenReturn(MembershipOperator.IN);
	...
	{
		Mockito.lenient().when(operatorParameter.getValue()).thenReturn(MembershipOperator.NOT_IN);
