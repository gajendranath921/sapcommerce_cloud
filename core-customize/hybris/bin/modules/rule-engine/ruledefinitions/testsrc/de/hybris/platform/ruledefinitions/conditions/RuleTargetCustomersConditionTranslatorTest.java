...
public class RuleTargetCustomersConditionTranslatorTest
{
	...
	public void setUp()
	{
		...
		Mockito.lenient().when(parameters.get(RuleTargetCustomersConditionTranslator.CUSTOMER_GROUPS_OPERATOR_PARAM)).thenReturn(
				customerGroupsOperatorParameter);
		Mockito.lenient().when(parameters.get(RuleTargetCustomersConditionTranslator.CUSTOMER_GROUPS_PARAM)).thenReturn(customerGroupsParameter);
		Mockito.lenient().when(parameters.get(RuleTargetCustomersConditionTranslator.CUSTOMERS_PARAM)).thenReturn(customersParameter);
		Mockito.lenient().when(parameters.get(RuleTargetCustomersConditionTranslator.EXCLUDED_USERS_PARAM)).thenReturn(excludedCustomersParameter);

		final List<String> customerGroups = new ArrayList<>();
		customerGroups.add("customerGroup1");
		customerGroups.add("customerGroup2");
		Mockito.lenient().when(customerGroupsParameter.getValue()).thenReturn(customerGroups);
		final List<String> customers = new ArrayList<>();
		customers.add("customer1");
		customers.add("customer2");
		Mockito.lenient().when(customersParameter.getValue()).thenReturn(customers);

		Mockito.lenient().when(context.generateVariable(UserRAO.class)).thenReturn(USER_RAO_VAR);
		Mockito.lenient().when(context.generateVariable(CartRAO.class)).thenReturn(CART_RAO_VAR);
		Mockito.lenient().when(context.generateVariable(UserGroupRAO.class)).thenReturn(USER_GROUP_RAO_VAR);
	}

	@Test
	public void testTranslateCustomersParamNull()
	{
		Mockito.lenient().when(parameters.get(RuleTargetCustomersConditionTranslator.CUSTOMER_GROUPS_PARAM)).thenReturn(null);
		Mockito.lenient().when(parameters.get(RuleTargetCustomersConditionTranslator.CUSTOMERS_PARAM)).thenReturn(null);
	...
	{
		Mockito.lenient().when(customerGroupsOperatorParameter.getValue()).thenReturn(CollectionOperator.NOT_CONTAINS);
		final RuleIrCondition ruleIrCondition = translator.translate(context, condition, conditionDefinition);
	...
	{
		Mockito.lenient().when(customerGroupsOperatorParameter.getValue()).thenReturn(CollectionOperator.CONTAINS_ANY);
	...
	public void testTranslateAnyOperatorWithExcludedCondition()
	{
		...
		Mockito.lenient().when(excludedCustomersParameter.getValue()).thenReturn(exludeCustomers);
	...
	{
		Mockito.lenient().when(customerGroupsOperatorParameter.getValue()).thenReturn(CollectionOperator.CONTAINS_ALL);
