...
public class DefaultPlatformRuleEngineServiceUnitTest
{
	...
	public void setUp()
	{
		...
		Mockito.lenient().when(kieServices.newReleaseId(anyString(), anyString(), anyString())).thenReturn(releaseId);
		Mockito.lenient().when(kieServices.newKieBuilder(any(KieFileSystem.class))).thenReturn(kieBuilder);
		Mockito.lenient().when(configurationService.getConfiguration()).thenReturn(configurationMock);
		...
		Mockito.lenient().when(postRulesModuleSwappingTasksProvider.getTasks(any())).thenReturn(singletonList(Object::new));
	}
	...
	public void testUpdateEngineRuleAbstractRuleEngineRuleModelForSuccessCondition()
	{
		...
		Mockito.lenient().when(droolsKIEBaseFinderStrategy.getKIEBaseForKIEModule(droolsKIEModuleModel)).thenReturn(kieBase);
	...
	{
		// given (Mockito.lenient().when switching causes an unrelated exception)
		final DroolsKIEModuleModel module = mock(DroolsKIEModuleModel.class);
		Mockito.lenient().when(module.getName()).thenReturn("myModuleName");
		...
		// Mockito.lenient().when (switching modules)
