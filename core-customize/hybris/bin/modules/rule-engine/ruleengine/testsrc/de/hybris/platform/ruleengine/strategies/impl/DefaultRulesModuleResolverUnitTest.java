...
public class DefaultRulesModuleResolverUnitTest
{
	...
	public void shouldReturnRulesModulesThatMapToDeployedDroolsRules()
	{
		...
		Mockito.lenient().when(engineRulesRepository.checkEngineRuleDeployedForModule(rule1, "module1")).thenReturn(true);
		Mockito.lenient().when(engineRulesRepository.checkEngineRuleDeployedForModule(rule2, "module2")).thenReturn(true);


		final DroolsKIEBaseModel kieBase4 = newKieBase("module2");
		final DroolsRuleModel rule4 = newDroolsRule(kieBase4, "rule4");

		Mockito.lenient().when(engineRulesRepository.checkEngineRuleDeployedForModule(rule4, "module2")).thenReturn(false);

		Mockito.lenient().when(ruleModel.getEngineRules()).thenReturn(ImmutableSet.of(rule1, rule2, rule3));
