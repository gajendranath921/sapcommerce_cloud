...
public class RuleGroupRemoveInterceptorUnitTest
{
	...
	public void setUp()
	{
		Mockito.lenient().when(l10NService.getLocalizedString(eq("error.rulegroup.cantremovehasrules"), any(Object[].class))).thenReturn(
				"err message");
		Mockito.lenient().when(removableRuleGroup.getRules()).thenReturn(Collections.emptySet());
		Mockito.lenient().when(nonRemovableRuleGroup.getRules()).thenReturn(Collections.singleton(new SourceRuleModel()));
	}
