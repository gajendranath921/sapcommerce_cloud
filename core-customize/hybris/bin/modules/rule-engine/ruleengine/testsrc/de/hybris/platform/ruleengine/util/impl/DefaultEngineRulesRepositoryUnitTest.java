...
public class DefaultEngineRulesRepositoryUnitTest
{
	...
	public void testCheckEngineRuleDeployedForModuleNoRuleFound()
	{
		...
		Mockito.lenient().when(engineRuleDao.getRuleByCodeAndMaxVersion(RULE_CODE, MODULE_NAME, 1L)).thenReturn(null);
	...
	public void testCheckEngineRuleDeployedForModuleRuleNotActive()
	{
		...
		Mockito.lenient().when(engineRuleDao.getRuleByCodeAndMaxVersion(RULE_CODE, MODULE_NAME, 1L)).thenReturn(engineRule);
	...
	public void testCheckEngineRuleDeployedForModuleRuleNotEqualTheLatestVersion()
	{
		...
		Mockito.lenient().when(engineRuleDao.getRuleByCodeAndMaxVersion(RULE_CODE, MODULE_NAME, 5L)).thenReturn(deployedEngineRule);
	...
	public void testGetDeployedEngineRulesForModuleNoRuleFound()
	{
		...
		Mockito.lenient().when(engineRuleDao.getActiveRulesForVersion(MODULE_NAME, 1L)).thenReturn(Collections.emptyList());
	...
	public void testGetDeployedEngineRulesForModuleOk()
	{
		...
		Mockito.lenient().when(engineRuleDao.getActiveRulesForVersion(MODULE_NAME, 1L)).thenReturn(Collections.singletonList(engineRule));
	...
	private DroolsKIEModuleModel createDroolsRulesModule(final Long version)
	{
		...
		Mockito.lenient().when(rulesModuleDao.findByName(MODULE_NAME)).thenReturn(module);
		Mockito.lenient().when(moduleVersionResolver.getDeployedModuleVersion(module)).thenReturn(Optional.ofNullable(version));
		return module;
	}
