...
public class RemoveOldKieModulePostRulesModuleSwappingTaskUnitTest
{
	...
	{
		Mockito.lenient().when(ruleEngineActionResult.isActionFailed()).thenReturn(Boolean.FALSE);
		Mockito.lenient().when(ruleEngineKieModuleSwapper.removeOldKieModuleIfPresent(ruleEngineActionResult)).thenReturn(Boolean.TRUE);
	...
	{
		Mockito.lenient().when(ruleEngineActionResult.isActionFailed()).thenReturn(Boolean.TRUE);
		final boolean success = swappingTask.execute(ruleEngineActionResult);
