...
public class DefaultPostRulesModuleSwappingTasksProviderUnitTest
{
	...
	public void setUp()
	{
		...
		Mockito.lenient().when(task1.execute(ruleEngineActionResult)).thenReturn(Boolean.TRUE);
		Mockito.lenient().when(task2.execute(ruleEngineActionResult)).thenReturn(Boolean.FALSE);
	}
