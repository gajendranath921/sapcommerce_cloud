...
public class RuleEngineRuleRemoveInterceptorUnitTest
{
	...
	public void setUp()
	{
		...
		Mockito.lenient().when(rulesBase.getKieModule()).thenReturn(rulesModule);
		Mockito.lenient().when(rulesModule.getName()).thenReturn(MODULE_NAME);

		Mockito.lenient().when(engineRuleDao.getRuleByCodeAndMaxVersion(RULE_CODE, MODULE_NAME, 0l)).thenReturn(previousVersionModel);
	}
	...
	{
		Mockito.lenient().when(l10NService.getLocalizedString("exception.ruleengineruleremoveinterceptor.version",new Object[]{model})).
				thenReturn("Cannot remove {0}. It is not a current rule version!");
	...
	{
		Mockito.lenient().when(engineRuleDao.getRuleByCodeAndMaxVersion(RULE_CODE, MODULE_NAME, 0L)).thenReturn(null);
		removeInterceptor.onRemove(model, context);
		verify(ruleEngineRemoveHandler, times(1)).handleOnRemove(model, context);
