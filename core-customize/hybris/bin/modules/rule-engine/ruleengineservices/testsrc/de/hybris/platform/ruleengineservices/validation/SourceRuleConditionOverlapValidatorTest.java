...
public class SourceRuleConditionOverlapValidatorTest
{
	...
	public void setUp()
	{
		...
		Mockito.lenient().when(sourceRuleModel.getConditions()).thenReturn("conditions");
	}
	...
	public void shouldReturnTrueOnContainersSizeLessThanThreshold()
	{
		...
		Mockito.lenient().when(ruleConditionsRegistry.getConditionDefinitionsForRuleTypeAsMap(sourceRuleModel.getClass()))
		...
		Mockito.lenient().when(ruleConditionsService.convertConditionsFromString("conditions", conditionDefinitions)).thenReturn(ruleConditions);
	...
	public void shouldReturnFalseOnContainersOverlap()
	{
		...
		final RuleConditionData ruleCondition1 = mock(RuleConditionData.class);
		Mockito.lenient().when(ruleCondition1.getDefinitionId()).thenReturn("containerDefinitionId");

		final RuleConditionData ruleCondition2 = mock(RuleConditionData.class);

		final RuleConditionData ruleCondition3 = mock(RuleConditionData.class);
		Mockito.lenient().when(ruleCondition3.getDefinitionId()).thenReturn("containerDefinitionId");
		...
		Mockito.lenient().when(ruleCondition5.getDefinitionId()).thenReturn("containerDefinitionId");
		...
		Mockito.lenient().when(conditionToCategoriesMapper.apply(ruleCondition1)).thenReturn(List.of(category11, category12));
		Mockito.lenient().when(conditionToCategoriesMapper.apply(ruleCondition3)).thenReturn(List.of(category31));
		Mockito.lenient().when(conditionToCategoriesMapper.apply(ruleCondition5)).thenReturn(List.of(category51, category52, category53));
		...
		Mockito.lenient().when(conditionToProductsMapper.apply(ruleCondition1)).thenReturn(List.of(product11));
		Mockito.lenient().when(conditionToProductsMapper.apply(ruleCondition3)).thenReturn(List.of());
		Mockito.lenient().when(conditionToProductsMapper.apply(ruleCondition5)).thenReturn(List.of(product51, product52));
	...
	protected CategoryModel category(final String code, final CategoryModel... relatedCategories)
	{
		...
		Mockito.lenient().when(category.getCode()).thenReturn(code);
		final Set<String> codes = Arrays.stream(relatedCategories).map(CategoryModel::getCode).collect(Collectors.toSet());
		codes.add(code);
		Mockito.lenient().when(categoryToCodesMapper.apply(category)).thenReturn(codes);
		return category;
	}
	...
	protected ProductModel product(final String code, final CategoryModel... superCategories)
	{
		...
		Mockito.lenient().when(product.getCode()).thenReturn(code);
		...
		Mockito.lenient().when(productToCodesMapper.apply(product)).thenReturn(categoryCodes);
		return product;
	}
}
