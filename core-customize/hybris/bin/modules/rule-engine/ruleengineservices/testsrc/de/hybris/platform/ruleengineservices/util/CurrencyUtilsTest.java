...
public class CurrencyUtilsTest
{
	...
	public void setUp()
	{
		...
		Mockito.lenient().when(commonI18NService.getCurrency(USD)).thenReturn(currencyModel);

		currencyModelEUR = new CurrencyModel();
		currencyModelEUR.setIsocode(EUR);
		currencyModelEUR.setDigits(2);
		Mockito.lenient().when(commonI18NService.getCurrency(EUR)).thenReturn(currencyModelEUR);
	}
	...
	public void testConvertCurrencyWithConverter()
	{
		...
		final BigDecimal value = new BigDecimal("100");
		Mockito.lenient().when(commonI18NService.convertAndRoundCurrency(Mockito.eq(conversionRateUS), Mockito.eq(conversionRateEUR),
