...
public class DefaultRuleCompilerSpliteratorUnitTest
{
	...
	public void setUp()
	{
		Mockito.lenient().when(ruleCompilationContext.getThreadTimeout()).thenReturn(1000L);
		Mockito.lenient().when(ruleCompilationContext.getNumberOfThreads()).thenReturn(10);
		Mockito.lenient().when(ruleCompilationContext.getCurrentTenant()).thenReturn(tenant);
		Mockito.lenient().when(ruleCompilationContext.getThreadFactory()).thenReturn(threadFactory);
		Mockito.lenient().when(ruleCompilationContext.getRuleCompilerService()).thenReturn(ruleCompilerService);
		Mockito.lenient().when(ruleCompilationContext.getSuspendResumeTaskManager()).thenReturn(suspendResumeTaskManager);
		Mockito.lenient().when(ruleCompilerService.compile(eq(ruleCompilationContext), any(SourceRuleModel.class), eq(MODULE_NAME)))
					 .thenReturn(ruleCompilerResult);
		Mockito.lenient().when(ruleCompilerResult.getResult()).thenReturn(RuleCompilerResult.Result.SUCCESS);
	...
	private RuleCompilerResult getSuccessfulRuleCompilerResult()
	{
		...
		Mockito.lenient().when(result.getResult()).thenReturn(RuleCompilerResult.Result.SUCCESS);
		return result;
	}
