...
public class DefaultRuleServiceUnitTest
{
	...
	public void setup()
	{
		Mockito.lenient().when(droolsRuleEngineContextModel.getKieSession()).thenReturn(droolsKIESessionModel);
		Mockito.lenient().when(droolsKIESessionModel.getKieBase()).thenReturn(droolsKIEBaseModel);
		Mockito.lenient().when(droolsKIEBaseModel.getKieModule()).thenReturn(droolsKIEModuleModel);
		Mockito.lenient().when(droolsKIEModuleModel.getName()).thenReturn(MODULE_NAME);
	}

	@Test
	public void testFindEngineRuleTypeForRuleType()
	{
		Mockito.lenient().when(ruleDao.findEngineRuleTypeByRuleType(SourceRuleModel.class)).thenReturn(RuleType.DEFAULT);
	...
	{
		Mockito.lenient().when(ruleDao.findEngineRuleTypeByRuleType(SourceRuleModel.class)).thenReturn(null);
	...
	{
		Mockito.lenient().when(droolsKIESessionModel.getKieBase()).thenReturn(null);
	...
	{
		Mockito.lenient().when(ruleEngineContextFinderStrategy.getRuleEngineContextForCatalogVersions(singletonList(catalogVersionModel),
	...
	public void testAllRulesForStatus()
	{
		...
		Mockito.lenient().when(ruleDao.findAllRulesWithStatuses(statuses)).thenReturn(daoResult);
	...
	public void testActiveRulesShouldPropagateIllegalArgumentException()
	{
		...
		Mockito.lenient().when(moduleVersionResolver.getDeployedModuleVersion(droolsKIEModuleModel)).thenThrow(IllegalArgumentException.class);
	...
	public void testActiveRulesShouldReturnEmptyListWhenActiveRulesAreFound()
	{
		Mockito.lenient().when(ruleEngineContextFinderStrategy.getRuleEngineContextForCatalogVersions(singletonList(catalogVersionModel),
				RuleType.DEFAULT)).thenReturn(Optional.of(droolsRuleEngineContextModel));
		Mockito.lenient().when(engineRuleDao.getActiveRulesForVersion(MODULE_NAME, 1)).thenReturn(emptyList());
		Mockito.lenient().when(moduleVersionResolver.getDeployedModuleVersion(droolsKIEModuleModel)).thenReturn(Optional.of(Long.valueOf(1)));
	...
	public void testActiveRulesShouldReturnSourceRules()
	{
		...
		Mockito.lenient().when(engineRule1.getSourceRule()).thenReturn(rule1);
		Mockito.lenient().when(engineRule2.getSourceRule()).thenReturn(rule2);

		Mockito.lenient().when(ruleEngineContextFinderStrategy.getRuleEngineContextForCatalogVersions(singletonList(catalogVersionModel),
				RuleType.DEFAULT)).thenReturn(Optional.of(droolsRuleEngineContextModel));
		Mockito.lenient().when(engineRuleDao.getActiveRulesForVersion(MODULE_NAME, 1)).thenReturn(asList(engineRule1, engineRule2));
		Mockito.lenient().when(moduleVersionResolver.getDeployedModuleVersion(droolsKIEModuleModel)).thenReturn(Optional.of(Long.valueOf(1)));
