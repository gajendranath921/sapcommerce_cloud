...
public class UpdateRulesStatusPostRulesModuleSwappingTaskUnitTest
{
	...
	public void testExecuteInactiveToPublished()
	{
		...
		verify(modelService, times(2)).saveAll(MockitoHamcrest.argThat(new SourceRulesMatcher(sourceRulesList)));
	}
	...
	private AbstractRuleEngineRuleModel createEngineRule(final String code, final Boolean active, final Long version)
	{
		...
		Mockito.lenient().when(kieModule.getDeployedMvnVersion()).thenReturn(DEPLOYED_MVN_VERSION);
		Mockito.lenient().when(kieModule.getMvnVersion()).thenReturn(MVN_VERSION);
	...
	private void mockRuleService(final int codeIdx, final AbstractRuleModel sourceRule)
	{
		...
		Mockito.lenient().when(ruleService.getAllRulesForCodeAndStatus("ruleCode" + codeIdx, RuleStatus.PUBLISHED, RuleStatus.INACTIVE)).thenReturn(
				rules);
	}
