...
public class ProductToCategoryCodesMapperTest
{
	...
	{
		Mockito.lenient().when(product.getSupercategories()).thenReturn(null);
	...
	{
		Mockito.lenient().when(product.getSupercategories()).thenReturn(Collections.emptyList());
	...
	public void shouldMapCategoryCodesCorrectly()
	{
		...
		Mockito.lenient().when(product.getSupercategories()).thenReturn(List.of(category1, category2, category3, category4));
		final Set<String> result = mapper.apply(product);
