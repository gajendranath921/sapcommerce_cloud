...
public class DefaultRuleCompilerServiceTest
{
	...
	{
		Mockito.lenient().when(ruleCompilerListenersFactory.getListeners(RuleCompilerListener.class)).thenReturn(Collections.emptyList());
		Mockito.lenient().when(ruleIrVariablesGeneratorFactory.createVariablesGenerator()).thenReturn(variablesGenerator);
		Mockito.lenient().when(ruleCompilerContextFactory.createContext(ruleCompilationContext, rule, MODULE_NAME, variablesGenerator))
				.thenReturn(context);
		Mockito.lenient().when(context.getProblems()).thenReturn(Collections.emptyList());
		Mockito.lenient().when(ruleCompilerResultFactory.create(eq(rule), ruleCompilerProblemsCaptor.capture()))
				.thenReturn(ruleCompilerResult);

		Mockito.lenient().when(ruleSourceCodeTranslatorFactory.getSourceCodeTranslator(context)).thenReturn(ruleSourceCodeTranslator);
		Mockito.lenient().when(ruleIrProcessorFactory.getRuleIrProcessors()).thenReturn(Collections.singletonList(ruleIrProcessor));
		Mockito.lenient().when(ruleTargetCodeGeneratorFactory.getTargetCodeGenerator(context)).thenReturn(ruleTargetCodeGenerator);
	...
	{
		Mockito.lenient().when(ruleSourceCodeTranslator.translate(context)).thenThrow(NullPointerException.class);
