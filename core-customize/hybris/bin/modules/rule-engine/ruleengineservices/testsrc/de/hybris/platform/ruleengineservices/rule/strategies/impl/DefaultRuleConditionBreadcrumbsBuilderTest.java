...
public class DefaultRuleConditionBreadcrumbsBuilderTest
{
	...
	public void setUp()
	{
		...
		Mockito.lenient().when(l10NService.getLocalizedString(buildLocalizationKey(RuleGroupOperator.AND)))
				.thenReturn(RuleGroupOperator.AND.toString());
		Mockito.lenient().when(l10NService.getLocalizedString(buildLocalizationKey(RuleGroupOperator.OR)))
				.thenReturn(RuleGroupOperator.OR.toString());
		Mockito.lenient().when(ruleParameterValueNormalizerStrategy.normalize(any(), nullable(String.class))).then(returnsFirstArg());
