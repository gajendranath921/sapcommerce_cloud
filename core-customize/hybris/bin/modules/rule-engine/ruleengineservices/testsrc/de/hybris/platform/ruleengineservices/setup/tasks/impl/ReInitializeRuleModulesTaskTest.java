...
public class ReInitializeRuleModulesTaskTest
{
	...
	{
		Mockito.lenient().when(ruleService.getEngineRuleTypeForRuleType(SourceRuleModel.class)).thenReturn(RuleType.DEFAULT);
		Mockito.lenient().when(activeModule.getActive()).thenReturn(Boolean.TRUE);
		Mockito.lenient().when(activeModule.getName()).thenReturn(LIVE_MODULE);
	}
	...
	public void shouldCompileAndPublishAllInactiveRules() throws Exception
	{
		...
		Mockito.lenient().when(rulesModuleDao.findActiveRulesModulesByRuleType(RuleType.DEFAULT)).thenReturn(
				newArrayList(activeModule));
		Mockito.lenient().when(ruleDao.findByVersionAndStatuses(RuleEngineServicesConstants.DEFAULT_RULE_VERSION, RuleStatus.INACTIVE))
				.thenReturn(newArrayList(sourceRule, sourceRule2, abstractRule));
		//when
	...
	{
		//Mockito.lenient().when
		Mockito.lenient().when(rulesModuleDao.findActiveRulesModulesByRuleType(RuleType.DEFAULT)).thenReturn(Collections.emptyList());
		//when
		reInitializeRuleModulesTask.execute(context);
		//then
