...
public class CategoryToProductCodesMapperTest
{
	...
	public void setUp()
	{
		...
		Mockito.lenient().when(i18NService.getCurrentLocale()).thenReturn(Locale.ENGLISH);
		Mockito.lenient().when(commonI18NService.getLanguage(Locale.ENGLISH.getLanguage())).thenReturn(language);

		Mockito.lenient().when(sessionService.executeInLocalView(any(SessionExecutionBody.class))).thenAnswer(
	...
	public void shouldMapCategoryToCodesCorrectly()
	{
		...
		Mockito.lenient().when(product1.getCode()).thenReturn("product1");

		final ProductModel product2 = mock(ProductModel.class);
		Mockito.lenient().when(product2.getCode()).thenReturn("product2");

		final ProductModel product3 = mock(ProductModel.class);
		Mockito.lenient().when(product3.getCode()).thenReturn("product3");

		final ProductModel product4 = mock(ProductModel.class);
		Mockito.lenient().when(product4.getCode()).thenReturn("product1");

		Mockito.lenient().when(productService.getProductsForCategory(category)).thenReturn(List.of(product1, product2, product3,
				product4));
