...
public class ObjectPatternValidatorUnitTest
{
	...
	public void setUp()
	{
		...
		Mockito.lenient().when(parameters.flags()).thenReturn(new Flag[]
		{ Flag.UNICODE_CASE });
		Mockito.lenient().when(parameters.regexp()).thenReturn(REGEXP);
	}
	...
	{
		Mockito.lenient().when(parameters.regexp()).thenReturn("\\M");
		validator.initialize(parameters);
	}
