...
public class DefaultRuleConditionsConverterTest
{
	...
	public void setUp() throws Exception
	{
		...
		Mockito.lenient().when(ruleParameterValueConverter.toString(STRING_VALUE)).thenReturn(STRING_VALUE_JSON);
		Mockito.lenient().when(ruleParameterValueConverter.toString(INTEGER_VALUE)).thenReturn(INTEGER_VALUE_JSON);

		Mockito.lenient().when(ruleParameterValueConverter.fromString(STRING_VALUE_JSON, String.class.getName())).thenReturn(STRING_VALUE);
		Mockito.lenient().when(ruleParameterValueConverter.fromString(INTEGER_VALUE_JSON, Integer.class.getName())).thenReturn(INTEGER_VALUE);
	...
	protected List<RuleConditionData> createRuleConditionsSimple()
	{
		...
		Mockito.lenient().when(ruleConditionsRegistry.getAllConditionDefinitionsAsMap()).thenReturn(
	...
	protected List<RuleConditionData> createRuleConditionsComplex()
	{
		...
		Mockito.lenient().when(ruleConditionsRegistry.getAllConditionDefinitionsAsMap()).thenReturn(ruleConditionDefinitions);
	...
	{
		if (ruleConditions1 == ruleConditions2)
		{
			return true;
		}
