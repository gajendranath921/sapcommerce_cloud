...
public class DefaultRuleEngineCalculationServiceTest extends AbstractRuleEngineTest
{
	...
	public void testSimpleCalculateTotals()
	{
		...
		simple01.setEntries(Collections.singleton(createOrderEntryRAO(simple01, "12.34",
				USD, 2, 0)));
		...
		assertEquals(new BigDecimal("100.03"), simple02.getTotalIncludingCharges());
		assertEquals(new BigDecimal("95.03"), simple02.getTotal());
		assertEquals(new BigDecimal("95.03"), simple02.getSubTotal());
	...
	@Test
	public void testAddOrderLevelDiscountPercentageAfterDiscountAbsolute()
	{
		// simple cart with 2 entries and delivery cost
		final CartRAO simple03 = createCartRAO("simple03", USD);
		simple03.setEntries(
				set(createOrderEntryRAO(simple03, "12.34", USD, 2, 0), createOrderEntryRAO(simple03, "23.45", USD, 3, 1)));
		simple03.setDeliveryCost(new BigDecimal("5.00"));
		getRuleEngineCalculationService().calculateTotals(simple03);
		assertEquals(new BigDecimal("100.03"), simple03.getTotalIncludingCharges());
		assertEquals(new BigDecimal("95.03"), simple03.getTotal());
		...
		// first add a 10USD oder-level discount
		...
		// then add a 10% oder-level discount
		// 10% of discounted total = 10% (95.03 - 10) = 10% (85.03) = 8.50
		// total is 95.03 - 10 - 8.50 = 76.53
		getRuleEngineCalculationService().addOrderLevelDiscount(simple03, false, new BigDecimal("10.00"));
		assertEquals(new BigDecimal("81.53"), simple03.getTotalIncludingCharges());
		assertEquals(new BigDecimal("76.53"), simple03.getTotal());
		assertEquals(new BigDecimal("5.00"), simple03.getDeliveryCost());
		assertEquals(new BigDecimal("95.03"), simple03.getSubTotal());
	}
	...
	@Test
	public void testAddOrderEntryLevelDiscountAddPerUnitParameterPercentage()
	{
		// simple cart with 2 entries and delivery cost
		final CartRAO simple04 = createCartRAO("simple04", USD);
		final OrderEntryRAO orderEntry1 = createOrderEntryRAO(simple04, "12.34", USD, 2, 0);
		...
		// subtotal is: 12.34*2-rounded_down(12.34*2*0.1) + 23.45*3 = 24.68 - rounded_down(2.468) + 70.35 = 24.68-2.46+70.35=92.57
		// total is: 12.34*2-rounded_down(12.34*2*0.1) + 23.45*3 + 5 = 24.68 - rounded_down(2.468) + 70.35 + 5 = 24.68-2.46+70.35+5 = 97.57
		getRuleEngineCalculationService().addOrderEntryLevelDiscount(orderEntry1, false, new BigDecimal("10.00"),false);
		assertEquals(new BigDecimal("5.00"), simple04.getDeliveryCost());
		assertEquals(new BigDecimal("92.57"), simple04.getSubTotal());
		assertEquals(new BigDecimal("92.57"), simple04.getTotal());
		assertEquals(new BigDecimal("97.57"), simple04.getTotalIncludingCharges());
	...
	@Test
	public void testAddOrderEntryLevelDiscountAddPerUnitParameterAbsolute()
	{
		// simple cart with 2 entries and delivery cost
		final CartRAO simple05 = createCartRAO("simple05", USD);
		final OrderEntryRAO orderEntry1 = createOrderEntryRAO(simple05, "12.34", USD, 2, 0);
		...
		// now add a 10USD oder-entry-level discount
		// subtotal is: (12.34)*2 -10 + 23.45*3 = 75.03
		// total is: (12.34)*2 -10 + 23.45*3 + 5 = 80.03
		getRuleEngineCalculationService().addOrderEntryLevelDiscount(orderEntry1, true, new BigDecimal("10.00"),false);
		assertEquals(new BigDecimal("5.00"), simple05.getDeliveryCost());
		assertEquals(new BigDecimal("85.03"), simple05.getSubTotal());
		assertEquals(new BigDecimal("85.03"), simple05.getTotal());
		assertEquals(new BigDecimal("90.03"), simple05.getTotalIncludingCharges());
	}
