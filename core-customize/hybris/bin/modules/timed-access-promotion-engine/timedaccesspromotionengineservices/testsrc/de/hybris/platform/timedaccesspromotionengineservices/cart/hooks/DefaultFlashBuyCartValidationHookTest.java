...
public class DefaultFlashBuyCartValidationHookTest
{
	...
	public void prepare()
	{
		...
		Mockito.lenient().when(cart.isProcessingFlashBuyOrder()).thenReturn(false);
		Mockito.lenient().when(cart.getAppliedCouponCodes()).thenReturn(couponCodes);
		Mockito.lenient().when(flashBuyCoupon.getProduct()).thenReturn(product);

		Mockito.lenient().when(promotionsService.getPromotionResults(cart)).thenReturn(promotionOrderResults);
		Mockito.lenient().when(promotionOrderResults.getAppliedProductPromotions()).thenReturn(appliedProductPromotions);
		Mockito.lenient().when(promotionResult.getPromotion()).thenReturn(promotion);
		Mockito.lenient().when(promotion.getCode()).thenReturn(PROMOTION_CODE);
		Mockito.lenient().when(flashBuyService.getPromotionByCode(PROMOTION_CODE)).thenReturn(promotionModel);
		Mockito.lenient().when(couponDao.findCouponById(COUPON_CODE)).thenReturn(flashBuyCoupon);


		Mockito.lenient().when(promotionResult.getPromotion()).thenReturn(promotion);
		Mockito.lenient().when(promotion.getCode()).thenReturn(PROMOTION_CODE);
		Mockito.lenient().when(flashBuyService.getPromotionByCode(PROMOTION_CODE)).thenReturn(promotionModel);
		Mockito.lenient().when(promotionModel.getCode()).thenReturn(PROMOTION_CODE);
		Mockito.lenient().when(flashBuyService.getFlashBuyCouponByPromotionCode(PROMOTION_CODE)).thenReturn(flashBuyCouponOptional);
		Mockito.lenient().when(flashBuyCoupon.getCouponId()).thenReturn(FLASHBUY_COUPON_CODE);
		Mockito.lenient().when(couponService.redeemCoupon(FLASHBUY_COUPON_CODE, cart)).thenReturn(response);
		Mockito.lenient().when(couponService.validateCouponCode(FLASHBUY_COUPON_CODE, null)).thenReturn(response);
	}
	...
	public void testBeforeValidateCart_checkout()
	{
		...
		Mockito.lenient().when(cart.isProcessingFlashBuyOrder()).thenReturn(false);
		Mockito.lenient().when(cartService.getEntriesForProduct(cart, product)).thenReturn(cartEntries);
	...
	public void testBeforeValidateCart_non_flashBuyCouppon_checkout()
	{
		response.setSuccess(false);
		Mockito.lenient().when(cart.isProcessingFlashBuyOrder()).thenReturn(false);
		Mockito.lenient().when(couponDao.findCouponById(COUPON_CODE)).thenReturn(multiCodeCoupon);
		Mockito.lenient().when(flashBuyService.getProductForPromotion(promotionModel)).thenReturn(productOptional);
		Mockito.lenient().when(cartService.getEntriesForProduct(cart, product)).thenReturn(cartEntries);
	...
	public void testBeforeValidateCart_non_flashBuyPromotion_checkout()
	{
		Mockito.lenient().when(cart.isProcessingFlashBuyOrder()).thenReturn(false);
		Mockito.lenient().when(promotionsService.getPromotionResults(cart)).thenReturn(promotionOrderResults);
		Mockito.lenient().when(promotionOrderResults.getAppliedProductPromotions()).thenReturn(Collections.emptyList());
		hook.beforeValidateCart(parameter, modifications);
	...
	{
		response.setSuccess(false);
		Mockito.lenient().when(cart.isProcessingFlashBuyOrder()).thenReturn(true);
		Mockito.lenient().when(couponDao.findCouponById(COUPON_CODE)).thenReturn(multiCodeCoupon);
		Mockito.lenient().when(flashBuyService.getFlashBuyCouponByPromotionCode(PROMOTION_CODE)).thenReturn(Optional.empty());
	...
	public void testBeforeValidateCart_flashCoupon_placeorder()
	{
		response.setSuccess(true);
		Mockito.lenient().when(cart.isProcessingFlashBuyOrder()).thenReturn(true);
		Mockito.lenient().when(cart.getAppliedCouponCodes()).thenReturn(flashBuyCouponCodes);
		Mockito.lenient().when(couponDao.findCouponById(FLASHBUY_COUPON_CODE)).thenReturn(flashBuyCoupon);
