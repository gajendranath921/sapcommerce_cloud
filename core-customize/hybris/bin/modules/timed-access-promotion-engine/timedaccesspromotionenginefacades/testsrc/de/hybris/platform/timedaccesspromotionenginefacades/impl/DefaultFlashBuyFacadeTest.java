...
public class DefaultFlashBuyFacadeTest
{
	...
	public void setup()
	{
		...
		Mockito.lenient().when(product.getPotentialPromotions()).thenReturn(promotions);
		Mockito.lenient().when(coupon.getCouponId()).thenReturn(COUPON_CODE);
	...
	{
		promotions.add(promotion);
		Mockito.lenient().when(flashBuyService.getFlashBuyCouponByPromotionCode(PROMOTION_CODE)).thenReturn(couponOptional);
		Mockito.lenient().when(coupon.getActive()).thenReturn(Boolean.FALSE);
	...
	public void testPrepareFlashBuyInfo()
	{
		promotions.add(promotion);
		Mockito.lenient().when(flashBuyService.getFlashBuyCouponByPromotionCode(PROMOTION_CODE)).thenReturn(couponOptional);
		Mockito.lenient().when(coupon.getActive()).thenReturn(Boolean.TRUE);
