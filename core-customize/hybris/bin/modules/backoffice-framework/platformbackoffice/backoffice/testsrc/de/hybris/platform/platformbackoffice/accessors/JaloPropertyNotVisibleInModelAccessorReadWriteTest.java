...
import de.hybris.platform.platformbackoffice.renderers.BackofficeEditorAreaRenderer;
...
public class JaloPropertyNotVisibleInModelAccessorReadWriteTest
{
	...
	protected WidgetModel createWidgetModel()
	{
		final ExpressionResolverFactory expressionResolverFactory = Mockito.mock(ExpressionResolverFactory.class);
		final ExpressionResolver resolver = new DefaultExpressionResolver(new DefaultEvaluationContextFactory());
		Mockito.when(expressionResolverFactory.createResolver()).thenReturn(resolver);
		final DefaultModelValueHandler handler = new DefaultModelValueHandler();
		handler.setExpressionResolverFactory(expressionResolverFactory);
		return new DefaultWidgetModel(new HashMap<>(), handler);
	}
	...
	protected void prepareTypeFacade(final boolean isWritable)
	{
		try
		{
			...
			Mockito.doReturn(Boolean.valueOf(isWritable)).when(dataAttribute).isWritable();
	...
	protected void prepareBackofficeEditorAreaRenderer()
	{
		...
		Mockito.doNothing().when(backofficeEditorAreaRenderer).delegateRendering(Mockito.any(),
	...
	protected void testSkeletonNotWritable(final String attributeName, final Object value)
	{
		try
		{
			final WidgetModel widgetModel = createWidgetModel();
			...
			final TypedValue readValue1 = jaloPropertyNotVisibleInModelAccessor.read(evaluationContext, productModel1,
					attributeName);
			...
			backofficeEditorAreaRenderer.render(null, null, productModel1, null, widgetInstanceManager);
			final TypedValue readValue2 = jaloPropertyNotVisibleInModelAccessor.read(evaluationContext, productModel1,
					attributeName);
	...
	protected void testSkeleton(final String attributeName, final Object value, final TypedValueAssertion typedValueAssertion)
	{
		try
		{
			...
			final TypedValue readValue3 = jaloPropertyNotVisibleInModelAccessor.read(evaluationContext, productModel1,
					attributeName);
			typedValueAssertion.doAssertions(readValue3);
			//render one more time the same object (refresh)
			backofficeEditorAreaRenderer.render(null, null, productModel1, null, widgetInstanceManager);
			final TypedValue readValue4 = jaloPropertyNotVisibleInModelAccessor.read(evaluationContext, productModel1,
					attributeName);
