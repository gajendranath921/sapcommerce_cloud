...
public class AmbiguousUniqueKeysExceptionTranslationHandlerTest
{
	...
	@InjectMocks
	@Spy
	private AmbiguousUniqueKeysExceptionTranslationHandler handler;
	@Mock
	private LabelService labelService;
	@Mock
	private UniqueAttributesInterceptor.AmbiguousUniqueKeysException uniqueKeysException;
	@Mock
	ItemModel model;
	@Captor
	...
	public void setUp()
	{
		...
		when(uniqueKeysException.getAttributesKeys()).thenReturn(new HashSet<String>(Arrays.asList(ATTR_KEY_ORIGINAL)));
		when(model.getItemtype()).thenReturn(ITEM_TYPE_ORIGINAL);
		when(uniqueKeysException.getModel()).thenReturn(model);
	...
	{
		assertThat(handler.toString(new Exception(uniqueKeysException))).isSameAs(LOCALIZED_MESSAGE);

		verify(handler).getLocalizedMessage(eq(AmbiguousUniqueKeysExceptionTranslationHandler.LABEL_KEY), argumentCaptor.capture(), eq(ITEM_TYPE_LOCALIZATION));
