...
public class WorkbookMandatoryColumnsValidatorTest
{
	...
	public void shouldNotReturnValidationErrorsWhenAllMandatoryColumnsAreSelected()
	{
		...
		final ExcelAttributeDescriptorAttribute excelAttribute = new ExcelAttributeDescriptorAttribute(code);
		Mockito.lenient().when(typeService.getAttributesForModifiers(eq(ProductModel._TYPECODE), any())).thenReturn(Sets.newHashSet(code));
		when(mapper.apply(ProductModel._TYPECODE)).thenReturn(Collections.emptyList());

		Mockito.lenient().when(workbookMandatoryColumnsValidator.prepareExcelAttribute(eq(code), any())).thenReturn(excelAttribute);
		Mockito.lenient().when(excelSheetService.findColumnIndex(typeSystemSheet, productSheet, excelAttribute)).thenReturn(0);
