...
public class AbstractClassificationAttributeTranslatorTest
{
	...
	public void shouldImportDataIfImportedValueIsNull()
	{
		...
		Mockito.lenient().when(abstractClassificationAttributeTranslator.importValue(excelClassificationAttribute, anyImportParameters,
				new ExcelImportContext())).thenReturn(null);
	...
	public void shouldCalculateReferenceFormatForAttributeWithUnitWithNoType()
	{
		// given
		final ClassAttributeAssignmentModel classAttributeAssignment = mock(ClassAttributeAssignmentModel.class);
		final ClassificationAttributeUnitModel unit = createUnit("g", 1);
		final ClassificationSystemVersionModel classificationSystemVersion = mock(ClassificationSystemVersionModel.class);
		...
		// then
		assertThat(referenceFormat).isEqualTo("value:unit[g]");
	}
	...
	public void shouldSubtranslatorRetrieveValueWithoutUnit()
	{
		...
		{
			@Override
			public boolean matches(ImportParameters importParameters) {
				final String inputValue = String.valueOf(importParameters.getCellValue());
