...
public class TypeSystemSheetPopulatorTest
{
	...
	public void shouldMergeAttributesByQualifierAndNameAndDisplayName()
	{
		...
		final String uniqueCatalogVersionDisplayName = "Catalog version*^";
		final String readOnlyCatalogVersionDisplayName = "Catalog version*=";
		...
		final TypeSystemRow uniqueCatalogVersionTypeSystemRow = mock(TypeSystemRow.class);
		final TypeSystemRow readOnlyCatalogVersionTypeSystemRow = mock(TypeSystemRow.class);

		final AttributeDescriptorModel productCodeAttr = mockAttributeDescriptor(code, articleNumber,
				uniqueCatalogVersionDisplayName, productCodeTypeSystemRow);
		final AttributeDescriptorModel shoeCodeAttr = mockAttributeDescriptor(code, shoeNumber, uniqueCatalogVersionDisplayName,
				shoeCodeTypeSystemRow);
		final AttributeDescriptorModel productUniqueCatalogVersionAttr = mockAttributeDescriptor(catalogVersionQualifier,
				catalogVersionName, uniqueCatalogVersionDisplayName, uniqueCatalogVersionTypeSystemRow);
		final AttributeDescriptorModel productReadOnlyCatalogVersionAttr = mockAttributeDescriptor(catalogVersionQualifier,
				catalogVersionName, uniqueCatalogVersionDisplayName, uniqueCatalogVersionTypeSystemRow);
		final AttributeDescriptorModel shoeCatalogVersionAttr = mockAttributeDescriptor(catalogVersionQualifier, catalogVersionName,
				readOnlyCatalogVersionDisplayName, readOnlyCatalogVersionTypeSystemRow);

		final List<AttributeDescriptorModel> attributes = List.of(productCodeAttr, shoeCodeAttr, productUniqueCatalogVersionAttr,
				productReadOnlyCatalogVersionAttr, shoeCatalogVersionAttr);
		...
		assertThat(result.get(code + ":" + articleNumber + ":" + uniqueCatalogVersionDisplayName))
				.isEqualTo(productCodeTypeSystemRow);
		assertThat(result.get(code + ":" + shoeNumber + ":" + uniqueCatalogVersionDisplayName)).isEqualTo(shoeCodeTypeSystemRow);
		assertThat(result.get(catalogVersionQualifier + ":" + catalogVersionName + ":" + uniqueCatalogVersionDisplayName))
				.isEqualTo(uniqueCatalogVersionTypeSystemRow);
		assertThat(result.get(catalogVersionQualifier + ":" + catalogVersionName + ":" + readOnlyCatalogVersionDisplayName))
				.isEqualTo(readOnlyCatalogVersionTypeSystemRow);

	}

	private AttributeDescriptorModel mockAttributeDescriptor(final String qualifier, final String name, final String displayName,
			final TypeSystemRow typeSystemRow)
	{
		...
		given(mockedTypeSystemRowFactory.getAttributeDisplayName(attributeDescriptor)).willReturn(displayName);
