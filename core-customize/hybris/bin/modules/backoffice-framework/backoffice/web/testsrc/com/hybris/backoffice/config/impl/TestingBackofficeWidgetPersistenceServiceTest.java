...
public class TestingBackofficeWidgetPersistenceServiceTest
{
	...
	public void setUp() throws IOException
	{
		...
		Mockito.lenient().when(firstSetting.getKey()).thenReturn(FIRST_SETTING_KEY);
		Mockito.lenient().when(firstSetting.getValue()).thenReturn(FIRST_SETTING_VALUE);
		Mockito.lenient().when(firstSetting.getType()).thenReturn(FIRST_SETTING_TYPE);

		final WidgetExtension extension = mock(WidgetExtension.class);
		Mockito.lenient().when(extension.getWidgetId()).thenReturn(WIDGET_ID);
		Mockito.lenient().when(extension.getSetting()).thenReturn(new ArrayList<>(Collections.singletonList(firstSetting)));
		...
		when(unmarshaller.unmarshal(any())).thenAnswer(invocationOnMock -> {
			final StreamSource arg = invocationOnMock.getArgument(0, StreamSource.class);
