...
public class CategoryToCategoryDropHandlerTest
{
	...
	public void testHandleAppendWhenDraggedHasSupercategoryAndCopyToCategory()
	{
		...
		Mockito.lenient().when(dragged.getCatalogVersion()).thenReturn(catalogVersion);
		Mockito.lenient().when(target.getCatalogVersion()).thenReturn(catalogVersion);
		...
		Mockito.lenient().when(dragContext.getParameter(CategoryToCategoryDropHandler.CONTEXT_PARAMETER_PARENT_OBJECT)).thenReturn(supercategory);
	...
	public void testHandleAppendWhenDraggedHasSupercategoryAndCopyToIt()
	{
		...
		Mockito.lenient().when(dragContext.getParameter(CategoryToCategoryDropHandler.CONTEXT_PARAMETER_PARENT_OBJECT)).thenReturn(target);
	...
	public void testAddRelatedObjectToUpdateToContextAppendWhenDraggedDoesNotHaveSupercategoriesInAssignedCategoryAndCopyToCategoryInOtherCatalogVersion()
	{
		...
		final CategoryModel supercategory = mock(CategoryModel.class);
		Mockito.lenient().when(supercategory.getCatalogVersion()).thenReturn(catalogVersion2);
		Mockito.lenient().when(dragged.getSupercategories()).thenReturn(Collections.emptyList());
	...
	public void testAddRelatedObjectToUpdateToContextReplaceWhenDraggedHasSupercategoryAndMoveToCategoryInTheSameCatalogVersion()
	{
		...
		final CategoryModel supercategory = mock(CategoryModel.class);
		when(supercategory.getCatalogVersion()).thenReturn(catalogVersion);
		Mockito.lenient().when(dragged.getSupercategories()).thenReturn(Arrays.asList(supercategory));
