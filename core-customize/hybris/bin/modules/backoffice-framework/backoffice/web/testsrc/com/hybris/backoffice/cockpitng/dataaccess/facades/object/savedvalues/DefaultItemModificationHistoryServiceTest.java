...
public class DefaultItemModificationHistoryServiceTest
{
	...
	final Map<String, Set<String>> attributesExcludedFromModificationHistory = new HashMap<>();

	@Before
	public void setup()
	{
		modificationHistoryService.setAttributesExcludedFromModificationHistory(attributesExcludedFromModificationHistory);
	}
	...
	public void shouldLogModificationOfARemovedEntityUsingPk()
	{
		...
		Mockito.lenient().when(removedEntity.getPk()).thenReturn(removedEntitiesPK);
		...
		Mockito.lenient().when(removedJalo.getPK()).thenReturn(removedEntitiesPK);
	...
	public void shouldHideEncryptedAttributes() throws TypeNotFoundException
	{
		...
		Mockito.lenient().when(itemModelContext.getValueHistory()).thenReturn(modelValueHistory);
		Mockito.lenient().when(modelValueHistory.isDirty()).thenReturn(true);
	...
	@Test
	public void shouldReturnTrueIfAttributeIncludeInEncryptedAttributes()
	{
		// given
		final String typeCode = "itemTypeCode";
		final String attribute = "attribute";
		...
		final ItemModel item = mock(ItemModel.class);
		attributesExcludedFromModificationHistory.clear();
		attributesExcludedFromModificationHistory.put(typeCode, Sets.newHashSet(attribute, "attr"));
		attributesExcludedFromModificationHistory.put("anotherTypeCode", Sets.newHashSet("attr1", "attr2", "attr3"));
		when(typeFacade.getType(item)).thenReturn(typeCode);

		assertThat(modificationHistoryService.isEncrypted(item, attribute)).isTrue();
	}

	@Test
	public void shouldReturnTrueIfAttributeIsEncrypted() throws TypeNotFoundException
	{
		// given
		final String typeCode = "itemTypeCode";
		final String attribute = "attribute";
		...
		when(dataAttribute.isEncrypted()).thenReturn(true);
		attributesExcludedFromModificationHistory.clear();
	...
	@Test
	public void shouldReturnFalseIfAttributeNotIncludeInEncryptedAttributes() throws TypeNotFoundException
	{
		// given
		final String typeCode = "itemTypeCode";
		final String attribute = "attribute";
		...
		when(typeFacade.load(typeCode)).thenReturn(dataType);
		when(dataType.getAttribute(attribute)).thenReturn(dataAttribute);
		when(dataAttribute.isEncrypted()).thenReturn(false);
		attributesExcludedFromModificationHistory.clear();
		attributesExcludedFromModificationHistory.put(typeCode, Sets.newHashSet("mockAttr", "attr"));
		attributesExcludedFromModificationHistory.put("anotherTypeCode", Sets.newHashSet("attr1", "attr2", "attr3"));

		assertThat(modificationHistoryService.isEncrypted(item, attribute)).isFalse();
	}

	@Test
	public void shouldReturnFalseIfAttributeNotEncrypted() throws TypeNotFoundException
	{
		// given
		final String typeCode = "itemTypeCode";
		final String attribute = "attribute";
