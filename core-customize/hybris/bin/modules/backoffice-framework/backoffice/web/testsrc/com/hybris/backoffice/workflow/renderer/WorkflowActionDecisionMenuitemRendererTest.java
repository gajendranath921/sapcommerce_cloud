...
public class WorkflowActionDecisionMenuitemRendererTest extends AbstractCockpitngUnitTest<WorkflowActionDecisionMenuitemRenderer>
{
	...
	public void whenUserHasNoPermissionsForWorkflowActionShouldNotRenderDecisionLabel()
	{
		...
		Mockito.lenient().when(labelService.getObjectLabel(decision)).thenReturn(DECISION_CODE);
	...
	@Test
	public void whenUserHasNoReadPermissionForWorkflowDecisionTypeShouldNotRenderDecisionLabel()
	{
		final WorkflowDecisionModel decision = createDecision();
		final WorkflowActionModel action = createAction(decision);
		final Menupopup popup = new Menupopup();
		...
		Mockito.lenient().when(labelService.getObjectLabel(decision)).thenReturn(DECISION_CODE);
		mockThatUserHasNoPermissionForWorkflowDecisionType();

		renderer.render(popup, null, new ImmutablePair<>(action, decision), null, CockpitTestUtil.mockWidgetInstanceManager());

		assertThat(popup.getChildren()).isEmpty();
	}

	@Test
	public void whenUserHasNoReadAndNoChangePermissionForWorkflowActionTypeShouldNotRenderDecisionLabel()
	{
		final WorkflowDecisionModel decision = createDecision();
		final WorkflowActionModel action = createAction(decision);
		final Menupopup popup = new Menupopup();
		...
		Mockito.lenient().when(labelService.getObjectLabel(decision)).thenReturn(DECISION_CODE);
		mockThatUserHasNoPermissionForWorkflowActionType();
	...
	private void mockThatUserHasNoPermissionForAction(final WorkflowActionModel action)
	{
		...
		Mockito.lenient().when(permissionFacade.canReadInstance(action)).thenReturn(false);
		Mockito.lenient().when(permissionFacade.canChangeInstance(action)).thenReturn(false);
	}
	...
	private void mockThatUserHasNoPermissionForDecision(final WorkflowDecisionModel decision)
	{
		...
		Mockito.lenient().when(permissionFacade.canReadInstance(decision)).thenReturn(false);
	}

	private void mockThatUserHasNoPermissionForWorkflowDecisionType()
	{
		Mockito.lenient().when(permissionFacade.canReadType(WorkflowDecisionModel._TYPECODE)).thenReturn(false);
	}

	private void mockThatUserHasNoPermissionForWorkflowActionType()
	{
		when(permissionFacade.canReadType(WorkflowActionModel._TYPECODE)).thenReturn(false);
		Mockito.lenient().when(permissionFacade.canChangeType(WorkflowActionModel._TYPECODE)).thenReturn(false);
	}
}
