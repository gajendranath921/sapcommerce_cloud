...
public class AbstractAttributesExportRendererTest
{
	...
	public void setUp()
	{
		...
		Mockito.lenient().when(pageable.getTypeCode()).thenReturn(TYPE_NAME);

		// locales
		Mockito.lenient().when(english.getActive()).thenReturn(true);
		when(english.getIsocode()).thenReturn("en");
		Mockito.lenient().when(german.getActive()).thenReturn(true);
		...
		when(permissionFacade.getEnabledReadableLocalesForCurrentUser()).thenReturn(locales);
