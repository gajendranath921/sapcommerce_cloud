...
public class ProductCloneStrategyTest
{
	...
	public void setUp()
	{
		Mockito.lenient().when(dataType.isSingleton()).thenReturn(false);
		Mockito.lenient().when(modelService.clone(any())).thenReturn(mockProduct);
		...
		Mockito.lenient().when(typeService.getAttributeDescriptor(any(ComposedTypeModel.class), any())).thenReturn(attributeDescriptorMock);
		Mockito.lenient().when(attributeDescriptorMock.getPartOf()).thenReturn(true);
	}
