...
public class WorkflowAttachmentTypeReferenceSearchFacadeTest
{
	...
	public void setUp()
	{
		...
		Mockito.lenient().when(searchQueryData.getSearchQueryText()).thenReturn("");
		Mockito.lenient().when(searchQueryData.getPageSize()).thenReturn(Integer.valueOf(PAGE_SIZE));

		product = mock(ComposedTypeModel.class);
		when(product.getName()).thenReturn("Product");
		Mockito.lenient().when(product.getCode()).thenReturn("Product");
		when(category.getName()).thenReturn("Category");
		Mockito.lenient().when(category.getCode()).thenReturn("Category");
