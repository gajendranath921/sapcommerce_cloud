...
public class DefaultObjectFacadeTest extends ServicelayerTransactionalTest
{
	...
	public void before() throws TypeNotFoundException
	{
		...
		when(typeFacade.load(nullable(String.class))).thenReturn(dataType);
		...
		objectFacadeStrategyRegistry.setStrategies(Lists.newArrayList(customStrategy));
		...
		objectFacade.setEventPublisher(eventPublisher);
	...
	public void createJavaObjectShouldThrowException()
	{
		...
		catch (final ObjectCreationException ex)
		{
			// expected behavior
		}
	...
	{
		Mockito.when(cockpitProperties.getProperty(DefaultObjectFacade.COCKPITNG_CRUD_COCKPIT_EVENT_NOTIFICATION))
				.thenReturn("true");
		...
		final ArgumentCaptor<Collection> collectionCaptor = ArgumentCaptor.forClass(Collection.class);
		verify(eventPublisher).publish(eq(ObjectFacade.OBJECTS_DELETED_EVENT), collectionCaptor.capture(), any());
		assertThat(collectionCaptor.getValue()).containsOnly(objectsToDelete.toArray());
	}
}
