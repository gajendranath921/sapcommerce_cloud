...
public class DefaultConfigUploadAuExtensionTest
{
	...
	{
		//given
		Mockito.lenient().when(request.getReader()).thenReturn(new BufferedReader(new NullReader()));
		Mockito.lenient().when(configurationService.validate(any())).thenReturn(SchemaValidationStatus.success());
	...
	public void handleConfigValidateShouldSetHeadersOnResponseOnValidationResult() throws IOException
	{
		...
		//then
		verify(response).addHeader("validationStatus", encodeURI("error"));
		verify(response).addHeader("validationLabel", encodeURI("I failed"));
	}
	...
	{
		//given
		Mockito.lenient().when(sessionService.getAttribute(eq("showFilterOptions"))).thenReturn(null);
	...
	@Test
	public void shouldFormatConfigValidationResponseHeaders() throws UnsupportedEncodingException {
		//given
		final SchemaValidationStatus successStatus = SchemaValidationStatus.success();
		final var testValidationLabel = "Test Validation Label";
		when(ext.getValidationLabelForStatus(successStatus)).thenReturn(testValidationLabel);

		//when
		ext.formatConfigValidationResponse(successStatus, response);

		//then
		verify(response).addHeader("validationStatus", successStatus.getCode().toString());
		verify(response).addHeader("validationLabel", encodeURI(testValidationLabel));
	}
}
