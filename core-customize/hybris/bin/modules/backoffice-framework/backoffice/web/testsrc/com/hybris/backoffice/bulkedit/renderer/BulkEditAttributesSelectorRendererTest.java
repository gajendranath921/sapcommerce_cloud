...
public class BulkEditAttributesSelectorRendererTest
{
	...
	public void allowOneToMany()
	{
		...
		Mockito.lenient().when(relationMetaType.getTargetTypeCardinality()).thenReturn(RelationEndCardinalityEnum.MANY);
	...
	public void localizedAttributeWithSubLanguages()
	{
		...
		when(permissionFacade.getEnabledWritableLocalesForCurrentUser())
	...
	@Test
	public void filtersOutLocalizedAttributesWhenAllLanguagesAreExcluded()
	{
		// given
		when(permissionFacade.getEnabledWritableLocalesForCurrentUser()).thenReturn(Sets.newHashSet());

		final DataAttribute articleStatus = mockDataAttribute(ProductModel.ARTICLESTATUS);
		when(articleStatus.isLocalized()).thenReturn(false);
		final DataAttribute description = mockDataAttribute(ProductModel.DESCRIPTION);
		when(description.isLocalized()).thenReturn(true);

		final ArrayList<DataAttribute> attributes = Lists.newArrayList(articleStatus, description);
		final DataType dataType = mockDataTypeWithAttributes(ProductModel._TYPECODE, attributes);
		...
		final Attribute attribute = bulkEditForm.getAttributesForm().getAvailableAttributes().iterator().next();
		assertThat(attribute.getQualifier()).isEqualTo(ProductModel.ARTICLESTATUS);
		assertThat(attribute.hasSubAttributes()).isFalse();
	}
			...
			final boolean writeThrough, final boolean writeThroughOnCreation)
	{
		...
		Mockito.lenient().when(da.isWritableOnCreation()).thenReturn(writeThroughOnCreation);
		when(da.isPartOf()).thenReturn(partOf);
		Mockito.lenient().when(da.isWriteThrough()).thenReturn(writeThrough);
		return da;
	}
