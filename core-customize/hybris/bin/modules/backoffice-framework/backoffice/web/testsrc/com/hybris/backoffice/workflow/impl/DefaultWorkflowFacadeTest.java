...
public class DefaultWorkflowFacadeTest
{
	...
	{
		when(workflowService.createWorkflow(nullable(String.class), eq(template), eq(attachments), eq(currentUser)))
				.thenReturn(workflow);
		doReturn(attachments).when(facade).assureAttachmentsWithoutDuplicates(attachments);
