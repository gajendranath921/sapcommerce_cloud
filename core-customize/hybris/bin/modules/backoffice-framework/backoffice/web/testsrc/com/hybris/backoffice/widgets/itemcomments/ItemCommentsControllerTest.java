...
public class ItemCommentsControllerTest extends AbstractWidgetUnitTest<ItemCommentsController>
{
	...
	@Test
	public void shouldInitializeDateFormatterWithCorrectTimezone()
	{
		// given
		final String dateFormat = "yyyy";
		final TimeZone timeZone = TimeZone.getTimeZone("GMT-12");
		when(uiDateRendererProvider.getFormatTimeZone()).thenReturn(timeZone);

		// when
		itemCommentsController.initDateFormatter();

		// then
		final SimpleDateFormat dateFormatter = itemCommentsController.getValue(itemCommentsController.MODEL_DATE_FORMATTER, SimpleDateFormat.class);
		assertThat(dateFormatter.getTimeZone()).isEqualTo(timeZone);
	}
	...
	public void shouldAddNewCommentWhenOkButtonIsClicked() throws Exception
	{
		...
		when(objectFacade.save(nullable(CommentModel.class))).thenReturn(commentModel);
		when(inputItem.getComments()).thenReturn(comments);
		final DomainModel domainModel = mock(DomainModel.class);
		when(commentService.getDomainForCode(nullable(String.class))).thenReturn(domainModel);
		final ComponentModel componentModel = mock(ComponentModel.class);
		when(commentService.getComponentForCode(eq(domainModel), nullable(String.class))).thenReturn(componentModel);
	...
	public void shouldRenderCommentsIfNewCommentsAppeared() throws ObjectNotFoundException
	{
		...
		when(itemCommentsController.getObjectFacade().reload(nullable(Object.class))).thenReturn(inputItem);
	...
	public void shouldNotRenderCommentsIfNewCommentsDidNotAppear() throws ObjectNotFoundException
	{
		// given
		when(itemCommentsController.getCommentsFromModel()).thenReturn(new ArrayList<>());
		when(itemCommentsController.getObjectFacade().reload(nullable(Object.class))).thenReturn(createItemModelWithoutCommentsStub());
	...
	public void shouldNotShowCommentsNoForNotAllowedUser()
	{
		...
		// then
		verify(commentsListContainer).setSclass(argThat(new ArgumentMatcher<>()
		{
			@Override
			public boolean matches(String s) {
				return Arrays.asList(CockpitTestUtil.getSClasses(s)).contains(SCLASS_COMMENTS_LIST_EMPTY);
			}
		}));
