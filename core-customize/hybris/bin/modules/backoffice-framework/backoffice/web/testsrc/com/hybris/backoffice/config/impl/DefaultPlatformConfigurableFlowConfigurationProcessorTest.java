...
public class DefaultPlatformConfigurableFlowConfigurationProcessorTest
{
	...
	public void retrieveMissingProperties()
	{
		...
		doReturn(typeWithOwnerAttribute).when(processor).loadDataType(nullable(String.class));
		...
		Mockito.lenient().when(catalogVersionAttribute.getValueType()).thenReturn(catalogVersionAttributeType);

		when(ownerAttributeType.getCode()).thenReturn(ItemModel._TYPECODE);
		Mockito.lenient().when(catalogVersionAttributeType.getCode()).thenReturn(CatalogVersionModel._TYPECODE);
