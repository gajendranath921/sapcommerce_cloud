...
public class DefaultBulkEditValidationHelperTest
{
	...
	public void testValidationProxyValidatesAttributesWithValue()
	{
		...
		verify(localizationAwareValidationHandler).validate(eq(templateObject),
				...
				{
					@Override
					public boolean matches(Collection<LocalizedQualifier> localizedQualifiers) {
						...
						return localizedQualifiers.size() == correctQualifiers.size() && localizedQualifiers.containsAll(correctQualifiers);
					}
	...
	public void shouldValidateSelectedAttributes()
	{
		...
		final DefaultValidationInfo validationError = new DefaultValidationInfo();
		validationError.setValidationSeverity(ValidationSeverity.ERROR);
		when(localizationAwareValidationHandler.validate(eq(product), (Collection<LocalizedQualifier>) any(Collection.class),
				nullable(ValidationContext.class))).thenReturn(Lists.newArrayList(validationError));
