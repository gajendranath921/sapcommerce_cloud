...
public class WorkflowDesignerCreateActionHandlerTest
{
	...
	public void shouldCreateAction()
	{
		...
		Mockito.lenient().when(mockedInitialElementLocationProvider.provideLocation(any())).thenReturn(initialLocation);
	...
	public void shouldRetrieveActionIfItsInModel()
	{
		...
		Mockito.lenient().when(action.getCode()).thenReturn(code);
		Mockito.lenient().when(node.getData()).thenReturn(code);
	...
	public void shouldCompareCodeUniqueness()
	{
		...
		Mockito.lenient().when(decision.getCode()).thenReturn(code);
		...
		Mockito.lenient().when(mockedNodeTypeService.hasCode(node, code)).thenReturn(true);
