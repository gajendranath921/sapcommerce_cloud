...
@UnitTest
public class DefaultSnExpressionEvaluatorSpec extends JUnitPlatformSpecification {

	static final LANGUAGE_ID_EN = "en"
	static final LANGUAGE_ID_DE = "de"
	...
	def "Evaluate pk attribute from item"() {
		...
		PK pk = PK.fromLong(2)
		ItemModel root = Mock()
		String expression = ItemModel.PK
	...
	@Test
	def "Evaluate itemtype attribute from item"() {
		given:
		String itemtype = "type1"
		ItemModel root = Mock()
		String expression = ItemModel.ITEMTYPE
	...
	def "Evaluate localized attribute from item using locales"() {
		...
		then:
		result == [
			(Locale.ENGLISH): value1,
			(Locale.GERMAN): value2
		]
	}

	@Test
	def "Evaluate not existing attribute from object returns null"() {
		given:
		String value = "testValue"
