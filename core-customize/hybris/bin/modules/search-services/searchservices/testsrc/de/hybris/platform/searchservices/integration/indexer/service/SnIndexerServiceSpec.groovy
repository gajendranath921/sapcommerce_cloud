...
public class SnIndexerServiceSpec extends AbstractSnIndexerSpec {
	...
	@Test
	public void runPartialOperation() {
		when:
		final SnIndexerItemSource fullItemSource = new FlexibleSearchSnIndexerItemSource(FULL_QUERY, Collections.emptyMap())
		final SnIndexerRequest fullIndexerRequest = snIndexerService.createFullIndexerRequest(INDEX_TYPE_ID, fullItemSource)
		snIndexerService.index(fullIndexerRequest)
		...
		final SnIndexerItemSource partialItemSource = new FlexibleSearchSnIndexerItemSource(INCREMENTAL_QUERY,
			Collections.singletonMap(START_TIME_PARAM, startDate))
		final SnIndexerItemSourceOperation partialItemSourceOperation = new DefaultSnIndexerItemSourceOperation(SnDocumentOperationType.PARTIAL_UPDATE, partialItemSource,
			Set.of("name"));
		List<SnIndexerItemSourceOperation> partialItemSourceOperations = List.of(partialItemSourceOperation)
		final SnIndexerRequest partialIndexerRequest = snIndexerService.createIncrementalIndexerRequest(INDEX_TYPE_ID, partialItemSourceOperations)
		final SnIndexerResponse partialIndexerResponse = snIndexerService.index(partialIndexerRequest)

		then:
		with (partialIndexerResponse) {
			getTotalItems() == PRODUCTS_UPDATED_COUNT
			getProcessedItems() == PRODUCTS_UPDATED_COUNT
		}
	}
}
