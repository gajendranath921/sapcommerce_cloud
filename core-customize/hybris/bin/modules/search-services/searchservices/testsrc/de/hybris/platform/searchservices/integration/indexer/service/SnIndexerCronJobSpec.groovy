...
public class SnIndexerCronJobSpec extends AbstractSnIndexerSpec {
	...
	public void failToRunIncrementalCronJobBeforeFullOperation() {
		...
		then:
		cronJobModel.getStatus() == CronJobStatus.ABORTED
	...
	@Test
	public void runPartialCronJobAfterFullOperation() {
		given:
		final FullSnIndexerCronJobModel fullCronJobModel = createFullIndexerCronJobModel(FULL_QUERY)
		final IncrementalSnIndexerCronJobModel incrementalCronJobModel = createIncrementalIndexerCronJobModel(
			INCREMENTAL_QUERY, SnDocumentOperationType.PARTIAL_UPDATE, ["code"])
		...
		def searchQuery = new SnSearchQuery()
		...
		searchResponse != null
		searchResponse.getSearchResult() != null
		searchResponse.getSearchResult().getTotalSize() == 2
		searchResponse.getSearchResult().getSearchHits().get(0).getFields().get("code") == "product1"
	}
	...
	protected IncrementalSnIndexerCronJobModel createIncrementalIndexerCronJobModel(final String query,
		final SnDocumentOperationType operationType = SnDocumentOperationType.CREATE_UPDATE, final List<String> fieldIds = null) {
	...
	protected List<SnFieldModel> getFieldsFromIndexType(final SnIndexTypeModel indexType, final List<String> fieldIds) {
		def fields = CollectionUtils.emptyIfNull(indexType.getFields())
		def fieldIdentifiers = CollectionUtils.emptyIfNull(fieldIds)
		return fields.stream().filter({ field -> fieldIdentifiers.contains(field.getId())}).collect(Collectors.toList());
	}
