...
public class DefaultSnIndexerBatchStrategy extends AbstractSnIndexerBatchStrategy implements ApplicationContextAware
{
			...
			throws SnException, InterruptedException
	{
		...
		final SnIdentityProvider<ItemModel> identityProvider = createIdentityProvider(indexerContext);
		...
		for (final SnIndexerItemSourceOperation indexerItemSourceOperation : indexerItemSourceOperations)
		{
			if (indexerItemSourceOperation.getIndexerItemSource() != null)
			{
				...
				{
					totalItems += pks.size();
					addIndexDocumentBatchOperationRequests(indexerContext, identityProvider, documentBatchOperationRequests,
		...
		else
		{
			...
			final SnSearchProvider<AbstractSnSearchProviderConfiguration> searchProvider = snSearchProviderFactory
					.getSearchProviderForContext(indexerContext);
		...
		return createIndexerBatchResponse(indexerContext, totalItems, processedItems, status);
	}
	...
	protected SnIdentityProvider<ItemModel> createIdentityProvider(final SnIndexerContext indexerContext) throws SnIndexerException
	{
