 ...
 */
package de.hybris.platform.searchservices.util.impex;
...
/**
 * Impex ImportProcessor that injects all config properties as impex definitions. All defined configuration properties
 * are added as impex macro definitions with the prefix of "config-". For example the config key
 * <tt>mail.smtp.server</tt> can be accessed via the macro <tt>$config-mail.smtp.server</tt>. In order to use this
 * import processor and to load the configuration properties the following must be added to the top of the impex file:
 *
