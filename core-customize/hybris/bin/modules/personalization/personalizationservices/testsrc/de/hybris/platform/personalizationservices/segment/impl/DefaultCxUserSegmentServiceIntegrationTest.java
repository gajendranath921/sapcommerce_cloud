...
public class DefaultCxUserSegmentServiceIntegrationTest extends AbstractCxServiceTest
{
	...
	public void testSetUserSegmentsForTwoBaseSites()
	{
		...
		final BaseSiteModel baseSite1 = baseSiteService.getBaseSiteForUID(BASE_SITE_1);
		final UserModel user = userService.getUserForUID(CUSTOMER_WITH_SEGMENT2_3); //
		final Collection<CxUserToSegmentModel> userSegmentsForBaseSite = createUserSegments(user, BigDecimal.valueOf(0.7), baseSite,
				SEGMENT2, SEGMENT4);
	...
	{
		return Arrays.stream(segments).map(s -> createUserSegment(user, affinity, s, baseSite)).collect(Collectors.toList());
	}
	...
	{
		return Arrays.stream(segments).map(s -> createUserSegment(user, providerId, affinity, s, baseSite))
	...
	protected void verifySegments(final Collection<CxUserToSegmentModel> expected, final Collection<CxUserToSegmentModel> current)
	{
		...
		Assert.assertTrue(
				current.stream().allMatch(us -> expectedAffinityMap.containsKey(cxUserSegmentService.getUserSegmentKey(us))
