...
export class PersonalizationsmarteditContextualMenuService {
    ...
    public async updateWorkflowStatus(): Promise<void> {
        this.isWorkflowRunningBoolean = await this.personalizationsmarteditContextService.isCurrentPageActiveWorkflowRunning();
    }
    ...
    private isSlotInCurrentCatalog(config: ContextMenuDto): boolean {
        ...
        const catalogUuid = this.personalizationsmarteditComponentHandlerService.getCatalogVersionUuid(
            slot
        );
    ...
    private getSelectedVariationCode(): string {
        if (this.personalizationsmarteditContextService.getCombinedView().enabled) {
            return (this.personalizationsmarteditContextService.getCombinedView().customize
                .selectedVariations as CustomizationVariation).code;
        }
    ...
    private getSlotsToRefresh(containerSourceId: string): string[] {
        ...
        const slots = this.personalizationsmarteditComponentHandlerService.getFromSelector(
            slotsSelector
        );
