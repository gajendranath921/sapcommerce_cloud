...
public class DefaultVariationFacadeTest
{
	...
	public void getVariationTest()
	{
		...
		Mockito.when(variationConverter.convert(Mockito.eq(variation), Mockito.anyCollectionOf(VariationConversionOptions.class))).thenReturn(variationData);
	...
	public void getVariationsTest()
	{
		...
		Mockito.when(customizationService.getCustomization(CUSTOMIZATION_ID, catalogVersionStage))
				.thenReturn(Optional.of(customization));
		Mockito.when(variationConverter.convertAll(Mockito.anyCollectionOf(CxVariationModel.class), Matchers.<VariationConversionOptions> anyVararg()))
