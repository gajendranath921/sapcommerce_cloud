...
public class DefaultCustomizationFacadeTest
{
	...
	public void getCustomizationTest()
	{
		...
		Mockito.when(customizationService.getCustomization(CUSTOMIZATION_ID, catalogVersionStage)).thenReturn(Optional.of(customization));
		Mockito.when(customizationConverter.convert(Mockito.any(), Mockito.anyCollectionOf(CustomizationConversionOptions.class))).thenReturn(customizationData);
