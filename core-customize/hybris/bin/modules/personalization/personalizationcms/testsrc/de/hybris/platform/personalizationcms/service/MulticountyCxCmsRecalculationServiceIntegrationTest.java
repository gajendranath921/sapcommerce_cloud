...
public class MulticountyCxCmsRecalculationServiceIntegrationTest extends ServicelayerTransactionalTest
{
	...
	public void checkActionsForSingleVariation()
	{
		...
		createPreviewTicket(Collections.singletonList(topCatalog), Collections.singletonList(variarion));

		catalogVersionService.setSessionCatalogVersions(Collections.singletonList(topCatalog));

		final List<CxCmsActionResult> expected = Collections.singletonList(buildActionResult("customization1", "variation1", "cmsaction1"));

		//when
		cxCmsRecalculationService.recalculate(user, Collections.singletonList(RecalculateAction.RECALCULATE));
	...
	public void checkActionsForMultipleVariations()
	{
		...
		createPreviewTicket(Collections.singletonList(topCatalog), Arrays.asList(variarion1, variarion2));
