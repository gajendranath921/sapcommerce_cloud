...
public class LeavesOrClosestAncestorsCatalogsLookupStrategyTest
{
	...
	public void setupTest()
	{
		...
		BDDMockito.given(c1.getCatalogVersions()).willReturn(new HashSet<>(Collections.singletonList(cv1)));
		BDDMockito.given(cc1.getCatalogVersions()).willReturn(new HashSet<>(Collections.singletonList(ccv1)));
		BDDMockito.given(cc2.getCatalogVersions()).willReturn(new HashSet<>(Collections.singletonList(ccv2)));
		BDDMockito.given(cc3a.getCatalogVersions()).willReturn(new HashSet<>(Collections.singletonList(ccv3a)));
		BDDMockito.given(cc3b.getCatalogVersions()).willReturn(new HashSet<>(Collections.singletonList(ccv3b)));
		...
		//setup hierarchy
		BDDMockito.given(cc1.getSubCatalogs()).willReturn(new HashSet<>(Collections.singletonList(cc2)));
		...
		intermediateCvs = Collections.singletonList(ccv2);
		topContentCvs = Collections.singletonList(ccv1);
		nonContentCv = Collections.singletonList(cv1);
	}
	...
	public void shouldHandleBrokenHierarchy()
	{
		...
		BDDMockito.given(cc1.getSubCatalogs()).willReturn(new HashSet<>(Collections.singletonList(cc2)));
		BDDMockito.given(cc2.getSubCatalogs()).willReturn(new HashSet<>(Collections.singletonList(cc1)));
		BDDMockito.given(cc2.getSuperCatalog()).willReturn(cc1);
