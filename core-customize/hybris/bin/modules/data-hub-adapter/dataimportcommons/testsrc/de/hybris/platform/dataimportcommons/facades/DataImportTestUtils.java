...
public class DataImportTestUtils
{
	...
	public static DataImportError error(final String msg)
	{
		...
		lenient().doReturn(msg).when(err).getMessage();
		return err;
	}
	...
	public static ImportResult successfulImportResult()
	{
		...
		lenient().doReturn(true).when(result).isSuccessful();
		lenient().doReturn(false).when(result).isError();
		lenient().doReturn(false).when(result).hasUnresolvedLines();
		return result;
	}
	...
	public static ImportResult importResult(final String impexFileLoc)
	{
		...
		lenient().doReturn(impexFileLoc).when(media).getLocation();
		lenient().doReturn(MediaUtil.URL_HAS_DATA).when(media).getInternalURL();

		final ItemModelContext itemModelContext = mock(ItemModelContext.class);
		lenient().doReturn(itemModelContext).when(media).getItemModelContext();

		final ImpExImportCronJobModel job = mock(ImpExImportCronJobModel.class);
		lenient().doReturn(media).when(job).getWorkMedia();

		final ImportResult res = mock(ImportResult.class);
		lenient().doReturn(job).when(res).getCronJob();
		lenient().doReturn(media).when(res).getUnresolvedLines();

		final String tenantId = "master";
		lenient().doReturn(tenantId).when(job).getTenantId();
