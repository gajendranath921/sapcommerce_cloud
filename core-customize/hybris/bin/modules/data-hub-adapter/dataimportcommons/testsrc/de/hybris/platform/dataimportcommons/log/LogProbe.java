 ...
 */
public class LogProbe extends ExternalResource implements Logger
{
	...
	private LogProbe(final Logger log)
	{
		logger = log;
	 ...
	 */
	public static LogProbe create(final Class<?> type)
	{
		final String loggerName = type.getName();
		return Log.getRegisteredLogger(loggerName)
		          .map(LogProbe::toProbe)
		          .orElseGet(() -> LogProbe.createProbe(loggerName));
	}

	private static LogProbe toProbe(final Logger l)
	{
		return l instanceof LogProbe
				? (LogProbe) l
				: createProbe(l);
	}

	private static LogProbe createProbe(final String loggerName)
	{
		final Logger logger = LoggerFactory.getLogger(loggerName);
		return createProbe(logger);
	}

	private static LogProbe createProbe(final Logger l)
	{
		final LogProbe probe = new LogProbe(l);
		Log.register(probe);
		return probe;
	}
	 ...
	 */
	public LogProbe withLogLevel(final LogLevel level)
	{
		logLevel = level;
		return this;
	}
