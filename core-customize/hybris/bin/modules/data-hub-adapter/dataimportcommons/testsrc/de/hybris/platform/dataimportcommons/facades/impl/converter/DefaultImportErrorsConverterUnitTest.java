...
public class DefaultImportErrorsConverterUnitTest
{
	...
	private ImportResult importResultWithErrors()
	{
		...
		lenient().doReturn(true).when(importResult).isError();
		lenient().doReturn(Stream.of(mock(ImpExHeaderError.class))).when(importService).collectImportErrors(importResult);
		return importResult;
	}
	...
	private ImportResult importResultWithErrorsExceedingLimit()
	{
		final ImportResult importResult = mock(ImportResult.class);
		lenient().doReturn(true).when(importResult).isError();
		lenient().doReturn(Stream.of(mock(ImpExHeaderError.class), mock(ImpExHeaderError.class))).when(importService).collectImportErrors(importResult);
		return importResult;
	}
	...
	public void testConverterForUnlimitedErrors()
	{
		...
		final ImportResult importResult = mock(ImportResult.class);
		lenient().doReturn(true).when(importResult).isError();
		lenient().doReturn(Stream.of(mock(ImpExHeaderError.class), mock(ImpExHeaderError.class), mock(ImpExHeaderError.class)))
	...
	private ImportResult errorImportResultWithoutErrors()
	{
		final ImportResult importResult = mock(ImportResult.class);
		lenient().doReturn(true).when(importResult).isError();
		lenient().doReturn(Stream.empty()).when(importService).collectImportErrors(importResult);
		return importResult;
	}
}
