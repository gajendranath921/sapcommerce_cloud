...
public class DefaultDataHubOutboundServiceUnitTest
{
	...
	{
		when(csvUtils.convertObjectToCsv(testProduct)).thenReturn(new String[0]);
	...
	{
		when(csvUtils.convertMapToCsv(testProductMap)).thenReturn(new String[0]);
	...
	public void testSendToDataHub_WithFeedNameAndListOfMaps() throws Exception
	{
		...
		when(csvUtils.convertListToCsv(testList)).thenReturn(new String[0]);
