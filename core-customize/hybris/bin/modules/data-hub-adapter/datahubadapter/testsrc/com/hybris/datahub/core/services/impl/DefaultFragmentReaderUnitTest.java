...
public class DefaultFragmentReaderUnitTest
{
	...
	public void setUp() throws ImpExException
	{
		...
		doReturn(new ImpExFragment[]{ impexMacroFragmentSpy, dataHubDataFragmentSpy, new ConstantTextFragment() }).when(reader)
		                                                                                                          .getFragmentsToTry();
	}
	...
	public void testValidationErrorIsAddedToContext() throws ImpExException
	{
		...
		doThrow(new ImpexValidationException(Collections.singletonList(new ImportError()))).when(dataHubDataFragmentSpy)
		                                                                                   .addLine(SEPARATOR_LINE, fragments);
		final ItemImportTaskData itemImportTaskData = scriptCtx();
		reader.readScriptFragments(itemImportTaskData);
		assertThat(itemImportTaskData.getHeaderErrors()).isNotEmpty();
	}
	...
	{
		reader.readScriptFragments(null);
	}

	@Test(expected = IllegalArgumentException.class)
	public void testThrowsExceptionWhenNullImpexMetaDataIsPassedToReadScriptFragments() throws ImpExException
	{
		final ItemImportTaskData itemImportTaskData = new ItemImportTaskData();
		itemImportTaskData.setImpexMetaData(null);
		reader.readScriptFragments(itemImportTaskData);
	}
