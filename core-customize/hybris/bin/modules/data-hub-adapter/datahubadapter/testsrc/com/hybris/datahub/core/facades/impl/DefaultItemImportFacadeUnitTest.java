...
public class DefaultItemImportFacadeUnitTest
{
	...
	public void setUp() throws ImpExException
	{
		...
		setupImportConfigStrategy();
		setUpImportResultConverter(importResult);
	}

	private void setupImportConfigStrategy() throws ImpExException
	{
		final ImportConfig importConfig = new ImportConfig();
		when(importConfigStrategy.createImportConfig(any(ItemImportTaskData.class))).thenReturn(importConfig);
	}

	private ImportResult setUpImportService()
	{
		final ImportResult res = Mockito.mock(ImportResult.class);
		when(importService.importData(any(ImportConfig.class))).thenReturn(res);
		return res;
	}
	...
	private void setUpImportResultConverter(final ImportResult importRes)
	{
		...
		when(itemImportResult.isSuccessful()).thenReturn(true);
