...
public class DataHubDataFragmentUnitTest extends AbstractScriptFragmentTest<DataHubDataFragment>
{
	...
	public void testMacroCannotBeAdded() throws IOException
	{
		testLineThatShouldNotBeAdded(
				"$catalogVersion=catalogversion(catalog(id[default=apparelProductCatalog]),version[default='Staged'])");
	}
	...
	public void testValidMacroAndHeader() throws IOException
	{
		...
		macroFragment.addLine(
				"$catalogVersion=catalogversion(catalog(id[default=$productCatalog]),version[default='Staged'])[unique=true,default=$productCatalog:Staged]" + CSVConstants.HYBRIS_LINE_SEPARATOR);
	...
	public void testValidMacroAndInvalidHeader() throws Exception
	{
		...
		doThrow(new ImpExException("Invalid Impex Header")).when(fragment)
		                                                   .validateImpexHeader(
				                                                   invalidImpexHeader + CSVConstants.HYBRIS_LINE_SEPARATOR,
				                                                   macroFragment.getContent());
