...
{
	private static final int CSV_SIZE = 2;
	...
	{
		final String[] csvData = new String[CSV_SIZE];
	 ...
	 */
	public Map<String, Object> transmissionSafe(final Map<String, Object> original)
	{
	...
	protected Map<String, Object> convertObjectToMap(final Object obj)
	{
		...
		try
		{
			...
			for (final PropertyDescriptor propertyDesc : beanInfo.getPropertyDescriptors())
			{
				...
				if ("class".equals(propertyDesc.getName()) || propertyDesc.getReadMethod() == null)
				{
					continue;
				}
		...
		{
			throw new IllegalArgumentException("Unable to introspect bean", e);
		}
		return result;
	}

	private static String createCsvHeader(final Set<String> csvHeaderValues, final Map<String, Object> map)
	{
	...
	{
		return obj == null ? null : transmissionValue(obj);
	}

	private String transmissionValue(final Object obj)
	{
		return obj instanceof Date ? DateFormatUtils.formatUTC((Date) obj, datePattern)
				: String.valueOf(obj);
	}
}
