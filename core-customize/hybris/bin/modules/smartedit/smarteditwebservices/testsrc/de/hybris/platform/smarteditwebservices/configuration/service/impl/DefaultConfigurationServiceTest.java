...
public class DefaultConfigurationServiceTest
{
	...
	public void setup()
	{
		...
		lenient().when(smarteditConfigurationDao.findByKey(KEY_ONE)).thenReturn(CONFIGURATION_MODEL_1);
		lenient().when(smarteditConfigurationDao.findByKey(KEY_TWO)).thenReturn(CONFIGURATION_MODEL_2);
		lenient().when(smarteditConfigurationDao.findByKey(KEY_THREE)).thenReturn(CONFIGURATION_MODEL_3);
	}
