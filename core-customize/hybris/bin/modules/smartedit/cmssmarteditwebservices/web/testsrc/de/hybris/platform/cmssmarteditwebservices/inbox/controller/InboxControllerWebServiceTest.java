...
public class InboxControllerWebServiceTest extends ApiBaseIntegrationTest
{
	...
	public void shouldFindAllWorkflowTasks() throws JAXBException
	{
		...
		final Response response = getCmsManagerWsSecuredRequestBuilder().path(WORKFLOW_TASKS_ENDPOINT)
				...
				.queryParam(URI_CURRENT_PAGE, Integer.valueOf(DEFAULT_CURRENT_PAGE)).build()
		...
		assertThat(entity.getPagination().getCount(), equalTo(Integer.valueOf(1)));
		assertThat(entity.getPagination().getTotalCount(), equalTo(Long.valueOf(1L)));
		assertThat(entity.getPagination().getPage(), equalTo(Integer.valueOf(0)));

		assertThat(entity.getTasks(), hasSize(1));

		final CMSWorkflowActionWsDTO action = entity.getTasks().get(0).getAction();
		assertThat(Boolean.valueOf(action.isIsCurrentUserParticipant()), is(Boolean.TRUE));
		...
		assertThat(attachments,
				hasItems(allOf(hasProperty(PAGE_UID, is(ContentPageModelMother.UID_HOMEPAGE))),
						...
						allOf(hasProperty(CATALOG_NAME,
								hasEntry(Locale.ENGLISH.getLanguage(),
										ContentCatalogModelMother.CatalogTemplate.ID_APPLE.getFirstInstanceOfHumanName()))),
