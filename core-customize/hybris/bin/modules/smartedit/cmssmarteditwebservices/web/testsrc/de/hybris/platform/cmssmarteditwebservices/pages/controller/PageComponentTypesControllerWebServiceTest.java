...
public class PageComponentTypesControllerWebServiceTest extends ApiBaseIntegrationTest
{
	...
	public void shouldReturnComponentTypesAllowedInPage()
	{
		...
		assertThat(result.getPagination(), allOf(hasProperty("totalCount", greaterThanOrEqualTo(3L)), hasProperty("page", is(0)),
				hasProperty("count", greaterThanOrEqualTo(3))));

		final List<ComponentTypeData> componentTypes = result.getComponentTypes();
		assertThat(componentTypes.size(), greaterThanOrEqualTo(3));
		assertThat(componentTypes, hasItems( //
				...
				allOf(//
						hasProperty("code", equalTo(BannerComponentModel._TYPECODE)) //
				) //
		));
	}
