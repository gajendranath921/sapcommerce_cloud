...
angular.module('customMocksModule', ['backendMocksUtilsModule']).run(function (httpBackendService) {
    ...
    backendEntry.mock = function (method, url, data, headers) {
        ...
        if (isEnabled) {
            if (result[1]) {
                var key = 'advancedMediaContainerManagement';
                var advancedMediaContainerManagementIdx = result[1].findIndex(
                    (cfgObj) => cfgObj.key === key
                );
                if (advancedMediaContainerManagementIdx === -1) {
                    result[1].push({
                        key,
                        value: true
                    });
