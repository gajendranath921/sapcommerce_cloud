    ...
    var componentObjects = {
        assertions: {
            ...
            disabledSlotWrapperIsNotPresent: function () {
                expect(
                    element(
                        by.css(componentObjects.constants.disabledSharedSlotWrapper())
                    ).isPresent()
            ...
            assertDisabledDecoratorIsNotDisplayedOnSlot: function (slotId) {
                ...
                expect(
                    browser.isAbsent(
                        by.css(componentObjects.constants.disabledSharedSlotCSSPath(slotId))
                    )
                ).toBe(true, message);
            },
            ...
            assertExternalDisabledDecoratorIsDisplayedOnSlot: function (slotId) {
                var message =
                    'Expected ' + slotId + ' to have an external slot disabled decorator.';
                expect(
                    browser.isPresent(
                        by.css(componentObjects.constants.disabledExternalSharedSlotCSSPath(slotId))
                    )
                ).toBe(true, message);
            },
            assertExternalDisabledDecoratorIsNotDisplayedOnSlot: function (slotId) {
                var message =
                    'Expected ' + slotId + ' not to have an external slot disabled decorator.';
                expect(
                    browser.isAbsent(
                        by.css(componentObjects.constants.disabledExternalSharedSlotCSSPath(slotId))
                    )
