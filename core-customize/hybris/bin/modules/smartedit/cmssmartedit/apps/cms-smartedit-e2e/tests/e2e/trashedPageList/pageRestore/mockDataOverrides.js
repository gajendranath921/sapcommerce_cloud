...
angular
    .module('mockDataOverridesModule', ['backendMocksUtilsModule', 'yLoDashModule'])
    .run(function (backendMocksUtils, httpBackendService, lodash) {
        ...
        const INVALID_NAME_EN = 'Page with duplicated name';
        const CATALOG_STAGED = 'apparel-ukContentCatalog/Staged';
        const TIMESTAMP1 = '2016-07-07T14:33:37+0000';
        const TIMESTAMP2 = '2016-07-12T01:23:41+0000';

        var CMS_ITEMS = {
            pages: [
                {
                    catalogVersion: CATALOG_STAGED,
                    creationtime: TIMESTAMP1,
                    modifiedtime: TIMESTAMP2,
                    name: 'Valid Page',
        ...
        componentsListGETMock.respond(function (method, url, data, headers) {
            if (
                isActivePageRequested(url) &&
                isDefaultPageRequested(url) &&
                isContentPageTypeRequested(url)
            ) {
                return [
                    200,
                    {
        ...
        httpBackendService
            .whenGET(
                /cmswebservices\/v1\/sites\/.*\/catalogs\/.*\/versions\/.*\/pages\/.*\/variations$/
            )
            .respond(function () {
                return [200, { uids: [] }];
            });
