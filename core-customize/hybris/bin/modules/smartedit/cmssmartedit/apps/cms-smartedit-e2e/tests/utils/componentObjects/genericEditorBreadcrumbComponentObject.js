...
module.exports = (function () {
    ...
    componentObject.elements = {
        getBreadcrumb: function () {
            ...
            var breadcrumb = modalElement.element(by.css('se-generic-editor-breadcrumb'));
        ...
        getElementTitleByNestingLevel: function (expectedNestingLevel) {
            var breadcrumb = componentObject.elements.getBreadcrumbElementInLevel(
    ...
    componentObject.assertions = {
        componentIsDisplayedInCorrectNestingLevel: function (
            componentName,
            componentType,
            expectedNestingLevel
        ) {
            expect(
                componentObject.elements
                    .getElementTitleByNestingLevel(expectedNestingLevel)
                    .getText()
            ).toBe(
                componentName,
                'Expected breadcrumb to show ' +
                    componentName +
                    ' at the ' +
                    expectedNestingLevel +
                    ' level.'
            ...
            expect(
                componentObject.elements
                    .getElementInfoByNestingLevel(expectedNestingLevel)
                    .getText()
            ).toBe(
                componentType,
                'Expected breadcrumb to show ' +
