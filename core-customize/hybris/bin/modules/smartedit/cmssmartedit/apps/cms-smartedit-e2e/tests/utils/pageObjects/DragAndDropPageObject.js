...
(function () {
    ...
    dragAndDropPageObject.structure = {
        ...
        bottomHeaderTotalComponents: 21 // see layouts/sf-layout-default.js
    };
    ...
    dragAndDropPageObject.elements = {
        getComponentDropHelpers: function (slotId, position) {
            ...
            var hintElement = by.css(
                "#smarteditoverlay .smartEditComponentX[data-smartedit-component-id='" +
                    slotId +
                    "'] .smartEditComponentX:nth-child(" +
                    position +
                    ') .overlayDropzone'
            );
        ...
        getParagraphComponentInComponentMenu: function () {
            return element(
                by.css(".smartEditComponent[data-smartedit-component-type='CMSParagraphComponent']")
            );
        },
        getComponentTypeByCodeInComponentMenu: (code) => {
            return element(by.css(`.smartEditComponent[data-smartedit-component-type='${code}']`));
        },
        getAbstractCMSComponentInComponentMenu: function () {
            return element(
                by.css(
                    "se-infinite-scrolling .smartEditComponent[data-smartedit-component-type='AbstractCMSComponent'][data-smartedit-component-id='Component10']"
                ),
                'Could not find Abstract CMS Component in Component Menu'
            );
        },
        ...
        getIframeOffset: function () {
            return browser.executeScript(
                'return window.smarteditJQuery(arguments[0]).offset()',
                element(by.id('ySmartEditFrame'))
            );
        },
    ...
    dragAndDropPageObject.actions = {
         ...
         */
        addFromComponentMenuToSlotInPosition: function (
            targetSlotId,
            position,
            expectedElementsInSlotCount
        ) {
            return browser.switchToIFrame().then(function () {
                return storefront.elements
                    .getComponentsBySlotId(targetSlotId, expectedElementsInSlotCount)
                    .then(function (elements) {
                        return dragAndDropPageObject.actions
                            .dragComponentInComponentMenuOverSlot(targetSlotId)
                            .then(function () {
                                var helperPosition =
                                    position < elements.length ? position + 1 : position;
                                var helperIndex = position < elements.length ? 0 : 1;
                                return dragAndDropPageObject.elements
                                    .getComponentDropHelpers(targetSlotId, helperPosition)
                                    .then(function (helpers) {
                                        return dragAndDropPageObject.elements
                                            .getElementBoundingClientRect(helpers[helperIndex])
                                            .then(function (elementRect) {
                                                return dragAndDropPageObject.actions
                                                    .dragOverDragAreaAtLocation(elementRect)
                                                    .then(function () {
        ...
        grabComponentInComponentMenu: function (componentTypeCode) {
            return _grabComponentInComponentMenu(componentTypeCode);
        },
        ...
        dropComponent: function (componentId, slotId) {
            return _dropComponent(componentId, slotId);
        },
        ...
        hoverOverTopHintFromOuter: function () {
            return dragAndDropPageObject.elements
                .getElementBoundingClientRect(dragAndDropPageObject.elements.getTopScrollHint())
                .then(function (elementRect) {
        ...
        hoverOverBottomHintFromOuter: function () {
            return dragAndDropPageObject.elements
                .getElementBoundingClientRect(dragAndDropPageObject.elements.getBottomScrollHint())
                .then(function (elementRect) {
                    return dragAndDropPageObject.actions.dragOverDragAreaAtLocation(elementRect);
                });
        },
        scrollToBottomFromOuter: function () {
            return browser
                .executeScript(
                    'window.smarteditJQuery(document.scrollingElement).scrollTop(document.scrollingElement.scrollHeight);'
                )
                .then(function () {
        ...
        dragComponentInComponentMenuOverSlot: function (slotId) {
            return browser.switchToIFrame().then(function () {
                return storefront.elements
                    .getComponentLocationById(slotId)
                    .then(function (elementPosition) {
                        ...
                        return dragAndDropPageObject.actions
                            .scrollToPosition(elementPosition.y)
                            .then(function () {
                                return dragAndDropPageObject.elements
                                    .getOriginalComponentRect(slotId)
                                    .then(function (elementRect) {
        ...
        scrollToPosition: function (value) {
            return browser.executeScript(
                'window.smarteditJQuery(document.scrollingElement).scrollTop(arguments[0])',
                value
            );
        }
    };
    ...
    dragAndDropPageObject.assertions = {
        pageHasScrolledUp: function (oldScroll) {
            browser.waitUntil(function () {
                return dragAndDropPageObject.actions
                    .getPageVerticalScroll()
                    .then(function (newScroll) {
    ...
    function _grabComponent(slotId, componentId) {
        ...
        return contextualMenu.elements
            .getMoveButtonForComponentId(componentId)
            .then(function (button) {
                browser.actions().mouseMove(button).mouseDown().perform();

                browser.executeScript(simulateDragAndDropOperation, button, EVENT_TYPES.DRAG_START);
                return button;
            });
    }

    async function _grabComponentInComponentMenu(componentTypeCode = 'CMSParagraphComponent') {
        console.log('_grabComponentInComponentMenu', componentTypeCode);
        await browser.click(componentMenu.elements.getComponentMenuButton());

        var elementToDrag = dragAndDropPageObject.elements.getComponentTypeByCodeInComponentMenu(
            componentTypeCode
        );
    ...
    function _enableCloneOnDropAndGrabCustomizedComponentInComponentMenu() {
        ...
        componentMenu.actions.searchComponents(
            dragAndDropPageObject.structure.components.COMPONENT10
        );

        var elementToDrag = dragAndDropPageObject.elements.getAbstractCMSComponentInComponentMenu();
        browser.actions().mouseMove(elementToDrag).perform();
    ...
    function simulateDragAndDropOperation(element, operationType, position) {
        function createCustomEvent(type, pos) {
            var event = document.createEvent('CustomEvent');
            event.initCustomEvent(type, true, true, null);
            event.dataTransfer = {
                data: {},
                setData: function (dataType, val) {
            ...
            if (pos) {
                ...
                event.pageX =
                    elementRect.left + Math.ceil((elementRect.right - elementRect.left) / 2);
                event.pageY =
                    window.scrollY +
                    elementRect.top +
                    Math.ceil((elementRect.bottom - elementRect.top) / 2);
            }
            return event;
        }
    ...
    function _isComponentInPosition(slotId, componentId, position, expectedElementsInSlotCount) {
        return browser.waitUntil(function () {
            return storefront.elements
                .getComponentsBySlotId(slotId, expectedElementsInSlotCount)
                .then(function (components) {
                    if (components.length > position) {
                        return components[position]
                            .getAttribute('data-smartedit-component-id')
                            .then(function (attr) {
