...
if (typeof require !== 'undefined') {
    cmsItemDropdown = require('../componentObjects/cmsItemDropdownComponentObject');
}
...
module.exports = (function () {
    ...
    pageObject.elements = {
        ...
        getTargetCatalogAndVersion: function () {
            return browser.findElement(
                by.css('div.target-catalog-version-selector-wrapper se-select')
            );
        },
        getTargetCatalogAndVersionToggle: function () {
            return this.getTargetCatalogAndVersion().element(by.css('.toggle-button'));
        },
        getTargetCatalogAndVersionOption: function () {
            return this.getTargetCatalogAndVersion().element(by.css('.se-item-printer'));
        },
        ...
        getTargetCatalogAndVersionInDropdown: function (name) {
            return browser
                .findElement(by.css('div.target-catalog-version-selector-wrapper se-select'))
                .all(by.cssContainingText('ol li span', name))
                .first();
        },
    ...
    pageObject.actions = {
        ...
        clickBack: function () {
            return wizard.actions.moveBack();
        },
        submit: function () {
        ...
        selectTargetCatalogAndVersion: function (name) {
            return this.openTargetCatalogAndVersionToggle().then(function () {
                var cl = browser.click(
                    pageObject.elements.getTargetCatalogAndVersionInDropdown(name)
                );
                cmsItemDropdown.actions.waitForDebounceToPass();
                return cl;
            });
        },
