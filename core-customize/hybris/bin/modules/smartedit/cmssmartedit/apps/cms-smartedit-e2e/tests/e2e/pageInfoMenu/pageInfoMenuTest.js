...
describe('Page Info Menu', function () {
    ...
    describe('content page', function () {
        ...
        it('GIVEN the user is on the storefront content page WHEN the user opens the page info menu THEN the page info is displayed', function () {
            ...
            // THEN
            expect(pageInfo.elements.getDivFieldValue('name')).toBe(
                'Homepage',
                'Expected page name to be "Homepage"'
            );
            expect(pageInfo.elements.getDivFieldValue('label')).toBe(
                'i-love-pandas',
                'Expected page label to be "i-love-pandas"'
            );
            expect(pageInfo.elements.getFieldValue('template')).toBe(
                'AccountPageTemplate',
                'Expected page template to be "AccountPageTemplate"'
            );
            expect(pageInfo.elements.getFieldValue('localizedType')).toBe(
                'ContentPage',
                'Expected localized type to be "ContentPage"'
            );
            expect(pageInfo.elements.getFieldValue('displayCondition')).toBe(
                'Variation',
                'Expected page display condition to be "Variation"'
            );
            expect(pageInfo.elements.getDivFieldValue('title')).toBe(
                'I love pandas',
                'Expected page title to be "I love pandas"'
            );
            expect(pageInfo.elements.getTimeFieldValue('creationtime')).toMatch(
                /\d+\/\d+\/\d+, \d+:\d+ (?:AM|PM)/,
                'Expected page creation time to be short date format'
            );
            expect(pageInfo.elements.getTimeFieldValue('modifiedtime')).toMatch(
                /\d+\/\d+\/\d+, \d+:\d+ (?:AM|PM)/,
                'Expected page modification time to be short date format'
            );
        });
        ...
        it('GIVEN the user is in the page info menu WHEN the user clicks the Edit button THEN the page editor modal is opened', function () {
            ...
            // THEN
            expect(pageInfo.elements.getPageEditorModal().isPresent()).toBe(
                true,
                'Expected Page Editor modal to be opened'
            );
    ...
    describe('product page', function () {
        ...
        it('GIVEN the user is on the storefront product page WHEN the user opens the page info menu THEN the page info is displayed', function () {
            ...
            expect(pageInfo.elements.getDivFieldValue('name')).toBe(
                'Some Other Page',
                'Expected page name to be "Some Other Page"'
            );
            expect(pageInfo.elements.getFieldValue('template')).toBe(
                'ProductPageTemplate',
                'Expected page label to be "ProductPageTemplate"'
            );
            expect(pageInfo.elements.getFieldValue('localizedType')).toBe(
                'ProductPage',
                'Expected localized type to be "ProductPage"'
            );
            expect(pageInfo.elements.getFieldValue('displayCondition')).toBe(
                'Primary',
                'Expected display condition to be "Primary"'
            );
            expect(pageInfo.elements.getDivFieldValue('title')).toBe(
                'I hate pandas',
                'Expected page title to be "I hate pandas"'
            );
            expect(pageInfo.elements.getTimeFieldValue('creationtime')).toMatch(
                /\d+\/\d+\/\d+, \d+:\d+ (?:AM|PM)/,
                'Expected page creation time to be short date format'
