...
angular
    .module('dragAndDropAndRemoveMocks', [])
    /*
    ...
    .run(function ($q, $timeout, httpBackendService, $window, lodash, componentMocksUpdater) {
        this.idCounter = 10;
        this.slots = [
            {
                id: 'topHeaderSlot',
            ...
            {
                id: 'bottomHeaderSlot',
                components: ['component4', 'component10'].concat(
                    Array.apply(null, {
                        length: 20
        ...
        httpBackendService
            .whenAsync(
                'PUT',
                /cmswebservices\/v1\/sites\/apparel-uk\/catalogs\/apparel-ukContentCatalog\/versions\/Staged\/pagescontentslotscomponents\/pages\/homepage\/contentslots\/.*/
            )
            .respond(
                function (method, url, data, headers) {
                    var deferred = $q.defer();
                    var parsedData = JSON.parse(data);
                    this.moveComponentToSlot(
                        parsedData.currentSlotId,
        ...
        httpBackendService
            .whenAsync(
                'DELETE',
                /cmswebservices\/v1\/sites\/.*\/catalogs\/.*\/versions\/.*\/pagescontentslotscomponents\/contentslots\/.*\/components/
            )
            .respond(
                function (method, url, data, headers) {
                    var deferred = $q.defer();
                    var slotId = /cmswebservices\/v1\/sites\/.*\/catalogs\/.*\/versions\/.*\/pagescontentslotscomponents\/contentslots\/(.*)/
                        .exec(url)[1]
                        .split('/')[0];
                    var componentId = /cmswebservices\/v1\/sites\/.*\/catalogs\/.*\/versions\/.*\/pagescontentslotscomponents\/contentslots\/.*\/components\/(.*)/.exec(
                        url
                    )[1];
        ...
        httpBackendService
            .whenGET('/cmswebservices/v1/sites/apparel-uk/cmsitems/component1')
            .respond(component1Data);
