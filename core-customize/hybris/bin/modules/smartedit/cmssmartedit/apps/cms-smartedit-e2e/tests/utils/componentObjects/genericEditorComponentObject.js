...
var genericEditorWidgets;
var TEXT_EXPECTED_FIELD = 'Expected field ';
    ...
    componentObject.elements = {
        ...
        getLocalizedFieldLanguageButton: function (fieldId, language) {
            return componentObject.elements
                .getField(fieldId)
                .element(by.cssContainingText('se-localized-element ul li a', language));
        },
        getLocalizedFieldInputs: function (fieldId) {
            return componentObject.elements
                .getField(fieldId)
                .all(by.css('se-localized-element se-tab input'));
        },
        getLocalizedFieldInputForLanguage: function (fieldId, language) {
            return componentObject.elements
                ...
                .element(
                    by.css(
                        'se-localized-element se-tab[tab-id="' + language.toLowerCase() + '"] input'
                    )
                );
        },
        ...
        modalWithName: function (componentName) {
            return by.xpath('//*[@id="fd-modal-title-type.' + componentName + '.name"]');
        },
        modalWithTitle: function (i18nKey) {
            return by.xpath('//*[@id="fd-modal-title-' + i18nKey + '"]');
        },
    ...
    componentObject.actions = {
        ...
        waitForNumberOfEditorModals: function (numberOfExpectedModals) {
            browser.waitUntil(function () {
                return genericEditorCommons.elements
                    .getOpenedEditorModals()
                    .count()
                    .then(function (count) {
                        return count === numberOfExpectedModals;
                    });
            }, 'Timeout waiting for number of editor modals to be ' +
                numberOfExpectedModals +
                '}.');
        },
        ...
        clickLocalizedFieldLanguageButton: function (fieldId, language) {
            return browser.click(
                componentObject.elements.getLocalizedFieldLanguageButton(fieldId, language)
            );
        },
        setValueForLanguage: function (fieldId, language, name) {
            return componentObject.actions
                .clickLocalizedFieldLanguageButton(fieldId, language)
                .then(function () {
                    var input = componentObject.elements.getLocalizedFieldInputForLanguage(
                        fieldId,
                        language
                    );
        ...
        setTextValueInLocalizedField: function (fieldId, valuesByLanguage) {
            ...
            Object.keys(valuesByLanguage).forEach(function (language) {
                promisesToResolve.push(
                    componentObject.actions.setValueForLanguage(
                        fieldId,
                        language,
        ...
        setEditorData: function (newData) {
            ...
            newData.forEach(function (fieldInfo) {
                var setterMethod = componentObject.utils.getSetterMethodForFieldType(
                    fieldInfo.type
                );
    ...
    componentObject.assertions = {
        ...
        saveIsDisabled: function () {
            return componentObject.elements
                ...
                .then(function (isButtonDisabled) {
        ...
        localizedFieldHasExpectedValueForLanguage: function (fieldId, language, expectedValue) {
            return componentObject.actions
                ...
                .then(function () {
                    return componentObject.elements
                        .getLocalizedFieldInputForLanguage(fieldId, language)
                        .getAttribute('value')
                        .then(function (currentValue) {
                            expect(currentValue).toEqual(
                                expectedValue,
                                'Invalid message in ' + language + ' for localized value ' + fieldId
                            );
                        });
                });
        },
        localizedFieldHasExpectedValues: async (fieldId, expectedValues) => {
            const assertions = Object.keys(expectedValues).map((language) => {
                return componentObject.assertions.localizedFieldHasExpectedValueForLanguage(
                    fieldId,
                    language,
        ...
        localizedFieldIsEmpty: function (fieldId) {
            ...
            for (var i = 0; i < localizedFieldInputs.length; i++) {
                ...
                expect(input.getText()).toBe(
                    '',
                    'Expected localized field ' + fieldId + ' to be empty'
                );
            }
        },

        topEditorIsNested: function (expectedLevelOfNesting) {
            expect(genericEditorCommons.elements.getOpenedEditorModals().count()).toBe(
                expectedLevelOfNesting,
        ...
        openEditorHasRightData: function (expectedData) {
            expectedData.forEach(function (fieldInfo) {
                var comparatorMethod = componentObject.utils.getComparatorMethodForFieldType(
                    fieldInfo.type
                );
        ...
        fieldHasValidationErrors: function (fieldName, language, expectedCount) {
            expect(componentObject.elements.getValidationErrors(fieldName, language).count()).toBe(
                expectedCount,
                `${TEXT_EXPECTED_FIELD}${fieldName} to have${expectedCount} errors.`
            );
        },
        ...
        fieldHasNoValidationErrors: function (fieldName) {
            var expectedCount = 0;
            expect(componentObject.elements.getValidationErrors(fieldName, null).count()).toBe(
                expectedCount,
                `${TEXT_EXPECTED_FIELD}${fieldName} not to have errors.`
            );
        }
    };
