...
describe('CmsLinkToSelectComponent', () => {
    ...
    beforeEach(() => {
        ...
        widgetData = {
            ...
            editor: {
                api: {
                    setAlwaysEnableSubmit(isAlwaysEnableSubmit: boolean) {
                        // This is intentional
                    },
                    considerFormDirty() {}
                } as GenericEditorAPI
    ...
    it('GIVEN option model is set an option WHEN initialized THEN it sets the option on "linkTo" property', () => {
        ...
        const spyConsiderFormDirty = spyOn(component.data.editor.api, 'considerFormDirty');
        const spySetAlwaysEnableSubmit = spyOn(component.data.editor.api, 'setAlwaysEnableSubmit');

        component.ngOnInit();

        expect(component.optionModel.linkTo).toBe('external');
        expect(spyConsiderFormDirty).toHaveBeenCalled();
        expect(spySetAlwaysEnableSubmit).toHaveBeenCalled();
    });

    it('GIVEN option model is set an option WHEN initialized THEN it sets the option on "linkTo" property', () => {
        model.linkToggle.urlLink = 'test';
