...
var sfBuilder = (function() {
    ...
    function StorefrontBuilder() {
        ...
        this.build = function(conf) {
            ...
            // Build Storefront
            domUtils.updatePageIdAttribute(
                this.config.tree.pageId,
                this.config.tree.catalogVersion
            );
        ...
        this.updatePageId = function(newPageId, newCatalogVersion) {
            if (newPageId) {
                if (newPageId.indexOf(' ') !== -1) {
                    throw 'StorefronBuilder.updatePageId() -  page Id cannot contains spaces: ' +
                        newPageId;
                }
        ...
        this.rerender = function(componentId, componentType, parentComponentIds) {
            ...
            if (!componentId) {
                ...
                var idChain = parentComponentIds
                    ? parentComponentIds + ',' + componentId
                    : componentId;
                ...
                if (str.endsWith(idChain)) {
                    ...
                    node.children.forEach(function(child, index) {
                        if (breadthFirstRemoval(child)) {
                            ...
                            var selectorForChildNode = createSelectorFromIdChain(
                                child.idChainFromRootNode
                            );
        ...
        this.removeNodesFromTree = function(componentId, parentComponentIds) {
            var selector = parentComponentIds
                ? parentComponentIds + ',' + componentId
                : componentId;
            treeUtils.treeForEach(
                this.config.tree,
                function f(node) {
                    var indicesToRemove = [];
                    node.children.forEach(function(child, index) {
        ...
        this.moveComponent = function(componentId, parentComponentIds, newPosition) {
            treeUtils.treeForEachParent(this.config.tree, parentComponentIds, function(parentNode) {
                ...
                var elementToMove = parentElement.children(
                    domUtils.SELECTORS.COMPONENT_AT_INDEX(oldPosition)
                );
                ...
                if (newPosition === 0) {
                    parentElement
                        .children(domUtils.SELECTORS.ANY_COMPONENT_INDEX)
                        .first()
                        .before(elementToMove);
                } else {
        ...
        function updateNodeIdChain(tree) {
            treeUtils.treeForEachWithDepthFirstCollector(
                tree,
                function(node, cumulatedValue) {
        ...
        function addNode(node, index) {
            ...
            var addNodeFn = addElementToDOMAndProcessNodeChildren.bind(
                this,
                newElement,
                node,
                index
        ...
        function getTemplateUrl(node) {
            ...
            var result = templateUrl.replace(
                'BUNDLE_ROOT_PLACEHOLDER',
                _conf.bundleRoot + '/dummystorefront/layouts'
            );

            return templateUrl.replace(
                'BUNDLE_ROOT_PLACEHOLDER',
                // _conf.bundleRoot + '/test/e2e/dummystorefront/layouts'
                _conf.bundleRoot + '/dummystorefront/layouts'
            );
        }
        ...
        function addAttributes(node, element, index) {
            if (node.nodeType === 'slot') {
                element.attr(
                    'class',
                    domUtils.COMPONENT_CLASSES.SLOT +
                        (node.view && node.view.class ? node.view.class : '')
                );
            } else if (node.nodeType === 'component') {
                element.attr(
                    'class',
                    domUtils.COMPONENT_CLASSES.COMPONENT +
                        (node.view && node.view.class ? node.view.class : '')
                );
            }
            ...
            element.attr(
                domUtils.ATTRIBUTES.COMPONENT_CATALOG_VERSION,
                node.catalogVersion || _conf.tree.catalogVersion
            );
    ...
    function DOMUtils() {
        ...
        this.SELECTORS = {
            ...
            COMPONENT_WITH_ID: function(id) {
                return (
                    this.SELECTORS.SLOTS_AND_COMPONENTS +
                    '[' +
        ...
        this.clearComponents = function() {
            this.getContentRoot()
                .find(this.SELECTORS.SLOTS_AND_COMPONENTS)
                .remove();
        };
        ...
        this.updatePageIdAttribute = function(pageId, catalogVersion) {
            ...
            var classes =
                uid + ' ' + uuid + ' ' + catalogClassName + ' ' + currentClasses.join(' ');
    ...
    function TreeUtils() {
        ...
        this.treeForEachWithDepthFirstCollector = function treeForEachWithDepthFirstCollector(
            node,
            fn,
            cumulatorFn,
            value
