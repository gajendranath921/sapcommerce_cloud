 ...
 *
 */
export const seInjectableInstrumenter = (fileNames: string[], injectableDecorators?: string[]) => {
    ...
    for (const sourceFile of program.getSourceFiles()) {
        ts.forEachChild(sourceFile, walkNode as any);
    }
    ...
    function walkNode(node: ts.Declaration) {
        ...
        if (node.kind === ts.SyntaxKind.ClassDeclaration && ts.isClassDeclaration(node)) {
            ...
            if (classDeclaration.decorators && classDeclaration.decorators.length) {
                ...
                if (
                    lodash.intersection(
                        classDecorators.map((d) => d.name),
                        decoratorsToInstrument
                    ).length
                ) {
                    const fileName: string = node.getSourceFile().fileName;
                    const className: string = node.name ? node.name.getFullText().trim() : '';
                    const intersectDecorators = classDecorators.filter(
                        (c) => decoratorsToInstrument.indexOf(c.name) > -1
                    );
            ...
            ts.forEachChild(node, walkNode as any);
        }
    }
    ...
    function serializeDecorator(decorator: ts.Decorator): DecoratorData {
        const symbol = checker.getSymbolAtLocation(decorator.expression.getFirstToken() as any);
