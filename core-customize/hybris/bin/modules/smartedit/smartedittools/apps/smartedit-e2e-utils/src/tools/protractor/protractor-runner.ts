...
interface ProtractorRunnerConfig {
    basePath: string;
    storefrontPath: string;
    htmlPath: string;
    testObjectsPath: string;
...
const getAllSpecsInPath = async (testProjectPath: string) => {
    return await fg('./generated/e2e/**/*Test.js', {
        cwd: testProjectPath,
    });
};

const getSpecsString = (tests: string[]) => {
    return tests.reduce((acc: string, file: string) => {
...
const buildParams = async (config: ProtractorRunnerConfig) => {
    const params = [
        configFilePath,
    ...
    if (shards > 0 && shard >= 0) {
        console.log(
            `protractor-runner - Sharding... total of shards: ${shards} - current shard #: ${shard}`
        );
...
const splitSpecs = (allSpecs: string[], shards: number, shard: number): string[] => {
...
export const runProtractor = async (config: ProtractorRunnerConfig) => {
