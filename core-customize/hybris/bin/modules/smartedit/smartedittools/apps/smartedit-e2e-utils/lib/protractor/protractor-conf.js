 ...
 */
const { bootstrapTests } = require('./test-bootstrapper');
const { loadTestObjects } = require('./object-loader');
const { setupMockBackend } = require('./mock-backend-service');
...
const ASYNC_WAIT = 20000; // the timedout value waiting for asynchronous Angular tasks to finish
...
const config = {
    ...
    baseUrl: 'http://127.0.0.1:7000/apps/',
    ...
    onPrepare: function () {
        // Use protractor.Key.CONTROL default.
        browser.controlKey = protractor.Key.CONTROL;
        browser.getCapabilities().then(function (capabilities) {
            // Use protractor.key.COMMAND instead of protractor.key.CONTROL in mac os.
        ...
        browser.driver
            ...
            .getSize()
            .then(function (size) {
                console.log(' PROTRACTOR BROWSER SIZE ' + JSON.stringify(size));
                return size;
            });

        const testsConfig = browser.params.tests;
        bootstrapTests(testsConfig);

        global.e2e = {};
        loadTestObjects(testsConfig, global.e2e);
        global.e2e.mockBackendClient = setupMockBackend();
        ...
        jasmine.getEnv().addReporter(
            new jasmineReporters.JUnitXmlReporter({
                ...
                savePath: testsConfig.reportOutputPath
            })
        );
            ...
            return browser
                ...
                .then(function (browserLogs) {
                    browserLogs
                        .filter(function (log) {
                            return (
                                log.message.indexOf('.png') === -1 &&
                                log.message.indexOf('favicon.ico') === -1
                            );
                        })
                        .filter(function (log) {
        ...
        browser.waitForContainerToBeReady = function () {
            ...
            return browser
                .waitUntil(
                    ...
                    'could not find user account toolbar menu when first loading app'
                )
                .then(function () {
        ...
        browser.waitUntilNoSpinner = function () {
            browser.waitForAbsence(by.css('.se-wait-spinner-dialog'));
        };

        browser.waitUntilNoModal = function () {
            return browser.waitForAbsence(
                by.css(
                    'body.modal-open, .fd-modal-container, .fd-overlay--modal, fd-modal-overlay, .modal-backdrop'
                )
            );
        };
        ...
        browser.waitUntilModalAppears = function () {
            return browser.waitForPresence(
                by.css(
                    'body.modal-open, .fd-modal-container, .fd-overlay--modal'
                    // ,
                    // 'fd-modal-overlay'
                )
            );
        ...
        browser.waitForFrameToBeReady = function () {
            ...
            }, "could not find data-smartedit-ready='true' attribute on the iframe body tag");
        };
        ...
        browser.moveTo = function (selector) {
            return browser.executeScript(
                'window.smarteditJQuery(arguments[0]).mouseover();',
                selector
            );
        };
        ...
        browser.click = function (source, errorMessage) {
            ...
            return browser
                .waitUntil(
                    protractor.ExpectedConditions.elementToBeClickable(
                        this._getElementFromSource(source)
                    ),
                    message
                )
                .then(
        ...
        browser.clearInput = async function (source) {
            const ele = this._getElementFromSource(source);
            await ele.sendKeys(protractor.Key.chord(browser.controlKey, 'a'));
        ...
        browser.clearAndSendKeys = function (source, text, errorMessage) {
            ...
            var message = errorMessage ? errorMessage : 'could not click on element ' + source;
            return browser
                .waitUntil(protractor.ExpectedConditions.elementToBeClickable(ele), message)
                .then(
                    function () {
            ...
            return browser.wait(
                function () {
                    return browser
                        ...
                        .then(function (initialWaitForAngularEnabled) {
                            return browser.waitForAngularEnabled(false).then(function () {
                                var promise;
                                if (assertionFunction.then) {
                                    promise = assertionFunction;
                                } else if (typeof assertionFunction === 'function') {
                                    const p2 = assertionFunction();
                                    if (p2.then) {
                                        promise = p2;
                                    } else {
                                        promise = protractor.promise.when(p2);
                                    }
                                } else {
                                    throw new Error(
                                        `unexpected type for assertionFunction ${assertionFunction} in browser.waitUntil`
                                    );
                                }
                                return promise.then(function (booleanResponse) {
                                    if (booleanResponse && initialWaitForAngularEnabled) {
                                        return browser
        ...
        browser.waitForSelectorToContainText = function (selector, text) {
            return browser.waitUntil(
                EC.textToBePresentInElement(browser._getElementFromSource(selector), text),
                'selector ' + selector + ' does not contain expected text ' + text
            );
        };

        browser.waitForSelectorToContainValue = function (selector, text) {
            return browser.waitUntil(
                EC.textToBePresentInElementValue(browser._getElementFromSource(selector), text),
                'selector ' + selector + ' does not contain expected text ' + text
            );
        };
        ...
        browser.waitForAbsence = function (source, errorMessage) {
            return browser.driver
                .manage()
                .timeouts()
                .implicitlyWait(NO_IMPLICIT_WAIT)
                .then(function () {
                    return waitForNonPresence(source, errorMessage).then(
                        function (result) {
                            return browser.driver
                                .manage()
                                .timeouts()
        ...
        browser.isAbsent = function (source) {
            return browser.waitForAbsence(source).then(
                function () {
                    return true;
                },
                function () {
        ...
        browser.isPresent = function (source) {
            return browser.waitForPresence(source).then(
                function () {
                    return true;
                },
                function () {
        ...
        browser.waitToBeDisplayed = function (source, errorMessage) {
            ...
            return browser.waitUntil(
                function () {
                    return this._getElementFromSource(source)
                        .isDisplayed()
                        .then(function () {
                            return true;
                        });
        ...
        browser.waitNotToBeDisplayed = function (source, errorMessage) {
            return browser.driver
                ...
                .implicitlyWait(NO_IMPLICIT_WAIT)
                .then(function () {
                    return waitNotToBeDisplayed(source, errorMessage).then(
                        function (result) {
                            return browser.driver
                                .manage()
                                .timeouts()
        ...
        browser.waitForVisibility = function (source) {
            return browser.waitFor(
                source,
                'visibilityOf',
                'Could not verify visibility of ' + source
            );
        };
        ...
        browser.waitFor = function (source, expectedConditions, errorMessage, specificWait) {
            specificWait = specificWait || EXPLICIT_WAIT;
            return browser.waitUntil(
                EC[expectedConditions](browser._getElementFromSource(source)),
                errorMessage
                    ? errorMessage
                    : 'Could not verify ' + expectedConditions + ' for ' + source,
                specificWait
            );
        };
        ...
        browser.hoverElement = function (source) {
            return browser.actions().mouseMove(browser.findElement(source)).perform();
        };

        browser.bringElementIntoView = function (source) {
            return browser.executeScript(
                'arguments[0].scrollIntoView(false)',
                browser._getElementFromSource(source).getWebElement()
            );
        };

        browser.elementNotClickable = function (source) {
            return browser
                .click(source)
                .then(function () {
                    return true;
        ...
        beforeEach(function () {
            ...
            browser.executeScript(
                'window.sessionStorage.setItem("isInstrumented", arguments[0])',
                browser.params.isInstrumented
            );
                ...
                .then(function () {
                            ...
                            compare: function (actual, expected) {
                                try {
                                    ...
                                    return {
                                        pass: false,
                                        message:
                                            "Error executing matcher '" + matcherName + "': " + e
                                    };
                                }
                            }
                        };
                    ...
                    jasmine.addMatchers({
                        ...
                        toEqualText: function () {
                            return getSafeMatcher('toEqualText', function (
                                actualLocatorOrElement,
                                expectedText
                            ) {
                                var actual = '';
                                return {
                                    pass: element(actualLocatorOrElement)
                                        .getText()
                                    ...
                                    message:
                                        ...
                                        ' to equal text ' +
                                        expectedText +
                                        '. Note: both will be trimmed.'
                                };
                            });
                        },
                        toContainClass: function () {
                            return getSafeMatcher('toContainClass', function (
                                actualLocatorOrElement,
                                expectedCssClass
                            ) {
                                var actualClasses = '';
                                return {
                                    pass: browser
                                        ._getElementFromSource(actualLocatorOrElement)
                                        .getAttribute('class')
                                        .then(function (classes) {
                                            actualClasses = classes;
                                            return (
                                                classes.split(' ').indexOf(expectedCssClass) !== -1
                                            );
                                        }),
                                    message:
                                        'Expected source classes "' +
                                        actualClasses +
                                        '" to contain the css class "' +
                                        expectedCssClass +
                                        '"'
                                };
                            });
                        ...
                        toBeAbsent: function () {
                            return {
                                compare: function (locator) {
                                    var message =
                                        'Expected element with locator "' +
                                        locator +
                                        '" to be present in DOM';
                                    return {
                                        pass: browser.driver
                                            .manage()
                                            .timeouts()
        ...
        afterEach(function (done) {
            ...
            // If you need to see detail browser logs produced by browser,
    ...
    onComplete: function () {
        if (browser.params.isInstrumented) {
            ...
            fs.mkdirs(
                bundlePaths.report.backwardCompatibilityResults +
                    '/' +
                    browser.params.instrumentPrefix
            );
            saveToFile(
                bundlePaths.report.instrument_functions_file.replace(
                    'VERSION',
                    browser.params.instrumentPrefix
                ),
                FUNCTIONS_LOG
            ...
            saveToFile(
                bundlePaths.report.instrument_directives_file.replace(
                    'VERSION',
                    browser.params.instrumentPrefix
                ),
                DIRECTIVES_LOG
            ...
            saveToFile(
                bundlePaths.report.instrument_service_not_exists_file.replace(
                    'VERSION',
                    browser.params.instrumentPrefix
                ),
                SERVICE_NOT_EXISTS_LOG
            );
        }
    },
    params: {
