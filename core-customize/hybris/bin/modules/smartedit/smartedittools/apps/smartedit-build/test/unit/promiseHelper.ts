...
class PromiseHelper {
    ...
    buildPromise<T>(
        name: string,
        promiseType: PromiseType = PromiseType.RESOLVES,
        value?: T
    ): IExtensiblePromise<T> {
        const promise = jasmine.createSpyObj<IPromise<T>>(name, [
            'then',
            'finally',
            'catch'
        ]) as IExtensiblePromise<T>;

        promise.promiseType = promiseType;
        promise.value = value;
        promise.name = name;
        promise.then.and.callFake(
            (resolveFunction: (response: any) => any, rejectFunction: (response: any) => any) => {
    ...
    $q(): SpyObj<angular.IQService> {
        const $q: SpyObj<angular.IQService> = jasmine.createSpyObj<angular.IQService>('$q', [
            'all',
            'defer',
            'when',
            'resolve',
        ...
        $q.all.and.callFake((promises: IExtensiblePromise<any>[]) => {
            ...
            const oneRejected = promises.find(
                (promise) => promise.promiseType === PromiseType.REJECTS
            );
