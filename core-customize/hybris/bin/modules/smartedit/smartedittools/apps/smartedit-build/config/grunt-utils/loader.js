...
module.exports = function(grunt) {
    ...
    class SmarteditBuildLoader {
        constructor() {
            ...
            this.config = {
                taskPaths: [bundlePaths.build.grunt.tasksDir, bundlePaths.external.grunt.tasksDir],
                configPaths: [
        ...
        load() {
            ...
            this.config.configPaths.forEach(function(configRoot) {
                ...
                recursiveCallForEachFile(configRoot, function(dir, file, fullPath) {
                    ...
                    try {
                        ...
                        grunt.log.error(
                            `ERROR: Problem reading grunt task config for ${fullPath}`['red']
                        );
            ...
            grunt.loadConfigForTask = function(taskName) {
                if (loadedConfigs.indexOf(taskName) === -1) {
                    ...
                    this.config.configPaths.forEach(
                        function(configRoot) {
                            ...
                            if (fs.existsSync(configFile)) {
                                try {
                                    grunt.config.set(
                                        taskName,
                                        require(configFile)(grunt).config(
                                            this.config.configData,
                                            lodash.cloneDeep(originalConfigGet(taskName) || {})
                                        )
                                    );
                                } catch (e) {
                                    grunt.log.error(
                                        `ERROR: Problem reading grunt task config for ${configFile}`[
                                            'red'
                                        ]
                                    );
                                    grunt.log.error(e.stack);
