...
module.exports = function(grunt) {
    ...
    return {
        ...
        config: function(data, conf) {
            ...
            const maxInstances = grunt.option(MAX_INSTANCES)
                ? parseInt(grunt.option(MAX_INSTANCES))
                : process.env.PROTRACTOR_CHROME_INSTANCES ||
                  getProtractorCapabilities().maxInstances ||
                  5;
            ...
            if (
                getProtractorCapabilities().chromeOptions &&
                getProtractorCapabilities().chromeOptions.args
            ) {
            ...
            if (openBrowser) {
                ...
                grunt.log.writeln(
                    'Note: In headless mode, we cannot modify the browser size in run-time.'
                );
            }
            ...
            conf.maxrun = {
                // multiple instance e2e (more performant)
                options: {
                    args: {
                        capabilities: {
                            shardTestFiles: true,
