...
`;
    ...
    grunt.registerMultiTask(taskName, taskDescription, function() {
        ...
        var templateFile = path.join(
            ...
            'config/grunt-utils/smartedit.index.tpl.html'
        );
        var templateContents = fs.readFileSync(templateFile, 'utf-8');

        templateContents = disclaimer.concat(templateContents);
        templateContents = templateContents.replace(
            /PRESMARTEDIT_PLACEHOLDER/g,
            gruntConfig.preSmarteditContent || ''
        );
        templateContents = templateContents.replace(
            /SMARTEDIT_PLACEHOLDER/g,
            gruntConfig.smarteditContent || ''
        );
        templateContents = templateContents.replace(
            /HEADER_PLACEHOLDER/g,
            gruntConfig.headerContent || ''
        );
        templateContents = templateContents.replace(
            /BUNDLE_PLACEHOLDER/g,
            gruntConfig.bundleContent || ''
        );
        ...
        if (global.smartedit.bundlePaths.bundleRoot) {
            LOGGER.verbose.detail(
                'Bundle root calculated to be at: ' + global.smartedit.bundlePaths.bundleRoot
            );
            LOGGER.verbose.detail(
                'Output dir calculated to be at: ' + path.resolve(path.dirname(gruntConfig.dest))
            );
            ...
            LOGGER.verbose.detail(
                'Replacing occurrences in content of BUNDLE_LOCATION with: ' + relativePath
            );
