        ...
        function() {
                        ...
                        grunt.file
                            ...
                            .forEach(function(row, index) {
                                ...
                                if (row.indexOf('#') !== 0 && row !== '') {
                                    // missing keys
                                    if (row.indexOf('=') === 0) {
                                        output.i18n[fileName].missing_keys.push(
                                            index + 1 + "|'" + row + "'"
                                        );
                                    }
                                    // missing values
                                    else if (row.indexOf('=') === row.length - 1) {
                                        output.i18n[fileName].missing_values.push(
                                            index + 1 + "|'" + row + "'"
                                        );
                                    }
                                    // multiple equal signs
                                    else if ((row.match(/=/g) || []).length !== 1) {
                                        output.i18n[fileName].not_one_equal_signs.push(
                                            index + 1 + "|'" + row + "'"
                                        );
                                    } else {
                                        row = row.split('=');
                                        ...
                                        if (row[0] in files.i18n[fileName]) {
                                            output.i18n[fileName].duplicated_keys.push(
                                                index + 1 + "|'" + row + "'"
                                            );
                                        }
                                        // validated key
                        ...
                        regExs.forEach(
                            function(regEx) {
                                ...
                                if (processedKeys !== null) {
                                    ...
                                    foundKeysPerFile[fileName][rowIndex + 1] =
                                        '|' + processedKeys.join('|') + '|';
                                }
                            }.bind(this)
                        );
                ...
                foundKeys.forEach(
                    function(key) {
                        ...
                        for (fileName in files.i18n) {
                            if (
                                files.i18n[fileName].hasOwnProperty(key) &&
            ...
            var checkPropertiesI18nCompliancy = function() {
                ...
                for (var fileName in files.i18n) {
                    ...
                    Object.getOwnPropertyNames(files.i18n[fileName]).forEach(
                        function(key, index) {
                                ...
                                case 'not_properly_prefixed':
                                    if (output.i18n[fileName][status].length > 0) {
                                        report =
                                            ...
                                            status.replace(/_/g, ' ') +
                                            ': ' +
                                            output.i18n[fileName][status].length +
                                            ' key' +
                                            singularOrPlurial(output.i18n[fileName][status]);
                                        grunt.log.writeln(report);
                                        grunt.log.writeln(
                                            '- ' + output.i18n[fileName][status].join('\n')
                                        );
                                    }
                                    break;
                ...
                var _publishSourceOutputLog = function() {
                    for (var type in output.sources) {
                        var report =
                            ...
                            type.replace(/_/g, ' ') +
                            ': ' +
                            output.sources[type].length +
                            ' key' +
                            singularOrPlurial(output.sources[type]) +
                            '';
