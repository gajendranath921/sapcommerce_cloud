...
export namespace ClientPagedListPageObject {
    ...
    export const Actions = {
        async navigateToTestPage(PERSPECTIVE_SERVICE_RESULT = false): Promise<void> {
            const pageURI = `smartedit-e2e/generated/e2e/clientPagedList/index.html?perspectiveServiceResult=${PERSPECTIVE_SERVICE_RESULT}`;
        ...
        async clickOnColumnHeader(key: string): Promise<any> {
        ...
        async hoverOnIcon(row: number): Promise<void> {
            await browser.actions().mouseMove(Elements.iconForRow(row)).perform();
        }
    };
