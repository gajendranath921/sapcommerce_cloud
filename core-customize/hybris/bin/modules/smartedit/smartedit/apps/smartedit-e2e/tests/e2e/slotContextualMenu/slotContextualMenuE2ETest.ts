...
describe('Slot Contextual Menu Decorator', () => {
    beforeEach(async () => {
        await Page.Actions.getAndWaitForWholeApp(
            'smartedit-e2e/generated/e2e/slotContextualMenu/#!/ng/storefront'
        );
    ...
    it('WHEN I click one of the slot contextual menu items THEN I expect the callback to be triggered', async () => {
        const DUMMY1 = 'slot.context.menu.title.dummy1';
        await hoverOverSlot(Storefront.Constants.BOTTOM_HEADER_SLOT_ID);
        await browser.click(
            by.css(
                '.smartEditComponentX[data-smartedit-component-id="' +
        ...
        await browser.click(
            await contextualMenuItemForSlot(DUMMY1, Storefront.Constants.BOTTOM_HEADER_SLOT_ID)
        );
        expect(
            await contextualMenuItemForSlot(
                DUMMY1,
