...
export namespace Login {
    ...
    export const Actions = {
        async logoutUser(): Promise<any> {
            ...
            await browser.waitUntil(
                ExpectedConditions.elementToBeClickable(Elements.userAccountButton()),
                'Timed out waiting for user account button'
            );
    ...
    export const Assertions = {
        async assertLanguageSelectorLanguage(language: string): Promise<any> {
            await browser.waitForPresence(Elements.languageSelectorDropdown());
