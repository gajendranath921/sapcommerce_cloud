...
describe('Multi Select -', () => {
    ...
    const testProduct1 = 'Test Product 1#';
    const testProduct2 = 'Test Product 2#';
    const testProduct3 = 'Test Product 3#';

    beforeEach(async () => {
        await browser.get('smartedit-e2e/generated/e2e/select/#!/ng/storefront');
    ...
    it('GIVEN a multi selector WHEN selecting a product in the dropdown list THEN the dropdown selection list should be updated', async () => {
        ...
        await Select.Actions.selectOptionByText(multiSelect1, testProduct1);
    ...
    it('GIVEN a custom item component THEN options and selected options should be displayed with that component', async () => {
        await Select.Assertions.selectedOptionContainsTextByIndex(multiSelect2, testProduct1, 0);
        await Select.Assertions.selectedOptionContainsTextByIndex(multiSelect2, testProduct2, 1);
        await Select.Actions.toggleMultiSelector(multiSelect2);
        await Select.Assertions.optionContainsTextByIndex(multiSelect2, testProduct3, 0);
    });
    ...
    it('GIVEN a Multi Selector WHEN selecting an option THEN that option should no longer be in dropdown list', async () => {
        ...
        await Select.Actions.toggleMultiSelector(multiSelect1);
        await Select.Assertions.optionNotInDropdownByText(multiSelect1, testProduct1);
    });
    ...
    it('GIVEN selected options WHEN removing a selected option THEN it should not be displayed among selected options BUT displayed in dropdown list', async () => {
        await Select.Actions.removeSelectedOptionByText(multiSelect1, testProduct2);

        await Select.Actions.toggleMultiSelector(multiSelect1);

        await Select.Assertions.optionContainsTextByIndex(multiSelect1, testProduct2, 1);
    });
    ...
    describe('fetchPage', () => {
        ...
        it('GIVEN "fetchPage" WHEN opening a dropdown THEN items should be displayed', async () => {
            ...
            await Select.Assertions.optionContainsTextByIndex(multiSelect1, testProduct1, 0);
            await Select.Assertions.optionContainsTextByIndex(multiSelect1, testProduct2, 1);
        });
