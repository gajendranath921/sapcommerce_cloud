...
describe('GenericEditor -', () => {
    ...
    const RICH_TEXT_FIELD_INVALID_VALUE_CHINESE =
        GenericEditorPageObject.Constants.CONTENT_FIELD_INVALID_TEXT_CHINESE;
    ...
    describe('form save', () => {
        ...
        it(
            'GIVEN invalid information in RichTextEditor WHEN user does not check the warning message THEN it will not display' +
                ' the submit button',
            async () => {
                // GIVEN
                await GenericEditorComponentObject.Actions.selectLocalizedFieldLanguage(
                ...
                await GenericEditorComponentObject.Actions.setValueInRichTextField(
                    RICH_TEXT_FIELD,
                    ENGLISH_TAB,
                    RICH_TEXT_FIELD_INVALID_VALUE_CHINESE
                );

                // THEN
                await GenericEditorComponentObject.Assertions.submitButtonIsNotDisplayed();

                await GenericEditorComponentObject.Assertions.richTextWarningMessageDisplayed();
            }
        );

        it(
            'GIVEN invalid information in RichTextEditor WHEN user check the warning message THEN it will display' +
                ' the submit button',
            async () => {
                // GIVEN
                await GenericEditorComponentObject.Actions.selectLocalizedFieldLanguage(
                ...
                await GenericEditorComponentObject.Actions.clickWarningCheckBox();

                // THEN
                await GenericEditorComponentObject.Assertions.submitButtonIsDisplayed();

                await GenericEditorComponentObject.Assertions.richTextWarningMessageNotDisplayed();
            }
        );
