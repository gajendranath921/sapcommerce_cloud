...
describe('DynamicPagedList - ', () => {
    ...
    describe('has no permission to display action dropdown', () => {
        beforeEach(async () => {
    ...
    describe('has permission to display action dropdown', () => {
        beforeEach(async () => {
        ...
        it('GIVEN I am on the page list WHEN permission service returns true AND I click on more menu button AND click on the first one THEN I expect to execute a callback function to be called which changes the style of the corresponding element', async () => {
            const moreMenu = await PagedListPageObject.Actions.openMoreMenuFirstElement();
            const dropdown = moreMenu.element(by.xpath('..'));
            const dropdownMenuItem = dropdown.element(
                by.css('.se-dropdown-menu__list se-dropdown-menu-item')
            );

            await dropdownMenuItem.getText().click();

            expect(await dropdown.getAttribute('class')).toMatch('dropdown-item-clicked');
        });
    });
});
