...
describe('outer featureService', () => {
    const toolbarServiceFactory = jasmine.createSpyObj('toolbarServiceFactory', [
        'getToolbarService'
    ]);
    ...
    beforeEach(() => {
        featureService = new FeatureService(toolbarServiceFactory, cloneableUtils);
    });
    ...
    it('register throws exception if key is not provided', () => {
        expect(() => {
            featureService.register({
                ...
                enablingCallback: lo.noop,
                disablingCallback: lo.noop
            } as IFeature);
    ...
    it('GIVEN that feature alias is found in the same frame, THEN enable will call the enabling callback is called and no request is sent across the gateway', function () {
        ...
        const key = 'key1';
        spyOn(featureService as any, '_remoteEnablingFromInner').and.returnValue(Promise.resolve());
        spyOn(featureService as any, '_remoteDisablingFromInner').and.returnValue(
            Promise.resolve()
        );
    ...
    describe('addToolbarItem will call register', () => {
        ...
        @Component({ selector: 'toolbar-item-comp' })
        class ToolbarItemComp {}
    ...
    describe('permissions', () => {
        ...
        it('featureService registers with permissions returns permissions for feature by calling getFeatureProperty', (done) => {
            // GIVEN
            featureService.register(configuration);

            // WHEN
            featureService
                .getFeatureProperty(FEATURE_KEY, 'permissions')
