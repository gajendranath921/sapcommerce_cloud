...
describe('ConfigurationEditor', () => {
    const VALUE_MALFORMED = '{malformed}';
    const ERROR_MSG_KEY_JSON_PARSE = 'se.configurationform.json.parse.error';
    const dataGet = [
        {
            key: '1',
            value: '{"location":"uri"}',
        ...
        expect((service as any).pristine).toEqualData([
            ...
            },
            {
                ...
                errors: {
                    values: [
                        {
                            message: ERROR_MSG_KEY_JSON_PARSE
                        }
                    ]
                }
            }
    ...
    it('submit will do nothing if configurationForm is not dirty', () => {
        const configurationForm = mockConfigurationForm({ dirty: false, valid: true });

        (service as any).configuration = [];

        service.submit(configurationForm).catch((reason) => {
