...
export class StorageManager implements IStorageManager {
    ...
    deleteStorage(storageId: string, force = false): Promise<boolean> {
    ...
    deleteExpiredStorages(force = false): Promise<boolean> {
    ...
    private applyDisposeDecorator(storageId: string, storage: IStorage<any, any>): void {
        ...
        storage.dispose = (): Promise<boolean> =>
            this.deleteStorage(storageId).then(() => originalDispose());
    }
    ...
    private verifyMetaData(
        ...
        if (metadata.storageType !== configuration.storageType) {
            this.logService.warn(
                `StorageManager - Detected a change in storage type for existing storage. Removing old storage with id ${configuration.storageId}`
            );
            return this.deleteStorage(metadata.storageId);
        }
        return Promise.resolve(true);
