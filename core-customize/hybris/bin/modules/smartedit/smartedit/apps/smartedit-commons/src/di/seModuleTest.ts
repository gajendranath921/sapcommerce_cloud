...
describe('SeModule Annotation', () => {
    ...
    beforeEach(() => {
        ...
        mockAModule = jasmine.createSpyObj('mockAModule', [
            'service',
            'constant',
            'factory',
            'config',
        ...
        mockBModule = jasmine.createSpyObj('mockBModule', [
            'service',
            'constant',
            'factory',
            'config',
    ...
    it('SmartEdit annotates module', () => {
        ...
        @SeModule({
            initialize,
            config: configure,
            imports: [SomeFunctionModule, 'anotherDummyModule']
        })
        class AModule {}
        ...
        expect(mockAngular.module.calls.argsFor(0)).toEqual([
            'aModule',
            ['someFunctionModule', 'anotherDummyModule']
        ]);
    });
    ...
    it('SeModule to throw error when configurable class module has not yet been annotated', () => {
        ...
        expect(() => {
            @SeModule({
                imports: [BModule.configure()]
            })
            class AModule {}
        }).toThrow(
            new Error(
                "bModule module was imported into aModule module but doesn't seem to have been @SeModule annotated"
            )
        );
    ...
    it('Module A will register the providers on SeComponents and SeDirectives', () => {
        ...
        @SeModule({
            declarations: [AComponent, BDirective]
        })
        class AModule {}

        expect(mockAModule.constant.calls.argsFor(0)).toEqual([
            'aComponentProvider',
            'AComponentProviderValue'
        ]);
        expect(mockAModule.constant.calls.argsFor(1)).toEqual([
            'bComponentProvider',
            'BComponentProviderValue'
        ]);
    ...
    it('SeComponent with attribute selector will throw an exception', () => {
        expect(() => {
            ...
            new Error(
                'component AComponent declared a selector on class or attribute. version 1808 of Smartedit DI limits SeComponents to element selectors'
            )
        );
    });
