...
describe('WizardService', () => {
    ...
    class DummyStepComponent1 {}
    class DummyStepComponent2 {}
    class DummyStepComponent3 {}
    const DummyStepComponent = {
        step1: DummyStepComponent1,
        step2: DummyStepComponent2,
        step3: DummyStepComponent3
    };
    ...
    const getStepComponent = (stepHandle: string) => {
        return DummyStepComponent[stepHandle];
    };
    ...
    const createDummyStep = (stepHandle: string): any => {
        return {
            ...
            component: getStepComponent(stepHandle)
        };
    };

    const initDefaultData = () => {
        wizardService.initialize({
            steps: [createDummyStep(handle1), createDummyStep(handle2), createDummyStep(handle3)]
        } as any);
    };

    beforeEach(() => {
        wizardService = new WizardService(
            new DefaultWizardActionStrategy(new WizardActions()),
            stringUtils
        );
    });
    ...
    it('begin with the first step', () => {
        ...
        expect(currentStep.component).toBe(DummyStepComponent[stepHandle]);
    });
    ...
    it('load a step by id', () => {
        ...
        expect(currentStep.component).toBe(DummyStepComponent[handle2]);
    });
    ...
    it('see an exception if I pass duplicate step ids', () => {
        const init = wizardService.initialize.bind(wizardService, {
            steps: [createDummyStep(handle1), createDummyStep(handle1)]
        });
