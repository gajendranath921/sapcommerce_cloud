...
describe('DragAndDropCrossOriginInner', () => {
    ...
    let event1: jasmine.SpyObj<any>;
    let event2: jasmine.SpyObj<any>;
    let event3: jasmine.SpyObj<any>;
    ...
    beforeEach(() => {
        ...
        inViewElementObserver.elementFromPoint.and.callFake((_mousePosition: IMousePosition) => {
            if (_mousePosition.x === mousePosition1.x && _mousePosition.y === mousePosition1.y) {
                ...
                throw new Error(
                    `inViewElementObserver.elementFromPoint1 called with unexpected coordinates: x:${_mousePosition.x}, y:${_mousePosition.y}`
                );
            }
        });
    ...
    describe('callbacks', () => {
        ...
        it('onDropElementInner will dispatch both a drop and a drag leave event', () => {
            ...
            expect(elementWrapper1.data as any).toHaveBeenCalledWith(
                SMARTEDIT_ELEMENT_HOVERED,
                false
            );
            ...
            expect(event1.pageX).toBe(mousePosition1.x);
            expect(event1.pageY).toBe(mousePosition1.y);
            ...
            expect(event2.pageX).toBe(mousePosition1.x);
            expect(event2.pageY).toBe(mousePosition1.y);
        });
        ...
        it('WHEN receiving a track event the first time, currentElementHovered will receive both a drag enter and a drag over', () => {
            ...
            expect(event1.pageX).toBe(mousePosition1.x + scrollLeft);
            expect(event1.pageY).toBe(mousePosition1.y + scrollTop);

            expect(elementFromPoint1.dispatchEvent).toHaveBeenCalledWith(event2);
            expect(event2.initCustomEvent).toHaveBeenCalledWith('dragover', true, true, null);
            expect(event2.pageX).toBe(mousePosition1.x + scrollLeft);
            expect(event2.pageY).toBe(mousePosition1.y + scrollTop);
        });
        ...
        describe('after initial track mouse event', () => {
            ...
            it('when different element is target, the last one is left and the new one is entered and dragged over', () => {
                ...
                expect(event1.pageX).toBe(mousePosition2.x + scrollLeft);
                expect(event1.pageY).toBe(mousePosition2.y + scrollTop);
                ...
                expect(event2.pageX).toBe(mousePosition2.x + scrollLeft);
                expect(event2.pageY).toBe(mousePosition2.y + scrollTop);
                ...
                expect(event3.pageX).toBe(mousePosition2.x + scrollLeft);
                expect(event3.pageY).toBe(mousePosition2.y + scrollTop);
            });
