...
public class DefaultSelectiveCartFacadeTest
{
	...
	public void testWishlistForSelectiveCart()
	{
		...
		Mockito.verify(wishlistConverter).convert(any());
	}
	...
	public void testRemoveWishlistEntryForProduct()
	{
		...
		Mockito.when(selectiveCartService.getWishlistForSelectiveCart()).thenReturn(wishlist2Model);
		Mockito.when(productService.getProductForCode(anyString())).thenReturn(productModel1);

		defaultSelectiveCartFacade.removeWishlistEntryForProduct(productModel1.getCode());

		Mockito.verify(selectiveCartService).removeWishlistEntryForProduct(any(), any());
	}
	...
	public void testAddToCartFromWishlist() throws CommerceCartModificationException
	{
		...
		Mockito.when(selectiveCartService.getWishlistForSelectiveCart()).thenReturn(wishlist2Model);
		Mockito.when(productService.getProductForCode(anyString())).thenReturn(productModel1);
		Mockito.when(selectiveCartService.getWishlistEntryForProduct(any(), any()))
				.thenReturn(wishlist2EntryModel1);
		Mockito.when(cartFacade.addToCart(anyString(), anyLong())).thenReturn(cartModification);
		Mockito.doNothing().when(selectiveCartService).updateCartTimeForOrderEntry(anyString(), anyInt(),
				any());
	...
	public void testAddToWishlistFromCartByOrderEntry() throws CommerceCartModificationException
	{
		...
		Mockito.when(selectiveCartService.getWishlistForSelectiveCart()).thenReturn(wishlist2Model);
		Mockito.when(productService.getProductForCode(anyString())).thenReturn(productModel1);
		Mockito.when(selectiveCartService.saveWishlistEntryForProduct(any(), any(), any()))
				.thenReturn(wishlist2EntryModel1);

		defaultSelectiveCartFacade.addToWishlistFromCart(orderEntryData);

		Mockito.verify(selectiveCartService).updateQuantityForWishlistEntry(any(), any());
	}
	...
	public void testaddToWishlistFromCartByProductCode() throws CommerceCartModificationException
	{
		...
		Mockito.doNothing().when(defaultSelectiveCartFacade).addToWishlistFromCart(nullable(OrderEntryData.class));

		defaultSelectiveCartFacade.addToWishlistFromCart(productCodes);

		Mockito.verify(defaultSelectiveCartFacade, Mockito.times(2)).addToWishlistFromCart(nullable(OrderEntryData.class));
	}
}
