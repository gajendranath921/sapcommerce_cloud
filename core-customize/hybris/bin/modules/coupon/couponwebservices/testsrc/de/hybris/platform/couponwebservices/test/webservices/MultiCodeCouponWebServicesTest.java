...
public class MultiCodeCouponWebServicesTest extends AbstractCouponWebServicesTest
{
	 ...
	 * which etrieves a multi-code coupon for a given couponId
	 * @param couponId
	 * @return status
	 */
	@Test
	 ...
	 * which retrieves all multi-code coupons that are registered in the system
	 * @return status
	 */
	@Test
			...
			final String genConfigName)
	{
		...
		if (putResult.getStatus() == Status.NO_CONTENT.getStatusCode())
		{
			...
			{
				assertNull(respMultiCodeCouponWsDTO.getStartDate());
			}
			else
			{
				assertTrue(startDateVal.startsWith(respMultiCodeCouponWsDTO.getStartDate())); // precision in nano seconds might be off
			}
			if (endDateVal == null)
			{
				assertNull(respMultiCodeCouponWsDTO.getEndDate());
			}
			else
			{
				assertTrue(endDateVal.startsWith(respMultiCodeCouponWsDTO.getEndDate())); // precision in nano seconds might be off
			}
	 ...
	 * which updates multi-code coupon data as provided in PUT body
	 * @param MultiCodeCouponWsDTO
	 * @return status
	 */
	@Test
	 ...
	 * which updates status of a multi-code coupon as provided in POST body
	 * @param  couponId,active=false
	 * @return status
	 */
	@Test
	 ...
	 * which validates a multi-code coupon with given coupon code
	 * @param couponId
	 * @return status=ok
	 */
	@Test
	 ...
	 * <p>the test is for occ api of
	 * "/couponservices/v2/multicodecoupon/validate/{couponCode}"
	 * which validates a multi-code coupon with given coupon code which is not active
	 * @param couponId
