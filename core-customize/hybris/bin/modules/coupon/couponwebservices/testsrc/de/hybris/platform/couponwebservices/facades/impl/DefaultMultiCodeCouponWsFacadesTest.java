...
public class DefaultMultiCodeCouponWsFacadesTest
{
	...
	{
		//given
		Mockito.lenient().when(couponPaginatedGenericDao.find(anyObject())).thenReturn(createResults(multiCodeCouponModel));
		Mockito.lenient().when(multiCodeCouponWsDTOConverter.convert(multiCodeCouponModel)).thenReturn(mutliCodeCuponDTO);
		//when
