...
public class SingleCodeCouponWebServicesTest extends AbstractCouponWebServicesTest
{
	 ...
	 * which retrieves all of the single-code coupons that are registered in the system
	 * @return SortWsDTO List
	 */
	@Test
	...
	private void assertSort(final SortWsDTO sort,final String code, final boolean asc)
	{
	 ...
	 * which retrieves a single-code coupon for a given couponId
	 * @param couponId
	 * @return status
	 */
	@Test
	 ...
	 * which creates a new single-code coupon object with the parameters provided in POST body
	 * @param SingleCodeCouponWsDTO
	 * @return status
	 */
	@Test
			...
			final Integer maxRedemptionsPerCustomer, final Integer maxTotalRedemptions)
	{
		...
		if (putResult.getStatus() == Status.NO_CONTENT.getStatusCode())
		{
			...
			{
				assertNull(respSingleCodeCouponWsDTO.getStartDate());
			}
			else
			{
				assertTrue(startDateVal.startsWith(respSingleCodeCouponWsDTO.getStartDate())); // precision in nano seconds might be off
			}
			if (endDateVal == null)
			{
				assertNull(respSingleCodeCouponWsDTO.getEndDate());
			}
			else
			{
				assertTrue(endDateVal.startsWith(respSingleCodeCouponWsDTO.getEndDate())); // precision in nano seconds might be off
			}
	 ...
	 * which updates single-code coupon data as provided in PUT body
	 * @param SingleCodeCouponWsDTO
	 * @return status
	 */
	@Test
	 ...
	 * which updates status of a single-code coupon as provided in POST body.
	 * @param SingleCodeCouponWsDTO
	 */
	@Test
	 ...
	 * which validates a single-code coupon with a given couponId
	 * @param  couponId
	 * @return CouponResponseWsDTO
	 */
	@Test
