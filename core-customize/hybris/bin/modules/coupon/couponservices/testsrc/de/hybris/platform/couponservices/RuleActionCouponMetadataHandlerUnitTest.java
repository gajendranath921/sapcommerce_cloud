...
public class RuleActionCouponMetadataHandlerUnitTest
{
	...
	public void setUp()
	{
		Mockito.lenient().when(promotionResultModel.getOrder()).thenReturn(orderModel);
		Mockito.lenient().when(orderModel.getAllPromotionResults()).thenReturn(Collections.singleton(promotionResultModel));
		final Optional<AbstractCouponModel> cpn1Optional = Optional.of(singleCodeCpn1);
		Mockito.lenient().when(couponService.getCouponForCode(SINGLE_COUPON1_CODE)).thenReturn(cpn1Optional);
		final Optional<AbstractCouponModel> cpn2Optional = Optional.of(singleCodeCpn2);
		Mockito.lenient().when(couponService.getCouponForCode(SINGLE_COUPON2_CODE)).thenReturn(cpn2Optional);
		final Optional<AbstractCouponModel> multiCpn1Optional = Optional.of(multiCodeCpn1);
		Mockito.lenient().when(couponService.getCouponForCode(MULTI_COUPON1_CODE)).thenReturn(multiCpn1Optional);
		Mockito.lenient().when(promotionResultUtils.getOrder(promotionResultModel)).thenReturn(orderModel);
	}
	...
	public void testHandleSingleCodeCoupon()
	{
		...
		Mockito.lenient().when(orderModel.getAppliedCouponCodes()).thenReturn(Collections.singleton(SINGLE_COUPON1_CODE));
		Mockito.lenient().when(promotionResultModel.getActions()).thenReturn(Collections.singleton(actionModel));
		Mockito.lenient().when(ruleActionCouponMetadataHandler.getMetadataId()).thenReturn("cpnHandler");
	...
	public void testHandleMultiCodeCoupon()
	{
		...
		actionModel.setPromotionResult(promotionResultModel);
		actionModel.setUsedCouponCodes(new ArrayList<String>());
		Mockito.lenient().when(orderModel.getAppliedCouponCodes()).thenReturn(Collections.singleton(MULTI_COUPON1_CODE));
		Mockito.lenient().when(promotionResultModel.getActions()).thenReturn(Collections.singleton(actionModel));
		Mockito.lenient().when(ruleActionCouponMetadataHandler.getMetadataId()).thenReturn("cpnHandler");
