...
public class DefaultCouponManagementServiceUnitTest
{
	...
	public void testValidateSingleCodeCoupon()
	{
		...
		Mockito.lenient().when(abstractOrder.getUser()).thenReturn(userModel);
		Mockito.lenient().when(findSingleCodeCouponStrategy.findValidatedCouponForCouponCode(COUPON_ID)).thenReturn(Optional.of(singleCodeCoupon));
		Mockito.lenient().when(
				Boolean.valueOf(singleCodeCouponRedemptionStrategy.isCouponRedeemable(nullable(SingleCodeCouponModel.class),
						nullable(UserModel.class), anyString()))).thenReturn(Boolean.TRUE);
	...
	public void testNotValidSingleCodeCoupon()
	{
		...
		Mockito.lenient().when(
				Boolean.valueOf(singleCodeCouponRedemptionStrategy.isCouponRedeemable(nullable(SingleCodeCouponModel.class),
						nullable(UserModel.class), anyString()))).thenReturn(Boolean.FALSE);
	...
	public void testRedeemSingleCodeCouponForCart()
	{
		...
		Mockito.lenient().when(
				Boolean.valueOf(singleCodeCouponRedemptionStrategy.isCouponRedeemable(nullable(SingleCodeCouponModel.class),
						nullable(UserModel.class), nullable(String.class)))).thenReturn(Boolean.TRUE);
	...
	public void testRedeemSingleCodeCouponForOrder()
	{
		...
		Mockito.lenient().when(modelService.create(CouponRedemptionModel.class)).thenReturn(couponRedemption);
