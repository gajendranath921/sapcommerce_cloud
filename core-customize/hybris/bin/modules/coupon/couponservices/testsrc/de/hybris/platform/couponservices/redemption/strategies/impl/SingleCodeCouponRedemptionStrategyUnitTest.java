...
public class SingleCodeCouponRedemptionStrategyUnitTest
{
	...
	public void setUp()
	{
		...
		Mockito.lenient().when(couponRedemptionDao.findCouponRedemptionsByCodeAndUser(anyString(), any(UserModel.class))).thenReturn(redemptionList);
		Mockito.lenient().when(couponRedemptionDao.findCouponRedemptionsByCode(anyString())).thenReturn(redemptionList);
