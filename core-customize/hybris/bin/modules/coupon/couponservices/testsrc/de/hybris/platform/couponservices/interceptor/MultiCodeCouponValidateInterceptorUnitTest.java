...
public class MultiCodeCouponValidateInterceptorUnitTest
{
	...
	public void setUp() throws Exception
	{
		...
		Mockito.lenient().when(configuration.getString(COUPON_CODE_GENERATION_PREFIX_REGEX_PROPERTY,
		...
		Mockito.lenient().when(l10NService.getLocalizedString(anyString())).thenReturn("Coupon code seed number cannot be decremented");
	}
			...
			final Class<? extends Throwable> expectedException) throws InterceptorException
	{
		...
		Mockito.lenient().when(Boolean.valueOf(ctx.isModified(coupon, MultiCodeCouponModel.COUPONCODENUMBER))).thenReturn(TRUE);
	...
	{
		Mockito.lenient().when(valueOf(ctx.isModified(nullable(MultiCodeCouponModel.class), eq(MultiCodeCouponModel.COUPONID)))).thenReturn(modified);
	}

	private void setCodeGenerationConfigModified(final Boolean modified)
	{
		Mockito.lenient().when(valueOf(ctx.isModified(nullable(MultiCodeCouponModel.class), eq(MultiCodeCouponModel.CODEGENERATIONCONFIGURATION))))
				.thenReturn(modified);
	}

	private void setCodeGeneratedCodesModified(final Boolean modified)
	{
		Mockito.lenient().when(valueOf(ctx.isModified(nullable(MultiCodeCouponModel.class), eq(MultiCodeCouponModel.GENERATEDCODES))))
				.thenReturn(modified);
	}
	...
	{
		doReturn(mediaModels).when(validator).getOriginal(nullable(MultiCodeCouponModel.class), eq(ctx),
	...
	{
		doReturn(activeFlag).when(validator).getOriginal(nullable(MultiCodeCouponModel.class), eq(ctx), eq(MultiCodeCouponModel.ACTIVE));
	...
	{
		doReturn(codeGenerationConfigurationModel).when(validator).getOriginal(nullable(MultiCodeCouponModel.class), eq(ctx),
	...
	{
		doReturn(origCouponCodeNumber).when(validator).getOriginal(nullable(MultiCodeCouponModel.class), eq(ctx),
