...
public class CodeGenerationConfigurationUsageValidateInterceptorTest
{
	...
	{
		Mockito.lenient().when(multiCodeCoupon.getName()).thenReturn("test");
		Mockito.lenient().when(multiCodeCouponWithoutName.getCouponId()).thenReturn("test-coupon-id");
	...
	{
		//given
		Mockito.lenient().when(couponDao.findMultiCodeCouponsByCodeConfiguration(modelToDelete)).thenReturn(newArrayList(multiCodeCoupon));
		//when
	...
	public void shouldTranslateExceptionMessage() throws Exception
	{
		//given
		Mockito.lenient().when(couponDao.findMultiCodeCouponsByCodeConfiguration(modelToDelete)).thenReturn(newArrayList(multiCodeCoupon));
		Mockito.lenient().when(l10nService.getLocalizedString(eq(EXCEPTION_MESSAGE_KEY), anyVararg())).thenReturn(EXCEPTION_MESSAGE);
		//when
		final Throwable throwable = catchThrowable(() -> validator.onRemove(modelToDelete, null));
		//then
	...
	{
		//given
		Mockito.lenient().when(couponDao.findMultiCodeCouponsByCodeConfiguration(modelToDelete)).thenReturn(newArrayList());
		//when
		final Throwable throwable = catchThrowable(() -> validator.onRemove(modelToDelete, null));
		//then
	...
	{
		//given
		Mockito.lenient().when(couponDao.findMultiCodeCouponsByCodeConfiguration(modelToDelete))
				.thenReturn(newArrayList(multiCodeCoupon, multiCodeCouponWithoutName));
		//when
