...
public class DefaultCouponFacadeTest
{
	...
	public void setUp()
	{
		...
		Mockito.lenient().when(couponCodeConverter.convert(COUPON_CODE)).thenReturn(voucherData);
		Mockito.lenient().when(couponModelConverter.convert(couponModel)).thenReturn(voucherData);
	...
	{
		Mockito.lenient().when(cartService.getSessionCart()).thenReturn(null);
	...
	public void testApplyVoucher() throws VoucherOperationException
	{
		...
		Mockito.lenient().when(couponService.redeemCoupon(COUPON_CODE, cart)).thenReturn(couponResponse);
		couponFacade.applyVoucher(COUPON_CODE);
	...
	{
		Mockito.lenient().when(cartService.getSessionCart()).thenReturn(null);
		Mockito.lenient().when(cart.getAppliedCouponCodes()).thenReturn(Collections.singleton(COUPON_CODE));
	...
	{
		Mockito.lenient().when(cart.getAppliedCouponCodes()).thenReturn(Collections.emptySet());
	...
	public void testCheckVoucherCode()
	{
		final CouponResponse couponResponse = new CouponResponse();
		couponResponse.setSuccess(Boolean.TRUE);
		Mockito.lenient().when(couponService.verifyCouponCode(COUPON_CODE, cart)).thenReturn(couponResponse);
	...
	{
		Mockito.lenient().when(couponService.getCouponForCode(Matchers.anyString())).thenReturn(Optional.of(couponModel));
