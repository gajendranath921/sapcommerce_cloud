...
public final class ServerCookie implements Serializable
{
	private static final long serialVersionUID = 1L;
	private static final int FLAGS_LENGTH = 128;
	// Other fields
	private static final int DEFAULT_DATE = 10000;
	...
	static
	{
		ANCIENT_DATE = OLD_COOKIE_FORMAT.get().format(new Date(DEFAULT_DATE));
	}
			...
			final boolean isHttpOnly)
	{
		...
		final int newVersion = getNewVersionBeforeCookieHeader(value, path, domain, comment, version);
	...
	public static final class CookieSupport
	{
		...
		private static final boolean[] V0_SEPARATOR_FLAGS = new boolean[FLAGS_LENGTH];
		...
		private static final boolean[] HTTP_SEPARATOR_FLAGS = new boolean[FLAGS_LENGTH];
		...
		{
			final int quoteLength = 2;
			if (value == null || value.length() < quoteLength)
			{
				return false;
			}
